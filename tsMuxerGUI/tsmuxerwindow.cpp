...
TsMuxerWindow::TsMuxerWindow()
   ...
   for (int i = 0; i < ui.listViewFont->rowCount(); ++i) {
      ...
      ui.listViewFont->item(i, 0)->setFlags(
          ui.listViewFont->item(i, 0)->flags() & (~Qt::ItemIsEditable));
      ui.listViewFont->item(i, 1)->setFlags(
          ui.listViewFont->item(i, 0)->flags() & (~Qt::ItemIsEditable));
   }
   ...
   connect(ui.memoChapters, SIGNAL(textChanged()), this,
           SLOT(onChapterParamsChanged()));
   connect(ui.noSplit, SIGNAL(clicked()), this,
...
void TsMuxerWindow::getCodecInfo() {
   ...
   for (int i = 0; i < procStdOutput.size(); ++i) {
      ...
      if (p >= 0)
         codecInfo->lang =
             procStdOutput[i].mid(QString("Stream lang: ").length());
...
void TsMuxerWindow::onVideoComboBoxChanged(int index) {
   ...
   if (!codecInfo) return;
   codecInfo->fpsText =
       ui.comboBoxFPS->itemText(ui.comboBoxFPS->currentIndex());
   codecInfo->levelText =
...
void TsMuxerWindow::updateCurrentColor(int dr, int dg, int db, int row) {
   ...
   for (int i = 0; i < 5; ++i) {
      ...
      ui.trackLV->model()->setData(index, QBrush(color),
...
bool TsMuxerWindow::checkFileDuplicate(const QString &fileName) {
   ...
   for (int i = 0; i < ui.inputFilesLV->count(); ++i)
      if (myUnquoteStr(
              ui.inputFilesLV->item(i)->data(FileNameRole).toString()) == t) {
         QMessageBox msgBox(this);
   ...
   for (int i = 0; i < codecList.size(); ++i) {
      ...
      if (info.displayName.isEmpty()) {
         ...
         if (codecList.size() == 0) {
            ...
            if (firstWarn) {
               msgBox.setText(
                   QString("Some tracks not recognized. This tracks was "
                           "ignored. File name: \"") +
                   newFileName + "\"");
               msgBox.exec();
               firstWarn = false;
...
QString TsMuxerWindow::getMuxOpts() {
   ...
   if (ui.checkBoxCBR->isChecked())
      rez += "--cbr --bitrate=" + toFixedDecPoint(QString::number(
                                      ui.editCBRBitrate->value(), 'f', 3));
   else {
      rez += "--vbr ";
      if (ui.checkBoxRVBR->isChecked()) {
         rez += QString("--minbitrate=") +
                toFixedDecPoint(
                    QString::number(ui.editMinBitrate->value(), 'f', 3));
         rez += QString(" --maxbitrate=") +
                toFixedDecPoint(
                    QString::number(ui.editMaxBitrate->value(), 'f', 3));
      }
   }
   ...
   if (isDiskOutput()) {
      if (ui.radioButtonAutoChapter->isChecked())
         rez += " --auto-chapters=" +
                QString::number(ui.spinEditChapterLen->value());
   ...
   for (int i = 0; i < ui.trackLV->rowCount(); ++i) {
      if (ui.trackLV->item(i, 0)->checkState() == Qt::Checked) {
         ...
         if (isVideoCodec(codecInfo->displayName)) {
            ...
            rez +=
                QString(",video-height=") + QString::number(codecInfo->height);
...
void TsMuxerWindow::updateMetaLines() {
   ...
   for (int i = 0; i < ui.trackLV->rowCount(); ++i) {
      ...
      if (codecInfo->subTrack != 0)
         postfix +=
             QString(", subTrack=") + QString::number(codecInfo->subTrack);
      if (isVideoCodec(codecInfo->displayName))
...
void TsMuxerWindow::deleteTrack(int idx) {
   ...
   if (ui.trackLV->rowCount() == 0) {
      ...
      if (lastItemIndex >= 0) {
         ...
         ui.trackLV->setRangeSelected(
             QTableWidgetSelectionRange(lastItemIndex, 0, lastItemIndex, 4),
             true);
      }
      updateNum();
   }
...
void TsMuxerWindow::moveRow(int index, int index2) {
   ...
   ui.trackLV->setRangeSelected(
       QTableWidgetSelectionRange(index2, 0, index2, 4), true);
...
void TsMuxerWindow::RadioButtonMuxClick() {
   ...
   if (ui.radioButtonBluRay->isChecked() ||
      ...
      if (!oldFileName.isEmpty()) {
         ui.outFileName->setText(
             QDir::toNativeSeparators(ui.outFileName->text()));
...
void TsMuxerWindow::startMuxing() {
   ...
   if (ui.radioButtonM2TS->isChecked()) {
      ...
      if (fi.suffix().toUpper() != "M2TS") {
         ...
         msgBox.setText(
             QString("The output file \"") + ui.outFileName->text() +
...
void TsMuxerWindow::updateBtns(QWidget *w) {
   if (w) {
      ...
      ui.btnAppend->setDefault(wName == "btnAppend" &&
                               ui.btnAppend->isEnabled());
