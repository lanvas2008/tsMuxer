...
class CombinedH264Reader {
 ...
 protected:
  ...
  void addDataToPrimary(const uint8_t* data, const uint8_t* dataEnd,
                        DemuxedData& demuxedData, int64_t& discardSize);
  void addDataToSecondary(const uint8_t* data, const uint8_t* dataEnd,
                          DemuxedData& demuxedData, int64_t& discardSize);
...
class CombinedH264Demuxer : public AbstractDemuxer, public CombinedH264Reader {
 public:
  CombinedH264Demuxer(const BufferedReaderManager& readManager,
...
class CombinedH264Filter : public SubTrackFilter, public CombinedH264Reader {
 public:
  ...
  virtual int demuxPacket(DemuxedData& demuxedData, const PIDSet& acceptedPIDs,
                          AVPacket& avPacket) override;
};
