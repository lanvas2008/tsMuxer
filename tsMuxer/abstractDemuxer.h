...
class MemoryBlock {
  public:
   ...
   void resize(int num) {
      m_size = num;
      if ((int)m_data.size() < m_size) m_data.resize(m_size);
   }
...
// Used to automatically switch to reading the next file while the current one
// ends. This class implements file-IO, which determines the name of the next
// file.
...
struct TrackInfo {
   ...
   TrackInfo(int trackType, const char* lang, int64_t delay)
...
class AbstractDemuxer {
  public:
   ...
   void setPidFilter(int pid, SubTrackFilter* pidFilter) {
      m_pidFilters[pid] = pidFilter;
   }
  ...
  protected:
   ...
   PIDFilters m_pidFilters;  // todo: refactor in case if several pidFilters
                             // required (>1 3D tracks)
};
