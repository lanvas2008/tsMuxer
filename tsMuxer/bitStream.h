...
//#define THROW_BITSTREAM_ERR throw BitStreamException(std::string(__FILE__) +
// std::string(" ") + std::string(__FUNCTION__) + std::string(" at line ") +
// int32ToStr(__LINE__))
...
class BitStream {
 ...
 protected:
  inline void setBuffer(uint8_t* buffer, uint8_t* end) {
    if (buffer >= end) THROW_BITSTREAM_ERR;
...
class BitStreamReader : public BitStream {
 ...
 public:
  ...
  inline unsigned getBits(unsigned num) {
    if (num > INT_BIT) THROW_BITSTREAM_ERR;
    if (m_totalBits < num) THROW_BITSTREAM_ERR;
    unsigned prevVal = 0;
    if (num <= m_bitLeft)
      m_bitLeft -= num;
    else {
  ...
  inline unsigned getBit() {
    if (m_totalBits < 1) THROW_BITSTREAM_ERR;
    if (m_bitLeft > 0)
      m_bitLeft--;
    else {
      m_buffer++;
  ...
  inline unsigned getBitsCount() const {
    return (unsigned)(m_buffer - m_initBuffer) * INT_BIT + INT_BIT - m_bitLeft;
  }
...
void updateBits(const BitStreamReader& bitReader, int bitOffset, int bitLen,
                int value);
