...
const static int MAX_TRACK_SIZE =
    (MAX(MAX(sizeof(MatroskaVideoTrack), sizeof(MatroskaAudioTrack)),
  ...
  while (res == 0) {
    ...
    switch (id) {
      ...
      case MATROSKA_ID_POINTENTRY:
        if ((res = ebml_read_master(&id)) < 0) break;
        ...
        while (res == 0) {
          ...
          switch (id) {
            ...
            case MATROSKA_ID_CUETIME: {
              uint64_t time;
              if ((res = ebml_read_uint(&id, &time)) < 0) break;
              ...
              while (res == 0) {
                ...
                switch (id) {
                  ...
                  case MATROSKA_ID_CUETRACK: {
                    uint64_t num;
                    if ((res = ebml_read_uint(&id, &num)) < 0) break;
                    idx.track = num;
                    break;
                  }
                  ...
                  default:
                    LTRACE(LT_INFO, 0,
                           "Unknown entry " << id << " in CuesTrackPositions");
                    /* fall-through */
            ...
            default:
              LTRACE(LT_INFO, 0,
                     "Unknown entry " << id << " in cuespoint index");
              /* fall-through */
        ...
        if (idx.pos != (uint64_t)-1 && idx.time != (uint64_t)-1 &&
            idx.track != (uint16_t)-1) {
...
int MatroskaDemuxer::ebml_read_header(char **doctype, int *version) {
  ...
  if (!(id = ebml_peek_id(&level_up)) || level_up != 0 ||
      id != EBML_ID_HEADER) {
    THROW(ERR_MATROSKA_PARSE,
          "This is not an EBML file (id=" << id << "/" << EBML_ID_HEADER);
  }
  if ((res = ebml_read_master(&id)) < 0) return res;

  while (res == 0) {
    if (!(id = ebml_peek_id(&level_up))) return -BufferedReader::DATA_EOF;
    ...
    switch (id) {
      ...
      case EBML_ID_EBMLREADVERSION: {
        ...
        if ((res = ebml_read_uint(&id, &num)) < 0) return res;
        if (num > EBML_VERSION) {
      ...
      case EBML_ID_EBMLMAXSIZELENGTH: {
        ...
        if (num > sizeof(uint64_t)) {
          THROW(ERR_MATROSKA_PARSE, "Integers of size " << num << " (> "
                                                        << sizeof(uint64_t)
                                                        << ") not supported");
        }
        break;
      ...
      case EBML_ID_EBMLMAXIDLENGTH: {
        ...
        if (num > sizeof(uint32_t)) {
          THROW(ERR_MATROSKA_PARSE, "IDs of size " << num << " (> "
                                                   << sizeof(uint32_t)
                                                   << ") not supported");
        }
        break;
      ...
      case EBML_ID_DOCTYPE: {
        ...
        if ((res = ebml_read_ascii(&id, &text)) < 0) return res;
        if (doctype) {
          if (*doctype) delete[] doctype;
          *doctype = text;
        } else
          delete[] text;
        break;
...
void MatroskaDemuxer::matroska_queue_packet(AVPacket *pkt) {
  ...
  // *)); packets[matroska->num_packets] = pkt; num_packets++;
  packets.push(pkt);
}
...
int MatroskaDemuxer::matroska_find_track_by_num(int num) {
  ...
  for (i = 0; i < num_tracks; i++)
    if (tracks[i]->num == num) return i;
  return -1;
}

int MatroskaDemuxer::matroska_ebmlnum_uint(uint8_t *data, uint32_t size,
                                           uint64_t *num) {
  ...
  if (read > 8) return AVERROR_INVALIDDATA;

  if ((total &= (len_mask - 1)) == len_mask - 1) num_ffs++;
  if (size < read) return AVERROR_INVALIDDATA;
  while (n < read) {
    if (data[n] == 0xff) num_ffs++;
...
int MatroskaDemuxer::matroska_ebmlnum_sint(uint8_t *data, uint32_t size,
                                           int64_t *num) {
  uint64_t unum;
  int res;

  /* read as unsigned number first */
  if ((res = matroska_ebmlnum_uint(data, size, &unum)) < 0) return res;
...
int MatroskaDemuxer::ebml_read_sint(uint32_t *id, int64_t *num) {
  ...
  if (size < 1 || size > 8) {
    offset_t pos = m_processedBytes;
    THROW(ERR_MATROSKA_PARSE,
          "Invalid sint element size " << size << " at position " << pos);
  }
  ...
  /* make signed */
  if (negative) *num = *num - (1LL << ((8 * size) - 1));
...
void MatroskaDemuxer::decompressData(const uint8_t *data, int size) {
  ...
  inflateEnd(&zstream);
  if (err != Z_STREAM_END) m_tmpBuffer.clear();
}

int MatroskaDemuxer::matroska_parse_block(uint8_t *data, int size, int64_t pos,
  ...
  if (tracks[track]->stream_index < 0) return res;
  /* block_time (relative to cluster time) */
  block_time = (int16_t)AV_RB16(data);
  data += 2;
  flags = *data++;
  size -= 3;
  if (is_keyframe == -1) is_keyframe = flags & 0x80 ? PKT_FLAG_KEY : 0;
  ...
  if (res == 0) {
    ...
    for (n = 0; n < laces; n++) {
      ...
      for (slice = 0; slice < slices; slice++) {
        ...
        if (real_v) slice_offset = rv_offset(data, slice, slices);
        if (slice + 1 == slices)
        ...
        if (tracks[track]->parsed_priv_data != 0) {
          tracks[track]->parsed_priv_data->extractData(pkt, curPtr,
                                                       slice_size + offset);
        ...
        if (offset) memcpy(curPtr, m_tmpBuffer.data(), offset);  // restore data
...
int MatroskaDemuxer::matroska_parse_blockgroup(uint64_t cluster_time) {
  ...
  while (res == 0) {
    ...
    switch (id) {
      ...
      case MATROSKA_ID_BLOCKDURATION: {
        if ((res = ebml_read_uint(&id, &duration)) < 0) break;
        break;
      }
      ...
      case MATROSKA_ID_BLOCKREFERENCE: {
        ...
        is_keyframe = 0;
        if (last_num_packets != packets.size()) packets.back()->flags = 0;
        if ((res = ebml_read_sint(&id, &num)) < 0) break;
        if (num > 0) is_bframe = 1;
        break;
      }
  ...
  if (size > 0)
    res = matroska_parse_block(data, size, pos, cluster_time, duration,
                               is_keyframe, is_bframe);
...
int MatroskaDemuxer::ebml_read_uint(uint32_t *id, uint64_t *num) {
  ...
  size = rlength;
  if (size < 1 || size > 8) {
    THROW(ERR_MATROSKA_PARSE, "Invalid uint element size " << size
                                                           << " at position "
                                                           << m_processedBytes);
  }
...
int MatroskaDemuxer::ebml_read_element_id(uint32_t *id, int *level_up) {
  ...
  if (peek_id != 0) {
    if (level_up) *level_up = 0;
    *id = peek_id;
    return 0;
  }

  /* read out the "EBML number", include tag in ID */
  if ((read = ebml_read_num(4, &total)) < 0) return read;
  *id = peek_id = total | (1 << (read * 7));

  /* level tracking */
  if (level_up) *level_up = ebml_read_element_level_up();
...
int MatroskaDemuxer::ebml_read_master(uint32_t *id) {
  ...
  if (num_levels >= EBML_MAX_DEPTH) {
    THROW(ERR_MATROSKA_PARSE,
          "File moves beyond max. allowed depth (" << EBML_MAX_DEPTH << ")");
  }
...
int MatroskaDemuxer::ebml_read_binary(uint32_t *id, uint8_t **binary,
                                      int *size) {
  uint64_t rlength;
  int res;
  ...
  *binary = new uint8_t[*size];
  ...
  if (get_buffer(*binary, *size) != *size) {
    THROW(ERR_MATROSKA_PARSE,
          "Matroska parser: read error at pos " << m_processedBytes);
  }
  return 0;
}
  ...
  while (res == 0) {
    ...
    switch (id) {
      ...
      case MATROSKA_ID_CLUSTER:
        return 0;  // I don't know why here is the next cluster without level
                   // up. Jim send me it. Probably file error
      default:
...
void MatroskaDemuxer::openFile(const std::string &streamName) {
  ...
  BufferedFileReader *fileReader =
      dynamic_cast<BufferedFileReader *>(m_bufferedReader);
...
int MatroskaDemuxer::ebml_read_num(int max_size, uint64_t *number) {
  ...
  if (!(total = get_byte())) {
    ...
    if (!m_isEOF)
      THROW(ERR_MATROSKA_PARSE,
            "Matroska parse error: Invalid EBML number size "
                << total << " at pos " << m_processedBytes - 1);
    return -BufferedReader::DATA_EOF;
  }
  ...
  if (read > max_size) {
    ...
    THROW(ERR_MATROSKA_PARSE, "Matroska parse error: Invalid EBML number size "
                                  << total << " at pos " << pos);
  }
...
uint32_t MatroskaDemuxer::ebml_peek_id(int *level_up) {
  uint32_t id;
  if (ebml_read_element_id(&id, level_up) < 0) return 0;
  return id;
}
...
int MatroskaDemuxer::readPacket(AVPacket &avPacket) {
  ...
  while (matroska_deliver_packet(newPacket) != 0) {
    ...
    if (done) return BufferedReader::DATA_EOF;
...
int MatroskaDemuxer::matroska_read_header() {
  ...
  if ((res = ebml_read_header(&doctype, &version)) < 0) return res;
  if ((doctype == NULL) || strcmp(doctype, "matroska")) {
    if (doctype) delete[] doctype;
    THROW(ERR_MATROSKA_PARSE, "Wrong EBML doctype ('"
  ...
  if (version > 2) {
    THROW(ERR_MATROSKA_PARSE,
          "Matroska demuxer version 2 too old for file version " << version);
  }

  /* The next thing is a segment. */
  while (1) {
    if (!(id = ebml_peek_id(&last_level))) return -BufferedReader::DATA_EOF;
    if (id == MATROSKA_ID_SEGMENT) break;

    /* oi! */
    // av_log(matroska->ctx, AV_LOG_INFO, "Expected a Segment ID (0x%x), but
    // received 0x%x!\n", MATROSKA_ID_SEGMENT, id);
    if ((res = ebml_read_skip()) < 0) return res;
  }
...
res = matroska_parse_seekhead();
  ...
  if (ebml_peek_id(NULL) == MATROSKA_ID_CLUSTER) {
    ...
    for (i = 0; i < num_tracks; i++) {
      ...
      if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_HEVC_FOURCC) &&
          (track->codec_priv != NULL)) {
        MatroskaVideoTrack *vtrack = (MatroskaVideoTrack *)track;
        track->parsed_priv_data =
            new ParsedH265TrackData(track->codec_priv, track->codec_priv_size);
      }
      if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_AVC_FOURCC) &&
          (track->codec_priv != NULL)) {
        MatroskaVideoTrack *vtrack = (MatroskaVideoTrack *)track;
        track->parsed_priv_data =
            new ParsedH264TrackData(track->codec_priv, track->codec_priv_size);
      }
      if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_VIDEO_VFW_FOURCC) &&
          (track->codec_priv != NULL)) {
        MatroskaVideoTrack *vtrack = (MatroskaVideoTrack *)track;
        track->parsed_priv_data =
            new ParsedVC1TrackData(track->codec_priv, track->codec_priv_size);
      }

      if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_AUDIO_AC3)) {
        track->parsed_priv_data =
            new ParsedAC3TrackData(track->codec_priv, track->codec_priv_size);
      }

      if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_AUDIO_AAC)) {
        track->parsed_priv_data =
            new ParsedAACTrackData(track->codec_priv, track->codec_priv_size);
      }
      ...
      if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_SRT)) {
        track->parsed_priv_data =
            new ParsedSRTTrackData(track->codec_priv, track->codec_priv_size);
      }
  ...
  while (res == 0) {
    ...
    switch (id) {
      ...
      case MATROSKA_ID_DURATION: {
        double num;
        if ((res = ebml_read_float(&id, &num)) < 0) break;
      ...
      case MATROSKA_ID_TITLE: {
        char *text;
        if ((res = ebml_read_utf8(&id, &text)) < 0) break;
      ...
      case MATROSKA_ID_DATEUTC: {
        int64_t time;
        if ((res = ebml_read_date(&id, &time)) < 0) break;
        created = time;
        break;
      }
...
int MatroskaDemuxer::matroska_parse_chapters() {
  ...
  while (res == 0) {
    ...
    switch (id) {
            ...
            case MATROSKA_ID_CHAPTERATOM:
              ...
              while (res == 0) {
                ...
                switch (id) {
                  ...
                  case MATROSKA_ID_CHAPTERDISPLAY:
                    ...
                    while (res == 0) {
                      ...
                      switch (id) {
                        ...
                        default:
                          LTRACE(LT_INFO, 0,
                                 "Ignoring unknown Chapter display ID " << id);
                        case EBML_ID_VOID:
                          res = ebml_read_skip();
                          break;
                      }
                  ...
                  default:
                    LTRACE(LT_INFO, 0,
                           "Ignoring unknown Chapter atom ID " << id);
      ...
      default:
        LTRACE(LT_INFO, 0,
               "Expected an Edition entry (" << MATROSKA_ID_EDITIONENTRY
                                             << "), but found " << id);
      case EBML_ID_VOID:
        res = ebml_read_skip();
        break;
  ...
  while (res == 0) {
    ...
    switch (id) {
      ...
      case MATROSKA_ID_ENCODINGCOMPSETTINGS: {
        uint8_t *data;
        int size;
        if ((res = ebml_read_binary(&id, &data, &size)) < 0) break;
        if (size > 0) {
...
int MatroskaDemuxer::matroska_add_stream() {
  ...
  if (num_tracks > MAX_STREAMS)
    THROW(ERR_COMMON,
          "Too many tracks. Max supported tracks count: " << MAX_STREAMS);
  ...
  while (res == 0) {
    ...
    switch (id) {
      ...
      case MATROSKA_ID_TRACKVIDEO: {
        ...
        if (!track->type) track->type = TRACK_TYPE_VIDEO;
        ...
        while (res == 0) {
          ...
          switch (id) {
            ...
            default:
              LTRACE(
                  LT_INFO, 0,
                  "Unknown video track header entry " << id << " - ignoring\n");
              /* pass-through */
      ...
      case MATROSKA_ID_TRACKAUDIO: {
        ...
        if (!track->type) track->type = TRACK_TYPE_AUDIO;
        ...
        while (res == 0) {
          ...
          switch (id) {
            ...
            default:
              LTRACE(
                  LT_INFO, 0,
                  "Unknown audio track header entry " << id << " - ignoring\n");
              /* pass-through */
      ...
      case MATROSKA_ID_CODECID: {
        char *text;
        if ((res = ebml_read_ascii(&id, &text)) < 0) break;
        track->codec_id = text;
        break;
      }
      ...
      case MATROSKA_ID_TRACKLANGUAGE: {
        char *text, *end;
        if ((res = ebml_read_utf8(&id, &text)) < 0) break;
        if ((end = strchr(text, '-'))) *end = '\0';
        if (strlen(text) == 3) strcpy(track->language, text);
        delete[] text;
        break;
      }
...
int MatroskaDemuxer::simpleDemuxBlock(DemuxedData &demuxedData,
                                      ...
                                      int64_t &discardSize) {
  for (std::set<uint32_t>::const_iterator itr = acceptedPIDs.begin();
       itr != acceptedPIDs.end(); ++itr)
    demuxedData[*itr];
  ...
  while (demuxedSize < m_fileBlockSize) {
    ...
    if (itr != m_pidFilters.end()) {
      demuxedSize +=
          itr->second->demuxPacket(demuxedData, acceptedPIDs, packet);
    } else {
      if (acceptedPIDs.find(packet.stream_index) != acceptedPIDs.end()) {
        MemoryBlock &vect = demuxedData[packet.stream_index];
        if (packet.size > 0) vect.append(packet.data, packet.size);
...
void MatroskaDemuxer::getTrackList(std::map<uint32_t, TrackInfo> &trackList) {
  for (int i = 0; i < num_tracks; i++)
    trackList.insert(std::make_pair(
        i + 1, TrackInfo(getTrackType(tracks[i]), tracks[i]->language, 0)));
}
...
std::vector<AVChapter> MatroskaDemuxer::getChapters() {
  ...
  for (std::map<int, AVChapter>::const_iterator itr = chapters.begin();
       itr != chapters.end(); ++itr)
