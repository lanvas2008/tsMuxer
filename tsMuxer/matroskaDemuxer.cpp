   ...
   while (res == 0) {
      ...
      switch (id) {
         ...
         case EBML_ID_EBMLREADVERSION: {
            ...
            if (num > EBML_VERSION) {
               THROW(ERR_MATROSKA_PARSE, "EBML version "
                                             << num << " > " << EBML_VERSION
                                             << " is not supported");
         ...
         case EBML_ID_EBMLMAXSIZELENGTH: {
            ...
            if (num > sizeof(uint64_t)) {
               THROW(ERR_MATROSKA_PARSE,
                     "Integers of size " << num << " (> " << sizeof(uint64_t)
                                         << ") not supported");
            }
            break;
         }
...
void MatroskaDemuxer::matroska_queue_packet(AVPacket *pkt) {
   // packets = av_realloc(matroska->packets, (num_packets + 1) *
   // sizeof(AVPacket
      ...
      for (n = 0; n < laces; n++) {
         ...
         for (slice = 0; slice < slices; slice++) {
            ...
            if (tracks[track]->parsed_priv_data != 0) {
               tracks[track]->parsed_priv_data->extractData(
                   pkt, curPtr, slice_size + offset);
...
int MatroskaDemuxer::ebml_read_uint(uint32_t *id, uint64_t *num) {
   ...
   size = rlength;
   if (size < 1 || size > 8) {
      THROW(ERR_MATROSKA_PARSE, "Invalid uint element size "
                                    << size << " at position "
                                    << m_processedBytes);
   }
   ...
   while (res == 0) {
      ...
      switch (id) {
         ...
         case MATROSKA_ID_CLUSTER:
            return 0;  // I don't know why here is the next cluster without
                       // level up. Jim send me it. Probably file error
         default:
   ...
   if (ebml_peek_id(NULL) == MATROSKA_ID_CLUSTER) {
      ...
      for (i = 0; i < num_tracks; i++) {
         ...
         if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_HEVC_FOURCC) &&
            ...
            track->parsed_priv_data = new ParsedH265TrackData(
                track->codec_priv, track->codec_priv_size);
         }
         if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_AVC_FOURCC) &&
             (track->codec_priv != NULL)) {
            MatroskaVideoTrack *vtrack = (MatroskaVideoTrack *)track;
            track->parsed_priv_data = new ParsedH264TrackData(
                track->codec_priv, track->codec_priv_size);
         }
         if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_VIDEO_VFW_FOURCC) &&
             (track->codec_priv != NULL)) {
            MatroskaVideoTrack *vtrack = (MatroskaVideoTrack *)track;
            track->parsed_priv_data = new ParsedVC1TrackData(
                track->codec_priv, track->codec_priv_size);
         }

         if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_AUDIO_AC3)) {
            track->parsed_priv_data = new ParsedAC3TrackData(
                track->codec_priv, track->codec_priv_size);
         }

         if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_AUDIO_AAC)) {
            track->parsed_priv_data = new ParsedAACTrackData(
                track->codec_priv, track->codec_priv_size);
         }
         ...
         if (!strcmp(track->codec_id, MATROSKA_CODEC_ID_SRT)) {
            track->parsed_priv_data = new ParsedSRTTrackData(
                track->codec_priv, track->codec_priv_size);
         }
   ...
   while (res == 0) {
      ...
      switch (id) {
         case MATROSKA_ID_EDITIONENTRY: {
            ...
            uint64_t uid = -1;
            char *title = NULL;
            // if there is more than one chapter edition we take only the first
            // one
            if (chapters.size() > 0) {
               ebml_read_skip();
               break;
            ...
            while (res == 0) {
               ...
               switch (id) {
                  ...
                  default:
                     LTRACE(LT_INFO, 0,
                            "Ignoring unknown Edition entry ID " << id);
            ...
            while (res == 0) {
               ...
               switch (id) {
                  ...
                  case MATROSKA_ID_AUDIOSAMPLINGFREQ: {
                     double num;
                     if ((res = ebml_read_float(&id, &num)) < 0) break;
                     audiotrack->internal_samplerate = audiotrack->samplerate =
                         num;
                     break;
                  }
