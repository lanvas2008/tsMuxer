...
// possible bitrates
const uint16_t ff_ac3_bitratetab[19] = {32,  40,  48,  56,  64,  80,  96,
...
const CodecInfo& AC3Codec::getCodecInfo() {
   if (m_true_hd_mode) return trueHDCodecInfo;
   if (isEAC3())
...
int AC3Codec::parseHeader(uint8_t* buf, uint8_t* end) {
   ...
   if (m_sync_word != 0x0B77) return AC3_PARSE_ERROR_SYNC;

   // read ahead to bsid to make sure this is AC-3, not E-AC-3
   int id = gbc.showBits(29) & 0x1F;
   if (id > 16) return AC3_PARSE_ERROR_BSID;
   ...
   if (m_bsid > 10)  // bsid = 16 => EAC3
   {
      ...
      if (strmtyp == 3) return AC3_PARSE_ERROR_SYNC;  // invalid stream type

      substreamid = gbc.getBits(3);

      m_frame_size = (gbc.getBits(11) + 1) * 2;
      if (m_frame_size < AC3_HEADER_SIZE) return 0;  // invalid header size
      ...
      if (m_fscod == 3) {
         ...
         numblkscod = 3;
         if (m_fscod2 == 3) return AC3_PARSE_ERROR_SYNC;
   ...
   } else  // AC-3
   {
      ...
      m_fscod = gbc.getBits(2);
      if (m_fscod == 3) return AC3_PARSE_ERROR_SAMPLE_RATE;

      m_frmsizecod = gbc.getBits(6);
      if (m_frmsizecod > 37) return AC3_PARSE_ERROR_FRAME_SIZE;
      ...
      m_bit_rate =
          (ff_ac3_bitratetab[m_frmsizecod >> 1] * 1000) >> m_halfratecod;
      m_channels = ff_ac3_channels[m_acmod] + m_lfeon;
...
bool AC3Codec::findMajorSync(uint8_t* buffer, uint8_t* end) {
   ...
   for (uint8_t* cur = buffer; cur < end - 4; cur++) {
      ...
      if (*testVal == majorSyncVal) return true;
   }
   return false;
}

bool AC3Codec::decodeDtsHdFrame(uint8_t* buffer, uint8_t* end) {
   if (end - buffer < 21) return false;
   int ratebits = 0;
   ...
   mh.frame_duration_nano =
       mh.access_unit_size * 1000000000ll / mh.group1_samplerate;
...
int AC3Codec::decodeFrame(uint8_t* buf, uint8_t* end, int& skipBytes) {
   try {
      ...
      if (end - buf < 2) return NOT_ENOUGHT_BUFFER;
      if (m_state != stateDecodeAC3 && buf[0] == 0x0B && buf[1] == 0x77) {
         if (testDecodeTestFrame(buf, end)) m_state = stateDecodeAC3;
      }
      ...
      if (getTestMode() && !m_true_hd_mode) {
         ...
         if (end - trueHDData < 8) return NOT_ENOUGHT_BUFFER;
         if (!isSyncWorld(trueHDData) && isHDSyncWorld(trueHDData + 4)) {
            if (end - trueHDData < 21) return NOT_ENOUGHT_BUFFER;
      ...
      {
         uint8_t* trueHDData = buf + rez;
         if (end - trueHDData < 7) return NOT_ENOUGHT_BUFFER;
         if (m_state == stateDecodeAC3) {
            // check if it is a real HD frame

            if (trueHDData[0] != 0x0B || trueHDData[1] != 0x77 ||
                !testDecodeTestFrame(trueHDData, end)) {
         ...
         trueHDFrameLen *= 2;
         if (end - trueHDData < trueHDFrameLen + 7) return NOT_ENOUGHT_BUFFER;
         if (!m_true_hd_mode) {
            // m_true_hd_mode |= findMajorSync(trueHDData, trueHDData +
            // trueHDFrameLen);
            m_true_hd_mode =
                decodeDtsHdFrame(trueHDData, trueHDData + trueHDFrameLen);
         }
         uint8_t* nextFrame = trueHDData + trueHDFrameLen;

         if (nextFrame[0] == 0x0B && nextFrame[1] == 0x77 &&
             testDecodeTestFrame(nextFrame, end))
...
int AC3Codec::testParseHeader(uint8_t* buf, uint8_t* end) {
   ...
   if (test_sync_word != 0x0B77) return AC3_PARSE_ERROR_SYNC;
   ...
   */
   ...
   if (test_bsid > 16) {
      ...
      if (test_fscod == 3) return AC3_PARSE_ERROR_SAMPLE_RATE;

      int test_frmsizecod = gbc.getBits(6);
      if (test_frmsizecod > 37) return AC3_PARSE_ERROR_FRAME_SIZE;
      ...
      if (test_fscod !=
              m_fscod || /*(test_frmsizecod>>1) != (m_frmsizecod>>1) ||*/
      ...
      if (m_acmod == AC3_ACMOD_STEREO) {
         ...
         if (test_dsurmod != m_dsurmod) return AC3_PARSE_ERROR_SYNC;
      }
      int test_lfeon = gbc.getBit();

      if (test_lfeon != m_lfeon) return AC3_PARSE_ERROR_SYNC;

      int test_halfratecod = max(test_bsid, 8) - 8;
      int test_sample_rate = ff_ac3_freqs[test_fscod] >> test_halfratecod;
      int test_bit_rate =
          (ff_ac3_bitratetab[test_frmsizecod >> 1] * 1000) >> test_halfratecod;
      ...
      if (test_halfratecod != m_halfratecod ||
          test_sample_rate != m_sample_rate || test_bit_rate != m_bit_rate ||
...
double AC3Codec::getFrameDurationNano() {
   if (m_bit_rateExt)
      return m_bsid > 10 ? m_frameDuration
                         : 0;  // E-AC3. finish frame after AC3 frame
...
const std::string AC3Codec::getStreamInfo() {
   ...
   if (m_true_hd_mode) {
      str << "Peak bitrate: " << mh.peak_bitrate / 1000 << "Kbps (core "
          << m_bit_rate / 1000 << "Kbps) ";
      ...
      if (m_bit_rateExt) str << "(core " << m_bit_rate / 1000 << "Kbps) ";
      str << "Sample Rate: " << m_sample_rate / 1000 << "KHz ";
   }

   str << "Channels: ";
   int channels = m_channels;
   if (m_extChannelsExists) channels += 2;
   if (mh.channels) channels = mh.channels;
