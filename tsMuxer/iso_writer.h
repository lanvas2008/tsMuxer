...
class ByteFileWriter {
 public:
  ...
  void writeLongAD(uint32_t lenBytes, uint32_t pos, uint16_t partition,
                   uint32_t id);
...
struct FileEntryInfo {
 public:
  FileEntryInfo(IsoWriter* owner, FileEntryInfo* parent, uint32_t objectId,
                FileTypes fileType);
  ~FileEntryInfo();
...
class IsoWriter {
 public:
  ...
  bool createInterleavedFile(const std::string& inFile1,
 ...
 private:
  ...
  int writeExtentFileDescriptor(uint8_t fileType, uint64_t len, uint32_t pos,
                                int linkCount, ExtentList* extents = 0);
  void writeFileSetDescriptor();
  void writeAllocationExtentDescriptor(ExtentList* extents, int start,
                                       int indexEnd);
...
class ISOFile : public AbstractOutputStream {
 public:
  ISOFile(IsoWriter* owner)
      : AbstractOutputStream(), m_owner(owner), m_entry(0) {}
  virtual ~ISOFile() { close(); }

  virtual int write(const void* data, uint32_t len) override;
  virtual bool open(const char* name, unsigned int oflag,
