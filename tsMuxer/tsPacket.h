...
struct PMTIndexData {
   uint32_t m_pktCnt;
   uint32_t m_frameLen;
   PMTIndexData(uint32_t pktCnt, uint32_t frameLen)
       : m_pktCnt(pktCnt), m_frameLen(frameLen) {}
};
...
struct PMTStreamInfo {
   ...
   PMTStreamInfo(int streamType, int pid, uint8_t* esInfoData, int esInfoLen,
                 AbstractStreamReader* codecReader, const std::string& lang,
                 bool secondary) {
   ...
   bool isSecondary;
   // ---------------------
   std::vector<PMTIndex> m_index;  // blu-ray seek index. key=number of tsFrame.
                                   // value=information about key frame
...
struct TS_program_map_section {
  ...
  private:
   void extractDescriptors(uint8_t* curPos, int es_info_len,
...
struct M2TSStreamInfo {
   ...
   static void blurayStreamParams(double fps, bool interlaced, int width,
...
struct CLPIStreamInfo : public M2TSStreamInfo {
   ...
   static void writeString(const char* dest, BitStreamWriter& writer,
                           int size) {
...
class CLPIParser {
  ...
  private:
   ...
   void composeEP_map_for_one_stream_PID(BitStreamWriter& writer,
...
struct MPLSStreamInfo : public M2TSStreamInfo {
   ...
   void composeStreamEntry(BitStreamWriter& reader, int entryNum,
                           int subPathID = 0);
...
struct PlayListMark {
   int m_playItemID;
   uint32_t m_markTime;
   PlayListMark(int playItemID, uint32_t markTime)
       : m_playItemID(playItemID), m_markTime(markTime) {}
};

struct ExtDataBlockInfo {
   ExtDataBlockInfo(uint8_t* a_data, int a_dataLen, int a_id1, int a_id2)
       : id1(a_id1), id2(a_id2) {
      data.resize(a_dataLen);
      if (!data.empty()) memcpy(&data[0], a_data, data.size());
   }
...
struct MPLSParser {
  ...
  private:
   void composeSubPlayItem(BitStreamWriter& writer, int playItemNum,
                           int subPathNum, std::vector<PMTIndex>& pmtIndexList);
   void composeSubPath(BitStreamWriter& writer, int subPathNum,
                       std::vector<PMTIndex>& pmtIndexList, int type);
   int composePip_metadata(uint8_t* buffer, int bufferSize,
                           std::vector<PMTIndex>& pmtIndexList);
   void composeExtensionData(BitStreamWriter& writer,
                             std::vector<ExtDataBlockInfo>& extDataBlockInfo);
   ...
   void composePlayItem(BitStreamWriter& writer, int playItemNum,
                        std::vector<PMTIndex>& pmtIndexList);
