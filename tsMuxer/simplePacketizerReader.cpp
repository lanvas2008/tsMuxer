...
void SimplePacketizerReader::doMplsCorrection() {
   if (m_curMplsIndex == -1) return;
   if (m_curPts >= (m_lastMplsTime - mplsEps) &&
       m_curMplsIndex < m_mplsInfo.size() - 1) {
      m_curMplsIndex++;
      ...
      m_lastMplsTime += (int64_t)((m_mplsInfo[m_curMplsIndex].OUT_time -
                                   m_mplsInfo[m_curMplsIndex].IN_time) *
                                  (1000000000 / 45000.0));
   }
}

void SimplePacketizerReader::setBuffer(uint8_t* data, int dataLen,
                                       bool lastBlock) {
...
int SimplePacketizerReader::readPacket(AVPacket& avPacket) {
   do {
      ...
      if (m_needSync) {
         ...
         if (decodeRez == NOT_ENOUGHT_BUFFER) {
            if (m_bufEnd - frame > DEFAULT_FILE_BLOCK_SIZE)
               THROW(ERR_COMMON, getCodecInfo().displayName
                                     << " stream (track " << m_streamIndex
                                     << "): invalid stream.");
      ...
      skipBeforeBytes = 0;
      int frameLen =
          decodeFrame(m_curPos, m_bufEnd, skipBytes, skipBeforeBytes);
      if (frameLen == NOT_ENOUGHT_BUFFER) {
         ...
         LTRACE(LT_INFO, 2,
                getCodecInfo().displayName
                    ...
                    << floatToTime((avPacket.pts - PTS_CONST_OFFSET) / 1e9, ',')
                    << ". Resync stream.");
         m_needSync = true;
         return 0;
      }
      ...
      if (m_halfFrameLen == 0) m_halfFrameLen = getFrameDurationNano() / 2.0;
      ...
      if (needMPLSCorrection()) {
         if (/*m_demuxMode && */ m_mplsOffset >
             m_halfFrameLen) {  // overlap frame detected. skip frame
            if (avPacket.duration)
               LTRACE(LT_INFO, 2,
                      getCodecInfo().displayName
                          << " stream (track " << m_streamIndex
                          << "): overlapped frame detected at position "
                          << floatToTime(
                                 (avPacket.pts - PTS_CONST_OFFSET) / 1e9, ',')
                          << ". Remove frame.");
            m_mplsOffset -= getFrameDurationNano();
            m_curPts -= getFrameDurationNano();
            return readPacket(
                avPacket);  // ignore overlapped packet, get next one
         } else {
            doMplsCorrection();
         }
      }

      if (needSkipFrame(avPacket)) continue;
