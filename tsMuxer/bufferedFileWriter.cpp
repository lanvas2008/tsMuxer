...
BufferedFileWriter::BufferedFileWriter()
    : m_started(false),
      m_terminated(false),
      m_writeQueue(WRITE_QUEUE_MAX_SIZE) {
  m_lastErrorCode = 0;
...
void BufferedFileWriter::thread_main() {
  while (!m_terminated) {
    ...
    try {
      ...
      m_lastErrorCode = -1;
      LTRACE(LT_ERROR, 0,
             "BufferedFileWriter::thread_main() throws runtime_error: "
                 << e.what());
      ...
      LTRACE(
          LT_ERROR, 0,
          "BufferedFileWriter::thread_main() throws exception: " << e.what());
    } catch (...) {
      m_lastErrorStr = "Unknown expcetion";
      m_lastErrorCode = -1;
      LTRACE(LT_ERROR, 0,
             "BufferedFileWriter::thread_main() throws unknown exception");
    }
