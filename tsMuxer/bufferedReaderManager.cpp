...
BufferedReaderManager::BufferedReaderManager(uint32_t readersCnt,
                                             uint32_t blockSize,
                                             uint32_t allocSize,
  ...
  for (uint32_t i = 0; i < readersCnt; i++) {
    BufferedReader* reader =
        new BufferedFileReader(m_blockSize, m_allocSize, m_prereadThreshold);
    reader->setId(i);
...
void BufferedReaderManager::init(uint32_t blockSize, uint32_t allocSize,
                                 uint32_t prereadThreshold) {
  ...
  m_prereadThreshold =
      prereadThreshold > 0 ? prereadThreshold : m_blockSize / 2;
}

BufferedReaderManager::~BufferedReaderManager() {
  for (size_t i = 0; i < m_fileReaders.size(); i++) {
    delete m_fileReaders[i];  // need to define destruction order first. This
                              // object MUST be deleted after MCVodStreamer
  }
}
