...
const static uint64_t TS_FREQ_TO_INT_FREQ_COEFF =
    INTERNAL_PTS_FREQ / PCR_FREQUENCY;
...
bool isM2TSExt(const std::string& streamName) {
   ...
   return strEndWith(sName, ".m2ts") || strEndWith(sName, ".mts") ||
          strEndWith(sName, ".ssif");
}

TSDemuxer::TSDemuxer(const BufferedReaderManager& readManager,
                     const char* streamName)
    : m_readManager(readManager) {
   m_firstPCRTime = -1;
   m_bufferedReader = (const_cast<BufferedReaderManager&>(m_readManager))
                          .getReader(streamName);
   m_readerID = m_bufferedReader->createReader(TS_FRAME_SIZE);
   if (m_bufferedReader == 0)
      THROW(
          ERR_COMMON,
          "TS demuxer can't accept reader because this reader does not support "
...
bool TSDemuxer::mvcContinueExpected() const {
   // return  m_pmt.video_type == STREAM_TYPE_VIDEO_MVC &&
   // strEndWith(m_streamNameLow, "ssif");
...
void TSDemuxer::getTrackList(std::map<uint32_t, TrackInfo>& trackList) {
   ...
   int lastReadRez = 0;
   uint8_t* curPos = 0;
   while (totalReadedBytes < DETECT_STREAM_BUFFER_SIZE &&
          lastReadRez != BufferedReader::DATA_EOF) {
      lastReadRez = 0;
      uint8_t* data =
          m_bufferedReader->readBlock(m_readerID, readedBytes, lastReadRez);
      ...
      for (curPos = data; curPos <= lastFrameAddr; curPos += TS_FRAME_SIZE) {
         if (!m2tsHdrDiscarded && m_m2tsMode) curPos += 4;
         ...
         */
         ...
         if (pid == 0) {  // PAT
            pat.deserialize(curPos + tsPacket->getHeaderSize(),
                            TS_FRAME_SIZE - tsPacket->getHeaderSize());
            if (firstPAT) {
               firstPAT = false;
               nonProcPMTPid = pat.pmtPids;
            }
         } else if (pat.pmtPids.find(pid) != pat.pmtPids.end()) {  // PMT
            if (tsPacket->payloadStart || pmtBufferLen > 0) {
               memcpy(pmtBuffer + pmtBufferLen,
                      curPos + tsPacket->getHeaderSize(),
                      TS_FRAME_SIZE - tsPacket->getHeaderSize());
               ...
               if (m_pmt.isFullBuff(pmtBuffer, pmtBufferLen)) {
                  ...
                  pmtBufferLen = 0;
                  for (PIDListMap::const_iterator itr = m_pmt.pidList.begin();
                       itr != m_pmt.pidList.end(); ++itr)
                     trackList.insert(std::make_pair(
                         itr->second.m_pid, TrackInfo(itr->second.m_streamType,
                                                      itr->second.m_lang, 0)));
                  nonProcPMTPid.erase(pid);
                  if (nonProcPMTPid.size() == 0 &&
                      !mvcContinueExpected()) {  // all pmt pids processed
                     ...
                     if (br)
                        ...
                        THROW(ERR_COMMON,
                              "Function TSDemuxer::getTrackList required "
                              "bufferedReader!");
                     return;
                  }
               }
   ...
   if (br)
      ...
      THROW(ERR_COMMON,
            "Function TSDemuxer::getTrackList required bufferedReader!");
   return;
}
...
bool TSDemuxer::checkForRealM2ts(uint8_t* buffer, uint8_t* end) {
   for (uint8_t* cur = buffer; cur < end; cur += 192)
      if (cur[4] != 0x47) return false;
   for (uint8_t* cur = buffer; cur < end; cur += 188)
      if (*cur != 0x47) {
         LTRACE(LT_WARN, 2,
                "Warning! The file " << m_streamName << " has a M2TS format.");
         return true;
      }
   return false;
}

int TSDemuxer::simpleDemuxBlock(DemuxedData& demuxedData,
                                const PIDSet& acceptedPIDs,
                                int64_t& discardSize) {
   if (m_firstDemuxCall) {
      for (set<uint32_t>::const_iterator itr = acceptedPIDs.begin();
   ...
   uint8_t* data = m_bufferedReader->readBlock(
       m_readerID, readedBytes, readRez, &isFirstBlock);  // blocked read mode
   if (readRez == BufferedFileReader::DATA_NOT_READY) {
      m_lastReadRez = readRez;
      return BufferedFileReader::DATA_NOT_READY;
   }
   if (readedBytes + m_tmpBufferLen == 0 ||
       readedBytes == 0 && m_lastReadRez == BufferedReader::DATA_EOF) {
      m_lastReadRez = readRez;
      return BufferedReader::DATA_EOF;
   }
   if (readedBytes > 0) m_bufferedReader->notify(m_readerID, readedBytes);
   m_lastReadRez = readRez;
   data += TS_FRAME_SIZE;
   if (m_tmpBufferLen > 0) {
   ...
   if (isFirstBlock) {
      if (m_curFileNum < m_mplsInfo.size())
         m_prevFileLen += (m_mplsInfo[m_curFileNum].OUT_time -
                           m_mplsInfo[m_curFileNum].IN_time) *
                          2;  // in 90Khz clock
      else
   ...
   if (m_pmt.pidList.size() == 0 || mvcContinueExpected()) {
      TS_program_association_section pat;
      for (m_curPos = data; m_curPos <= lastFrameAddr;
           m_curPos += TS_FRAME_SIZE) {
         if (!forpmtm2tsHdrDiscarded && m_m2tsMode) m_curPos += 4;
         while (*m_curPos != 0x47 && m_curPos <= lastFrameAddr) m_curPos++;
         if (m_curPos > lastFrameAddr) break;
         forpmtm2tsHdrDiscarded = false;

         TSPacket* tsPacket = (TSPacket*)m_curPos;
         int pid = tsPacket->getPID();
         if (pid == 0) {  // PAT
            pat.deserialize(m_curPos + tsPacket->getHeaderSize(),
                            TS_FRAME_SIZE - tsPacket->getHeaderSize());
         } else if (pat.pmtPids.find(pid) != pat.pmtPids.end()) {  // PMT
            if (tsPacket->payloadStart || pmtBufferLen > 0) {
               memcpy(pmtBuffer + pmtBufferLen,
                      m_curPos + tsPacket->getHeaderSize(),
                      TS_FRAME_SIZE - tsPacket->getHeaderSize());
               pmtBufferLen += TS_FRAME_SIZE - tsPacket->getHeaderSize();
               if (m_pmt.isFullBuff(pmtBuffer, pmtBufferLen)) {
                  m_pmt.deserialize(pmtBuffer, pmtBufferLen);
   ...
   for (m_curPos = data; m_curPos <= lastFrameAddr; m_curPos += TS_FRAME_SIZE) {
      ...
      if (pesStartCode) {
         ...
         int payloadLen =
             pesPacket->getPacketLength() - pesPacket->getHeaderLength();

         PIDListMap::iterator streamInfo = m_pmt.pidList.find(pid);

         if ((pesPacket->flagsLo & 0x80) == 0x80) {  // 2880 - ac3 frame len
    ...
    static int64_t prevDts = 0;
                    ...
                    uint64_t tmpDts = ((pesPacket->flagsLo & 0xc0) == 0xc0) ?
    pesPacket->getDts() : pesPacket->getPts(); int32_t dtsDif = (int64_t)tmpDts
    - prevDts; uint8_t* afterPesData = frameData + pesPacket->getHeaderLength()
    - 1; LTRACE(LT_INFO, 2, "PID=" << pid << "  PTS: " << pesPacket->getPts()
    ...
    */
            ...
            if (m_lastPTS == -1 || curPts > m_lastPTS) m_lastPTS = curPts;

            if (m_firstPTS == -1 || curPts < m_firstPTS) m_firstPTS = curPts;
         ...
         if (streamInfo != m_pmt.pidList.end() &&
             streamInfo->second.m_streamType !=
                 STREAM_TYPE_SUB_PGS)  // demux PGS with PES headers
      ...
      if (!m_acceptedPidCache[pid]) continue;
...
void TSDemuxer::setFileIterator(FileNameIterator* itr) {
   ...
   if (br)
      ...
      THROW(ERR_COMMON,
            "Can not set file iterator. Reader does not support bufferedReader "
            "interface.");
}

int64_t getLastPCR(File& file, int bufferSize, int frameSize,
                   int64_t fileSize) {
   ...
   while (curPtr <= bufferEnd - frameSize) {
      ...
      if (tsPacket->afExists && tsPacket->adaptiveField.length &&
          tsPacket->adaptiveField.pcrExist) {
...
int64_t getTSDuration(const char* fileName) {
   try {
      int frameSize = 188;
      if (isM2TSExt(fileName)) frameSize = 192;
      uint64_t fileSize;
