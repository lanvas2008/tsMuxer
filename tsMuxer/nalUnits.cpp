...
uint8_t BDROM_METADATA_GUID[] =
    "\x17\xee\x8c\x60\xf8\x4d\x11\xd9\x8c\xd6\x08\x00\x20\x0c\x9a\x66";
...
void NALUnit::write_byte_align_bits(BitStreamWriter& writer) {
   ...
   if (rest == 8) return;
   writer.putBit(1);
   if (rest > 1) writer.putBits(rest - 1, 0);
}
...
uint8_t* NALUnit::findNALWithStartCode(const uint8_t* buffer,
                                       const uint8_t* end,
                                       bool longCodesAllowed) {
...
int NALUnit::encodeNAL(uint8_t* srcBuffer, uint8_t* srcEnd, uint8_t* dstBuffer,
   ...
   for (srcBuffer += 2; srcBuffer < srcEnd;) {
      if (*srcBuffer > 3)
         ...
         if (dstBufferSize < (size_t)(srcBuffer - srcStart + 2)) return -1;
   ...
   if (dstBufferSize < srcEnd - srcStart) return -1;
   memcpy(dstBuffer, srcStart, srcEnd - srcStart);
   dstBuffer += srcEnd - srcStart;
   return dstBuffer - initDstBuffer;
}

int NALUnit::decodeNAL(const uint8_t* srcBuffer, const uint8_t* srcEnd,
                       uint8_t* dstBuffer, size_t dstBufferSize) {
   uint8_t* initDstBuffer = dstBuffer;
   ...
   for (srcBuffer += 3; srcBuffer < srcEnd;) {
      if (*srcBuffer > 3)
         ...
         if (dstBufferSize < (size_t)(srcBuffer - srcStart)) return -1;
...
int NALUnit::decodeNAL2(uint8_t* srcBuffer, uint8_t* srcEnd, uint8_t* dstBuffer,
                        size_t dstBufferSize, bool* keepSrcBuffer) {
   uint8_t* initDstBuffer = dstBuffer;
   uint8_t* srcStart = srcBuffer;
   *keepSrcBuffer = true;
   for (srcBuffer += 3; srcBuffer < srcEnd;) {
   ...
   if (!*keepSrcBuffer) memcpy(dstBuffer, srcStart, srcEnd - srcStart);
   dstBuffer += srcEnd - srcStart;
   return dstBuffer - initDstBuffer;
}
...
int NALUnit::extractUEGolombCode() {
   ...
   for (; bitReader.getBits(1) == 0; cnt++)
      ;
   if (cnt > INT_BIT) THROW_BITSTREAM_ERR;
...
int NALUnit::deserialize(uint8_t* buffer, uint8_t* end) {
   if (end == buffer) return NOT_ENOUGHT_BUFFER;

   // assert((*buffer & 0x80) == 0);
   if ((*buffer & 0x80) != 0) {
      LTRACE(LT_WARN, 0,
             "Invalid forbidden_zero_bit for nal unit " << (*buffer & 0x1f));
   }
...
int NALUnit::serializeBuffer(uint8_t* dstBuffer, uint8_t* dstEnd,
                             bool writeStartCode) const {
   if (m_nalBufferLen == 0) return 0;
   if (writeStartCode) {
      if (dstEnd - dstBuffer < 4) return -1;
      *dstBuffer++ = 0;
      *dstBuffer++ = 0;
      *dstBuffer++ = 0;
      *dstBuffer++ = 1;
   }
   int encodeRez = NALUnit::encodeNAL(m_nalBuffer, m_nalBuffer + m_nalBufferLen,
                                      dstBuffer, dstEnd - dstBuffer);
   if (encodeRez == -1)
      return -1;
   else
...
int ceil_log2(double val) {
   ...
   if (iVal - mask == 0 && frac == 0)
      return bits -
             1;  // For example: cail(log2(8.0)) = 3, but for 8.2 or 9.0 it's 4
   else
      return bits;
}
...
int NALDelimiter::deserialize(uint8_t* buffer, uint8_t* end) {
   ...
   if (rez != 0) return rez;
   if (end - buffer < 2) return NOT_ENOUGHT_BUFFER;
...
int PPSUnit::deserialize() {
   ...
   if (rez != 0) return rez;
   if (nalEnd - m_nalBuffer < 2) return NOT_ENOUGHT_BUFFER;
   try {
      ...
      /*
         ...
         num_slice_groups_minus1; iGroup++ ) run_length_minus1[iGroup] =
         extractUEGolombCode();
                      }
                      else if( slice_group_map_type  ==  2)
                      {
                              if (num_slice_groups_minus1 >= 256)
                                      THROW_BITSTREAM_ERR;
                              for( int iGroup = 0; iGroup <
         num_slice_groups_minus1; iGroup++ ) { top_left[ iGroup ] =
         extractUEGolombCode(); bottom_right[ iGroup ] = extractUEGolombCode();
                              }
                      }
         ...
         bitReader.getBits(1); slice_group_change_rate = extractUEGolombCode() +
         1; } else if( slice_group_map_type  ==  6 )
                      {
                              int pic_size_in_map_units_minus1 =
         extractUEGolombCode(); if (pic_size_in_map_units_minus1 >= 256)
                                      THROW_BITSTREAM_ERR;
                              for( int i = 0; i <= pic_size_in_map_units_minus1;
         i++ ) { int bits = ceil_log2( num_slice_groups_minus1 + 1 );
...
void PPSUnit::duplicatePPS(PPSUnit& oldPPS, int ppsID, bool cabac)
{
        ...
        m_nalBuffer = new uint8_t[oldPPS.m_nalBufferLen + 400]; // 4 bytes
reserved for new ppsID and cabac values m_nalBuffer[0] = oldPPS.m_nalBuffer[0];
        ...
        bitReader.setBuffer(oldPPS.m_nalBuffer+1, oldPPS.m_nalBuffer +
oldPPS.m_nalBufferLen); extractUEGolombCode(); extractUEGolombCode();
...
void HRDParams::resetDefault(bool mvc) {
   ...
   if (!mvc) {
      bit_rate_scale = 3;
      cpb_size_scale = 3;
      bit_rate_value_minus1[0] = 78124;  // 78124*512 = 40 mbit. It is max
                                         // allowed value for Blu-ray disks
      cpb_size_value_minus1[0] =
          234374;  // 234374*128 = 30 mbit per frame max. It is max allowed
                   // value for Blu-ray disks
   } else {
      bit_rate_scale = 2;
      cpb_size_scale = 3;
      bit_rate_value_minus1[0] = 234374;  // 60 mbit for MVC+AVC It is max
                                          // allowed value for Blu-ray disks
...
void SPSUnit::scaling_list(int* scalingList, int sizeOfScalingList,
...
int SPSUnit::deserialize() {
   ...
   */
   if (m_nalBufferLen < 4) return NOT_ENOUGHT_BUFFER;
   ...
   try {
      ...
      pic_size_in_map_units =
          pic_width_in_mbs *
          pic_height_in_map_units;  // * (2 - frame_mbs_only_flag);
      ...
      full_sps_bit_len =
          calcNalLenInBits(m_nalBuffer, m_nalBuffer + m_nalBufferLen);

      // m_pulldown = ((abs(getFPS() - 23.97) < FRAME_RATE_EPS) || (abs(getFPS()
      // - 59.94) < FRAME_RATE_EPS)) && pic_struct_present_flag;
...
int SPSUnit::deserializeSubSPS() {
   if (profile_idc == 83 || profile_idc == 86) {
      ...
      */
      ...
      if (bitReader.getBit() != 1) return INVALID_BITSTREAM_SYNTAX;
...
void SPSUnit::serializeHDRParameters(BitStreamWriter& writer,
   ...
   for (int SchedSelIdx = 0; SchedSelIdx <= params.cpb_cnt_minus1;
        SchedSelIdx++) {
...
void SPSUnit::insertHdrParameters() {
   for (int i = mvcNalHrdParams.size() - 1; i >= 0; --i) {
      if (!mvcNalHrdParams[i].isPresent || !mvcVclHrdParams[i].isPresent) {
         ...
         insertHrdData(mvcHdrParamsBitPos[i], nalBitLen, vclBitLen, false,
                       mvcNalHrdParams[i]);
   ...
   if (!nalHrdParams.isPresent || !vclHrdParams.isPresent) {
      ...
      if (hdrParamsBitPos != -1)
         insertHrdData(hdrParamsBitPos, nalBitLen, vclBitLen, false,
                       nalHrdParams);
      else
   ...
   if (!fixed_frame_rate_flag) updateTimingInfo();
}
...
void SPSUnit::updateTimingInfo() {
   ...
   writer.setBuffer(newNalBuffer + beforeBytes,
                    newNalBuffer + m_nalBufferLen + EXTRA_SPACE);
...
void SPSUnit::insertHrdData(int bitPos, int nal_hdr_len, int vcl_hdr_len,
                            bool addVuiHeader, const HRDParams& params) {
   // replace hrd parameters not implemented. only insert
   const int EXTRA_SPACE = 64;
...
int SPSUnit::getMaxBitrate() {
   if (nalHrdParams.bit_rate_value_minus1.empty() == 0)
      ...
      return (nalHrdParams.bit_rate_value_minus1[0] + 1)
             << (6 + nalHrdParams.bit_rate_scale);
}
...
int SPSUnit::getCropY() {
   if (chroma_format_idc == 0)
      return (2 - frame_mbs_only_flag) *
             (frame_crop_top_offset + frame_crop_bottom_offset);
   else {
      int SubHeightC = 1;
      if (chroma_format_idc == 1) SubHeightC = 2;
      return SubHeightC * (2 - frame_mbs_only_flag) *
             (frame_crop_top_offset + frame_crop_bottom_offset);
   }
}
...
int SPSUnit::getCropX() {
   if (chroma_format_idc == 0)
      ...
      int SubWidthC = 1;
      if (chroma_format_idc == 1 || chroma_format_idc == 2) SubWidthC = 2;
...
double SPSUnit::getFPS() const {
   if (num_units_in_tick != 0) {
      double tmp = time_scale / (float)num_units_in_tick /
                   2;  //(float)(frame_mbs_only_flag+1);
...
void SPSUnit::seq_parameter_set_mvc_extension() {
   ...
   for (int i = 0; i <= num_level_values_signalled_minus1; i++) {
      ...
      for (int j = 0; j <= num_applicable_ops_minus1[i]; j++) {
         ...
         int dummy =
             extractUEGolombCode();  // applicable_op_num_target_views_minus1[
                                     // i ][ j ]
...
void SPSUnit::mvc_vui_parameters_extension() {
   ...
   for (int i = 0; i < vui_mvc_num_ops; i++) {
      ...
      int vui_mvc_timing_info_present_flag =
          bitReader.getBit();  // vui_mvc_timing_info_present_flag[ i ]
      if (vui_mvc_timing_info_present_flag) {
         int vui_mvc_num_units_in_tick =
             bitReader.getBits(32);  // vui_mvc_num_units_in_tick[i]
      ...
      int picStructpresent =
          bitReader.getBit();  // vui_mvc_pic_struct_present_flag[i]
...
bool SliceUnit::isIFrame() const {
   return nal_unit_type == nuSliceExt ? anchor_pic_flag
                                      : slice_type == SliceUnit::I_TYPE;
}
...
int SliceUnit::deserializeSliceType(uint8_t* buffer, uint8_t* end) {
   ...
   try {
      ...
      if (nal_unit_type == nuSliceExt) {
         if (end - buffer < 5) return NOT_ENOUGHT_BUFFER;
         offset += 3;
      ...
      if (slice_type >= 5)
         slice_type -= 5;  // +5 flag is: all other slice at this picture must
                           // be same type
...
void NALUnit::updateBits(int bitOffset, int bitLen, int value) {
   ...
   uint8_t* ptr_end =
       (uint8_t*)bitReader.getBuffer() + (bitOffset + bitLen) / 8;
...
int SliceUnit::deserializeSliceHeader(
    const std::map<uint32_t, SPSUnit*>& spsMap,
    const std::map<uint32_t, PPSUnit*>& ppsMap) {
   first_mb_in_slice = extractUEGolombCode();
   orig_slice_type = slice_type = extractUEGolombCode();
   if (slice_type >= 5)
      slice_type -=
          5;  // +5 flag is: all other slice at this picture must be same type
   pic_parameter_set_id = extractUEGolombCode();
   std::map<uint32_t, PPSUnit*>::const_iterator itr =
       ppsMap.find(pic_parameter_set_id);
   if (itr == ppsMap.end()) return SPS_OR_PPS_NOT_READY;
   pps = itr->second;

   std::map<uint32_t, SPSUnit*>::const_iterator itr2 =
       spsMap.find(pps->seq_parameter_set_id);
   if (itr2 == spsMap.end()) return SPS_OR_PPS_NOT_READY;
   sps = itr2->second;
   ...
   if (sps->frame_mbs_only_flag == 0) {
      ...
      if (m_field_pic_flag) bottom_field_flag = bitReader.getBits(1);
   }
   if (isIDR()) idr_pic_id = extractUEGolombCode();
   m_picOrderBitPos = -1;
...
void SEIUnit::deserialize(SPSUnit& sps, int orig_hrd_parameters_present_flag) {
   ...
   try {
      ...
      while (curBuff < nalEnd - 1) {
         ...
         payloadSize += *curBuff++;
         if (curBuff >= nalEnd) return;
         sei_payload(sps, payloadType, curBuff, payloadSize,
                     orig_hrd_parameters_present_flag);
...
int SEIUnit::isMVCSEI() {
   ...
   try {
      int rez = NALUnit::deserialize(m_nalBuffer, nalEnd);
      if (rez != 0) return NOT_ENOUGHT_BUFFER;
      uint8_t* curBuff = m_nalBuffer + 1;
      while (curBuff < nalEnd - 1) {
         int payloadType = 0;
         for (; *curBuff == 0xFF && curBuff < nalEnd; curBuff++)
            payloadType += 0xFF;
         if (curBuff >= nalEnd) return NOT_ENOUGHT_BUFFER;
         payloadType += *curBuff++;
         if (curBuff >= nalEnd) return NOT_ENOUGHT_BUFFER;
         ...
         payloadSize += *curBuff++;
         if (curBuff >= nalEnd) return NOT_ENOUGHT_BUFFER;
         if (payloadType == 37) return 1;  // mvc scalable nesting message
         curBuff += payloadSize;
      }
   } catch (BitStreamException) {
      ;
...
void SEIUnit::sei_payload(SPSUnit& sps, int payloadType, uint8_t* curBuff,
                          int payloadSize,
   ...
   if (payloadType == 0)
      ...
      mvc_scalable_nesting(sps, curBuff, payloadSize,
                           orig_hrd_parameters_present_flag);
   else
...
void SEIUnit::serialize_pic_timing_message(const SPSUnit& sps,
                                           BitStreamWriter& writer,
                                           bool seiHeader) {
   if (seiHeader) {
   ...
   if (sps.nalHrdParams.isPresent || sps.vclHrdParams.isPresent) {
      ...
      writer.putBits(sps.nalHrdParams.cpb_removal_delay_length_minus1 + 1,
                     cpb_removal_delay);
      writer.putBits(sps.nalHrdParams.dpb_output_delay_length_minus1 + 1,
                     dpb_output_delay);
   }
...
void SEIUnit::serialize_buffering_period_message(const SPSUnit& sps,
                                                 BitStreamWriter& writer,
                                                 bool seiHeader) {
   if (seiHeader) {
      writer.putBits(8, nuSEI);
   ...
   if (sps.nalHrdParams.isPresent) {  // NalHrdBpPresentFlag
      for (int SchedSelIdx = 0; SchedSelIdx <= sps.nalHrdParams.cpb_cnt_minus1;
           SchedSelIdx++) {
         writer.putBits(
             sps.nalHrdParams.initial_cpb_removal_delay_length_minus1 + 1,
   ...
   if (sps.vclHrdParams.isPresent) {  // NalHrdBpPresentFlag
      for (int SchedSelIdx = 0; SchedSelIdx <= sps.vclHrdParams.cpb_cnt_minus1;
           SchedSelIdx++) {
         writer.putBits(
             sps.vclHrdParams.initial_cpb_removal_delay_length_minus1 + 1,
             initial_cpb_removal_delay[SchedSelIdx]);
         writer.putBits(
             sps.vclHrdParams.initial_cpb_removal_delay_length_minus1 + 1,
             initial_cpb_removal_delay_offset[SchedSelIdx]);
...
void SEIUnit::pic_timing(SPSUnit& sps, uint8_t* curBuff, int payloadSize,
...
void SEIUnit::pic_timing(SPSUnit& sps, bool orig_hrd_parameters_present_flag) {
   ...
   if (CpbDpbDelaysPresentFlag) {
      cpb_removal_delay = bitReader.getBits(
          sps.nalHrdParams.cpb_removal_delay_length_minus1 + 1);
      dpb_output_delay = bitReader.getBits(
          sps.nalHrdParams.dpb_output_delay_length_minus1 + 1);

      // LTRACE(LT_INFO, 2, "got PicTimings " << cpb_removal_delay << "x" <<
      // dpb_output_delay);
   }
   if (sps.pic_struct_present_flag) {
              ...
              if( clock_timestamp_flag[i] ) {
                      ...
                      if( full_timestamp_flag ) {
                              ...
                              if( seconds_flag ) {
                                      ...
                                      if( minutes_flag ) {
                                              minutes_value =
      bitReader.getBits(6); hours_flag = bitReader.getBit(); if( hours_flag )
                                                      hours_value =
      bitReader.getBits(5);
                                      }
                              }
                      ...
                      if( time_offset_length > 0 ) {
                              //time_offset	=
      bitReader.getBits(time_offset_length); // signed bits!
                      }
              }
      }
      */
...
void SEIUnit::mvc_scalable_nesting(SPSUnit& sps, uint8_t* curBuf, int size,
                                   int orig_hrd_parameters_present_flag) {
   try {
      ...
      if (!operation_point_flag) {
         ...
         for (int i = 0; i <= num_view_components_op_minus1; i++) {
            ...
            int sei_op_temporal_id =
                bitReader.getBits(3);  // sei_op_temporal_id
            sei_op_temporal_id = sei_op_temporal_id;
         }
      }
      int byteBits = bitReader.getBitsCount() % 8;
      if (byteBits) bitReader.skipBits(8 - byteBits);  // byte align
     ...
     LTRACE(LT_INFO, 2, "got BufferingPeriod " << initialDelay << "x" <<
     initialDelayOffset);
         */
         buffering_period(payloadSize);
         m_processedMessages.insert(payloadType);
...
void SEIUnit::processBlurayOffsetMetadata() {
   ...
   bitReader.skipBits(18);  // skip: marker_bit, PTS[14..0], marker_bit,
                            // reserved_for_future_use bit
