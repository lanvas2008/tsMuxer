...
void HRDParams::resetDefault(bool mvc) {
  ...
  if (!mvc) {
    bit_rate_scale = 3;
    cpb_size_scale = 3;
    bit_rate_value_minus1[0] = 78124;  // 78124*512 = 40 mbit. It is max allowed
                                       // value for Blu-ray disks
    cpb_size_value_minus1[0] =
        234374;  // 234374*128 = 30 mbit per frame max. It is max allowed value
                 // for Blu-ray disks
  } else {
    bit_rate_scale = 2;
    cpb_size_scale = 3;
    bit_rate_value_minus1[0] = 234374;  // 60 mbit for MVC+AVC It is max allowed
                                        // value for Blu-ray disks
...
int SliceUnit::deserializeSliceHeader(
  ...
  if (sps->separate_colour_plane_flag) bitReader.getBits(2);  // colour_plane_id
...
void SEIUnit::pic_timing(SPSUnit& sps, bool orig_hrd_parameters_present_flag) {
  ...
  if (CpbDpbDelaysPresentFlag) {
    cpb_removal_delay =
        bitReader.getBits(sps.nalHrdParams.cpb_removal_delay_length_minus1 + 1);
    dpb_output_delay =
        bitReader.getBits(sps.nalHrdParams.dpb_output_delay_length_minus1 + 1);
