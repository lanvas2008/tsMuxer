...
uint8_t* MP3Codec::mp3FindFrame(uint8_t* buff, uint8_t* end) {
  ...
  for (uint8_t* cur = buff; cur < end - 4; cur++) {
    ...
    // layer check
    if ((header & (3 << 17)) == 0) continue;
    // bit rate
    if ((header & (0xf << 12)) == 0xf << 12) continue;
    // frequency
    if ((header & (3 << 10)) == 3 << 10) continue;
    return cur;
  }
  return 0;
}
...
int MP3Codec::mp3DecodeFrame(uint8_t* buff, uint8_t* end) {
  ...
  if (m_layer == 4) return 0;
  /* extract frequency */
  m_sample_rate_index = (header >> 10) & 3;
  if (m_sample_rate_index == 3) return 0;  // invalid sample rate
  ...
  if (m_bitrate_index == 15) return 0;  // invalid bitrate index
  padding = (header >> 9) & 1;
