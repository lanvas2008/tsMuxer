...
static const int dca_ext_audio_descr_mask[] = {
    DCA_EXT_XCH, -1, DCA_EXT_X96,  DCA_EXT_XCH | DCA_EXT_X96,
    -1,          -1, DCA_EXT_XXCH, -1,
};
...
void DTSStreamReader::writePESExtension(PESPacket* pesPacket,
                                        const AVPacket& avPacket) {
  // 0f 81 71 - from blu ray DTS-HD  ( can use 0x01 instead 0x0f. bits 1-3 are
  // reserved.)
...
int DTSStreamReader::getSurroundModeCode() {
  int rez = 0;
  if ((pi_channels_conf & AOUT_CHAN_LEFT) &&
      (pi_channels_conf & AOUT_CHAN_RIGHT) &&
      (pi_channels_conf & AOUT_CHAN_CENTER) &&
      (pi_channels_conf & AOUT_CHAN_REARLEFT) &&
      (pi_channels_conf & AOUT_CHAN_REARRIGHT))
    rez = 0x09;
  else if ((pi_channels_conf & AOUT_CHAN_LEFT) &&
           (pi_channels_conf & AOUT_CHAN_RIGHT) &&
           (pi_channels_conf & AOUT_CHAN_REARLEFT) &&
           (pi_channels_conf & AOUT_CHAN_REARRIGHT))
    rez = 0x08;
...
uint8_t* DTSStreamReader::findFrame(uint8_t* buff, uint8_t* end) {
  ...
  while (end - buff >= 16) {
    ...
    if (hdrType == AUPRINFO || hdrType == BITSHVTB || hdrType == BLACKOUT ||
        hdrType == BRANCHPT || hdrType == BUILDVER || hdrType == CORESSMD ||
        hdrType == EXTSS_MD || hdrType == FILEINFO || hdrType == NAVI_TBL ||
        hdrType == TIMECODE || hdrType == DTSHDHDR) {
      if (buff + hdrSize > end) return 0;  // need more data
      buff += hdrSize;
  ...
  for (uint8_t* p_buf = buff; p_buf < end - 4; p_buf++) {
    if (p_buf < end - 6) {
      if (p_buf[0] == 0xff && p_buf[1] == 0x1f && p_buf[2] == 0x00 &&
          p_buf[3] == 0xe8 && (p_buf[4] & 0xf0) == 0xf0 && p_buf[5] == 0x07) {
        return p_buf;
      }
      // 14 bits, big endian version of the bitstream
      else if (p_buf[0] == 0x1f && p_buf[1] == 0xff && p_buf[2] == 0xe8 &&
               p_buf[3] == 0x00 && p_buf[4] == 0x07 &&
               (p_buf[5] & 0xf0) == 0xf0) {
        return p_buf;
      }
    }
    // 16 bits, big endian version of the bitstream
    if (p_buf[0] == 0x7f && p_buf[1] == 0xfe && p_buf[2] == 0x80 &&
        p_buf[3] == 0x01) {
      return p_buf;
    }
    // 16 bits, little endian version of the bitstream
    else if (p_buf[0] == 0xfe && p_buf[1] == 0x7f && p_buf[2] == 0x01 &&
             p_buf[3] == 0x80) {
      return p_buf;
    }
  }
...
int DTSStreamReader::decodeHdInfo(uint8_t* buff, uint8_t* end) {
  try {
    ...
    if (m_hdType == DTS_SUBTYPE_UNINITIALIZED) {
      if (buff + headerSize + 4 > end) return NOT_ENOUGHT_BUFFER;
    ...
    if (bStaticFieldsPresent) {
      ...
      if (pi_frame_length == 0) pi_frame_length = nuExSSFrameDurationCode * 512;
    ...
    for (int i = 0; i < nuNumAssets; i++) {
      ...
      if (bStaticFieldsPresent) {
        ...
        if (m_hdType != DTS_SUBTYPE_MASTER_AUDIO)
          m_hdBitrate =
              hd_pi_sample_rate / (double)pi_frame_length * hdFrameSize * 8;
...
bool DTSStreamReader::isPriorityData(AVPacket* packet) {
  return packet->size >= 2 &&
         ((packet->data[0] == 0x7f && packet->data[1] == 0xfe) ||
          (packet->data[0] == 0xfe && packet->data[1] == 0x7f));
}

int DTSStreamReader::decodeFrame(uint8_t* buff, uint8_t* end, int& skipBytes,
                                 int& skipBeforeBytes) {
  uint8_t* afterFrameData = buff;
  i_frame_size = 0;
  if (m_isCoreExists) {
    skipBeforeBytes = skipBytes = 0;
    if (m_state == stDecodeHD &&
        *((uint32_t*)buff) == my_htonl(DTS_HD_PREFIX)) {
      m_state = stDecodeHD2;
      return m_hdFrameLen;
    }
    m_state = stDecodeDTS;
    ...
    // if (m_state == stDecodeDTS)
    {
      ...
      if (buff[0] == 0xff && buff[1] == 0x1f && buff[2] == 0x00 &&
          buff[3] == 0xe8 && (buff[4] & 0xf0) == 0xf0 && buff[5] == 0x07) {
      ...
      // 14 bits, big endian version of the bitstream
      else if (buff[0] == 0x1f && buff[1] == 0xff && buff[2] == 0xe8 &&
               buff[3] == 0x00 && buff[4] == 0x07 && (buff[5] & 0xf0) == 0xf0) {
        uint8_t conv_buf[DTS_HEADER_SIZE];
      ...
      // 16 bits, big endian version of the bitstream
      else if (buff[0] == 0x7f && buff[1] == 0xfe && buff[2] == 0x80 &&
               buff[3] == 0x01) {
        i_frame_size = syncInfo16be(buff);
      }
      // 16 bits, little endian version of the bitstream
      else if (buff[0] == 0xfe && buff[1] == 0x7f && buff[2] == 0x01 &&
               buff[3] == 0x80) {
        uint8_t conv_buf[DTS_HEADER_SIZE];
      ...
      if (pi_sample_rate_index >=
          sizeof(ppi_dts_samplerate) / sizeof(ppi_dts_samplerate[0])) {
        return 0;
      }
      pi_sample_rate = ppi_dts_samplerate[pi_sample_rate_index];
      if (!pi_sample_rate) return 0;

      if (pi_bit_rate_index >=
          sizeof(ppi_dts_bitrate) / sizeof(ppi_dts_bitrate[0])) {
        return 0;
      }
    ...
    if (afterFrameData > end - 4) return NOT_ENOUGHT_BUFFER;
    ...
    if (m_testMode && m_dtsEsChannels == 0) {
      ...
      for (int i = 0; i < findSize; ++i) {
        if (*curPtr32++ == 0x5a5a5a5a) {
          ...
          int frameSize = (int(exHeader[0]) << 2) + (exHeader[1] >> 6) -
                          4;  // remove 4 bytes of ext world
  ...
  if (*(uint32_t*)afterFrameData == my_htonl(DTS_HD_PREFIX)) {
    ...
    if (hdFrameSize == NOT_ENOUGHT_BUFFER) return NOT_ENOUGHT_BUFFER;

    uint8_t* nextFrame = afterFrameData + hdFrameSize;
    if (nextFrame >= end) return NOT_ENOUGHT_BUFFER;
...
int DTSStreamReader::buf14To16(uint8_t* p_out, const uint8_t* p_in, int i_in,
                               int i_le) {
...
bool DTSStreamReader::needSkipFrame(const AVPacket& packet) {
  if (m_skippingSamples == 0) return false;

  if (getFrameDurationNano() > 0) m_skippingSamples -= pi_frame_length;
  return true;
}
...
const std::string DTSStreamReader::getStreamInfo() {
  ...
  if (m_hdType == DTS_SUBTYPE_MASTER_AUDIO) str << "core + MLP data.";
  str << "Sample Rate: "
      << (hd_pi_sample_rate ? hd_pi_sample_rate : pi_sample_rate) / 1000
      << "KHz  ";
  str << "Channels: ";
  if (hd_pi_channels) {
    if (hd_pi_lfeCnt)
    ...
    if (pi_lfeCnt)
      str << (int)(pi_channels + m_dtsEsChannels - pi_lfeCnt) << '.'
          << pi_lfeCnt;
    else
