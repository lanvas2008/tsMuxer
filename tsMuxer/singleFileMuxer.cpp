...
void SingleFileMuxer::intAddStream(const std::string& streamName,
  ...
  /*
  if (codecName[0] == 'A') {
          // Used for "more correct" switching between the boundaries of Blu-Ray
  disc files
...
void SingleFileMuxer::writeOutBuffer(StreamInfo* streamInfo) {
  ...
  // if (lpcmReader && streamInfo->m_totalWrited >= 0x0ffffffful)
  {
    ...
    if (rename(streamInfo->m_fileName.c_str(), newName.c_str()) != 0)
      THROW(ERR_COMMON, "Can't rename file " << streamInfo->m_fileName << " to "
                                             << newName);
    streamInfo->m_part++;
    int systemFlags = 0;
...
void SingleFileMuxer::parseMuxOpt(const std::string& opts) {
  ...
  for (int i = 0; i < params.size(); i++) {
    ...
    if (paramPair[0] == "--split-duration") {
      LTRACE(
          LT_WARN, 2,
          "Warning! Splitting does not implemented for demux mode. Parameter "
              << paramPair[0] << " ignored.");
    }
