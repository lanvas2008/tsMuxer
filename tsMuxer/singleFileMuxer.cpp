...
std::string getNewName(const std::string& oldName, int cnt) {
   if (strEndWith(oldName, ".wav"))
      return oldName.substr(0, oldName.size() - 4) + "." + int32ToStr(cnt) +
             ".wav";
   else
...
SingleFileMuxer::SingleFileMuxer(MuxerManager* owner)
    : AbstractMuxer(owner), m_lastIndex(-1) {}

SingleFileMuxer::~SingleFileMuxer() {
   for (map<int, StreamInfo*>::iterator itr = m_streamInfo.begin();
...
void SingleFileMuxer::intAddStream(const std::string& streamName,
   ...
   /*
   if (codecName[0] == 'A') {
           // Used for "more correct" switching between the boundaries of
   Blu-Ray disc files
           ((SimplePacketizerReader*)
   codecReader)->setMPLSInfo(m_mplsParser.m_playItems);
   }
   */
...
void SingleFileMuxer::openDstFile() {
   ...
   //	THROW(ERR_CANT_CREATE_FILE, "Can't create output directory " <<
   // dstFileName);
   int systemFlags = 0;
#ifdef WIN32
   ...
   for (map<int, StreamInfo*>::iterator itr = m_streamInfo.begin();
      ...
      if (!itr->second->m_file.open(itr->second->m_fileName.c_str(),
                                    File::ofWrite, systemFlags))
         THROW(ERR_CANT_CREATE_FILE,
               "Can't create output file " << itr->second->m_fileName);
   }
}
...
void SingleFileMuxer::writeOutBuffer(StreamInfo* streamInfo) {
   ...
   if (streamInfo->m_bufLen >= blockSize) {
      ...
      if (m_owner->isAsyncMode()) {
         ...
         memcpy(newBuf, streamInfo->m_buffer + toFileLen,
                streamInfo->m_bufLen - toFileLen);
         m_owner->asyncWriteBuffer(this, streamInfo->m_buffer, toFileLen,
                                   &streamInfo->m_file);
         streamInfo->m_buffer = newBuf;
      } else {
         m_owner->syncWriteBuffer(this, streamInfo->m_buffer, toFileLen,
                                  &streamInfo->m_file);
         memcpy(streamInfo->m_buffer, streamInfo->m_buffer + toFileLen,
                streamInfo->m_bufLen - toFileLen);
      }
   ...
   LPCMStreamReader* lpcmReader =
       dynamic_cast<LPCMStreamReader*>(streamInfo->m_codecReader);
   ...
   // if (lpcmReader && streamInfo->m_totalWrited >= 0x0ffffffful)
   {
      ...
      if (m_owner->isAsyncMode()) systemFlags += FILE_FLAG_NO_BUFFERING;
#endif
      if (!streamInfo->m_file.open(streamInfo->m_fileName.c_str(),
                                   File::ofWrite + systemFlags))
...
bool SingleFileMuxer::muxPacket(AVPacket& avPacket) {
   if (avPacket.data == 0 || avPacket.size == 0) return true;
   ...
   memcpy(streamInfo->m_buffer + streamInfo->m_bufLen, avPacket.data,
          avPacket.size);
...
bool SingleFileMuxer::doFlush() {
   for (map<int, StreamInfo*>::iterator itr = m_streamInfo.begin();
      ...
      unsigned lastBlockSize =
          streamInfo->m_bufLen & 0xffff;  // last 64K of data
      ...
      if (m_owner->isAsyncMode()) {
         if (lastBlockSize > 0) {
            ...
            m_owner->asyncWriteBuffer(this, streamInfo->m_buffer, roundBufLen,
                                      &streamInfo->m_file);
         ...
         m_owner->syncWriteBuffer(this, streamInfo->m_buffer, roundBufLen,
                                  &streamInfo->m_file);
         memcpy(streamInfo->m_buffer, streamInfo->m_buffer + roundBufLen,
                lastBlockSize);
      }
...
bool SingleFileMuxer::close() {
   for (map<int, StreamInfo*>::iterator itr = m_streamInfo.begin();
        itr != m_streamInfo.end(); ++itr) {
      StreamInfo* streamInfo = itr->second;
      if (!streamInfo->m_file.close()) return false;
      if (streamInfo->m_bufLen > 0) {
         if (!streamInfo->m_file.open(streamInfo->m_fileName.c_str(),
                                      File::ofWrite + File::ofAppend))
            return false;
         if (!streamInfo->m_file.write(streamInfo->m_buffer,
                                       streamInfo->m_bufLen))
            return false;
         if (streamInfo->m_codecReader)
            if (!streamInfo->m_codecReader->beforeFileCloseEvent(
                    streamInfo->m_file))
               return false;
         if (!streamInfo->m_file.close()) return false;
...
void SingleFileMuxer::parseMuxOpt(const std::string& opts) {
   ...
   for (int i = 0; i < params.size(); i++) {
      ...
      if (paramPair[0] == "--split-duration") {
         LTRACE(LT_WARN, 2,
                "Warning! Splitting does not implemented for demux mode. "
                "Parameter "
                    << paramPair[0] << " ignored.");
      }
