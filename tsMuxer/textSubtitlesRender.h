...
const static double BROWSER_FONT_STYLE_INC_COEFF =
...
struct YUVQuad {
   ...
   bool operator==(const YUVQuad& second) const {
      return Y == second.Y && Cr == second.Cr && Cb == second.Cb &&
             alpha == second.alpha;
   }
   bool operator<(const YUVQuad& second) const {
      if (Y != second.Y)
...
struct Font {
   ...
   bool operator!=(const Font& other) const {
      return m_name != other.m_name || m_size != other.m_size ||
             m_opts != other.m_opts || m_borderWidth != other.m_borderWidth ||
             m_charset != other.m_charset || m_color != other.m_color ||
...
class TextSubtitlesRender {
  ...
  protected:
   ...
   std::vector<std::pair<Font, std::wstring>> processTxtLine(
       const std::wstring& line, std::vector<Font>& fontStack);
