...
void MPEGStreamReader::setBuffer(uint8_t* data, int dataLen, bool lastBlock) {
   if (lastBlock) m_eof = true;

   if (m_tmpBufferLen + dataLen > TMP_BUFFER_SIZE)
      THROW(ERR_COMMON_SMALL_BUFFER,
            "Not enought buffer for parse video stream. Current frame num "
                << m_totalFrameNum);
...
int MPEGStreamReader::flushPacket(AVPacket& avPacket) {
   ...
   avPacket.dts = m_curDts + m_timeOffset -
                  m_pcrIncPerFrame * getFrameDepth();  // shift dts back
...
int MPEGStreamReader::readPacket(AVPacket& avPacket) {
   ...
   if (!m_syncToStream) {
      uint8_t* nal =
          NALUnit::findNALWithStartCode(m_curPos, m_bufEnd, m_longCodesAllowed);
      if (nal != m_bufEnd) {
         m_syncToStream = true;
         m_curPos = nal;
      } else
      ...
      prevPos = m_curPos;
      if (!m_syncToStream) return NEED_MORE_DATA;
   }

   uint8_t* nextNal = NALUnit::findNALWithStartCode(
       std::min(m_curPos + 3, m_bufEnd), m_bufEnd, m_longCodesAllowed);
   if (nextNal == m_bufEnd) {
      storeBufferRest();
   ...
   if (isNal) {
      ...
      while (1) {
         ...
         if (rez != UNIT_SKIPPED) break;
         uint8_t* nal = NALUnit::findNALWithStartCode(
             m_curPos + isNal, m_bufEnd, m_longCodesAllowed);
         // assert(nal < findEnd); // if unit is skipped, next unit MUST be in
         // buffer
      ...
      if (prevDts != m_curDts) {
         ...
         avPacket.pts = m_curPts + m_timeOffset;
         avPacket.dts =
             m_curDts + m_timeOffset -
             m_pcrIncPerFrame * getFrameDepth();  // shift dts back to 1 frame;
         if (isIFrame()) avPacket.flags |= AVPacket::IS_IFRAME;
         if (isPriorityData(&avPacket))
            avPacket.flags |= AVPacket::PRIORITY_DATA;
         if (m_spsPpsFound) avPacket.flags |= AVPacket::IS_SPS_PPS_IN_GOP;

         return 0;  // return zero AV packet for new frame
      }
   }
   uint8_t* findEnd = std::min(m_bufEnd, m_curPos + MAX_AV_PACKET_SIZE);
   uint8_t* nal = NALUnit::findNALWithStartCode(m_curPos + isNal, findEnd,
                                                m_longCodesAllowed);
   ...
   avPacket.dts = m_curDts + m_timeOffset -
                  m_pcrIncPerFrame * getFrameDepth();  // shift dts back
   if (isIFrame()) avPacket.flags |= AVPacket::IS_IFRAME;
   if (isPriorityData(&avPacket)) avPacket.flags |= AVPacket::PRIORITY_DATA;
   if (m_spsPpsFound) avPacket.flags |= AVPacket::IS_SPS_PPS_IN_GOP;
...
int MPEGStreamReader::bufFromNAL() {
   if (m_bufEnd - m_curPos < (3 + (m_longCodesAllowed ? 1 : 0))) return 0;
...
/*
int MPEGStreamReader::bufFromNAL(const uint8_t* buff, const uint8_t* bufEnd,
bool longCodesAllowed) { if (bufEnd - buff < 4) return false; if (buff[0] == 0
&& buff[1] == 0) { if (longCodesAllowed && buff[2] == 0 && buff[3] == 1 ) return
4; else if (buff[2] == 1) return 3; else return 0;
        }
        else
                return 0;
}
...
int MPEGStreamReader::decodeNal(uint8_t* buff) {
   ...
   if (buff > m_lastDecodedPos) {
      ...
      if (m_lastDecodedPos < buff) m_lastDecodedPos = buff;
   }
...
void MPEGStreamReader::updateFPS(void* curNALUnit, uint8_t* buff,
   ...
   if (spsFps == 0 && m_fps == 0) {
      ...
      LTRACE(
          ...
          "This " << getCodecInfo().displayName
                  << " stream doesn't contain fps value. Muxing fps is absent "
                     "too. Set muxing FPS to default 25.0 value.");
   } else if (m_fps == 0) {
      setFPS(spsFps);
      LTRACE(LT_INFO, 2,
             getCodecInfo().displayName
                 << " muxing fps is not set. Get fps from stream. Value: "
                 << spsFps);
      ...
      if (m_isFirstFpsWarn) {
         LTRACE(
             ...
             getCodecInfo().displayName
                 ...
                 << getCodecInfo().displayName << " fps from " << spsFps
                 << " to " << m_fps);
      ...
      if (m_isFirstFpsWarn) {
         LTRACE(LT_INFO, 2,
                getCodecInfo().displayName
                    << " stream doesn't contain fps field. Muxing fps="
                    << m_fps);
         m_isFirstFpsWarn = false;
      }
   }
...
void MPEGStreamReader::checkPulldownSync() {
   ...
   if (m_testPulldownDts != 0 &&
       asyncValue > MAX_PULLDOWN_ASYNC * m_pulldownWarnCnt) {
      LTRACE(
          LT_ERROR, 2,
          "Warning! Source stream contain irregular pulldown marks. Mistiming "
          "between original fps and fps/1.25(without pulldown) exceed "
...
void MPEGStreamReader::fillAspectBySAR(double sar) {
   if (m_streamAR == AR_KEEP_DEFAULT) {
      ...
      const static double base_ar[] = {0.0, 1.0, 4.0 / 3.0, 16.0 / 9.0,
                                       221.0 / 100.0};
