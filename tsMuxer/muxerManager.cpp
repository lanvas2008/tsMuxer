...
void MuxerManager::preinitMux(const std::string& outFileName,
  ...
  if (m_subMuxer) {
    ...
    m_subMuxer->setBlockMuxMode(SUB_INTERLEAVE_BLOCKSIZE - MAX_FRAME_SIZE,
                                BLURAY_SECTOR_SIZE);
    if (m_mainMuxer)
      m_mainMuxer->setBlockMuxMode(MAIN_INTERLEAVE_BLOCKSIZE - MAX_FRAME_SIZE,
                                   BLURAY_SECTOR_SIZE);
  }
...
void MuxerManager::asyncWriteBuffer(AbstractMuxer* muxer, uint8_t* buff,
  ...
  if (m_interleave && muxer == m_mainMuxer) {
    // do interlieave of SSIF blocks. Place sub channel blocks first, delay main
    // muxer blocks
  ...
  for (size_t i = 0; i < params.size(); i++) {
    ...
    if (paramPair[0] == "--start-time" && paramPair.size() > 1) {
      if (paramPair[1].find(":") != string::npos)
        ...
        m_ptsOffset =
            strToInt64u(paramPair[1].c_str()) * 2;  // source in a 45Khz clock
