...
MuxerManager::MuxerManager(const BufferedReaderManager& readManager,
                           AbstractMuxerFactory& factory)
...
void MuxerManager::preinitMux(const std::string& outFileName,
   ...
   for (vector<StreamInfo>::iterator itr = ci.begin(); itr != ci.end(); ++itr) {
      ...
      H264StreamReader* h264Reader =
          dynamic_cast<H264StreamReader*>(si.m_streamReader);
      if (h264Reader) {
         h264Reader->setStartPTS(m_ptsOffset);
         if (firstH264Track) {
            if (si.m_isSubStream) mvcTrackFirst = true;
      ...
      if (si.m_isSubStream && m_allowStereoMux) {
         if (!m_subMuxer) {
            ...
            if (tsMuxer) tsMuxer->setPtsOffset(m_ptsOffset);
   ...
   if (m_subMuxer) {
      ...
      if (strEndWith(strToLowerCase(outFileName), ".ssif")) {
         ...
         m_subMuxer->setFileName(m_subMuxer->getNextName(outFileName),
                                 fileFactory);
         if (fileFactory && fileFactory->isVirtualFS()) m_interleave = true;
      }
      m_subMuxer->setBlockMuxMode(SUB_INTERLEAVE_BLOCKSIZE - MAX_FRAME_SIZE,
                                  BLURAY_SECTOR_SIZE);
      if (m_mainMuxer)
         m_mainMuxer->setBlockMuxMode(
             MAIN_INTERLEAVE_BLOCKSIZE - MAX_FRAME_SIZE, BLURAY_SECTOR_SIZE);
   }
   ...
   for (vector<StreamInfo>::iterator itr = ci.begin(); itr != ci.end(); ++itr) {
      ...
      if (si.m_isSubStream && m_allowStereoMux) {
         ...
         m_subMuxer->intAddStream(si.m_fullStreamName, si.m_codec,
                                  si.m_streamReader->getStreamIndex(),
                                  si.m_addParams, si.m_streamReader);
      } else {
         m_mainMuxer->intAddStream(si.m_fullStreamName, si.m_codec,
                                   si.m_streamReader->getStreamIndex(),
                                   si.m_addParams, si.m_streamReader);
      }
   }
   ...
   if (m_mainMuxer) m_mainMuxer->openDstFile();
   if (m_subMuxer) {
...
void MuxerManager::checkTrackList(const vector<StreamInfo>& ci) {
   ...
   for (vector<StreamInfo>::const_iterator itr = ci.begin(); itr != ci.end();
        ++itr) {
   ...
   if (!avcFound && mvcFound)
      THROW(ERR_INVALID_STREAMS_SELECTED,
            "Fatal error: MVC depended view track can't be muxed without AVC "
            "base view track");
}
...
void MuxerManager::doMux(const string& outFileName, FileFactory* fileFactory) {
   ...
   while (true) {
      ...
      if (avRez == BufferedReader::DATA_EOF) break;
      if (m_cutStart > 0) {
         if (avPacket.pts < m_cutStart) continue;
      }
      if (m_cutEnd > 0 && avPacket.pts >= m_cutEnd) break;

      if (m_subStreamIndex.find(avPacket.stream_index) !=
          m_subStreamIndex.end())
   ...
   if (m_subMuxer) m_subMuxer->doFlush();
   m_mainMuxer->doFlush();
   ...
   m_mainMuxer->close();
   if (m_subMuxer) m_subMuxer->close();
...
int MuxerManager::addStream(const string& codecName, const string& fileName,
...
void MuxerManager::asyncWriteBuffer(AbstractMuxer* muxer, uint8_t* buff,
                                    int len, AbstractOutputStream* dstFile) {
   WriterData data;
   data.m_buffer = buff;
   ...
   if (m_interleave && muxer == m_mainMuxer) {
      // do interlieave of SSIF blocks. Place sub channel blocks first, delay
      // main muxer blocks
...
int MuxerManager::syncWriteBuffer(AbstractMuxer* muxer, uint8_t* buff, int len,
   ...
   for (size_t i = 0; i < params.size(); i++) {
      ...
      if (paramPair[0] == "--start-time" && paramPair.size() > 1) {
         if (paramPair[1].find(":") != string::npos)
            ...
            m_ptsOffset = strToInt64u(paramPair[1].c_str()) *
                          2;  // source in a 45Khz clock
         ...
         if (paramPair[0] == "--cut-start")
            ...
            setCutEnd(strToInt64(prefix.c_str()) * coeff);
                 ...
                 paramPair[0] == "--split-size") {
         if (m_extraIsoBlocks == 0) m_extraIsoBlocks = 4;
