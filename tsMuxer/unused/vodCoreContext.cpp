...
class VodCoreContext : public AbstractCoreContext {
  ...
  public:
   unsigned startStreaming(const char* session, int streamingMode,
                           double ntpPos, int scale, const char* fileName,
                           const char* address, unsigned short port,
                           const char* rtpInfo, long tcpSocketHandle) {
      return vodcore::startStreaming(session, streamingMode, ntpPos, scale,
                                     fileName, address, port, rtpInfo,
                                     tcpSocketHandle);
   }
   ...
   unsigned startSmartBridgeStreaming(const char* sessionID, double ntpPos,
      ...
      return vodcore::startSmartBridgeStreaming(sessionID, ntpPos, url, address,
                                                port, rtpInfo, tcpSocketHandle);
   }

   void setYTCacheParams(const string& sCacheDirectory,
   ...
   void setRTCacheParams(const string& sCacheDirectory,
                         uint64_t nTotalCacheDuration) {
   ...
   unsigned seekYouTubeStreaming(const char* streamName, double nptPos) {
      ...
      {
         Mutex::ScopedLock lock(&m_demuxerMutex);
         map<string, AbstractDemuxer*>::iterator itr =
             m_demuxers.find(streamName);
         if (itr != m_demuxers.end()) demuxer = itr->second;
      }
      if (demuxer) {
   ...
   uint64_t getYTStreamDuration(const std::string& streamName) {
      ...
      if (demuxer) duration = demuxer->getDuration();

      return duration;
   }

   void setYTVideoInfo(const std::string& streamName, uint8_t existingType,
                       double fps) {
      m_videoInfoMap.insert(InfoPair(streamName, ParamPair(existingType, fps)));
   }

   void setRTVideoInfo(const std::string& streamName, uint8_t existingType,
                       double fps) {
      setYTVideoInfo(streamName, existingType, fps);
   }

   void getYTVideoInfo(const std::string& streamName, uint8_t& existingType,
                       double& fps, bool cleanUp) {
      if (m_videoInfoMap.count(streamName) > 0) {
         ...
         if (iter != m_videoInfoMap.end()) {
            ...
            if (cleanUp) m_videoInfoMap.erase(streamName);
         }
      }
   }

   void getRTVideoInfo(const std::string& streamName, uint8_t& existingType,
                       double& fps, bool cleanUp) {
      getYTVideoInfo(streamName, existingType, fps, cleanUp);
   }

   uint64_t getYTCacheDuration() { return m_ytCacheDuration; }

   uint64_t getRTCacheDuration() { return m_rtCacheDuration; }
   ...
   void setYTDomesticHost(const std::string& sDomesticHost) {
      if (!sDomesticHost.empty()) m_sYTDomesticHost = sDomesticHost;
   }
   ...
   void disableYTCache() { m_ytCacheDisabled = true; }

   void disableRTCache() { m_rtCacheDisabled = true; }

   void disableYTMp4Transcoding() { m_ytMp4TranscodingDisabled = true; }

   void setYTVideoSize(const uint32_t width, const uint32_t height,
                       const bool autoPal) {
      m_ytVideoWidth =
          !autoPal && width > 0 && width < uint32_t(-1) ? width : 352;
      m_ytVideoHeight =
          !autoPal && height > 0 && height < uint32_t(-1) ? height : 288;

      m_ytAutoPal = autoPal;
   }

   void setRTVideoSize(const uint32_t width, const uint32_t height,
                       const bool autoPal) {
      m_rtVideoWidth =
          !autoPal && width > 0 && width < uint32_t(-1) ? width : 352;  // 476
      m_rtVideoHeight = !autoPal && height > 0 && height < uint32_t(-1)
                            ? height
                            : 288;  // 320
   ...
   bool isYTCacheDisabled() { return m_ytCacheDisabled; }

   bool isRTCacheDisabled() { return m_rtCacheDisabled; }
   ...
   uint8_t getRTScalingType() { return m_nRTScalingType; }
   ...
   uint8_t getYTScalingType() { return m_nYTScalingType; }
   ...
   int createStreamingGroup(const char* address, const char* name,
                            double nptPos, int scale, int lowLevelTransport,
                            int loopCount, int eoplDelay, int eofDelay,
                            bool suspended) {
      return vodcore::createStreamingGroup(address, name, nptPos, scale,
                                           lowLevelTransport, loopCount,
                                           eoplDelay, eofDelay, suspended);
   }
   ...
   int startStreamingDVDExt(const char* groupName, int llTransport,
                            const char* dvdDrive, const char* folder) {
      return vodcore::startStreamingDVDExt(groupName, llTransport, dvdDrive,
                                           folder);
   }
   ...
   uint32_t addDestination(const char* session, const char* fileName,
                           const char* ip, uint16_t port, const char* rtpInfo,
                           long tcpSocketHandle, const char* ixface) {
      return vodcore::addDestination(session, fileName, ip, port, rtpInfo,
                                     tcpSocketHandle, ixface);
   }
   ...
   double getNptPos(unsigned streamer) { return vodcore::getNptPos(streamer); }
   ...
   unsigned createIndexFileAsync(const char* displayName, const char* fileName,
                                 bool createRewind) {
   ...
   bool stopAutoIndexing() { return vodcore::stopAutoIndexing(); }

   long multicastJoin(const char* assetName, const char* address, int port,
                      bool overWrite, int cycleBufferLen, const char* ixface,
                      bool createRewind, int nBlockSize,
                      int splitByDurationSize) {
      return vodcore::multicastJoin(assetName, address, port, overWrite,
                                    cycleBufferLen, ixface, createRewind,
                                    nBlockSize, splitByDurationSize);
   }
   ...
   void initLog(int messageLevel, const char* logName, const char* logDir,
                int logRotationPeriod) {
      return vodcore::initLog(messageLevel, logName, logDir, logRotationPeriod);
   }

   void getAssetDescription(const char* sessionID, const char* cfileName,
                            int descrType, char* dstBuffer, unsigned len) {
      return vodcore::getAssetDescription(sessionID, cfileName, descrType,
                                          dstBuffer, len);
   }
   ...
   int getAssetInfoFromFile(const char* fileName, char* dstBuffer,
                            int bufSize) {
   ...
   bool setRootDir(const char* rootDir) { return vodcore::setRootDir(rootDir); }

   bool setM3uRoot(const char* rootDir) { return vodcore::setM3uRoot(rootDir); }
   ...
   int LIB_STD_CALL muxTSFile(std::map<int, MuxTrackInfo> trackInfo,
                              const std::string& outName, int indexingType,
                              int nBitrate, int nVBVLen, int nFileBlockSize) {
      return vodcore::muxTSFile(trackInfo, outName, indexingType, nBitrate,
                                nVBVLen, nFileBlockSize);
   }
   AbstractReader* getReader(const char* streamName) {
      return vodcore::getReader(streamName);
   }

   bool checkTS(string fileName) { return vodcore::checkTS(fileName); }

   int getTSDuration(const char* fileName) {
      return vodcore::getTSDuration(fileName);
   }

   string getMediaInterface() { return m_sMediaInterface; }
   ...
   void setTsMtuSize(const int value) {
      if (value > 1500)
         LTRACE(LT_WARN, 0,
                "Max TS MTU size in Ethernet is 1500B. Value "
                    << value
                    << " is wrong. Used default TS MTU size ( 1400B ). Line: "
                    << __LINE__)
      else if (value > 0) {
         m_tsMtuSize = value;
         LTRACE(LT_DEBUG, 0,
                "TS MTU size is " << value << "B. Line: " << __LINE__)
      } else
         LTRACE(LT_WARN, 0,
                "TS MTU size must be > 0. Value "
                    << value
                    << " is wrong. Used default TS MTU size ( 1400B ). Line: "
                    << __LINE__)
   }
   ...
   void setMaxMIpPacketCount(const int value) {
      if (value == 0)
         LTRACE(LT_WARN, 0,
                "Value 'auto' is wrong for max multicast ip packet count. Used "
                "default value ( 6 ). Line: "
                    << __LINE__)
      else if (value < 0)
         LTRACE(LT_WARN, 0,
                "Value " << value
                         << " is wrong for max multicast ip packet count. Used "
                            "default value ( 6 ). Line: "
                         << __LINE__)
      else if (value * m_tsMtuSize > 16 * 1024)
         LTRACE(LT_WARN, 0,
                "Value " << value
                         << " is wrong for max multicast ip packet count ( "
                            "maxMIpPacketCount * tsMtuSize > 16K ). Used "
                            "default value ( 6 ). Line: "
                         << __LINE__)
      else {
         m_maxMIpPacketCount = value;
         LTRACE(LT_DEBUG, 0,
                "Max multicast ip packet count is " << value
                                                    << ". Line: " << __LINE__)
      }
   }
   ...
   void setMaxUIpPacketCount(const int value) {
      if (value < 0)
         LTRACE(LT_WARN, 0,
                "Value " << value
                         << " is wrong for max unicast ip packet count. Used "
                            "default value ( 6 ). Line: "
                         << __LINE__)
      else if (value * m_tsMtuSize > 16 * 1024)
         LTRACE(LT_WARN, 0,
                "Value " << value
                         << " is wrong for max unicast ip packet count ( "
                            "maxUIpPacketCount * tsMtuSize > 16K ). Used "
                            "default value ( 6 ). Line: "
                         << __LINE__)
      else {
         ...
         if (value == 0)
            LTRACE(LT_DEBUG, 0,
                   "Max unicast ip packet count is 'auto'. Line: " << __LINE__)
         else
            LTRACE(LT_DEBUG, 0,
                   "Max unicast ip packet count is " << value
                                                     << ". Line: " << __LINE__)
      }
   }
   ...
   virtual SocketAddress getDestinationForStream(const string& streamName) {
      ...
      AbstractCoreContext::StreamInfoMap::iterator itr =
          m_streamInfoMap.find(streamName);
   ...
   virtual void setTrafficBandWidth(int value) { m_trafficBandWidth = value; }

   virtual uint32_t getTrafficBandWidth() { return m_trafficBandWidth; }

   virtual int getStatisticForPeriod(int period, double& avgBroadcastBitrate,
                                     double& maxBroadcastBitrate,
                                     double& avgRunoffBitrate,
                                     double& maxRunoffBitrate, char* interval,
                                     int intervalSize) {
      return vodcore::getStatisticForPeriod(
          period, avgBroadcastBitrate, maxBroadcastBitrate, avgRunoffBitrate,
          maxRunoffBitrate, interval, intervalSize);
   }
   ...
   */
   ...
   virtual void addStreamInfo(const string& streamName,
