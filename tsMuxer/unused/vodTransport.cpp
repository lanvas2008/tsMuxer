...
struct VodApplication {
   VodApplication() {
      ...
      fileReader = new BufferedFileReader(
          ...
          DEFAULT_FILE_BLOCK_SIZE + MAX_TS_PACKET_LEN + MAX_AV_PACKET_SIZE,
          DEFAULT_FILE_BLOCK_SIZE / 2);
      tsAlignedReadManager = new BufferedReaderManager(
          2, DEFAULT_ROUND_BLOCK_SIZE,
          DEFAULT_ROUND_BLOCK_SIZE + MAX_AV_PACKET_SIZE);
      unalignedReadManager = new BufferedReaderManager(
          2, DEFAULT_FILE_BLOCK_SIZE,
          DEFAULT_FILE_BLOCK_SIZE + MAX_AV_PACKET_SIZE);
      ytReadManager = new BufferedReaderManager(
          2, YT_ROUND_BLOCK_SIZE, YT_ROUND_BLOCK_SIZE + MAX_AV_PACKET_SIZE,
          YT_ROUND_BLOCK_SIZE / 2);
      streamer = new MCVodStreamer();

      for (int i = 0; i < STREAMERS_COUNT; ++i) streamer->addWorkerThread();
  ...
  public:
   ...
   /*
   BufferedFileReader fileReader ( DEFAULT_ROUND_BLOCK_SIZE,
   DEFAULT_ROUND_BLOCK_SIZE + MAX_TS_PACKET_LEN + MAX_AV_PACKET_SIZE,
   DEFAULT_ROUND_BLOCK_SIZE / 2); BufferedReaderManager readManager ( 2 );
   BufferedReaderManager ytReadManager ( 2, YT_ROUND_BLOCK_SIZE,
   YT_ROUND_BLOCK_SIZE + MAX_TS_PACKET_LEN + MAX_AV_PACKET_SIZE,
   YT_ROUND_BLOCK_SIZE / 2 ); BufferedFileWriter bufferedFileWrited; map<string,
   MulticastEncoder*>  multicastEncoders;
...
#ifdef WIN32
PROCESS_INFORMATION execProcessAsync(const string& appName,
   ...
   // memcpy(runbuffer, (string(" ") + commandLine).c_str(),
   // commandLine.size()+1); runbuffer[commandLine.size()+1] = 0;
   memcpy(runbuffer, appName.c_str(), appName.size());
   runbuffer[appName.size()] = 0;
   string folder = extractFilePath(appName);
   // bool rez = CreateProcessA(appName.c_str(), runbuffer,  NULL, NULL, FALSE,
   // 0, NULL, 	                      folder.c_str(), &si, &pi);
   ...
   if (rez == FALSE) {
      ...
      THROW(ERR_CANT_EXEC_PROCESS,
            "Can't execute " << appName << ". Error code: " << retCode);
   }
...
LIB_EXPORT bool LIB_STD_CALL checkTS(string fileName) {
   ...
   ResultPair rp = METADemuxer::DetectStreamReader(
       *vodApplication.tsAlignedReadManager, fileName);
...
LIB_EXPORT int64_t LIB_STD_CALL getFileLength(const char* fileName,
                                              double fromOffset) {
   try {
      uint64_t fileSize;
      File file(fileName, File::ofRead);
      if (!file.size(&fileSize)) return -1;
      ...
      if (assetIndex) {
         ...
         if (fromOffset > 0) {
            LTRACE(LT_WARN, 0,
                   "getFileLength: No index found for file "
                       << fileName
                       << ". return full file length ignoring offset "
                       << fromOffset);
         }
         return fileSize;
      }
...
LIB_EXPORT int LIB_STD_CALL getTSDuration(const char* fileName) {
   try {
      ...
      int frameSize = 188;
      if (strEndWith(fileName, ".m2ts")) frameSize = 192;
      uint64_t fileSize;
      File file(fileName, File::ofRead);
      if (!file.size(&fileSize)) return -1;
      fileSize = fileSize / frameSize * frameSize;
      fileSize -=
          frameSize * 2000;  // step a little backwards from the end of the file
...
BufferedFileReader& getFileReader() { return *vodApplication.fileReader; }
...
LIB_EXPORT void LIB_STD_CALL getAssetDescription(const char* sender,
                                                 const char* cfileName,
                                                 int descrType, char* dstBuffer,
                                                 unsigned len) {
   *dstBuffer = 0;
   unsigned tmpLen = len;
   ...
   LTRACE(LT_DEBUG, 0,
          "getAssetDescription called. fileName=" << cfileName << " sender="
                                                  << (sender ? sender : ""));
   ...
   try {
      ...
      uint8_t videoService =
          serviceName == "youtube"
              ? MCVodStreamer::vsYouTube
              : (serviceName == "rutube"
                     ? MCVodStreamer::vsRuTube
                     : (serviceName == "dvtube"
                            ? MCVodStreamer::vsDvTube
                            : (serviceName == "bbc"
                                   ? MCVodStreamer::vsBBC
                                   : MCVodStreamer::vsIndefinite)));
      ...
      if (videoService != MCVodStreamer::vsIndefinite) {
         ...
         AssetIndex* assetIndex =
             AssetIndexManager::getInstance()->getFileIndex(fileName);

         if (assetIndex) {
            string tmp = assetIndex->getFirstMainFileName();

            if (tmp.size() > 0) fileName = tmp;
         }

         assetDescr = vodApplication.streamer->getAssetDescr(fileName.c_str(),
                                                             descrType);
         ...
         if (len < tmpLen) THROW(ERR_COMMON_SMALL_BUFFER, "")
      }
   } catch (runtime_error& e) {
      LTRACE(LT_ERROR, ERR_COMMON,
             "vodTransport::getAssetDescription runtime error: "
                 << e.what() << ". Line: " << __LINE__)
      errMessage = "ERROR:" + int32ToStr(ERR_FILE_NOT_FOUND) + ',' + e.what();

   } catch (VodCoreException& e) {
      LTRACE(LT_ERROR, e.m_errCode,
             "vodTransport::getAssetDescription error: "
                 << e.m_errStr << ". Line: " << __LINE__)
...
LIB_EXPORT void LIB_STD_CALL reservePorts(
    const char* session, int streamingMode, const char* address,
   ...
   try {
      ...
      vodApplication.streamer->reservePorts(
          session, streamingMode, SocketAddress(IPAddress(address), port),
          sRtpInfo);
...
LIB_EXPORT unsigned LIB_STD_CALL
startStreaming(const char* session, int lowLevelTransport, double ntpPos,
               int scale, const char* fileName, const char* address,
               unsigned short port, const char* rtpInfo, long tcpSocketHandle,
               bool doDownload, int64_t downloadLimit) {
   unsigned handle = 0;
   ...
   try {
      ...
      LTRACE(LT_DEBUG, 0,
             "vodTransport::StartStreaming called. FileName: "
                 << fileName << ", ip: " << address << "."
                 << " sessionID=" << (session ? session : ""));
      LTRACE(LT_DEBUG, 0,
             "vodTransport::StartStreaming. total starts: " << ++stCounter);
      ...
      if (usePluginStreaming)
         handle = startSmartBridgeStreaming(session, ntpPos, fileName, address,
                                            port, rtpInfo, tcpSocketHandle);
      else {
         /*
            if ( cyclicMulticastEncoder ( fileName ) )
            {
            Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
                MulticastEncoder* me = vodApplication.multicastEncoders [
            fileName ]; if (me) { if (ntpPos <= 0) ntpPos += me->getCurNptPos();
         ...
         */
         ...
         {
            Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
            map<string, MulticastEncoder*>::iterator itr =
                vodApplication.multicastEncoders.find(fileName);
            MulticastEncoder* me =
                itr != vodApplication.multicastEncoders.end() ? itr->second : 0;
            ...
            if (me) {
               if (ntpPos <= 0)
                  ...
                  ntpPos = me->getCurNptPos() -
                           (me->getCyclicBufferLength() - ntpPos);

               ntpPos = FFMIN(FFMAX(0, ntpPos), me->getCurNptPos());
            }
         }

         handle = vodApplication.streamer->startStreaming(
             session,
             usePluginStreaming
             ...
             assetName.c_str(), ntpPos, scale,
             SocketAddress(IPAddress(address), port), lowLevelTransport,
             rtpInfo, (TCPSocket*)tcpSocketHandle, streamingContext, doDownload,
             downloadLimit);

         // if ( ntpPos >= 0 && ntpPos < 1 &&
         // vodCoreContext->cyclicMulticastEncoder ( fileName ) )
         //    vodApplication.streamer->pauseStreaming ( handle );
      }
      if (handle == 0) {
         LTRACE(
             LT_DEBUG, 0,
             "vodTransport::startStreaming: can't start streaming ( session: "
                 << (session ? session : "") << ", file:" << fileName
                 << ". Remove streaming context. Line: " << __LINE__)
         delete streamingContext;
      }
   } catch (VodCoreException& e) {
      LTRACE(LT_ERROR, 0,
             "vodTransport::startStreaming error: can't StartStreaming, "
                 << e.m_errStr)
   } catch (exception& e) {
      LTRACE(LT_ERROR, 0,
             "vodTransport::startStreaming error: " << e.what()
                                                    << ". Line: " << __LINE__)
   } catch (...) {
      LTRACE(LT_ERROR, 0,
             "vodTransport::startStreaming error: unknown. Line: " << __LINE__)
   }
...
string startBridgeMuxing(SmartBridgeSource sb, const string& url,
                         TSMuxer* tsMuxer) {
   ...
   tsMuxer->setFileBlockSize(YT_DEFAULT_PREBUFFERING_TIME,
                             YT_MEMORY_FILE_BUFFER_SIZE);
   ...
   switch (sb) {
      case sbsUdp: {
         ...
         keepAudio = transcode ? strstr(source.c_str(), "keepsound=1") != NULL
                               : keepAudio;
         keepVideo = true;
         ...
         if (transcode && strstr(source.c_str(), "codec=h264") != NULL) {
            ...
            {
                string s = source.substr ( pos + strlen( "bitrate=" ),
            source.size() );
   ...
   if (keepVideo) tsMuxer->addStream(videoCodecName, source, videoParams);

   if (keepAudio) tsMuxer->addStream(audioCodecName, source, audioParams);

   tsMuxer->parseMuxOpt(string("--vbv-len=") + int32ToStr(vbvLength) +
                        string(" --maxbitrate=") +
                        doubleToStr(bitRate / 1000.0));
...
LIB_EXPORT int LIB_STD_CALL createStreamingGroup(const char* address,
                                                 const char* name,
                                                 double nptPos, int scale,
                                                 int lowLevelTransport,
                                                 int loopCount, int eoplDelay,
                                                 int eofDelay, bool suspended) {
   int res = -1;

   LTRACE(
       LT_DEBUG, 0,
       "CreateStreamingGroup called. address: " << address << " name:" << name);
        ...
        if ( strstr ( address, "icca_demo_" ) )
           {
               ...
               LTRACE ( LT_DEBUG, 0, "Create streaming group at: " <<
        (int)nptPos )
           }
           */

         SmartBridgeSource sb =
             strStartWith(address, "radio://")
                 ? sbsRadio
                 : (strStartWith(address, "rutube://")
                        ? sbsRT
                        : (strStartWith(address, "youtube://")
                               ? sbsYT
                               : (strStartWith(address, "udpt://")
                                      ? sbsUdp
                                      : (strStartWith(address, "bbc://")
                                             ? sbsBBC
                                             : sbsNone))));

         BufferedReaderManager& arm =
             lowLevelTransport == StreamingContext::llTransportRAW
                 ? *vodApplication.unalignedReadManager
                 : *vodApplication.tsAlignedReadManager;
         const BufferedReaderManager& rm = sb == sbsNone || sb == sbsUdp
                                               ? arm
                                               : *vodApplication.ytReadManager;
         ...
         if (sb != sbsNone) {
            if (sb == sbsDVT || sb == sbsUdp) {
               memName =
                   memName.substr(string("udpt://").length(), memName.size());
               memName = "udp://" + memName;
         ...
         StreamingContext* context =
             vodApplication.streamer->createStreamingGroup(
                 rm, memName.c_str(), name, nptPos, scale, lowLevelTransport,
                 loopCount, eoplDelay, eofDelay, suspended);
         if (context) {
            if (tsMuxer) {
               if (lowLevelTransport == StreamingContext::llTransportRTP)
                  ((StreamingContextRTP*)context)
                      ->setVBVMode(StreamingContextRTP::VBV_BITRATE_AUTO,
...
LIB_EXPORT uint32_t LIB_STD_CALL addDestination(
    const char* session, const char* fileName, const char* ip, uint16_t port,
    const char* rtpInfo, long tcpSocketHandle, const char* ixface) {
   if (vodApplication.stopped) return 0;
   try {
      LTRACE(LT_DEBUG, 0, "addDestination called0. fileName: " << fileName);
      return vodApplication.streamer->addDestination(
          session, fileName, SocketAddress(ip, port), rtpInfo,
...
LIB_EXPORT int LIB_STD_CALL removeDestination(const char* fileName,
                                              long handle) {
   if (vodApplication.stopped) return 0;
   try {
...
LIB_EXPORT double LIB_STD_CALL getCyclicNptPos ( std::string name )
{
    ...
    try
    {
        if ( !vodApplication.stopped )
        {
         Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
            map < string, MulticastEncoder* >::iterator itr =
vodApplication.multicastEncoders.find ( name );
    ...
    {
        LTRACE ( LT_ERROR, 0, "Can't get cyclic ntp of multicast encoder '" <<
name <<  "': " << e.m_errStr )
    }
    catch ( exception& e)
    {
...
LIB_EXPORT int cyclicStreamingFirstFileNum(const char* streamName) {
   if (!cyclicMulticastEncoder(streamName)) return -1;
   AssetIndexManager* assetIndexManager = AssetIndexManager::getInstance();
   AssetIndex* assetIndex = assetIndexManager->getFileIndex(streamName);
   int fileNum = -1;
   if (assetIndex) {
      double nptPos = -INT_MAX;  // relative pos for cyclic streaming (get
                                 // begining of buffer)
...
LIB_EXPORT int LIB_STD_CALL cyclicMulticastEncoderLen(std::string name) {
   ...
   try {
      ...
      LTRACE(LT_ERROR, 0,
             "Can't check cyclic of multicast encoder '" << name
                                                         << "': " << e.m_errStr)
   } catch (exception& e) {
      LTRACE(LT_ERROR, 0, e.what())
   } catch (...) {
   ...
   try {
      if (!vodApplication.stopped) {
         ...
         /*
            ...
            ]; if (me) { res -= me->getCurNptPos(); res =
            FFMAX(-me->getCyclicBufferLength(), FFMIN(0, res));
            }
            }
         */
         {
            Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
            map<string, MulticastEncoder*>::iterator itr =
                vodApplication.multicastEncoders.find(streamName);
            MulticastEncoder* me =
                itr != vodApplication.multicastEncoders.end() ? itr->second : 0;
            if (me) {
               res -= me->getCurNptPos();
...
LIB_EXPORT int LIB_STD_CALL stopStreaming(unsigned readerID) {
   ...
   LTRACE(LT_DEBUG, 0,
          "vodTransport::stopStreaming. total stops: " << ++stCounter);
...
LIB_EXPORT int LIB_STD_CALL setPosition(unsigned readerID, double position) {
   ...
   try {
      LTRACE(0, LT_DEBUG,
             "setPosition to " << position << ". ReaderID " << readerID);
      ...
      if ( cyclicMulticastEncoder ( streamName ) && position <= 0 )
        {
         Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
            MulticastEncoder* me = vodApplication.multicastEncoders [ streamName
      ]; if (me) { position += me->getCurNptPos(); position =
      FFMIN(me->getCyclicBufferLength(), FFMAX(0, position));
         }
        }
      */
      {
         ...
         if (me) {
            if (position <= 0) {
               ...
               position = me->getCurNptPos() -
                          (me->getCyclicBufferLength() - position);
            }
...
bool supportedRTPCodecs(const string& fileName) {
   pair<AVChapters, vector<CheckStreamRez>> fullRez =
       METADemuxer::DetectStreamReader(*vodApplication.tsAlignedReadManager,
                                       fileName);
...
LIB_EXPORT int LIB_STD_CALL updateIndexes() {
   ...
   try {
      ...
      LTRACE(
          LT_ERROR, 0,
          "Vod core" << at_str << ": " << e.m_errStr << ". Line: " << __LINE__)
   } catch (exception& e) {
      LTRACE(LT_ERROR, 0,
             "Std" << at_str << ": " << e.what() << ". Line: " << __LINE__)
   } catch (...) {
   ...
   LTRACE(
       LT_DEBUG, 0,
       "vodTransport.updateIndexes time: " << (uint32_t)(end_time - start_time))
...
string getLongFileLocation(const string& fileName) {
   ...
   if (res.empty()) res = extractFileDir(fileName);
...
bool generateVLCM3uFile(const string& fullName) {
   ...
   string m3uName = m3uRootDir +
                    shortName.substr(0, shortName.length() - ext.length()) +
                    "m3u";
   TextFile file;
   if (!file.open(m3uName.c_str(), File::ofWrite)) return false;
   file.writeLine("#EXTM3U");
   file.writeLine("#EXTVLCOPT--input-repeat=-1");
   file.writeLine("#EXTVLCOPT--http-reconnect=true");
   file.writeLine(string("#EXTVLCOPT:realrtsp-caching=") +
                  int32ToStr(defaultRTPSPTimeout));
   file.writeLine(string("#EXTVLCOPT:rtsp-caching=") +
                  int32ToStr(defaultRTPSPTimeout));
   ...
   for (size_t i = 0; i < nFileName.length(); i++)
      if (nFileName[i] == '\\') nFileName[i] = '/';
   file.writeLine(httpServerURL + nFileName);
   file.close();
   return true;
}

LIB_EXPORT unsigned LIB_STD_CALL createIndexFile(const char* fileName,
                                                 bool createRewind) {
   if (vodApplication.stopped) return 0;
   if (AssetIndexManager::getInstance()->isReadOnlyIndexes()) {
      LTRACE(LT_WARN, 0,
             "Can't start file indexing for file "
                 << fileName << " because readOnly mode selected.");
      return 0;
   }
   try {
      AssetIndexer indexer;
      int rez =
          indexer.createIndexFile(fileName, getFileReader(), createRewind);
...
LIB_EXPORT unsigned LIB_STD_CALL getAssetStatus(const char* fileName) {
   {
      Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
      if (vodApplication.stopped ||
          (vodApplication.multicastEncoders.count(fileName) &&
           vodApplication.multicastEncoders[fileName]->isWorking()))
         return 0;
   }
   AssetIndexManager* aim = AssetIndexManager::getInstance();
   ...
   if (aiw && !aiw->isFinished()) {
      ...
      AssetIndex* assetIndex =
          new AssetIndex(fileName, aim->isReadOnlyIndexes());
      ...
      if (assetIndex->indexFileExists()) {
         ...
         if (crc == 0) {
            ...
            for (size_t i(0); i < size; ++i) {
               ...
               ai_size += header->m_indexCurDuration *
                              AssetIndexHeader::ASSET_INDEX_DATA_SIZE +
                          AssetIndexHeader::HEADER_SIZE;
            }
         ...
         if ((firstMainFileName.empty() || !fileExists(firstMainFileName)) &&
             indexIsReady && assetLength != 0)
      ...
      if (!indexIsReady) {
         LTRACE(
             LT_WARN, 0,
             "Invalid index file ( "
                 << (crc == 0 ? "file contet check failed" : "CRC check failed")
                 << " ) of asset '" << fileName
                 << "'. Asset will be re-indexed. Line" << __LINE__)
...
LIB_EXPORT bool LIB_STD_CALL getEncodedAssetList(char* rezBuffer,
                                                 int bufferSize) {
   if (vodApplication.stopped) return false;
...
LIB_EXPORT unsigned LIB_STD_CALL createIndexFileAsync(const char* displayName,
                                                      const char* fileName,
                                                      bool createRewind) {
   if (vodApplication.stopped) return 0;
   ...
   try {
      AssetIndexer indexer;
      int rez = indexer.createIndexFileAsync(displayName, fileName,
                                             getFileReader(), createRewind);
      return rez;
   } catch (VodCoreException& e) {
      LTRACE(LT_ERROR, 0, "Can't start indexing: " << e.m_errStr);
   } catch (exception& e) {
...
LIB_EXPORT int LIB_STD_CALL deleteAsset(const char* fileName) {
   ...
   try {
         ...
         if (AssetIndexManager::getInstance()->deleteFileIndex(fileName)) {
            ...
            if (fileExists(fileName)) {
               if (deleteFile(fileName)) {
                  LTRACE(LT_WARN, 0,
                         "Media file " << fileName << " deleted successfully")
                  res = 0;
               } else {
            ...
            LTRACE(LT_WARN, 0,
                   "Can't delete index files for asset: " << fileName)
            res = -3;
         }
      } else
         res = -4;
      ...
      LTRACE(LT_ERROR, 0,
             "Vod core exception at deleteAsset: " << e.m_errStr
                                                   << ". Line: " << __LINE__)
   } catch (exception& e) {
      LTRACE(LT_ERROR, 0,
             "Exception at deleteAsset: " << e.what() << ". Line: " << __LINE__)
   } catch (...) {
      LTRACE(LT_ERROR, 0,
             "Unknown exception at deleteAsset for: " << fileName
                                                      << ". Line: " << __LINE__)
   }
...
LIB_EXPORT int LIB_STD_CALL getRewindSpeed(int requestSpeed) {
   if (vodApplication.stopped) return 0;
   if (requestSpeed > 1) return DEFAULT_REWIND_SPEED;
   if (requestSpeed < 0)
...
LIB_EXPORT long LIB_STD_CALL multicastJoin(const char* assetName,
                                           const char* address, int port,
                                           bool overWrite, int cycleBufferLen,
                                           const char* ixface,
                                           bool createRewind, int nBlockSize,
                                           int splitByDurationSize) {
   LTRACE(LT_DEBUG, 0,
          "vodTransport::multicastJoin called. assetName:"
              << assetName << ", source=" << address << ":" << port)
   ...
   try {
      if (!vodApplication.stopped) {
         if (!overWrite && fileExists(assetName)) {
            res = -2;
            LTRACE(LT_WARN, 0,
                   "vodTransport::multicastJoin: file '"
                       << assetName
                       << "' exists and can't be overwrited because param "
                          "overWrite is false. Line: "
                       << __LINE__)
         } else {
            Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
            map<string, MulticastEncoder*>::iterator itr =
                vodApplication.multicastEncoders.find(assetName);
            if (itr == vodApplication.multicastEncoders.end()) {
               if (nBlockSize == 0) {
               ...
               MulticastEncoder* rez = MulticastEncoder::addEncoder(
                   address, port, vodApplication.getBufferedFileWriter(),
                   ixface, nBlockSize);
               ...
               if (res == 0)
                  vodApplication.multicastEncoders.insert(
                      make_pair(assetName, rez));
               else
                  delete rez;
            } else
               res = -1;
      ...
      LTRACE(LT_ERROR, 0,
             "Vod core exception at vodTransport::multicastJoin: "
                 << e.m_errStr << ". Line: " << __LINE__)
   } catch (exception& e) {
      LTRACE(LT_ERROR, 0,
             "Exception at vodTransport::multicastJoin: "
                 << e.what() << ". Line: " << __LINE__)
   } catch (...) {
      LTRACE(LT_ERROR, 0,
             "Unknown exception at vodTransport::multicastJoin for: "
                 << assetName << ". Line: " << __LINE__)
   }
...
LIB_EXPORT void LIB_STD_CALL retransmitStream(const char* srcAddress,
                                              ...
                                              bool emulateLack) {
   MulticastRetransmitter* mr = MulticastRetransmitter::create(
       srcAddress, srcIxface, dstAddress, fwdAddress, emulateLack);

   if (mr) mcstRetransmitters.push_back(mr);
}

LIB_EXPORT long LIB_STD_CALL
createRaptorStream(const char* srcAddress, int srcPort, const char* srcIxface,
                   const char* dstAddress, int dstPort, const char* dstIxface,
                   int repairSymbolCount, int symbolCount) {
   LTRACE(LT_DEBUG, 0,
          "vodTransport::createRaptorStream called. srcAddress:"
              << srcAddress << ":" << srcPort << ", dstAddress=" << dstAddress
              << ":" << dstPort)
   ...
   try {
      if (!vodApplication.stopped) {
         ...
         currentEncoder->addEncoder(srcAddress, srcPort, srcIxface, dstAddress,
                                    dstPort, dstIxface, repairSymbolCount,
                                    symbolCount);
         // return (long) rez;
      ...
      LTRACE(LT_ERROR, 0,
             "Vod core exception at vodTransport::createRaptorStream: "
                 << e.m_errStr << ". Line: " << __LINE__)
   } catch (exception& e) {
      LTRACE(LT_ERROR, 0,
             "Exception at vodTransport::createRaptorStream: "
                 << e.what() << ". Line: " << __LINE__)
   } catch (...) {
      LTRACE(LT_ERROR, 0,
             "Unknown exception at vodTransport::createRaptorStream for: "
             "srcAddress="
                 << srcAddress << ". Line: " << __LINE__)
   }
...
LIB_EXPORT int LIB_STD_CALL multicastLeave(const char* encoderName) {
   ...
   LTRACE(LT_DEBUG, 0,
          "vodTransport::multicastLeave called. assetName:" << encoderName)

   try {
      if (!vodApplication.stopped) {
         Mutex::ScopedLock lock(&vodApplication.multicastEncodersLock);
         map<string, MulticastEncoder*>::iterator itr =
             vodApplication.multicastEncoders.find(encoderName);
         if (itr != vodApplication.multicastEncoders.end()) {
      ...
      LTRACE(LT_ERROR, 0,
             "Vod core exception at multicastLeave: " << e.m_errStr
                                                      << ". Line: " << __LINE__)
   } catch (exception& e) {
      LTRACE(
          LT_ERROR, 0,
          "Exception at multicastLeave: " << e.what() << ". Line: " << __LINE__)
   } catch (...) {
      LTRACE(LT_ERROR, 0,
             "Unknown exception at multicastLeave for: "
                 << encoderName << ". Line: " << __LINE__)
   }
...
LIB_EXPORT void LIB_STD_CALL initLog(int messageLevel, const char* logName,
                                     const char* logDir,
                                     int logRotationPeriod) {
   if (vodApplication.stopped) return;

   SystemLog::initializeSystemLog(
       messageLevel, logName, logDir, logRotationPeriod * 60, 50 * 1024 * 1024,
       true, AlternativeFileStream::Day,
       LogStream::MessageLevel | LogStream::MessageDate |
           LogStream::MessageTime | LogStream::MessageThreadID,
       "");
...
LIB_EXPORT int LIB_STD_CALL getAssetInfoFromFile(const char* fileName,
                                                 char* dstBuffer, int bufSize) {
   int res = -3;
   ...
   *dstBuffer = 0;
   ...
   try {
      if (!vodApplication.stopped) {
         ...
         if (duration > 0) bitrate = fileSize / (double)duration / 1024.0 * 8;
         ...
         for (size_t i = 0; i < rez.size(); i++) {
            ...
            if (rez[i].codecInfo.codecID) {
               stream << "Stream type: " << rez[i].codecInfo.displayName
                      << endl;
               stream << "Stream ID:   " << rez[i].codecInfo.programName
                      << endl;
      ...
      if (stream.str().size() > (size_t)bufSize) {
         ...
         LTRACE(LT_WARN, 0,
                "vodTransport::getAssetInfoFromFile: too small out buffer "
                "size. Line: "
                    << __LINE__)
      }
   } catch (VodCoreException& e) {
      LTRACE(LT_ERROR, 0,
             "Vod core exception at getAssetInfoFromFile: "
                 << e.m_errStr << ". Line: " << __LINE__)
   } catch (exception& e) {
      LTRACE(LT_ERROR, 0,
             "Exception at getAssetInfoFromFile: " << e.what()
                                                   << ". Line: " << __LINE__)
   } catch (...) {
      LTRACE(LT_ERROR, 0,
             "Unknown exception at getAssetInfoFromFile for: "
...
unsigned LIB_STD_CALL testRuTubeStreaming(uint32_t nPort,
                                          const string& sVideoID) {
   ...
   getAssetDescription ( sPort.c_str(), ( "rutube://" + sVideoID ).c_str(), 0,
   dstBuffer, sizeof(dstBuffer));
   */
   int mcasthandle = multicastJoin("tstv2.ts", "239.255.2.100", 5500, true, 20,
                                   0, true, 0, 0);
   Process::sleep(1000 * 30);
   int sHandle = startStreaming(0, StreamingContext::llTransportRAW, -2, 1,
                                "tstv2.ts", "127.0.0.1", 35000, 0, 0);
   ...
   // unsigned handle = startStreaming ( sPort.c_str(),
   // StreamingContext::llTransportRAW, 0, 1, ( "rutube://" + sVideoID /*+
   // "&pos=20"*/ ).c_str(), "127.0.0.1", nPort, 0, 0 );

   unsigned handle =
       startStreaming(sPort.c_str(), StreamingContext::llTransportRAW, 0, 1,
                      ("rutube://" + sVideoID /*+ "&pos=20"*/).c_str(),
                      "127.0.0.1", nPort, 0, 0);

   return handle;
}

unsigned LIB_STD_CALL testYouTubeStreaming(uint32_t nPort,
                                           const string& sVideoID) {
   // Saving
   ...
   tsMuxer->parseMuxOpt ( string ( "--vbv-len=350 --maxbitrate=" ) + int32ToStr
   ( YOUTUBE_BITRATE ) );
   ...
   return 0;*/
   // Streaming
   //    vodCoreContext->setYTCacheParams ( "C:/MediaCache", "./log", 24 * 3600
   //    * 365 );
   vodCoreContext->setYTCacheParams("C:/rtspServer/Media", "./log",
                                    3600 * 24 * 365);
   ...
   char dstBuffer[1024 * 2];
   string sPort = int32uToStr(nPort);
   // getAssetDescription ( sPort.c_str(), ( "youtube://" + sVideoID ).c_str(),
   // 0, dstBuffer, sizeof(dstBuffer)); vodCoreContext->setMediaInterface (
   // "10.40.1.30" );

   unsigned handle =
       startStreaming(sPort.c_str(), StreamingContext::llTransportRAW, 0, 1,
                      ("youtube://" + sVideoID /*+ "&pos=20"*/).c_str(),
                      "127.0.0.1", nPort, 0, 0);

   // vodCoreContext->setYTCacheParams ( "c:/rtspSrever/MediaCache",
   // "c:/rtspSrever/log", 3600 * 24 * 365 );
   //
   // unsigned handle = startStreaming ( StreamingContext::llTransportRAW, 0, 1,
   // ( "bbc://" + sVideoID ).c_str(),
   //                                   "127.0.0.1", nPort, 0, 0 );
...
int LIB_STD_CALL muxTSFile(map<int, MuxTrackInfo> trackInfo,
                           const string& outName, int indexingType,
                           int nBitrate, int nVBVLen, int nFileBlockSize) {
   int res = -1;
   ...
   try {
      if (!vodApplication.stopped) {
         ...
         tsMuxer->parseMuxOpt(string("--vbv-len=") + int32ToStr(nVBVLen) +
                              string(" --maxbitrate=") +
                              doubleToStr(nBitrate / 1000.0));

         for (map<int, MuxTrackInfo>::const_iterator itr = trackInfo.begin();
              itr != trackInfo.end(); ++itr) {
            map<string, string> params;
            params["track"] = int32ToStr(itr->first);
            const string& codecName = itr->second.codecName;
            for (map<string, string>::const_iterator paramItr =
                     itr->second.addParam.begin();
                 paramItr != itr->second.addParam.end(); ++paramItr) {
               params[paramItr->first] = paramItr->second;
            }
            tsMuxer->addStream(codecName.c_str(), itr->second.streamName,
                               params);
         }
...
LIB_EXPORT void LIB_STD_CALL setYTLogoPos(uint32_t pos) {
   if (!vodApplication.stopped) vodCoreContext->setYTLogoPos(pos);
}

LIB_EXPORT void LIB_STD_CALL setYTLogoPath(const string& path) {
   if (!vodApplication.stopped) vodCoreContext->setYTLogoPath(path);
}

LIB_EXPORT void LIB_STD_CALL setYTCacheParams(const string& sCacheDirectory,
                                              const string& sLogDirectory,
                                              uint64_t nTotalCacheDuration) {
   if (vodApplication.stopped) return;
   vodCoreContext->setYTCacheParams(sCacheDirectory, sLogDirectory,
                                    nTotalCacheDuration);
}

LIB_EXPORT void LIB_STD_CALL setRTCacheParams(const string& sCacheDirectory,
                                              uint64_t nTotalCacheDuration) {
   if (vodApplication.stopped) return;
   vodCoreContext->setRTCacheParams(sCacheDirectory, nTotalCacheDuration);
}

LIB_EXPORT void LIB_STD_CALL getYTVideoInfo(const string& streamName,
                                            uint8_t& existingType, double& fps,
                                            bool cleanUp) {
   if (vodApplication.stopped) return;
...
LIB_EXPORT void LIB_STD_CALL getRTVideoInfo(const string& streamName,
                                            uint8_t& existingType, double& fps,
                                            bool cleanUp) {
   if (vodApplication.stopped) return;
...
LIB_EXPORT void LIB_STD_CALL setYTDomesticHost(const string& sDomesticHost) {
   if (vodApplication.stopped) return;
   LTRACE(LT_DEBUG, 0,
          "setYTDomesticHost called. sDomesticHost: " << sDomesticHost)
...
LIB_EXPORT void LIB_STD_CALL setYTVideoSize(const uint32_t width,
...
LIB_EXPORT void LIB_STD_CALL setRTVideoSize(const uint32_t width,
                                            const uint32_t height,
                                            const bool autoPal) {
   if (vodApplication.stopped) return;
...
LIB_EXPORT void LIB_STD_CALL setMediaInterface(const string& sMediaInterface) {
   if (vodApplication.stopped) return;
   LTRACE(LT_DEBUG, 0,
          "setMediaInterface called. sMediaInterface: " << sMediaInterface)
   vodCoreContext->setMediaInterface(sMediaInterface);
}

LIB_EXPORT void LIB_STD_CALL getYTDomesticHost(char* pDstBuff,
                                               uint32_t& nDstBuffSize) {
   if (vodApplication.stopped) return;
...
LIB_EXPORT bool LIB_STD_CALL checkSmartBridgeCache(const char* streamName) {
   ...
   if (prefix != "igmp" && prefix != "radio" && prefix != "dvtube") {
      typedef bool(LIB_STD_CALL * CheckCacheFuncPtr)(AbstractCoreContext*,
                                                     const char*);
...
unsigned LIB_STD_CALL startSmartBridgeStreaming(
    const char* sessionID, double ntpPos, const char* url, const char* address,
    unsigned short port, const char* rtpInfo, long tcpSocketHandle) {
   if (vodApplication.stopped) return 0;
   ...
   try {
      ...
      SmartBridgeSource sb =
          strStartWith(url, "radio://")
              ? sbsRadio
              : (strStartWith(url, "rutube://")
                     ? sbsRT
                     : (strStartWith(url, "youtube://")
                            ? sbsYT
                            : (strStartWith(url, "udp://")
                                   ? sbsUdp
                                   : (strStartWith(url, "dvtube:/")
                                          ? sbsDVT
                                          : (strStartWith(url, "bbc://")
                                                 ? sbsBBC
                                                 : sbsNone)))));
      ...
      handle = vodApplication.streamer->startStreaming(
          sessionID, *vodApplication.ytReadManager, memName.c_str(), ntpPos, 1,
          SocketAddress(IPAddress(address), port),
          StreamingContext::llTransportRAW, rtpInfo,
          (TCPSocket*)tcpSocketHandle, context);
      ...
      LTRACE(LT_DEBUG, 0,
             "DNM_T vodTransport::startSmartBridgeStreaming exception")
   } catch (...) {
      LTRACE(LT_DEBUG, 0,
             "DNM_T vodTransport::startSmartBridgeStreaming exception")
   }
...
int32_t getInitialDelay(const string& toolsRoot, const string& groupName) {
   int32_t lastDelay = 0;
   TextFile file;
   if (file.open((closeSlash(toolsRoot) + "stream_delay.txt").c_str(),
                 File::ofRead)) {
      string line;
...
LIB_EXPORT unsigned LIB_STD_CALL
startEncodingServer(const char* xmlConfigFileName) {
   if (vodApplication.stopped) return 0;
   ...
   bool useRaptor = v == "true" || v == "1" || v == "yes" || v == "да" ||
                    v == "t" || v == "y" || v == "д";
   ...
   tsBitrate = tsBitrate <= 0
                   ? (encVideoBitrate + encAudioBitrate) * TS_BITRATE_COEFF
                   : tsBitrate;
   coderVBVBufferSize =
       coderVBVBufferSize <= 0 ? encVideoBitrate / 2 : coderVBVBufferSize;

   double vbvBufferLength =
       1000.0 * (double)coderVBVBufferSize / encVideoBitrate;

   const string defaultEncodePattern =
       "--level 3.1 --keyint 25 --min-keyint 5 --subme 6 --bframes 2 --ref 3 "
       "--analyse p8x8,b8x8,i4x4,p4x4 --ratetol 0.1 --qcomp 0.1 --me umh "
       "--threads 0 --thread-input --no-psnr --no-ssim --sar 48:45";

   string encodePattern = pConfig->Attribute("encodePattern");

   if (encodePattern.empty()) encodePattern = defaultEncodePattern;
   ...
   if (batFile.open((toolsDir + "capture_all.bat").c_str(), File::ofWrite)) {
      // batFile.writeLine ( ( "x264.exe --bitrate " + int32ToStr (
      // encVideoBitrate ) + " --vbv-bufsize " + int32ToStr ( encVideoBitrate/2 )
      // + " --vbv-maxrate " + int32ToStr ( encVideoBitrate ) + " --level 3.1
      // --keyint 25 --min-keyint 5 --subme 6 --bframes 2 --ref 3 --analyse
      // p8x8,b8x8,i4x4,p4x4 --ratetol 0.1 --qcomp 0.1 --me umh --threads 0
      // --thread-input --no-psnr --no-ssim --sar 48:45 --output
      // \\\\.\\pipe\\sm_encoded_video " + toolsDir + "capture_all.avs" ).c_str()
      // );
      batFile.writeLine(("x264.exe --bitrate " + int32ToStr(encVideoBitrate) +
                         string(" ") + encodePattern + " --vbv-bufsize " +
                         int32ToStr(coderVBVBufferSize) + " --vbv-maxrate " +
                         int32ToStr(encVideoBitrate) +
                         " --output \\\\.\\pipe\\sm_encoded_video " + toolsDir +
                         "capture_all.avs")
                            .c_str());
      batFile.close();
   }
   ...
   LTRACE(
       LT_DEBUG, 0,
       "TS mux bitrate:" << tsBitrate << ". ts VBV buffer=" << vbvBufferLength);
   ...
   if (useRaptor) {
      ...
      context = vodApplication.streamer->createStreamingGroup(
          *vodApplication.tsAlignedReadManager, "memory://sm_encoded_ts",
          "live_encoding", 0, 1, StreamingContext::llTransportRAW, 0, 0, 0,
          false);
   ...
   if (context) {
      ...
      while (pDest) {
         ...
         if (port > 0 && port < 65536 && !dstAddress.empty())
            addDestination(0, "live_encoding", dstAddress.c_str(), port, 0, 0,
                           0);
...
long LIB_STD_CALL startEncodingServer2(const char* groupName,
                                       const char* srcAddr, const char* ipAddr,
                                       const char* rtpInfo) {
   if (vodApplication.stopped) return 0;
   ...
   Process::sleep(5);
   long handle =
       createStreamingGroup(srcAddr, groupName, 0, 1,
                            StreamingContext::llTransportRTP, 0, 0, 0, false);
   Process::sleep(5);
   addDestination(0, groupName, ipAddr, 0, rtpInfo, 0, 0);
   return handle;
}

unsigned LIB_STD_CALL startEncodingServer3(
    const string transport, const string groupName, const string& toolsRoot,
    const string& batName, const string& videoPipeName,
    const string& audioPipeName, const string& syncSockAddr, const char* ipAddr,
    const char* rtpInfo, int vbvBufferSize) {
   if (vodApplication.stopped) return 0;
   ...
   if (audioPipeName != "none") {
      ...
      LTRACE(LT_INFO, 0,
             "Added AAC audio stream from source " << audioPipeName);
   } else
   ...
   if (transport == "RAW") llTransport = StreamingContext::llTransportRAW;
   tsMuxer->doMux(string("memory://prog_") + groupName, true);
   StreamingContext* context = vodApplication.streamer->createStreamingGroup(
       *vodApplication.tsAlignedReadManager,
       (string("memory://prog_") + groupName).c_str(),
       ("prog_" + groupName).c_str(), 0, 1, llTransport, 0, 0, 0);
   if (vbvBufferSize > 0)
      ((StreamingContextRTP*)context)->setVBVMode(vbvBufferSize, 3);
   if (isExtSync) context->setCorrectTimeOnWait(false);
   if (llTransport == StreamingContext::llTransportRTP)
      addDestination(0, ("prog_" + groupName).c_str(), ipAddr, 0, rtpInfo, 0,
                     0);
   else {
...
unsigned LIB_STD_CALL startEncodingServer3Test(
    const string groupName, const string& toolsRoot, const string& batName,
    const string& videoPipeName, const string& audioPipeName,
    const string& syncSockAddr, const char* ipAddr, const char* rtpInfo) {
   if (vodApplication.stopped) return 0;
   BOOL rez = SetPriorityClass(GetCurrentProcess(), HIGH_PRIORITY_CLASS);
   if (rez == FALSE) LTRACE(LT_WARN, 0, "Can't change process priority.");
   execProcessAsync(closeSlash(toolsRoot) + batName, "");
   // Process::sleep(3000);
   TSMuxer* tsMuxer = new TSMuxer(*vodApplication.tsAlignedReadManager);
   tsMuxer->setAsyncMode(false);
...
LIB_EXPORT int LIB_STD_CALL startStreamingDVD(const char* groupName,
                                              int llTransport) {
   if (vodApplication.stopped) return 0;
...
LIB_EXPORT int LIB_STD_CALL startStreamingDVDExt(const char* groupName,
                                                 int llTransport,
                                                 const char* drive,
                                                 const char* folder) {
   if (vodApplication.stopped) return 0;
   TSMuxer* tsMuxer = 0;
   string sDrive = drive;
   if (!strEndWith(sDrive, "\\") && !strEndWith(sDrive, "/")) sDrive += "\\";
   ...
   try {
      ...
      tsMuxer->setAsyncMode(false);
      tsMuxer->setNewStyleAudioPES(false);
      vector<string> vobList =
          DVDHelper::getFilmVobList(sDrive.c_str(), sFolder.c_str());
      if (vobList.size() == 0) {
      ...
      for (size_t i = 0; i < tracks.size(); i++) {
         if (tracks[i].streamDescr.size() > 0) {  // tracks recognized
            ...
            tsMuxer->addStream(tracks[i].codecInfo.programName, sourceFile,
                               param);
         }
      }
      ...
      // unsigned handle = createStreamingGroup("memory://sm_DVD", groupName, 0,
      // 1, llTransport, 0, 0, 0); StreamingContext* context =
      // (StreamingContext*) handle;
      StreamingContext* context = vodApplication.streamer->createStreamingGroup(
          *vodApplication.tsAlignedReadManager, "memory://sm_DVD", groupName, 0,
          1, llTransport, 0, 0, 0);
      context->setMuxer(tsMuxer);
...
LIB_EXPORT bool LIB_STD_CALL startAutoIndexing(const string& dir,
                                               bool createRewind) {
   if (vodApplication.stopped) return false;
...
LIB_EXPORT void LIB_STD_CALL loadSmartBridge(const char* configDir) {
   try {
      LTRACE(LT_DEBUG, 0,
             "vodTransport::loadSmartBridge called. from: " << configDir)
      ...
      LTRACE(LT_ERROR, e.m_errCode,
             "vodTransport::loadSmartBridge error: " << e.m_errStr)
   } catch (...) {
      LTRACE(LT_ERROR, e.m_errCode,
             "vodTransport::loadSmartBridge error: unknown. Line: " << __LINE__)
   }
}

LIB_EXPORT int LIB_STD_CALL getChecksumForAsset(const char* fileName,
                                                char* buffer, int& size) {
   LTRACE(LT_DEBUG, 0,
          "vodTransport::getChecksumForAsset called. Asset: " << fileName)
         ...
         *buffer = 0;
         ...
         if (fileExists(fileName)) {
            ...
            if (file.open(fileName, File::ofRead)) {
               ...
               if (fileSize == 0) {
                  res = -2;
                  THROW(ERR_COMMON, "file '" << fileName << "' is empty. Line: "
                                             << __LINE__)
               }
               ...
               if (count == -1) {
                  res = -3;
                  THROW(ERR_COMMON, "couldn't read file '"
                                        << fileName << "'. Line: " << __LINE__)
               }
               ...
               if (checkSumSize == 0) {
                  res = -4;
                  THROW(ERR_COMMON,
                        "can't represent checksum like a hex string. Line: "
                            << __LINE__)
               ...
               res = -6;
               THROW(ERR_COMMON, "couldn't open file '"
                                     << fileName << "'. Line: " << __LINE__)
            }
         } else {
            res = -7;
            THROW(ERR_COMMON,
                  "file '" << fileName << "' not found. Line: " << __LINE__)
         }
      }
   } catch (VodCoreException& e) {
      LTRACE(LT_ERROR, e.m_errCode,
             "vodTransport::getChecksumForAsset error: " << e.m_errStr)
   } catch (...) {
      res = -8;
      LTRACE(LT_ERROR, e.m_errCode,
             "vodTransport::getChecksumForAsset error: unknown. Line: "
                 << __LINE__)
   }
...
uint8_t assumeStat(TrafficMonitor* tm, int period, double& avgBitrate,
                   double& maxBitrate) {
   int res = 0;
...
LIB_EXPORT int LIB_STD_CALL getStatisticForPeriod(
    int period, double& avgBroadcastBitrate, double& maxBroadcastBitrate,
    double& avgRunoffBitrate, double& maxRunoffBitrate, char* interval,
    int intervalSize) {
   size_t size = 0;
   ...
   if (vodApplication.stopped) {
      ...
      avgBroadcastBitrate = avgRunoffBitrate = maxRunoffBitrate =
          maxBroadcastBitrate = 0.0;

      *interval = 0;

      bool isValidPeriod = period == 60 || period == 600 || period == 1800 ||
                           period == 3600 || period == 86400;

      if (isValidPeriod) {
         for (int i = 0; i < STREAMERS_COUNT; ++i) {
            nB += assumeStat(
                s->getTrafficMonitor(StreamingContext::smBroadcast, i), period,
                avgBroadcastBitrate, maxBroadcastBitrate);
            nR +=
                assumeStat(s->getTrafficMonitor(StreamingContext::smRunoff, i),
                           period, avgRunoffBitrate, maxRunoffBitrate);
         }
      ...
      if (isValidPeriod) {
         if (nB == 0) LTRACE(LT_WARN, 0, err + "no broadcast traffic info.")

         if (nR == 0) LTRACE(LT_WARN, 0, err + "no runoff traffic info.")
         ...
         if (size < (size_t)intervalSize) {
            ...
            size = -1;
            LTRACE(LT_ERROR, ERR_COMMON,
                   err + "not enough out buffer for interval.")
         }
      } else {
         size = -2;
...
void testContinuityCounter() {
   ...
   while (ts_file.read(buffer, nSize) != -1) {
      ...
      if (pid == 4113 || pid == 4352) {
         if (cnt != 0 && counters[pid] - cnt != 0) counters[pid] = cnt;
      }
...
bool convertCacheInfoFile(const string& fileName) {
   ...
   try {
      if (!fileExists(fileName))
         THROW(ERR_COMMON, "cache info file '"
                               << fileName
                               << "' not faound. Line: " << __LINE__)
      ...
      if (!file.open(fileName.c_str(), File::ofRead))
         THROW(ERR_COMMON,
               "can't open file '" << fileName << "'. Line: " << __LINE__)
      ...
      LTRACE(LT_FATAL_ERROR, __LINE__,
             "convertCacheInfoFile error: unknown. Line: " << __LINE__)
   }
...
void testTCP() {
   ...
   */
   ...
   startStreaming(0, StreamingContext::llTransportRAW, 1024 * 1024, 1,
                  "udp://239.255.4.97:5500", "127.0.0.1", 50000, 0, (long)sck,
                  false);
   // startStreaming ( 0, StreamingContext::llTransportRAW, 1024*1024, 1,
   // "C:/rtspServer/Media/valli.ts", "127.0.0.1", 50000, 0, 0, false );
      ...
      if (idListSize > 0) {
         ...
         for (int j = 0; j < 1; ++j) {
            ...
            for (int i = 0; i < concurrentlyStreamingFilms; ++i) {
               ...
               do {
                  randomIndex =
                      (((double)rand() / (double)RAND_MAX) * RANGE_MAX +
                       RANGE_MIN);
               ...
               LTRACE(LT_DEBUG, 0,
                      "DBG ytTester action start for '"
                          << "youtube://" + rid + "&sid=" + int32uToStr(port)
                          << "'")
               LTRACE(LT_DEBUG, 0, "DBG ytTester action start: before start")

               uint32_t handle = testYouTubeStreaming(port, rid);
               // uint32_t handle = startStreaming ( 0,
               // StreamingContext::llTransportRAW, 0 , 1,
               // "C:\\rtspServer\\Media\\300.ts", "127.0.0.1", 24000, 0, 0 );
            ...
            for (StreamingMap::iterator i = usingIndexes.begin();
                 i != usingIndexes.end(); ++i) {
               int32_t randomAction =
                   (((double)rand() / (double)RAND_MAX) * RANGE_MAX +
                    RANGE_MIN);

               int32_t handle = i->second.first;
               string streamName = "youtube://" + idList[i->first] +
                                   "&sid=" + int32uToStr(i->second.second);

               /*if ( randomAction < 50 )
               {
                  uint64_t duration = 40 * 1000000;
               //vodCoreContext->getYTStreamDuration ( streamName );

                  RANGE_MAX = floor ( ( double )duration / 1000000.0 );

                  uint32_t randomPos = ( ( ( double ) rand() / ( double )
               RAND_MAX ) * RANGE_MAX + RANGE_MIN );

                  randomPos = 120;

                  LTRACE ( LT_DEBUG, 0, "DBG ytTester action set-pos for '" <<
               streamName << "', pos = " << randomPos << ", handle = " << handle
               ) LTRACE ( LT_DEBUG, 0, "DBG ytTester action set-pos: before
               setPos" ) setPosition ( handle, randomPos ); LTRACE ( LT_DEBUG,
               0, "DBG ytTester action set-pos: after setPos" )
               ...
               */
               ...
               LTRACE(LT_DEBUG, 0,
                      "DBG ytTester action stop for '"
                          << streamName << "', handle: " << handle)
...
void startEncodingServer() {
   ...
   if (fileExists("encodingServerCfg.xml")) {
      ...
      while (true) Process::sleep(50);
   } else {
      cout << "SmartLabs encoding server need config file encodeServerCfg.xml!"
           << endl;
...
void testStreamingGroup() {
   ...
   createStreamingGroup("C:\\rtspServer\\Media\\300_10sec.ts", "demuxerTest", 0,
                        1, StreamingContext::llTransportRAW, 0, 0, 0, false);

   addDestination(0, "demuxerTest", "239.255.11.113", 5000, 0, 0, 0);

   addDestination(0, "demuxerTest", "127.0.0.1", 1234, 0, 0, 0);

   // createStreamingGroup ( "udp://239.255.0.100:5500", "demuxerTest", 0, 1,
   // StreamingContext::llTransportRAW, 0, createStreamingGroup (
   // "C:\\rtspServer\\Media\\300.ts", "demuxerTest", 0, 1,
   // StreamingContext::llTransportRAW, 0, 0, 0, false, false, 128, 96, 256, 25,
   // 27, false, 0, true );
...
void testIndexer() {
   ...
   // int rez = createIndexFile ( "C:/rtspServer/Media/NPVR_RECORD_5200455.ts" ,
   // true);
   int rez =
       createIndexFile("C:/rtspServer/Media/NPVR_RECORD_5199513.ts", true);
   ...
   for (int i = 0; i < 1000; ++i)
   {
       ...
       fileSize = (int)( ( ( double ) rand() / ( double ) RAND_MAX ) * rmax +
   rmin );
...
void testMulticastJoin() {
   // multicastJoin(
   // "C:/rtspServer/Media/cyclic_records/pause_live/pause_live.ts",
   // "239.255.0.92", 5500, 1, 300, 0, true , 0, 0 ); multicastJoin(
   // "C:/rtspServer/Media/pause_live.ts", "239.255.0.101", 5500, 1, 0, 0, true ,
   // 0, 0 ); multicastJoin( "C:/rtspServer/ffmpeg.ts", "229.255.0.101", 5700, 1,
   // 0, 0, false , 0, 0 );
   MulticastRetransmitter* mr = MulticastRetransmitter::create(
       "239.255.2.100:5500", NULL, "239.255.2.105:5700", "10.65.10.81:11500",
       true);
   ...
   // char dstBuffer [ 1024*2 ];
   // getAssetDescription ( 0, "C:/rtspServer/Media/pause_live.ts", 0,
   // dstBuffer, sizeof ( dstBuffer ) );
   ...
   // int readerID = startStreaming ( 0, StreamingContext::llTransportRAW, 0, 1,
   // "C:/rtspServer/Media/pause_live.ts", "127.0.0.1", 50000 , 0, 0 );

   int readerID =
       startStreaming(0, StreamingContext::llTransportRAW, 0, 1,
                      "C:/rtspServer/ffmpeg.ts", "127.0.0.1", 50000, 0, 0);
...
void test101() {
   // unsigned handle = startStreaming ( "", StreamingContext::llTransportRAW,
   // 0, 1, "radio://http://www.101.ru/play.m3u?uid=1&bit=2&serv=", "127.0.0.1",
   // 50000, 0, 0 );

   createStreamingGroup("radio://uid=31&proxy=10.65.10.4&port=3128",
                        "radioTest", 0, 1, StreamingContext::llTransportRAW, 0,
                        0, 0, false);
...
void testRTSPServer(int count) {
   ...
   for (int i = 0; i < count; ++i) {
      ...
      if (i > 0) name += "_" + int32ToStr(i);

      name += ".ts";

      RTSPRCGThread* t = new RTSPRCGThread("10.65.10.50:50000",
                                           "10.65.10.79:" + int32ToStr(port++),
                                           name /*list [ i % count ]*/);
...
void testMulticastStreamer() {
   // unsigned handle = startStreaming ( 0, StreamingContext::llTransportRAW, 0,
   // 1, "udp://239.255.0.100:5500", "127.0.0.1", 50000, 0, 0 ); Process::sleep (
   // 5000 ); stopStreaming ( handle ); handle = startStreaming ( 0,
   // StreamingContext::llTransportRAW, 0, 1, "udp://239.255.0.101:5500",
   // "127.0.0.1", 50000, 0, 0 );

   // createStreamingGroup (
   // "igmp://239.255.0.101:5500/transcode?bitrate=900&keepsound=1&codec=h264ld",
   // "mcstTest", 0, 1, StreamingContext::llTransportRAW, 0, 0, 0, false );
   ...
   for (int i = 0; i < count; ++i) {
      ...
      createStreamingGroup(("udp://" + list_src[i] +
                            "/transcode?bitrate=500&keepsound=1&codec=h264ld&"
                            "width=480&height=320")
                               .c_str(),
                           name.c_str(), 0, 1, StreamingContext::llTransportRAW,
                           0, 0, 0, false);
...
void testInternetStreaming() {
   int sHandle =
       startStreaming(0, StreamingContext::llTransportRAW, 0, 1,
                      "udp://239.255.4.97:5500", "10.65.10.79", 50000, 0, 0);
   /*while (1) {
         LTRACE(LT_DEBUG, 0, "before start streaming");
         TCPSocket* tcpSocket = new TCPSocket();
         tcpSocket->connect(SocketAddress("10.65.100.7",8080)); //
   rtsp://195.34.30.227:50000/udp://239.255.4.97:5500 int sHandle =
   startStreaming ( 0, StreamingContext::llTransportRAW, 0, 1,
   "udp://239.255.4.97:5500", "10.65.10.79", 35000, 0, (int)tcpSocket);
...
void testStartStreaming() {
   int h =
       startStreaming(0, StreamingContext::llTransportRAW, 0, 1,
                      "C:/rtspServer/Media/300.ts", "127.0.0.1", 50000, 0, 0);
   ...
   npt = getNptPos(h);
   // int sHandle = startStreaming ( 0, StreamingContext::llTransportRAW, 0, 8,
   // "C:/rtspServer/Media/cyclic_records/pause_live/pause_live.ts", "127.0.0.1",
   // 50000, 0, 0);

   // int sHandle = startStreaming ( 0, StreamingContext::llTransportRAW, 0, 8,
   // "C:/rtspServer/ffmpeg.ts", "127.0.0.1", 50000, 0, 0); int sHandle =
   // startStreaming ( 0, StreamingContext::llTransportRAW, 0, 8,
   // "udp://127.0.0.1:55000", "127.0.0.1", 50000, 0, 0);
}
//-----------------------------------------------------------------------------------------------------------------------------------------
...
void testSmartBridge() {
   ...
   // int sHandle = startStreaming ( 0, StreamingContext::llTransportRAW, 0, 1,
   // name.c_str(), "127.0.0.1", 50000, 0, 0 );
   createStreamingGroup(name.c_str(), "test", 0, 1,
                        StreamingContext::llTransportRAW, 0, 0, 0, false);
   ...
   /*   string name = "dvtube:/s58X0LD5UfsE_13875.flv";
      ...
      //int sHandle = startStreaming ( 0, StreamingContext::llTransportRAW, 0,
      1, name.c_str(), "127.0.0.1", 50000, 0, 0 );
...
void testPlayerCmdEmu(int argc, char** argv) {
  ...
  public:
      Foo ( int j )
      {
         i = new int[j]; //potential garbage. Solution: i = j > 0 ? new int [ j
 ] : NULL;
      }
  ...
  public:
      Bar(int j)
      {
         i = new char[j]; // potential garbage. Solution: i = j > 0 ? new int [
 j ] : NULL; ( k = j > 0 ? new int [ j ] : NULL; )
      }
      ~Bar()
      {
         delete i; // potential memory leak. Solution: delete [] i; ( delete []
 k; )
      }
  private:
  ...
  {
      Foo* f = new Foo ( 100 );
      Foo* b = new Bar ( 200 ); //Forbidden conversion from derived to base
 class (access specifier "private"). Solution: class Bar: public Foo

      *f = *b; // potential memory leak. Solution: explicitly overload Foo&
 Foo::operator= ( const Foo& s ){...}

      delete f;
      delete b; //garbage. like in the example above, b->i can point at invalid
 memory. Solution: char* Bar::k + previous comment.
  }
 ...
 public:
     Foo ( int j )
     {
         i = new int[j]; // potential garbage here. i = j > 0 ? new int [ j ] ?
 NULL;
     }
     virtual ~Foo()
     {
