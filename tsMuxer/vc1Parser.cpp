...
double VC1SequenceHeader::getFPS() {
   if (time_base_num == 0 || time_base_den == 0) return 0;
...
void VC1SequenceHeader::setFPS(double value) {
   ...
   if (m_fpsFieldBitVal > 0) {
      ...
      if ((time_scale == 24000 || time_scale == 25000 || time_scale == 30000 ||
           time_scale == 50000 || time_scale == 60000) &&
         ...
         THROW(ERR_VC1_ERR_FPS,
               "Can't overwrite stream fps. Non standart fps values not "
               "supported "
               "for VC-1 streams");
      if (time_scale == 24000)
         nr = 1;
      ...
      THROW(ERR_VC1_ERR_FPS,
            "Can't overwrite stream fps. Non standart fps values not supported "
            "for VC-1 streams");
}
...
int VC1SequenceHeader::decode_sequence_header() {
   try {
      bitReader.setBuffer(
          m_nalBuffer,
          m_nalBuffer + m_nalBufferLen);  // skip 00 00 01 xx marker
      ...
      if (profile == PROFILE_ADVANCED)
         ...
         if (res_sm) {
            LTRACE(LT_ERROR, 0,
                   "Reserved RES_SM=" << res_sm << " is forbidden");
      ...
      if (res_x8) LTRACE(LT_WARN, 0, "1 for reserved RES_X8 is forbidden");
      ...
      if (res_rtm_flag) LTRACE(LT_WARN, 0, "Old WMV3 version detected.");
...
int VC1SequenceHeader::decode_sequence_header_adv() {
   ...
   if (level >= 5) LTRACE(LT_WARN, 0, "Reserved LEVEL " << level);
 ...
 if(psf) { //PsF, 6.1.13
   ...
   */
   ...
   if (bitReader.getBit()) {  // Display Info - decoding is not affected by it
      ...
      if (bitReader.getBit()) ar = bitReader.getBits(4);
      if (ar && ar < 14) {
...
int VC1SequenceHeader::decode_entry_point() {
   ...
   try {
      ...
      if (extended_mv) extended_dmv = bitReader.getBit();
      if (bitReader.getBit()) {
         // av_log(avctx, AV_LOG_ERROR, "Luma scaling is not supported, expect
         // wrong picture\n");
         bitReader.skipBits(3);  // Y range, ignored for now
      }
      if (bitReader.getBit()) {
         // av_log(avctx, AV_LOG_ERROR, "Chroma scaling is not supported, expect
         // wrong picture\n");
...
int VC1Frame::decode_frame_direct(const VC1SequenceHeader& sequenceHdr,
...
int VC1Frame::vc1_parse_frame_header(const VC1SequenceHeader& sequenceHdr) {
   interpfrm = -1;
   if (sequenceHdr.finterpflag) interpfrm = bitReader.getBit();
   framecnt = bitReader.getBits(2);  // framecnt unused
   rangeredfrm = 0;
   if (sequenceHdr.rangered) rangeredfrm = bitReader.getBit();
...
int VC1Frame::vc1_parse_frame_header_adv(const VC1SequenceHeader& sequenceHdr) {
   fcm = 0;
   if (sequenceHdr.interlace) fcm = decode012(bitReader);
   ...
   */
   ...
   if (sequenceHdr.tfcntrflag) int TFCNTR = bitReader.getBits(8);
