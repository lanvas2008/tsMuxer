...
enum SubPathType { SUBPATH_PIP = 7 };
...
bool isAudioStreamType(int stream_coding_type) {
   return stream_coding_type == 0x80 || stream_coding_type == 0x81 ||
          ...
          stream_coding_type == STREAM_TYPE_AUDIO_AAC ||
          stream_coding_type == STREAM_TYPE_AUDIO_AAC_RAW ||
          stream_coding_type == STREAM_TYPE_AUDIO_MPEG1 ||
          stream_coding_type == STREAM_TYPE_AUDIO_MPEG2;
}
...
bool PS_stream_pack::deserialize(uint8_t* buffer, int buf_size) {
   ...
   if (bitReader.getBits(2) != 1) return false;  // 0b01 required
   m_pts = bitReader.getBits(3) << 30;
   if (bitReader.getBit() != 1) return false;
   ...
   if (bitReader.getBits(2) != 3) return false;
...
bool TS_program_association_section::deserialize(uint8_t* buffer,
                                                 int buf_size) {
   m_nitPID = -1;
   buffer++;
   BitStreamReader bitReader;
   try {
      ...
      if (table_id != 0x0) return false;

      int indicator = bitReader.getBits(
          2);  // section syntax indicator and reserved '0' bit
      ...
      if (bitReader.getBitsCount() != crcBit) return false;
      // uint32_t crc = bitReader.getBits(32);
      // uint32_t rez = av_crc(tmpAvCrc, -1, buffer,
      // get_bits_count(&bitContext)/8-4); rez = my_htonl(rez);
      return true;
...
uint32_t TS_program_association_section::serialize(uint8_t* buffer,
                                                   int buf_size) {
   buffer[0] = 0;
   buffer++;
   ...
   for (std::map<int, int>::const_iterator itr = pmtPids.begin();
        itr != pmtPids.end(); ++itr) {
   ...
   // uint32_t crc = av_crc(tmpAvCrc, 0xffffffff, buffer,
   // bitWriter.getBitsCount()/8);
   bitWriter.flushBits();
...
bool TS_program_map_section::isFullBuff(uint8_t* buffer, int buf_size) {
   ...
   try {
      ...
      return bitReader.getBuffer() + bitReader.getBitsCount() / 8 +
                 section_length <=
             bufEnd;
   } catch (BitStreamException&) {
      return false;
...
void TS_program_map_section::extractPMTDescriptors(uint8_t* curPos,
                                                   int es_info_len) {
...
bool TS_program_map_section::deserialize(uint8_t* buffer, int buf_size) {
   ...
   try {
      ...
      int section_length = bitReader.getBits(12);
      uint8_t* crcPos = bitReader.getBuffer() + bitReader.getBitsCount() / 8 +
                        section_length - 4;
      if (crcPos > bufferEnd) {
      ...
      // the video PID is not available in the scrambled channel veriMatrix
      // 239.255.2.58:5500. Is that similar to the es_info_len crap?
      video_pid = pcr_pid;
      ...
      while (curPos < crcPos) {
         ...
         PMTStreamInfo pmtStreamInfo(stream_type, elementary_pid, 0, 0, 0, "",
                                     false);
...
void TS_program_map_section::extractDescriptors(uint8_t* curPos,
                                                int es_info_len,
                                                PMTStreamInfo& pmtInfo) {
   uint8_t* end = curPos + es_info_len;
   while (curPos < end) {
      uint8_t tag = *curPos;
      uint8_t len = curPos[1];
...
uint32_t TS_program_map_section::serialize(uint8_t* buffer, int max_buf_size,
                                           bool addLang) {
   buffer[0] = 0;
   buffer++;
   ...
   if (casPID) program_info_len += 6;

   int sectionLen = 9 + (video_pid ? 5 : 0) + (audio_pid ? 5 : 0) +
                    (sub_pid ? 5 : 0) + (pidList.size() * 5) + 4 +
                    program_info_len;
   for (PIDListMap::const_iterator itr = pidList.begin(); itr != pidList.end();
        ++itr) {
   ...
   bitWriter.putBits(8, 0xfc);  // scenarist: 0xfc, prev example: 0x84 here 1 0
                                // 000 1 00               1 1 111 1 00
   ...
   for (PIDListMap::const_iterator itr = pidList.begin(); itr != pidList.end();
      ...
      bitWriter.putBits(4, 15);  // reserved
      bitWriter.putBits(
          12, itr->second.m_esInfoLen +
                  (*itr->second.m_lang && addLang ? 6 : 0));  // es_info_len
...
void CLPIParser::parseProgramInfo(
    uint8_t* buffer, uint8_t* end, std::vector<CLPIProgramInfo>& programInfoMap,
   ...
   for (int i = 0; i < number_of_program_sequences; i++) {
      ...
      for (int stream_index = 0;
           stream_index < programInfoMap[i].number_of_streams_in_ps;
           stream_index++) {
...
void CLPIParser::composeProgramInfo(BitStreamWriter& writer, bool isSsExt) {
   uint32_t* lengthPos =
       (uint32_t*)(writer.getBuffer() + writer.getBitsCount() / 8);
   ...
   // for (int i=0; i < number_of_program_sequences; i++)
   {
      ...
      writer.putBits(16,
                     DEFAULT_PMT_PID);  // m_programInfo[i].program_map_PID =

      int streams = 0;
      for (std::map<int, CLPIStreamInfo>::const_iterator itr =
               m_streamInfo.begin();
           itr != m_streamInfo.end(); ++itr) {
         const CLPIStreamInfo& si = itr->second;
         bool streamOK = isSsExt && si.stream_coding_type == 0x20 ||
                         !isSsExt && si.stream_coding_type != 0x20;
         if (streamOK) streams++;
      }
   ...
   if (isSsExt && writer.getBitsCount() % 32 != 0) writer.putBits(16, 0);
...
void CLPIParser::composeTS_type_info_block(BitStreamWriter& writer) {
   // uint16_t length = reader.getBits(16);
   uint16_t* lengthPos =
       (uint16_t*)(writer.getBuffer() + writer.getBitsCount() / 8);
...
void CLPIParser::parseClipInfo(BitStreamReader& reader) {
   ...
   application_type =
       reader.getBits(8);           // 1 - Main TS for a main-path of Movie
...
void CLPIParser::parseSequenceInfo(uint8_t* buffer, uint8_t* end) {
   ...
   for (int atc_id = 0; atc_id < number_of_ATC_sequences; atc_id++) {
      ...
      for (int stc_id = offset_STC_id;
           stc_id < number_of_STC_sequences + offset_STC_id; stc_id++) {
...
void CLPIParser::composeSequenceInfo(BitStreamWriter& writer) {
   ...
   // for (int atc_id = 0; atc_id < number_of_ATC_sequences; atc_id++)
   {
      ...
      // for (int stc_id=offset_STC_id; stc_id < number_of_STC_sequences +
      // offset_STC_id; stc_id++)
      {
...
void CLPIParser::composeCPI(BitStreamWriter& writer, bool isCPIExt) {
   ...
   if (isDependStream && !isCPIExt || !isDependStream && isCPIExt)
      return;  // CPI_SS for MVC depend stream only and vice versa: standart CPI
               // for standart video stream
   ...
   if (isCPIExt && writer.getBitsCount() % 32 != 0) writer.putBits(16, 0);
...
void CLPIParser::composeEP_map(BitStreamWriter& writer, bool isSSExt) {
   ...
   for (std::map<int, CLPIStreamInfo>::iterator itr = m_streamInfo.begin();
        itr != m_streamInfo.end(); ++itr) {
      int coding_type = itr->second.stream_coding_type;
      if (isSSExt) {
         if (coding_type == 0x20) processStream.push_back(itr->second);
      } else {
   ...
   if (processStream.size() == 0)
      THROW(ERR_COMMON,
            "Can't create EP map. One audio or video stream is needed.");
   // ------------------
   writer.putBits(8, 0);  // reserved_for_word_align 8 bslbf
   writer.putBits(
       8, processStream.size());  // number_of_stream_PID_entries 8 uimsbf
   ...
   for (int i = 0; i < processStream.size(); i++) {
      ...
      std::vector<BluRayCoarseInfo> coarseInfo =
          buildCoarseInfo(processStream[i]);
      writer.putBits(
          16, coarseInfo.size());  // number_of_EP_coarse_entries[k] 16 uimsbf
      if (processStream[i].m_index.size() > 0)
         writer.putBits(18,
                        processStream[i]
                            .m_index[m_clpiNum]
                            .size());  // number_of_EP_fine_entries[k] 18 uimsbf
      else
         writer.putBits(18, 0);
      epStartAddrPos.push_back(
          (uint32_t*)(writer.getBuffer() + writer.getBitsCount() / 8));
      writer.putBits(
          32,
          0);  // EP_map_for_one_stream_PID_start_address[k] 32 uimsbf
   }
...
std::vector<BluRayCoarseInfo> CLPIParser::buildCoarseInfo(
    M2TSStreamInfo& streamInfo) {
   std::vector<BluRayCoarseInfo> rez;
   if (streamInfo.m_index.size() == 0) return rez;
   uint32_t cnt = 0;
   uint32_t lastPktCnt = 0;
   uint32_t lastCoarsePts = 0;
   PMTIndex& curIndex = streamInfo.m_index[m_clpiNum];
   for (PMTIndex::const_iterator itr = curIndex.begin(); itr != curIndex.end();
        ++itr) {
      ...
      if (rez.size() == 0 || newCoarsePts != lastCoarsePts ||
          lastCoarseSPN != newCoarseSPN) {
...
void CLPIParser::composeEP_map_for_one_stream_PID(BitStreamWriter& writer,
                                                  M2TSStreamInfo& streamInfo) {
   ...
   for (int i = 0; i < coarseInfo.size(); i++) {
      writer.putBits(
          18, coarseInfo[i].m_fineRefID);  // ref_to_EP_fine_id[i] 18 uimsbf
      writer.putBits(14, coarseInfo[i].m_coarsePts);  // PTS_EP_coarse[i] 14
                                                      // uimsbf
   ...
   *epFineStartAddr = my_htonl(writer.getBitsCount() / 8 - beforePos);
   if (streamInfo.m_index.size() > 0) {
      ...
      for (PMTIndex::const_iterator itr = curIndex.begin();
           itr != curIndex.end(); ++itr) {
         const PMTIndexData& indexData = itr->second;
         ...
         writer.putBits(3,
                        endCode);  // I_end_position_offset[EP_fine_id] 3 bslbf
         writer.putBits(11, (itr->first >> 9) %
                                2048);  // PTS_EP_fine[EP_fine_id] 11 uimsbf
         writer.putBits(17,
                        indexData.m_pktCnt %
                            (65536 * 2));  // SPN_EP_fine[EP_fine_id] 17 uimsbf
      }
   }
}
...
bool CLPIParser::parse(const char* fileName) {
   File file;
   if (!file.open(fileName, File::ofRead)) return false;
   uint64_t fileSize;
...
void CLPIParser::HDMV_LPCM_down_mix_coefficient(uint8_t* buffer,
                                                int dataLength) {}
...
void CLPIParser::ProgramInfo_SS(uint8_t* buffer, int dataLength) {
   parseProgramInfo(buffer, buffer + dataLength, m_programInfoMVC,
                    m_streamInfoMVC);
}
...
void CLPIParser::composeExtentInfo(BitStreamWriter& writer) {
   ...
   if (interleaveInfo.empty()) return;
   ...
   lengthPos[1] = my_htonl(writer.getBitsCount() / 8 -
                           beforeCount);  // data_block_start_address, same as
                                          // extentStart point start
   ...
   *extentStartPos = my_htonl(writer.getBitsCount() / 8 - beforeCount);
   ...
   if (isDependStream) {
      ...
      *ProgramInfo_StartPos = my_htonl(writer.getBitsCount() / 8 - beforeCount);
      ...
      ProgramInfo_StartPos[1] =
          my_htonl(writer.getBitsCount() / 8 - beforeExtentCount);
...
void MPLSParser::parse(uint8_t* buffer, int len) {
   ...
   try {
      ...
      parsePlayList(buffer + playList_start_address,
                    len - playList_start_address);
      /*
      for (int i=0; i<N1; i++) {
      ...
      */
      parsePlayListMark(buffer + playListMark_start_address,
                        len - playListMark_start_address);
      ...
      */
      ...
      THROW(ERR_COMMON,
            "Can't parse media playlist file: unexpected end of data");
   }
}
...
int MPLSParser::composeSubPathEntryExtension(uint8_t* buffer, int bufferSize) {
   ...
   try {
      ...
      // composeSubPath(writer, 0, dependStreamInfo.m_index, 5); // MVC dep
      // stream type
      writer.putBits(16, 1);  // one subpath
      SubPath_extension(writer);

      if (writer.getBitsCount() % 32 != 0) writer.putBits(16, 0);
...
int MPLSParser::composeSTN_tableSS(uint8_t* buffer, int bufferSize) {
   ...
   try {
      ...
      for (int PlayItem_id = 0; PlayItem_id < streamInfoMVC.m_index.size();
           PlayItem_id++) {
...
int MPLSParser::compose(uint8_t* buffer, int bufferSize, DiskType dt) {
   for (int i = 0; i < m_streamInfo.size(); i++) {
      ...
      if (m_streamInfo[i].isSecondary &&
          isVideoStreamType(stream_coding_type)) {
         number_of_SubPaths++;
   ...
   writer.putBits(32, 0);
   uint32_t* playListMark_bit_pos =
       (uint32_t*)(buffer + writer.getBitsCount() / 8);
   writer.putBits(32, 0);
   ...
   if (number_of_SubPaths > 0 || isDependStreamExist) {
      *extDataStartAddr = my_htonl(writer.getBitsCount() / 8);
      ...
      if (number_of_SubPaths > 0) {
         int bufferSize = composePip_metadata(buffer, sizeof(buffer),
                                              mainStreamInfo.m_index);
...
void MPLSParser::AppInfoPlayList(BitStreamReader& reader) {
   ...
   if (PlayList_playback_type == 2 ||
       PlayList_playback_type == 3) {  // 1 == Sequential playback of PlayItems
...
void MPLSParser::composeAppInfoPlayList(BitStreamWriter& writer) {
   ...
   writer.putBit(1);  // audio_mix_app_flag. 0 == no secondary audio, 1- allow
                      // secondary audio if exist
...
void MPLSParser::UO_mask_table(BitStreamReader& reader) {
   reader.skipBit();  // reserved_for_future_use // reserved for menu call mask
                      // 1 bslbf
   reader.skipBit();  // reserved_for_future_use // reserved for title search
                      // mask 1 bslbf
   ...
   reader.skipBit();  // reserved_for_future_use // reserved for play FirstPlay
                      // mask 1 bslbf
   stop_mask = reader.getBit();      // 1 bslbf
   pause_on_mask = reader.getBit();  // 1 bslbf
   reader.skipBit();  // reserved_for_future_use // reserved for pause off mask
                      // 1 bslbf
...
MPLSStreamInfo& MPLSParser::getMainStream() {
   for (int i = 0; i < m_streamInfo.size(); i++) {
      ...
      if (isVideoStreamType(coding_type)) return m_streamInfo[i];
   }
   for (int i = 0; i < m_streamInfo.size(); i++) {
      int coding_type = m_streamInfo[i].stream_coding_type;
      if (isAudioStreamType(coding_type)) {
   ...
   THROW(ERR_COMMON,
         "Can't find stream index. One audio or video stream is needed.");
}
...
MPLSStreamInfo MPLSParser::getStreamByPID(int pid) const {
   for (int i = 0; i < m_streamInfo.size(); i++) {
      if (m_streamInfo[i].streamPID == pid) return m_streamInfo[i];
   }
...
std::vector<MPLSStreamInfo> MPLSParser::getPgStreams() const {
   ...
   for (int i = 0; i < m_streamInfo.size(); i++) {
      int coding_type = m_streamInfo[i].stream_coding_type;
      if (coding_type == 0x90) pgStreams.push_back(m_streamInfo[i]);
   }
   return pgStreams;
}
...
MPLSStreamInfo& MPLSParser::getMVCDependStream() {
   for (int i = 0; i < m_streamInfoMVC.size(); i++) {
      ...
      if (coding_type == 0x20) return m_streamInfoMVC[i];
   }
   THROW(ERR_COMMON,
         "Can't find stream index. One audio or video stream is needed.");
}
...
void MPLSParser::composePlayList(BitStreamWriter& writer) {
   ...
   MPLSStreamInfo& mainStreamInfo = getMainStream();
   writer.putBits(
       16, mainStreamInfo.m_index.size());  // 16 uimsbf number_of_PlayItems
   writer.putBits(16, number_of_SubPaths);  // number_of_SubPaths
   // connection_condition = 1;
   for (int PlayItem_id = 0; PlayItem_id < mainStreamInfo.m_index.size();
        PlayItem_id++) {
   ...
   for (int SubPath_id = 0;
        SubPath_id < number_of_SubPaths * dependStreamInfo.m_index.size();
        SubPath_id++) {
      composeSubPath(writer, SubPath_id, dependStreamInfo.m_index,
                     subPath_type);  // pip
   }
...
void MPLSParser::composeSubPath(BitStreamWriter& writer, int subPathNum,
                                std::vector<PMTIndex>& pmtIndexList, int type) {
   uint32_t* lengthPos =
       (uint32_t*)(writer.getBuffer() + writer.getBitsCount() / 8);
   writer.putBits(32, 0);  // skip lengthField
   int beforeCount = writer.getBitsCount() / 8;
   ...
   writer.putBits(8, type);  // SubPath_type = 7 (In-mux and Synchronous type of
                             // Picture-in-Picture), 5 - MVC depend stream
   ...
   {
           int stream_coding_type = m_streamInfo[i].stream_coding_type;
           if (m_streamInfo[i].isSecondary && (stream_coding_type==0x02 ||
   stream_coding_type==0x1B || stream_coding_type==0xEA))
...
void MPLSParser::composeSubPlayItem(BitStreamWriter& writer, int playItemNum,
                                    int subPathNum,
   ...
   std::string clip_Information_file_name =
       strPadLeft(int32ToStr(fileNum + m_m2tsOffset), 5, '0');
   ...
   if (playItemNum == pmtIndexList.size() - 1)
      ...
      writer.putBits(
          32, pmtIndexList[playItemNum + 1].begin()->first / 2);  // 32 uimsbf
   else
      writer.putBits(32, OUT_time);  // 32 uimsbf

   writer.putBits(16,
                  playItemNum);  // sync_PlayItem_id. reference to play_item id.
...
int MPLSParser::composePip_metadata(uint8_t* buffer, int bufferSize,
                                    std::vector<PMTIndex>& pmtIndexList) {
   // The ID1 value and the ID2 value of the ExtensionData() shall be set to
   // 0x0001 and 0x0001
   BitStreamWriter writer;
   writer.setBuffer(buffer, buffer + bufferSize);
   ...
   for (int i = 0; i < pmtIndexList.size(); ++i) {
      for (int k = 0; k < pipStreams.size(); k++) {
         ...
         writer.putBits(8, 0);   // reserved_for_future_use
         writer.putBits(4, pipParams.lumma >= 0
                               ? 1
                               : 0);  // pip_timeline_type == 1. Synchronous
                                      // type of Picture-in-Picture
         writer.putBit(1);  // is_luma_key = 0
         writer.putBit(
             1);  // trick_playing_flag. keep PIP windows when tricking
   ...
   for (int i = 0; i < pmtIndexList.size(); ++i) {
      for (int k = 0; k < pipStreams.size(); k++) {
         ...
         *(blockDataAddressPos[i * pipStreams.size() + k]) =
             my_htonl(writer.getBitsCount() / 8);
         ...
         writer.putBits(16, 1);  // number_of_pip_metadata_entries
         {
            ...
            if (!pipParams.isFullScreen()) {
               ...
               if (pipParams.corner == PIPParams::TopRight ||
                   pipParams.corner == PIPParams::BottomRight)
                  hPos = mainHSize - pipWidth - pipParams.hOffset;
               if (pipParams.corner == PIPParams::BottomRight ||
                   pipParams.corner == PIPParams::BottomLeft)
            ...
            writer.putBits(
                4,
                pipParams.scaleIndex);  // pip_scale[i] 4 uimsbf. 1 == no_scale
...
void MPLSParser::parseStnTableSS(uint8_t* data, int dataLength) {
   try {
      ...
      int firstPgIndex = number_of_primary_video_stream_entries +
      ...
      if (size > 0) {
         ...
         if (type == 8 || type == 9) {
            ...
            for (int i = 0; i < subPlayItems; ++i) {
               ...
               CLPIStreamInfo::readString(clip_Information_file_name, reader,
                                          5);
               ...
               if (isMulticlip) {
                  ...
                  for (int j = 1; j < numberOfClipEntries; ++j) {
                     char clip_Information_file_name[6];
                     CLPIStreamInfo::readString(clip_Information_file_name,
                                                reader, 5);
                     m_mvcFiles.push_back(clip_Information_file_name);
                     reader.skipBits(32);  // clip codec identifier
                     reader.skipBits(8);   // ref to stc id
...
void MPLSParser::composeExtensionData(
    BitStreamWriter& writer, vector<ExtDataBlockInfo>& extDataBlockInfo) {
   ...
   if (extDataBlockInfo.size() > 0) {
      ...
      for (int i = 0; i < extDataBlockInfo.size(); i++) {
         ...
         extDataStartAddrPos[i] =
             (uint32_t*)(writer.getBuffer() + writer.getBitsCount() / 8);
         writer.putBits(32, 0);  // ext_data_start_address
         writer.putBits(32,
                        extDataBlockInfo[i].data.size());  // ext_data_length
      }
      while ((writer.getBitsCount() / 8 - initPos) % 4 != 0)
         writer.putBits(16, 0);
      *(lengthPos + 1) = my_htonl(writer.getBitsCount() / 8 - initPos +
                                  4);  // data_block_start_address
      for (int i = 0; i < extDataBlockInfo.size(); i++) {
         *(extDataStartAddrPos[i]) =
             my_htonl(writer.getBitsCount() / 8 - initPos + 4);
...
void MPLSParser::parsePlayItem(BitStreamReader& reader, int PlayItem_id) {
   ...
   if (is_multi_angle == 1) {
      ...
      for (int angle_id = 1;                       // angles except angle_id=0
         ...
         CLPIStreamInfo::readString(clip_Information_file_name, reader,
                                    5);  // 8*5 bslbf
         CLPIStreamInfo::readString(clip_codec_identifier, reader,
                                    4);      // 8*4 bslbf
         ref_to_STC_id = reader.getBits(8);  // 8 uimsbf
      }
   }
...
void MPLSParser::composePlayItem(BitStreamWriter& writer, int playItemNum,
                                 std::vector<PMTIndex>& pmtIndexList) {
   uint16_t* lengthPos =
       (uint16_t*)(writer.getBuffer() + writer.getBitsCount() / 8);
   writer.putBits(16, 0);  // skip lengthField
   int beforeCount = writer.getBitsCount() / 8;
   int fileNum = playItemNum;
   if (isDependStreamExist) fileNum *= 2;
   std::string clip_Information_file_name =
       strPadLeft(int32ToStr(fileNum + m_m2tsOffset), 5, '0');
   CLPIStreamInfo::writeString(clip_Information_file_name.c_str(), writer, 5);
   char clip_codec_identifier[] = "M2TS";
...
void MPLSParser::parsePlayListMark(uint8_t* buffer, int len) {
   ...
   for (int PL_mark_id = 0; PL_mark_id < number_of_PlayList_marks;
        PL_mark_id++) {
      reader.skipBits(8);                 // reserved_for_future_use 8 bslbf
...
int MPLSParser::calcPlayItemID(MPLSStreamInfo& streamInfo, uint32_t pts) {
   for (int i = 0; i < streamInfo.m_index.size(); i++) {
      if (streamInfo.m_index[i].size() > 0) {
         if (streamInfo.m_index[i].begin()->first > pts) return FFMAX(i - 1, 0);
      }
   }
...
void MPLSParser::composePlayListMark(BitStreamWriter& writer) {
   ...
   for (int i = 0; i < m_marks.size(); i++) {
      ...
      if (m_marks[i].m_playItemID >= 0)
         ...
         writer.putBits(16, calcPlayItemID(streamInfo, m_marks[i].m_markTime *
                                                           2));  // play item ID
...
void MPLSParser::composePlayListMark(BitStreamWriter& writer)
{
        uint32_t* lengthPos = (uint32_t*) (writer.getBuffer() +
writer.getBitsCount()/8); writer.putBits(32,0); // skip lengthField int
beforeCount = writer.getBitsCount()/8;
...
void MPLSParser::composeSTN_table(BitStreamWriter& writer, int PlayItem_id,
                                  bool isSSEx) {
   uint16_t* lengthPos =
       (uint16_t*)(writer.getBuffer() + writer.getBitsCount() / 8);
   writer.putBits(16, 0);  // skip lengthField
   ...
   std::vector<MPLSStreamInfo>& streamInfo =
       isSSEx ? m_streamInfoMVC : m_streamInfo;
   ...
   for (int i = 0; i < streamInfo.size(); i++) {
      ...
      if (isVideoStreamType(stream_coding_type)) {
         ...
         THROW(ERR_COMMON, "Unsupported media type "
                               << stream_coding_type
                               << " for AVCHD/Blu-ray muxing");
   }

   if (!isSSEx) {
      writer.putBits(
          8,
          number_of_primary_video_stream_entries);  // 8 uimsbf // test subpath
      writer.putBits(8, number_of_primary_audio_stream_entries);
      writer.putBits(8, number_of_PG_textST_stream_entries);
      writer.putBits(8,
                     0);  // int number_of_IG_stream_entries =
                          // reader.getBits(8); //8 uimsbf
      writer.putBits(8, number_of_secondary_audio_stream_entries);
      writer.putBits(
          8,
          number_of_secondary_video_stream_entries);  // int
                                                      // number_of_secondary_video_stream_entries.
                                                      // Now subpath used for
                                                      // second video stream
      writer.putBits(8, 0);  // int number_of_PiP_PG_textST_stream_entries_plus
                             // = reader.getBits(8); //8 uimsbf
   ...
   for (int i = 0; i < number_of_primary_video_stream_entries; i++) {
      ...
      if (!streamInfo[i].isSecondary && isVideoStreamType(stream_coding_type)) {
         ...
         if (stream_coding_type == 0x20) {
            ...
            writer.putBits(6,
                           FFMAX(1, streamInfo[i].number_of_offset_sequences));
         }
      }
   }
   ...
   for (int i = 0; i < streamInfo.size(); i++) {
      ...
      if (isAudioStreamType(stream_coding_type) && streamInfo[i].isSecondary) {
         ...
         if (number_of_secondary_video_stream_entries == 0) {
            writer.putBits(
                8,
                number_of_primary_audio_stream_entries);  // number_of_primary_audio_ref_entries
                                                          // allowed to join
                                                          // with this stream
            ...
            for (int j = 0; j < streamInfo.size(); j++) {
               ...
               if (isAudioStreamType(stream_coding_type) &&
                   !streamInfo[j].isSecondary)
   ...
   for (int i = 0; i < streamInfo.size(); i++) {
      ...
      if (streamInfo[i].isSecondary && isVideoStreamType(stream_coding_type)) {
         ...
         int useSecondaryAudio =
             number_of_secondary_audio_stream_entries > secondaryVNum ? 1 : 0;
   ...
   if (isSSEx && writer.getBitsCount() % 32 != 0) writer.putBits(16, 0);
...
void MPLSParser::STN_table(BitStreamReader& reader, int PlayItem_id) {
   ...
   for (int primary_video_stream_id = 0;
        primary_video_stream_id < number_of_primary_video_stream_entries;
        primary_video_stream_id++) {
      MPLSStreamInfo streamInfo;
      streamInfo.parseStreamEntry(reader);
      streamInfo.parseStreamAttributes(reader);
      if (PlayItem_id == 0) m_streamInfo.push_back(streamInfo);
   }
   for (int primary_audio_stream_id = 0;
        primary_audio_stream_id < number_of_primary_audio_stream_entries;
   ...
   for (int PG_textST_stream_id = 0;
        PG_textST_stream_id < number_of_PG_textST_stream_entries +
                                  number_of_PiP_PG_textST_stream_entries_plus;
        PG_textST_stream_id++) {
      MPLSStreamInfo streamInfo;
      streamInfo.parseStreamEntry(reader);
      streamInfo.parseStreamAttributes(reader);
   ...
   for (int IG_stream_id = 0; IG_stream_id < number_of_IG_stream_entries;
        IG_stream_id++) {
      MPLSStreamInfo streamInfo;
      streamInfo.parseStreamEntry(reader);
      streamInfo.parseStreamAttributes(reader);
   ...
   for (int secondary_audio_stream_id = 0;
        secondary_audio_stream_id < number_of_secondary_audio_stream_entries;
   ...
   for (int secondary_video_stream_id = 0;
        secondary_video_stream_id < number_of_secondary_video_stream_entries;
      ...
      // comb_info_Secondary_video_Secondary_audio(){
      int number_of_secondary_audio_ref_entries =
          reader.getBits(8);  // 8 uimsbf
      reader.skipBits(8);     // reserved_for_word_align 8 bslbf
...
void M2TSStreamInfo::blurayStreamParams(double fps, bool interlaced, int width,
                                        int height, float ar, int* video_format,
   ...
   *aspect_ratio_index = 3;  // 16:9; 2 = 4:3
   ...
   bool isNtsc = width <= 854 && height <= 480 &&
                 (fabs(25 - fps) >= 0.5 && fabs(50 - fps) >= 0.5);
   bool isPal = width <= 1024 && height <= 576 &&
                (fabs(25 - fps) < 0.5 || fabs(50 - fps) < 0.5);
   ...
   if (ar == AR_3_4 || ar == AR_VGA) *aspect_ratio_index = 2;  // 4x3
}
...
M2TSStreamInfo::M2TSStreamInfo(const PMTStreamInfo& pmtStreamInfo) {
   ...
   if (pmtStreamInfo.m_codecReader != 0) {
      MPEGStreamReader* vStream =
          dynamic_cast<MPEGStreamReader*>(pmtStreamInfo.m_codecReader);
      if (vStream) {
         width = vStream->getStreamWidth();
         height = vStream->getStreamHeight();
         VideoAspectRatio ar = vStream->getStreamAR();
         blurayStreamParams(vStream->getFPS(), vStream->getInterlaced(),
                            vStream->getStreamWidth(),
                            vStream->getStreamHeight(), ar, &video_format,
      ...
      H264StreamReader* h264Stream =
          dynamic_cast<H264StreamReader*>(pmtStreamInfo.m_codecReader);
      if (h264Stream)
         number_of_offset_sequences = h264Stream->getOffsetSeqCnt();

      SimplePacketizerReader* aStream =
          dynamic_cast<SimplePacketizerReader*>(pmtStreamInfo.m_codecReader);
      if (aStream) {
         ...
         // todo: add index 12 and 14. 12: 48Khz core, 192Khz mpl, 14: 48Khz
         // core, 96Khz mlp
         switch (freq) {
            case 48000:
...
MPLSStreamInfo::MPLSStreamInfo(const MPLSStreamInfo& other)
   ...
   leftEye = 0;
   rightEye = 0;
   if (other.leftEye) leftEye = new MPLSStreamInfo(*other.leftEye);
   if (other.rightEye) rightEye = new MPLSStreamInfo(*other.rightEye);
}
...
void MPLSStreamInfo::composePGS_SS_StreamEntry(BitStreamWriter& writer,
                                               int entryNum) {
...
void MPLSStreamInfo::composeStreamEntry(BitStreamWriter& writer, int entryNum,
                                        int subPathID) {
   uint8_t* lengthPos = writer.getBuffer() + writer.getBitsCount() / 8;
   ...
   if (type == 1) {
      ...
      writer.putBits(8,
                     subPathID);  // ref_to_SubPath_id. constant subPathID == 0
      writer.putBits(16, streamPID);  // 16 uimsbf
      writer.putBits(32, 0);          // reserved_for_future_use 48 bslbf
      writer.putBits(8, 0);
   } else
...
void MovieObject::parseMovieObjects(BitStreamReader& reader) {
   ...
   for (int mobj_id = 0; mobj_id < number_of_mobjs; mobj_id++) {
      ...
      int number_of_navigation_commands =
          reader.getBits(16);  //[mobj_id] 16 uimsbf
      for (int command_id = 0; command_id < number_of_navigation_commands;
           command_id++) {
         parseNavigationCommand(
             reader);  // navigation_command[mobj_id][command_id] 96 bslbf
      }
   }
}
...
void MovieObject::parseNavigationCommand(BitStreamReader& reader) {
   ...
   */
   ...
   bool iFlag2 = false;
   if (iFlag1 == 0) {
      bool reg_flag1 = reader.getBit();  // 0: General Purpose Register, 1:
