...
bool TS_program_association_section::deserialize(uint8_t* buffer,
  ...
  try {
    ...
    int indicator =
        bitReader.getBits(2);  // section syntax indicator and reserved '0' bit
...
uint32_t TS_program_map_section::serialize(uint8_t* buffer, int max_buf_size,
  ...
  for (PIDListMap::const_iterator itr = pidList.begin(); itr != pidList.end();
    ...
    if (*itr->second.m_lang && addLang) sectionLen += 6;  // lang descriptor len
  }
  ...
  *epFineStartAddr = my_htonl(writer.getBitsCount() / 8 - beforePos);
  if (streamInfo.m_index.size() > 0) {
    ...
    for (PMTIndex::const_iterator itr = curIndex.begin(); itr != curIndex.end();
      ...
      writer.putBits(
          11, (itr->first >> 9) % 2048);  // PTS_EP_fine[EP_fine_id] 11 uimsbf
      writer.putBits(17, indexData.m_pktCnt %
...
int MPLSParser::composeSubPathEntryExtension(uint8_t* buffer, int bufferSize) {
  ...
  try {
    ...
    // composeSubPath(writer, 0, dependStreamInfo.m_index, 5); // MVC dep stream
    // type
  ...
  if (number_of_SubPaths > 0 || isDependStreamExist) {
    *extDataStartAddr = my_htonl(writer.getBitsCount() / 8);
    ...
    if (number_of_SubPaths > 0) {
      int bufferSize =
          composePip_metadata(buffer, sizeof(buffer), mainStreamInfo.m_index);
...
void MPLSParser::UO_mask_table(BitStreamReader& reader) {
  reader.skipBit();  // reserved_for_future_use // reserved for menu call mask 1
                     // bslbf
  ...
  reader.skipBit();  // reserved_for_future_use // reserved for pause off mask 1
                     // bslbf
  ...
  for (int i = 0; i < pmtIndexList.size(); ++i) {
    for (int k = 0; k < pipStreams.size(); k++) {
      ...
      writer.putBits(4, pipParams.lumma >= 0
                            ? 1
                            : 0);  // pip_timeline_type == 1. Synchronous type
    ...
    for (int k = 0; k < pipStreams.size(); k++) {
      ...
      *(blockDataAddressPos[i * pipStreams.size() + k]) =
      ...
      writer.putBits(16, 1);  // number_of_pip_metadata_entries
      {
        ...
        writer.putBits(
            4, pipParams.scaleIndex);  // pip_scale[i] 4 uimsbf. 1 == no_scale
...
void MPLSParser::parseStnTableSS(uint8_t* data, int dataLength) {
  try {
    ...
    for (int i = 0; i < number_of_PG_textST_stream_entries; ++i) {
      ...
      if (idx != -1) m_streamInfo[idx].offsetId = PG_textST_offset_sequence_id;
...
void MPLSParser::parseExtensionData(uint8_t* data, uint8_t* dataEnd) {
  try {
    ...
    for (int i = 0; i < entries; ++i) {
      ...
      if (dataAddress + dataLength > dataEnd - data) continue;  // invalid entry
...
void MPLSParser::composeSTN_table(BitStreamWriter& writer, int PlayItem_id,
  ...
  if (!isSSEx) {
    writer.putBits(
        8, number_of_primary_video_stream_entries);  // 8 uimsbf // test subpath
    writer.putBits(8, number_of_primary_audio_stream_entries);
    writer.putBits(8, number_of_PG_textST_stream_entries);
    writer.putBits(
        8,
        0);  // int number_of_IG_stream_entries = reader.getBits(8); //8 uimsbf
  ...
  if (pmtStreamInfo.m_codecReader != 0) {
    ...
    if (vStream) {
      ...
      blurayStreamParams(vStream->getFPS(), vStream->getInterlaced(),
                         vStream->getStreamWidth(), vStream->getStreamHeight(),
                         ar, &video_format, &frame_rate_index,
                         &aspect_ratio_index);
      if (ar == AR_3_4)
        width = height * 4 / 3;
      else if (ar == AR_16_9)
    ...
    if (h264Stream) number_of_offset_sequences = h264Stream->getOffsetSeqCnt();
    ...
    if (aStream) {
      ...
      // todo: add index 12 and 14. 12: 48Khz core, 192Khz mpl, 14: 48Khz core,
      // 96Khz mlp
      switch (freq) {
        case 48000:
...
void MovieObject::parseNavigationCommand(BitStreamReader& reader) {
  ...
  */
  ...
  bool iFlag2 = false;
  if (iFlag1 == 0) {
    bool reg_flag1 = reader.getBit();  // 0: General Purpose Register, 1: Player
                                       // Status Registers
    reader.skipBits(19);
