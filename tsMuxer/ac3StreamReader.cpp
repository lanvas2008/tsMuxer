...
bool AC3StreamReader::isPriorityData(AVPacket* packet) {
   // return packet->size >= 2 && packet->data[0] == 0x0B && packet->data[1] ==
   // 0x77 && m_bsid <= 10 && m_state != stateDecodeTrueHD;
   bool rez = packet->size >= 2 && packet->data[0] == 0x0B &&
              packet->data[1] == 0x77 && m_bsid <= 10;
...
void AC3StreamReader::writePESExtension(PESPacket* pesPacket,
...
int AC3StreamReader::getTSDescriptor(uint8_t* dstBuff) {
   ...
   *dstBuff++ = 0x4;   // descriptor len
   ...
   bitWriter.putBits(4,
                     m_acmod);  // when MSB == 0 then high (4-th) bit always 0
   ...
   if (rez > 0 && m_true_hd_mode && !m_downconvertToAC3) {
      if (!(avPacket.flags & AVPacket::PRIORITY_DATA))
         avPacket.pts = avPacket.dts =
             m_totalTHDSamples * 1000000000ll / mh.group1_samplerate -
