...
std::string toNativeSeparators(const std::string& dirName) {
  ...
  for (int i = 0; i < rez.length(); ++i)
    if (rez[i] == '\\' || rez[i] == '/') rez[i] = getDirSeparator();
  return rez;
}

bool isFillerNullPacket(uint8_t* curBuf) {
  uint32_t* endPos = (uint32_t*)(curBuf + TS_FRAME_SIZE);
  for (uint32_t* curBuf32 = (uint32_t*)(curBuf + 4); curBuf32 < endPos;
       curBuf32++)
    if (*curBuf32 != 0xffffffff && *curBuf32 != 0) return false;
  return true;
}

std::string unquoteStr(const std::string& val) {
  std::string tmp = val;
  if (val.size() > 0 && val[0] == '\"') tmp = tmp.substr(1, tmp.size());
  ...
  for (int i = 0; i < val.size(); i++) {
    if (val[i] == '"')
      ...
      if (i > lastStartPos) {
        ...
        tmp = trimStr(tmp);
        tmp = unquoteStr(tmp);
        if (tmp.size() > 0) rez.push_back(tmp);
      }
      lastStartPos = i + 1;
    }
  }
...
uint32_t AV_RB32(const uint8_t* buffer) {
  return my_ntohl(*((int32_t*)buffer));
}
...
void AV_WB16(uint8_t* buffer, uint16_t value) {
  *((uint16_t*)buffer) = my_htons(value);
}
...
void AV_WB32(uint8_t* buffer, uint32_t value) {
  *((uint32_t*)buffer) = my_htonl(value);
}
...
std::string floatToTime(double time, char msSeparator) {
  ...
  str << strPadLeft(int32ToStr(sec), 2, '0') << msSeparator
      << strPadLeft(int32ToStr(msec), 3, '0');
  return str.str();
}
