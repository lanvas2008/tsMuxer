...
ProgramStreamDemuxer::ProgramStreamDemuxer(
    const BufferedReaderManager& readManager)
...
void ProgramStreamDemuxer::openFile(const std::string& streamName) {
   ...
   readClose();
   // BufferedFileReader* fileReader = dynamic_cast <BufferedFileReader*>
   // (m_bufferedReader);
...
int ProgramStreamDemuxer::mpegps_psm_parse(uint8_t* buff, uint8_t* end) {
   if (end - buff < 7) return -1;
   uint8_t* curBuff = buff + 4;
   // int map_stream_id = *curBuff++;
   int psm_length = (*curBuff << 8) + curBuff[1];
   if (psm_length > MAX_PES_HEADER_SIZE) {
      THROW(ERR_COMMON, "Can't parse Program Stream Map. Too large size "
                            << psm_length << ". Max allowed size 1018 bytes.");
   }
   if (end - buff < psm_length + 7) return -1;
...
long ProgramStreamDemuxer::processPES(uint8_t* buff, uint8_t* end,
                                      int& afterPesHeader) {
   afterPesHeader = 0;
   ...
   if (startcode == PACK_START_CODE) return 0;
   if (startcode == SYSTEM_HEADER_START_CODE) return 0;
   ...
   if (!((startcode >= 0xc0 && startcode <= 0xdf) ||
         (startcode >= 0xe0 && startcode <= 0xef) || (startcode == 0xbd) ||
         (startcode == 0xfd)))
      return 0;
   ...
   if (pesPacket->flagsLo & 0x01) {  // PES_extension_flag 1 bslbf
      ...
      if (extFlag & 0x01) {  // PES_extension_flag_2
         ...
         if (ext2_len > 0) startcode = (startcode << 8) + *curBuf;
         curBuf += ext2_len;
      }
   }
   ...
   if (startcode == PES_PRIVATE_DATA1 && m_psm_es_type[startcode] == 0) {
      ...
      if (startcode >= 0xb0 &&
          startcode <=
              0xbf) {  // MLP/TrueHD audio has a 4-byte header, keep header
         ...
         if (waveHeader.isEmpty()) {
            ...
            wave_format::buildWaveHeader(waveHeader, samplerate, channels,
                                         channels >= 6, bitdepth);
         }

         // uint16_t dataLen = pesPacket->getPacketLength() -
         // pesPacket->getHeaderLength() - 7; AV_WB16(curBuf+1, dataLen);
         // afterPesHeader++;
         afterPesHeader += 6;

         uint8_t* payloadData = curBuf + afterPesHeader - 1;
         uint32_t pesPayloadLen = pesPacket->getPacketLength() -
                                  pesPacket->getHeaderLength() - afterPesHeader;
         wave_format::toLittleEndian(payloadData, payloadData, pesPayloadLen,
                                     bitdepth);
...
void ProgramStreamDemuxer::getTrackList(
    std::map<uint32_t, TrackInfo>& trackList) {
   for (int i = 0x20; i < 0xff; i++) {
      if (i >= 0xa0 && i <= 0xaf)
         trackList.insert(std::make_pair(
             i, TrackInfo(STREAM_TYPE_AUDIO_LPCM, "", 0)));  // set track hint
      else
         trackList.insert(
             std::make_pair(i, TrackInfo(0, "", 0)));  // autodetect
   }
   return;
}
...
int ProgramStreamDemuxer::simpleDemuxBlock(DemuxedData& demuxedData,
   ...
   while (curBuf <= end - 9) {
      ...
      if ((startcode >= 0xc0 && startcode <= 0xef) ||
         ...
         if (curBuf + pesPacket->getHeaderLength() + 4 > end) break;
         ...
         if (acceptedPIDs.find(startcode) != acceptedPIDs.end()) {
            if ((pesPacket->flagsLo & 0x80) == 0x80) {
               ...
               pesPacket->getDts() : pesPacket->getPts(); int32_t dtsDif =
               (int64_t)tmpDts - prevDts; uint8_t* afterPesData = curBuf +
               pesPacket->getHeaderLength(); LTRACE(LT_INFO, 2, "Cnt=" <<
               ++ggCnt << "  PTS: " << pesPacket->getPts() << " DTS:" << tmpDts
               << "  dtsDif:"
               << dtsDif << " after pes 0x" << int32ToStr(afterPesData[0],16) <<
               int32ToStr(afterPesData[1],16) << int32ToStr(afterPesData[2],16)
               << int32ToStr(afterPesData[3],16)); prevDts = tmpDts;
               }
               */
            }
            ...
            if (idx >= 0 && idx <= 15 && !m_lpcpHeaderAdded[idx]) {
               vect.append(m_lpcmWaveHeader[idx].data(),
                           m_lpcmWaveHeader[idx].size());
               m_lpcpHeaderAdded[idx] = true;
            }

            uint8_t* payloadData =
                curBuf + pesPacket->getHeaderLength() + afterPesHeader;
            uint32_t pesPayloadLen = pesPacket->getPacketLength() -
                                     pesPacket->getHeaderLength() -
                                     afterPesHeader;
   ...
   if (m_tmpBufferLen > 0) memcpy(m_tmpBuffer, curBuf, end - curBuf);
   return 0;
}
...
int64_t getPSDuration(const char* fileName) {
   ...
   try {
      ...
      if (!file.size(&fileSize)) return 0;
