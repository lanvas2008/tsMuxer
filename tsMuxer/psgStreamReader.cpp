  ...
  while (pos < objEnd) {
    if (pos >= end) {
      if (*pos != OBJECT_DEF_SEGMENT)
        THROW(ERR_COMMON, "Unexpected byte "
                              << *pos
                              << " during parsing Object definition segment");
...
void PGSStreamReader::renderTextShow(int64_t inTime) {
  ...
  // m_objectWindowTop = FFMAX(0, m_render->m_videoHeight - m_objectWindowHeight
...
void PGSStreamReader::renderTextHide(int64_t outTime) {
  ...
  rLen = m_render->composeWindowDefinition(
      curPos, outTime - windowsTransferTime, outTime - windowsTransferTime - 90,
      m_objectWindowTop, m_objectWindowHeight, m_demuxMode);
  m_renderedBlocks.push_back(
  ...
  try {
    ...
    switch (segment_type) {
      ...
      case OBJECT_DEF_SEGMENT:
        // object definition
        // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " object definition
        // size " << segment_len
      ...
      case WINDOWS_DEF_SEGMENT:
        ...
        // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " Window Definition
        // Segment");
        break;
      case 0x18:
