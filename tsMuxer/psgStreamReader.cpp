...
void PGSStreamReader::composition_object(BitStreamReader& bitReader) {
   ...
   bitReader.skipBits(6);
   composition_object_horizontal_position[object_id_ref] =
       bitReader.getBits(16);
...
int PGSStreamReader::calcFpsIndex(double fps) {
   for (int i = 0; i < 16; i++)
      if (fabs(pgs_frame_rates[i] - fps) < 1e-4) return i;
...
void PGSStreamReader::readPalette(uint8_t* pos, uint8_t* end) {
   ...
   while (pos < end) {
      ...
      color.Cb = *pos++;
      color.alpha = *pos++;
      if (color.alpha != 0) m_palette.insert(make_pair(index, color));
   }
}
...
void PGSStreamReader::yuvToRgb(int minY) {
   ...
   for (map<uint8_t, YUVQuad>::const_iterator itr = m_palette.begin();
        itr != m_palette.end(); ++itr) {
...
int PGSStreamReader::readObjectDef(uint8_t* pos, uint8_t* end) {
   ...
   if (m_bufEnd < objEnd) return NEED_MORE_DATA;
   ...
   while (pos < objEnd) {
      if (pos >= end) {
         if (*pos != OBJECT_DEF_SEGMENT)
            THROW(ERR_COMMON,
                  "Unexpected byte "
                      << *pos << " during parsing Object definition segment");
   ...
   if (m_needRescale) {
      ...
      if (m_fontBorder)
         TextSubtitlesRender::addBorder(m_fontBorder, m_scaledRgbBuffer,
                                        m_scaled_width, m_scaled_height);
      // memcpy(bmpDest.buffer, bmpRef.buffer, bmpDest.Width * bmpDest.Height *
      // 4);
   }
   return 0;
}
...
void PGSStreamReader::renderTextShow(int64_t inTime) {
   ...
   int64_t compositionDecodeTime =
       90000.0 * decodedObjectSize / PIXEL_DECODING_RATE + 0.999;
   int64_t windowsTransferTime =
       90000.0 * decodedObjectSize / PIXEL_COMPOSITION_RATE + 0.999;
   const int64_t PLANEINITIALIZATIONTIME =
       90000.0 * (m_scaled_width * m_scaled_height) / PIXEL_COMPOSITION_RATE +
       0.999;
   const int64_t PRESENTATION_DTS_DELTA =
       PLANEINITIALIZATIONTIME + windowsTransferTime;

   m_objectWindowHeight = FFMAX(0, m_render->renderedHeight());
   // m_objectWindowTop = FFMAX(0, m_render->m_videoHeight -
   // m_objectWindowHeight
   // - m_render->m_bottomOffset);
   m_objectWindowTop = m_render->maxLine();

   // show text
   uint8_t* curPos = m_renderedData;
   // composition segment. pts=x,  dts = x-0.0648 (pts alignment to video grid)
   // (I get constant value from real PGS track as example)
   int rLen = m_render->composePresentationSegment(
       curPos, CM_Start, inTime, inTime - PRESENTATION_DTS_DELTA,
       m_objectWindowTop, m_demuxMode, m_forced_on_flag);
   m_renderedBlocks.push_back(
       PGSRenderedBlock(inTime, inTime - PRESENTATION_DTS_DELTA, rLen, curPos));
   curPos += rLen;
   // window definition.   pts=x-0.001, dts = x-0.0648
   rLen = m_render->composeWindowDefinition(
       curPos, inTime - windowsTransferTime, inTime - PRESENTATION_DTS_DELTA,
       m_objectWindowTop, m_objectWindowHeight, m_demuxMode);
   m_renderedBlocks.push_back(PGSRenderedBlock(inTime - windowsTransferTime,
                                               inTime - PRESENTATION_DTS_DELTA,
                                               rLen, curPos));
   curPos += rLen;

   // palette.             pts=x-0.0648, dts = x-0.0648
   rLen = m_render->composePaletteDefinition(
       m_render->m_paletteByColor, curPos, inTime - PRESENTATION_DTS_DELTA,
       inTime - PRESENTATION_DTS_DELTA, m_demuxMode);
   m_renderedBlocks.push_back(PGSRenderedBlock(inTime - PRESENTATION_DTS_DELTA,
                                               inTime - PRESENTATION_DTS_DELTA,
                                               rLen, curPos));
   curPos += rLen;
   ...
   rLen = m_render->composeObjectDefinition(
       curPos, odfPTS, inTime - PRESENTATION_DTS_DELTA, m_render->minLine(),
       m_render->maxLine(), m_demuxMode);
   m_renderedBlocks.push_back(
       PGSRenderedBlock(odfPTS, inTime - PRESENTATION_DTS_DELTA, rLen, curPos));
   curPos += rLen;
...
void PGSStreamReader::renderTextHide(int64_t outTime) {
   ...
   // hide text
   uint8_t* curPos = m_renderedData;
   // composition segment. pts=x,       dts = x-0.001 (pts alignment to video
   // grid)
   int rLen = m_render->composePresentationSegment(
       curPos, CM_Finish, outTime, outTime - windowsTransferTime - 90,
       m_objectWindowTop, m_demuxMode, false);
   m_renderedBlocks.push_back(PGSRenderedBlock(
       outTime, outTime - windowsTransferTime - 90, rLen, curPos));
   curPos += rLen;
   // windows              pts=x-0.001, dts = x-0.001
   rLen = m_render->composeWindowDefinition(
       curPos, outTime - windowsTransferTime,
       outTime - windowsTransferTime - 90, m_objectWindowTop,
       m_objectWindowHeight, m_demuxMode);
   m_renderedBlocks.push_back(
       PGSRenderedBlock(outTime - windowsTransferTime,
                        outTime - windowsTransferTime - 90, rLen, curPos));
   curPos += rLen;
   // end                  pts=x-0.001, dts = x-0.001
   rLen = m_render->composeEnd(curPos, outTime - 90, outTime - 90, m_demuxMode);
   m_renderedBlocks.push_back(
       PGSRenderedBlock(outTime - 90, outTime - 90, rLen, curPos));
   curPos += rLen;
   m_renderedLen = curPos - m_renderedData;
}
...
int PGSStreamReader::readPacket(AVPacket& avPacket) {
   ...
   if (m_renderedBlocks.size() >
       0) {  // rendered data block (rescaled PGS). send it.
      if (m_firstRenderedPacket) avPacket.flags += AVPacket::FORCE_NEW_FRAME;
   ...
   if (m_curPos == 0) return NEED_MORE_DATA;

   if (m_video_height == 0) {
      intDecodeStream(m_curPos, m_bufEnd - m_curPos);
      if (m_video_height == 0) return NEED_MORE_DATA;

      int tmpWidth;
      int tmpHeight;
      m_render->enlargeCrop(m_video_width, m_video_height, &tmpWidth,
                            &tmpHeight);
      m_needRescale = m_scaled_width && m_scaled_width != tmpWidth ||
                      m_scaled_height && m_scaled_height != tmpHeight;
      // m_needRescale |= true;
      if (m_needRescale) {
         ...
         m_renderedData =
             new uint8_t[(m_scaled_width + 16) * m_scaled_height + 16384];
      ...
      LTRACE(LT_INFO, 2,
             "Decoding PGS stream (track " << m_streamIndex << "): "
                                           << " Resolution: " << m_video_width
                                           << ':' << m_video_height
                                           << "  Frame rate: " << m_frame_rate);
      if (m_newFps != 0 && fabs(m_newFps - m_frame_rate) > 1e-4) {
         LTRACE(LT_INFO, 2,
                "Change FPS from " << m_frame_rate << " to " << m_newFps
                                   << " for PGS stream #" << m_streamIndex);
         m_scale = m_frame_rate / m_newFps;
      }
      if (m_needRescale)
         LTRACE(LT_INFO, 2,
                "Change PGS resolution from "
                    << m_video_width << ':' << m_video_height << " to "
                    << m_scaled_width << ':' << m_scaled_height
   ...
   if (pgStartCode) {
      ...
      if (m_lastDTS == 0) m_lastDTS = m_lastPTS;
      m_curPos += 10;
      m_processedSize += 10;
      m_state = stParsePGS;
      avPacket.pts = m_lastPTS;
      avPacket.dts = m_lastDTS;
      m_isNewFrame = true;
      // LTRACE(LT_INFO, 2, "PGS PES#" << m_streamIndex << ". PTS=" <<
      // m_lastPTS/1e9 << " DTS=" << m_lastDTS/1e9);
      if (m_needRescale) {
         avPacket.dts = FFMAX(0, avPacket.dts);
         avPacket.pts = FFMAX(0, avPacket.pts);
      }
   ...
   bool pesStartCode =
       m_curPos[0] == 0 && m_curPos[1] == 0 && m_curPos[2] == 01;
   ...
   try {
      int number_of_composition_objects, palette_update_flag, palette_id_ref,
          number_of_windows;
      switch (segment_type) {
         case PALETTE_DEF_SEGMENT:
            ...
            if (m_needRescale) readPalette(m_curPos, m_curPos + segment_len);
            // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " palette
            // definition");
            break;
         case OBJECT_DEF_SEGMENT:
            // object definition
            // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " object
            // definition size " << segment_len
            //	  << " objectID=" << (int)m_curPos[0] << " version=" << (int)
            // m_curPos[2]);
            if (m_needRescale) {
               memset(m_imgBuffer, 0xff, m_video_width * m_video_height);
               memset(m_rgbBuffer, 0x00, m_video_width * m_video_height * 4);
               memset(m_scaledRgbBuffer, 0x00,
                      m_scaled_width * m_scaled_height * 4);
               if (readObjectDef(m_curPos, m_curPos + segment_len) ==
                   NEED_MORE_DATA) {
                  m_tmpBufferLen = m_bufEnd - m_curPos;
                  memcpy(&m_tmpBuffer[0], m_curPos, m_tmpBufferLen);
                  return NEED_MORE_DATA;
         ...
         case PCS_DEF_SEGMENT:
            ...
            // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " Presentation
            // Composition Segment. State = "
         ...
         case WINDOWS_DEF_SEGMENT:
            ...
            // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " Window
            // Definition Segment");
            break;
         case 0x18:
            // Interactive Composition Segment
            // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " Interactive
            // Composition Segment");
            break;
         case 0x80:
         ...
         case 0x82:
            ...
            break;
         default:
            // LTRACE(LT_INFO, 2, "PGS #" << m_streamIndex << " unknown type "
            // << (int)segment_type);
            break;
      }
   ...
   avPacket.dts = m_lastDTS;
   if (m_isNewFrame) {
      if (!m_needRescale) avPacket.flags += AVPacket::FORCE_NEW_FRAME;
      m_isNewFrame = false;
   }
...
void PGSStreamReader::setBuffer(uint8_t* data, int dataLen, bool lastBlock) {
   ...
   if (m_tmpBuffer.size() > 0)
      memcpy(&m_tmpBuffer[0] + m_tmpBufferLen, data + MAX_AV_PACKET_SIZE,
             dataLen);
...
CheckStreamRez PGSStreamReader::checkStream(uint8_t* buffer, int len,
   ...
   CheckStreamRez rez;
   if ((containerType == ctM2TS || containerType == ctTS) &&
      ...
      if (containerStreamIndex >= 0x1200)
         rez.streamDescr +=
             std::string(" #") + int32ToStr(containerStreamIndex - 0x1200);
   } else if (containerType == ctMKV &&
              containerDataType == STREAM_TYPE_SUB_PGS) {
      rez.codecInfo = pgsCodecInfo;
      rez.streamDescr = "Presentation Graphic Stream";
      rez.streamDescr += std::string(" #") + int32ToStr(containerStreamIndex);
   } else if (containerType == ctSUP && len > 2 && buffer[0] == 'P' &&
              buffer[1] == 'G') {
      rez.codecInfo = pgsCodecInfo;
   ...
   if (m_video_height != 0) {
      ...
      ostr << " Resolution: " << m_video_width << ':' << m_video_height
           << " Frame rate: " << m_frame_rate;
...
int PGSStreamReader::writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd,
                                       AVPacket& avPacket,
                                       PriorityDataInfo* priorityData) {
   if (!m_demuxMode) return 0;
   if (dstEnd - dstBuffer < 10)
      THROW(ERR_COMMON,
            "PGS stream error: Not enought buffer for write headers");
   if (!m_needRescale) {
      *dstBuffer++ = 'P';
      *dstBuffer++ = 'G';
