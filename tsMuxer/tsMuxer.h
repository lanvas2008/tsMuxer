...
class TSMuxer : public AbstractMuxer {
 ...
 public:
  ...
  virtual void setFileName(const std::string& fileName,
                           FileFactory* fileFactory) override;
 ...
 private:
  ...
  void flushTSFrame();
  int writeTSFrames(int pid, uint8_t* buffer, int len, bool priorityData,
                    bool payloadStart);
  void writeSIT();
  void writePMT();
  void writePAT();
  ...
  bool appendM2TSNullPacketToFile(uint64_t curFileSize, int counter,
                                  int* packetsWrited);
  int writeOutFile(uint8_t* buffer, int len);

  void joinToMasterFile() override;
  virtual void setSubMode(AbstractMuxer* mainMuxer,
                          bool flushInterleavedBlock) override;
  virtual void setMasterMode(AbstractMuxer* subMuxer,
                             bool flushInterleavedBlock) override;

  AbstractOutputStream* getDstFile() { return m_muxFile; }
  void flushTSBuffer();
  void finishFileBlock(uint64_t newPts, uint64_t newPCR, bool doChangeFile,
                       bool recursive = true);
 ...
 private:
  ...
  std::vector<std::pair<uint8_t*, int>>
      m_m2tsDelayBlocks;  // postpone M2TS PCR processing (fill previous data on
                          // next PCR after several PES packets)
  ...
  std::vector<std::vector<int32_t>>
      m_interliaveInfo;  // ssif interliave info, should be written to CLPI
  bool m_masterMode;
  bool m_subMode;
...
class TSMuxerFactory : public AbstractMuxerFactory {
 public:
  virtual AbstractMuxer* newInstance(MuxerManager* owner) const override {
    return new TSMuxer(owner);
  }
};
