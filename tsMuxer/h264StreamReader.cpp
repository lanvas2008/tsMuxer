...
H264StreamReader::H264StreamReader() : MPEGStreamReader() {
   ...
   m_startPts = 0;
   m_decodedSliceHeader.resize(
       8 + 8);  // 8 for slice data, 8 - extra size for increase decoded data
   m_removalDelay = 0;
   m_spsChangeWarned = false;
}

H264StreamReader::~H264StreamReader() {
   for (map<uint32_t, SPSUnit*>::iterator itr = m_spsMap.begin();
        itr != m_spsMap.end(); ++itr)
      delete itr->second;
   for (map<uint32_t, PPSUnit*>::iterator itr = m_ppsMap.begin();
        itr != m_ppsMap.end(); ++itr)
      delete itr->second;
}
...
CheckStreamRez H264StreamReader::checkStream(uint8_t* buffer, int len) {
   ...
   for (uint8_t* nal = NALUnit::findNextNAL(buffer, end); nal < end - 4;
      ...
      if (*nal & 0x80) return rez;
      uint8_t nalType = *nal & 0x1f;
      switch (nalType) {
         case nuSubSPS:
            m_mvcSubStream = true;
         case nuSPS: {
            if (nalType == nuSPS) m_mvcPrimaryStream = true;
            ...
            // m_spsMap.insert(make_pair<uint32_t,
            // SPSUnit*>(sps->seq_parameter_set_id, sps));
            m_spsMap.insert(make_pair(sps->seq_parameter_set_id, sps));
            if (tmpDescr.empty()) tmpDescr = sps->getStreamDescr();
            break;
         }
         case nuPPS: {
            ...
            // m_ppsMap.insert(make_pair<uint32_t,
            // PPSUnit*>(pps->pic_parameter_set_id, pps));
         ...
         case nuSEI:
            if (!m_spsMap.empty()) {
               ...
               nextNal = NALUnit::findNALWithStartCode(nal, end, true);
               if (nextNal == m_bufEnd) break;
               lastSEI.decodeBuffer(nal, nextNal);
               lastSEI.deserialize(*sps, sps->nalHrdParams.isPresent ||
                                             sps->vclHrdParams.isPresent);
               ...
               if (!offsetsInserted &&
                   lastSEI.number_of_offset_sequences >= 0) {
                  offsetsInserted = true;
                  tmpDescr += "  3d-pg-planes: " +
                              int32ToStr(lastSEI.number_of_offset_sequences);
               }
            }
            break;
         ...
         case nuSliceC:
         case nuSliceExt:
            if (m_ppsMap.empty() || m_spsMap.empty()) break;
            try {
               const uint8_t* nextNal =
                   NALUnit::findNALWithStartCode(nal, end, true);
               uint8_t tmpBuffer[512];
               int toDecode = FFMIN(sizeof(tmpBuffer) - 8, nextNal - nal);
               int decodedLen = slice.decodeNAL(nal, nal + toDecode, tmpBuffer,
                                                sizeof(tmpBuffer));
               int nalRez = slice.deserialize(tmpBuffer, tmpBuffer + decodedLen,
                                              m_spsMap, m_ppsMap);
               if (nalRez != 0) return rez;
   ...
   if (m_mvcSubStream && m_mvcPrimaryStream) rez.multiSubStream = true;
...
int H264StreamReader::writeSEIMessage(uint8_t* dstBuffer, uint8_t* dstEnd,
                                      SEIUnit& sei, uint8_t payloadType) {
   uint8_t* curPos = dstBuffer;

   if (dstEnd - curPos < 4)
      THROW(ERR_COMMON,
            "H264 stream error: Not enought buffer for write headers");
   ...
   if (sps) {
      ...
      int sRez = sei.encodeNAL(tmpBuffer, tmpBuffer + writer.getBitsCount() / 8,
                               curPos, dstEnd - curPos);
      if (sRez == -1)
         THROW(ERR_COMMON,
               "H264 stream error: Not enought buffer for write headers");
      curPos += sRez;
...
int H264StreamReader::writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd,
                                        AVPacket& avPacket,
   ...
   if (!m_delimiterFound && m_lastSliceSPS != -1) {
      ...
      if (m_mvcSubStream && m_blurayMode) {
         ...
         *curPos++ = nuDelimiter;
         *curPos++ =
             (m_pict_type << 5) + 0x10;  // primary_pic_type << 5 + rbsp bits
      }
   }

   bool needInsTimingSEI = m_needSeiCorrection && m_lastSliceSPS != -1;  //
   /*
   bool needInsSpsPps = isIFrame() && m_lastSliceSPS != -1 && m_lastSlicePPS !=
   -1
       && (m_totalFrameNum > 1 || m_spsAfterFirstSlice || needInsTimingSEI)
       && ((m_h264SPSCont || needInsTimingSEI) && m_spsCounter < 2 ||
   m_firstFileFrame);
   */
   ...
   bool spsDiscontinue =
       m_h264SPSCont &&
       ((m_spsCounter < 2 && (m_totalFrameNum > 1 && m_lastSliceIDR ||
                              m_totalFrameNum > 250 && isIFrame())) ||
   ...
   if (needInsSpsPps) {
      ...
      // Why the check for 250 : unless we see the 2nd IDR frame, we can only
      // see that the stream runs with open GOPs (open GOPs with one IDR at
      // start for the whole movie do happen). That's why there's also a check
      // that there's a SPS before for the first I-frame in the first GOP in a
      // normal stream. There's nothing scary about that - we simply insert one
      // more SPS - but in order to avoid outputting messages into the console,
      // I left the check at 250.
      ...
      for (map<uint32_t, PPSUnit*>::const_iterator itr = m_ppsMap.begin();
           itr != m_ppsMap.end(); ++itr) {
   ...
   if (needInsTimingSEI) {
      ...
      if (m_firstSEIWarn) {
         ...
         LTRACE(
             LT_INFO, 2,
             "H264 bitstream changed: insert pict timing and buffering period "
             "SEI units");
      }
      ...
      if (m_lastIFrame) {
         if (m_lastSliceIDR) {
            ...
            curPos +=
                writeSEIMessage(curPos, dstEnd, sei, SEI_MSG_BUFFERING_PERIOD);
         }
         ...
         if (m_mvcSubStream) {
            if (!m_bdRomMetaDataMsg.empty()) {
               ...
               int64_t pts90k =
                   m_curDts / INT_FREQ_TO_TS_FREQ + 0.5 + m_startPts;
               uint8_t* srcData = &m_bdRomMetaDataMsg[0];
               SEIUnit::updateMetadataPts(srcData + m_bdRomMetaDataMsgPtsPos,
                                          pts90k);

               uint8_t* prevPos = curPos;
               curPos = writeNalPrefix(curPos);
               curPos += NALUnit::encodeNAL(srcData,
                                            srcData + m_bdRomMetaDataMsg.size(),
                                            curPos, dstEnd - curPos);
               if (priorityData)
                  priorityData->push_back(std::pair<int, int>(
                      prevPos - dstBuffer, curPos - prevPos));
      ...
      sei.dpb_output_delay =
          ((m_curPts - m_curDts + m_pcrIncPerFrame * getFrameDepth()) * 2) /
          m_pcrIncPerFrame;
      sei.pic_struct = m_lastPictStruct;

      if (m_lastSliceIDR) m_removalDelay = 0;
      m_removalDelay += 2;
...
int H264StreamReader::getTSDescriptor(uint8_t* dstBuff) {
   ...
   for (uint8_t* nal = NALUnit::findNextNAL(m_buffer, m_bufEnd);
        nal < m_bufEnd - 4; nal = NALUnit::findNextNAL(nal, m_bufEnd)) {
      uint8_t nalType = *nal & 0x1f;
      if (nalType == nuSPS || nalType == nuSubSPS) {
         ...
         if (m_blurayMode) {
            ...
            M2TSStreamInfo::blurayStreamParams(
                ...
                getStreamAR(), &video_format, &frame_rate_index,
                &aspect_ratio_index);
...
void H264StreamReader::updateStreamFps(void* nalUnit, uint8_t* buff,
                                       uint8_t* nextNal, int oldSpsLen) {
   SPSUnit* sps = (SPSUnit*)nalUnit;
   sps->setFps(m_fps);
   uint8_t* tmpBuffer = new uint8_t[oldSpsLen + 16];
   long newSpsLen =
       sps->serializeBuffer(tmpBuffer, tmpBuffer + oldSpsLen + 16, false);
   if (newSpsLen == -1) THROW(ERR_COMMON, "Not enought buffer");
...
void H264StreamReader::updateHDRParam(SPSUnit* sps) {
   ...
   if (!m_needSeiCorrection) return;

   // if (!orig_hrd_parameters_present_flag &&
   // !orig_vcl_parameters_present_flag)
   //{
   //}
   if (!sps->vui_parameters_present_flag) sps->setFps(m_fps);
   sps->insertHdrParameters();
}

// All the remaining checks of the stream, executed at start

void H264StreamReader::checkPyramid(int frameNum, int* fullPicOrder,
                                    bool nextFrameFound) {
   int depth = frameNum - *fullPicOrder;
   if (depth > 3)
      depth = 3;  // prevent large delay. It is not to be happend in normanl
                  // stream, but this protection for invalid streams.
   if (depth > m_frameDepth) {
      if (nextFrameFound) {
         m_frameDepth = depth;
         LTRACE(LT_INFO, 2,
                "B-pyramid level " << m_frameDepth - 1
                                   << " detected. Shift DTS to " << m_frameDepth
                                   << " frames");
      } else {
         *fullPicOrder =
             frameNum - m_frameDepth;  // special case for last frame
      }
   }
}

void H264StreamReader::additionalStreamCheck(uint8_t* buff, uint8_t* end) {
   // Make sure that the stream doesn't appear to be progressive, since
   // pic_order_cnt_lsb doesn't reach values higher than 2 in these streams.
   bool SEIFound = false;
   SliceUnit slice;
   SPSUnit tmpsps;
   ...
   for (uint8_t* nal = NALUnit::findNextNAL(buff, end); nal != end;
        nal = NALUnit::findNextNAL(nal, end)) {
      switch (*nal & 0x1f) {
         case nuSubSPS:
            m_mvcSubStream = true;
         ...
         case nuPPS: {
            ...
            if (pps1.pic_parameter_set_id == pps2.pic_parameter_set_id) {
               if (pps2.m_nalBufferLen != pps1.m_nalBufferLen ||
                   memcmp(pps2.m_nalBuffer, pps1.m_nalBuffer,
                          pps1.m_nalBufferLen))
                  differPPS = true;
            }
         ...
         case nuSEI:
            if (tmpspsfound) {
               ...
               sei.deserialize(tmpsps, orig_hrd_parameters_present_flag ||
                                           orig_vcl_parameters_present_flag);
               if (sei.m_processedMessages.find(SEI_MSG_BUFFERING_PERIOD) !=
                       sei.m_processedMessages.end() ||
                   sei.m_processedMessages.find(SEI_MSG_PIC_TIMING) !=
                       sei.m_processedMessages.end())
                  SEIFound = true;
   ...
   if (m_removePulldown || m_insertSEIMethod == SEI_InsertForce)
      ...
      m_needSeiCorrection = orig_hrd_parameters_present_flag == 0 ||
                            orig_vcl_parameters_present_flag == 0 || !SEIFound;
      if (m_needSeiCorrection && differPPS) {
         LTRACE(LT_INFO, 2,
                "H264 stream contains different PPS with same pps_id. SEI "
   ...
   if (m_forceLsbDiv == -1) m_forceLsbDiv = 1;
   ...
   for (uint8_t* nal = NALUnit::findNextNAL(buff, end); nal != end;
      ...
      switch (*nal & 0x1f) {
         ...
         case nuSliceExt:
            rez = deserializeSliceHeader(slice, nal, end);
            if (rez == 0 && slice.first_mb_in_slice == 0 &&
                slice.getSPS()->pic_order_cnt_type != 2) {
               int picOrder = calcPicOrder(slice);
               if (slice.isIDR()) frameNum = -1;
...
int H264StreamReader::calcPicOrder(SliceUnit& slice) {
   if (slice.getSPS()->pic_order_cnt_type != 0)
      THROW(ERR_H264_UNSUPPORTED_PARAMETER,
            "SPS picture order " << slice.getSPS()->pic_order_cnt_type
                                 << " not supported.");

   if (slice.isIDR()) prevPicOrderCntMsb = prevPicOrderCntLsb = 0;
      ...
      switch (*nal & 0x1f) {
         ...
         case nuSliceExt:
            ...
            if (slice.isIDR() && slice.first_mb_in_slice == 0)
               ...
               break;
            MaxPicOrderCntLsbHalf =
                1 << (slice.getSPS()->log2_max_pic_order_cnt_lsb - 1);
            if (slice.pic_order_cnt_lsb > MaxPicOrderCntLsbHalf) {
               if (slice.bottom_field_flag == 0) prevPicCnt++;
            } else
               return prevPicCnt;
      }
   }
...
int H264StreamReader::intDecodeNAL(uint8_t* buff) {
   ...
   switch (nal_unit_type) {
      ...
      case nuDRD:
      case nuDelimiter:
         if (nal_unit_type == nuDelimiter ||
             (m_blurayMode && nal_unit_type == nuDRD)) {
            // LTRACE(LT_DEBUG, 0, "NAUD type " << int32ToStr(buff[1],16));
            m_delimiterFound = true;
         }
         nextNal = NALUnit::findNextNAL(buff, m_bufEnd);
         while (1) {
            if (nextNal == m_bufEnd) return NOT_ENOUGHT_BUFFER;
            switch (*nextNal & 0x1f) {
               case nuSEI:
                  ...
                  break;
               case nuDRD:
                  if (m_blurayMode) m_delimiterFound = true;
                  break;
               case nuDelimiter:
                  // LTRACE(LT_DEBUG, 0, "NAUD type " <<
                  // int32ToStr(nextNal[1],16));
                  m_delimiterFound = true;
                  break;
               case nuSPS:
               case nuSubSPS:
...
bool H264StreamReader::skipNal(uint8_t* nal) {
   ...
   if (nalType == nuFillerData) return true;
   ...
   if (!m_needSeiCorrection) return false;

   if (nalType == nuPPS || nalType == nuSPS || nalType == nuSubSPS) {
      return true;  // SPS/PPS will be composed from our side on writePesHeader.
                    // It is required to compose SPS/PPS before SEI
      ...
      for (std::set<int>::iterator itr = sei.m_processedMessages.begin(); itr !=
      sei.m_processedMessages.end(); ++itr)
      {
...
int H264StreamReader::processSEI(uint8_t* buff) {
   ...
   if (m_spsMap.size() > 0) {
      ...
      if (nextNal == m_bufEnd) return NOT_ENOUGHT_BUFFER;
      if (nextNal + 4 > m_bufEnd) return NOT_ENOUGHT_BUFFER;
      lastSEI.pic_struct = -1;
      lastSEI.decodeBuffer(buff, nextNal);
      lastSEI.deserialize(
          *(m_spsMap.begin()->second),
      ...
      if (m_mvcSubStream) {
         if (lastSEI.number_of_offset_sequences >= 0) {
            if (m_needSeiCorrection) {
               ...
               memcpy(&m_bdRomMetaDataMsg[0], lastSEI.m_nalBuffer,
                      lastSEI.m_nalBufferLen);
            } else {
               // update inplace
         ...
         if (timingSEI && lastSEI.m_mvcHeaderLen > 0) {
            ...
            memcpy(&m_lastSeiMvcHeader[0], lastSEI.m_mvcHeaderStart,
                   lastSEI.m_mvcHeaderLen);
         }
      }

      if (timingSEI && !nonTimingSEI)

         // LTRACE(LT_INFO, 2, "got SEI " << lastSEI.cpb_removal_delay << " : "
         // << lastSEI.dpb_output_delay);
         return 0;
      ...
      if (timingSEI && nonTimingSEI && m_needSeiCorrection) {
         ...
         int newSize =
             lastSEI.serializeBuffer(tmpBuff, tmpBuff + sizeof(tmpBuff), false);
         if (newSize == -1) THROW(ERR_COMMON, "Not enought buffer");
...
int H264StreamReader::deserializeSliceHeader(SliceUnit& slice, uint8_t* buff,
                                             uint8_t* sliceEnd) {
   int nalRez;
   ...
   do {
      ...
      int decodedLen =
          slice.decodeNAL(buff, buff + toDecode, tmpBuffer, tmpBufferSize);
      nalRez = slice.deserialize(tmpBuffer, tmpBuffer + decodedLen, m_spsMap,
                                 m_ppsMap);
...
int H264StreamReader::processSliceNal(uint8_t* buff) {
   ...
   if (slice.first_mb_in_slice != 0) return 0;
   ...
   // if (!m_delimiterFound)
   {
      if (detectPrimaryPicType(slice, buff) != 0) return NOT_ENOUGHT_BUFFER;
   }
   ...
   if (slice.getSPS()->frame_mbs_only_flag && m_forceLsbDiv != 0) {
      if (slice.pic_order_cnt_lsb % 2 == 1) {
         if (m_totalFrameNum < 250) {
            m_forceLsbDiv = 0;
            LTRACE(LT_WARN, 2,
                   "H264 warn: Force frame_mbs_only_flag division invalid "
                   "detected. Cancel this flag.");
         } else {
            LTRACE(LT_WARN, 2,
                   "H264 warn: Unexpected pic_order_cnt_lsb value "
                       << slice.pic_order_cnt_lsb
                       << ". FrameNum: " << m_totalFrameNum
   ...
   if (slice.isIDR()) {
      ...
      if (slice.getSPS()->pic_order_cnt_type != 2)
         ...
         m_iFramePtsOffset = 0;
      if (m_iFramePtsOffset == -1) return NOT_ENOUGHT_BUFFER;
   } else {
      m_frameNum++;
      // m_frameNum = slice.frame_num;

      if (slice.getSPS()->pic_order_cnt_type == 2) fullPicOrder = m_frameNum;
   }
   ...
   if (m_idrSliceCnt == 0) {
      ...
      if (m_bSliceFound) {
         ...
         if (fullPicOrder < 0) fullPicOrder = 0;
      } else
   ...
   // LTRACE(LT_INFO, 2, "delta=" << fullPicOrder - m_frameNum << "
   // m_lastDtsInc=" << m_lastDtsInc);

   if (m_lastPicStruct == 5 ||
       m_lastPicStruct == 6)  // 3 fields per frame. Used in pulldown
...
int H264StreamReader::detectPrimaryPicType(SliceUnit& firstSlice,
                                           uint8_t* buff) {
   SliceUnit slice;
   ...
   m_pict_type = -1;
   m_pict_type =
       std::max(m_pict_type, sliceTypeToPictType(firstSlice.slice_type));

   // if (firstSlice.orig_slice_type >= 5) // all other slice at this picture
   // must be same type 	return 0; // OK
   ...
   while (1) {
      ...
      switch (nalUnitType & 0x1f) {
         ...
         case nuSliceExt:
            ...
            m_pict_type =
                std::max(m_pict_type, sliceTypeToPictType(slice.slice_type));
            break;
            /*
            case nuDelimiter:
            case nuDRD:
...
int H264StreamReader::processSPS(uint8_t* buff) {
   if (replaceToOwnSPS()) {
      ...
      int seq_parameter_set_id =
          NALUnit::extractUEGolombCode(buff + 4, m_bufEnd);
      if (seq_parameter_set_id == -1) return NOT_ENOUGHT_BUFFER;
   ...
   // long sps fields can cause 00 00 0x code and it is required decode slice
   // header
   if (nalRez != 0) {
      delete sps;
   ...
   if (m_spsMap.find(sps->seq_parameter_set_id) == m_spsMap.end()) {
      LTRACE(LT_INFO, 2,
             "Decoding H264 stream (track " << m_streamIndex
                                            << "): " << sps->getStreamDescr());
      if (m_forcedLevel != 0)
         LTRACE(LT_INFO, 2,
                "Change H264 level from " << sps->level_idc / 10.0 << " to "
                                          << m_forcedLevel / 10.0);
   }
...
bool H264StreamReader::getInterlaced() {
   if (m_spsMap.size() == 0) return true;
...
bool H264StreamReader::isPriorityData(AVPacket* packet) {
   if (packet->data + 3 == m_priorityNalAddr ||
       packet->data + 4 == m_priorityNalAddr) {
      m_priorityNalAddr = 0;
      m_OffsetMetadataPtsAddr = 0;
      return true;
   }
