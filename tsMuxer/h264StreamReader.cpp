    ...
    switch (nalType) {
      ...
      case nuSEI:
        if (!m_spsMap.empty()) {
          ...
          lastSEI.deserialize(
              *sps, sps->nalHrdParams.isPresent || sps->vclHrdParams.isPresent);
...
int H264StreamReader::writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd,
  ...
  */
  ...
  if (needInsSpsPps) {
    ...
    // Why the check for 250 : unless we see the 2nd IDR frame, we can only see
    // that the stream runs with open GOPs (open GOPs with one IDR at start for
    // the whole movie do happen). That's why there's also a check that there's
    // a SPS before for the first I-frame in the first GOP in a normal stream.
    // There's nothing scary about that - we simply insert one more SPS - but in
    // order to avoid outputting messages into the console, I left the check at
    // 250.
    ...
    if (m_lastIFrame) {
      ...
      if (m_mvcSubStream) {
        if (!m_bdRomMetaDataMsg.empty()) {
          ...
          curPos = writeNalPrefix(curPos);
          curPos +=
              NALUnit::encodeNAL(srcData, srcData + m_bdRomMetaDataMsg.size(),
                                 curPos, dstEnd - curPos);
          if (priorityData)
            priorityData->push_back(
                std::pair<int, int>(prevPos - dstBuffer, curPos - prevPos));
