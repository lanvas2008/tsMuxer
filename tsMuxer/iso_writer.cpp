...
void writeLongAD(uint8_t* buffer, uint32_t lenBytes, uint32_t pos,
                 uint16_t partition, uint32_t id) {
   uint32_t* buff32 = (uint32_t*)buffer;
   uint16_t* buff16 = (uint16_t*)buffer;
...
ByteFileWriter::ByteFileWriter()
    : m_buffer(0), m_bufferEnd(0), m_curPos(0), m_tagPos(0) {}
...
void ByteFileWriter::writeDescriptorTag(DescriptorTag tag,
...
void ByteFileWriter::closeDescriptorTag(int dataSize) {
   if (dataSize == -1) dataSize = m_curPos - m_tagPos;
...
void ByteFileWriter::writeIcbTag(uint8_t fileType) {
   ...
   buff16[18 / 2] = 0x20;  // flags. Archive: This bit shall be set to ONE when
                           // the file is created or is written.

   m_curPos += 20;
}

void ByteFileWriter::writeLongAD(uint32_t lenBytes, uint32_t pos,
                                 uint16_t partition, uint32_t id) {
...
void ByteFileWriter::writeDString(const char* value, int len) {
   int writeLen = len;
   if (writeLen == -1) writeLen = strlen(value) + 2;
...
FileEntryInfo::FileEntryInfo(IsoWriter* owner, FileEntryInfo* parent,
                             uint32_t objectId, FileTypes fileType)
    : m_owner(owner),
      m_parent(parent),
      m_sectorNum(0),
      m_sectorsUsed(0),
...
void FileEntryInfo::writeEntity(ByteFileWriter& writer, FileEntryInfo* subDir) {
   ...
   writer.writeDescriptorTag(DESC_TYPE_FileId,
                             m_owner->absoluteSectorNum() + 1);
   ...
   writer.writeLE8(
       !subDir->isFile()
           ...
           : (isSystemFile
                  ? 0x10
                  : 0));  // File Characteristics, 'directory' bit (1-th)
   writer.writeLE8(subDir->m_name.length() +
                   1);  // Length of File Identifier (=L_FI)
   writer.writeLongAD(0x800, subDir->m_sectorNum, 0x01,
                      subDir->m_objectId);  // ICB
...
int FileEntryInfo::allocateEntity(int sectorNum) {
   ...
   if (!isFile())
      m_sectorsUsed = 2;  // restriction here: amount of files/subdirs is
                          // limited by sector size.
   else if (m_extents.size() <= MAX_EXTENTS_IN_EXTFILE)
      m_sectorsUsed = 1;
   else {
      int sz = m_extents.size() - 1 - MAX_EXTENTS_IN_EXTFILE;
      m_sectorsUsed =
          1 + (sz + MAX_EXTENTS_IN_EXTCONT - 1) / MAX_EXTENTS_IN_EXTCONT;
   }
...
void FileEntryInfo::serializeFile() {
   ...
   int writed = m_owner->writeExtentFileDescriptor(m_fileType, m_fileSize,
...
void FileEntryInfo::serializeDir() {
   ...
   writer.writeDescriptorTag(DESC_TYPE_FileId,
                             m_owner->absoluteSectorNum() + 1);
   writer.writeLE16(0x01);  // File Version Number
   writer.writeLE8(0x0A);   // File Characteristics, parent flag (3-th bit) and
                            // 'directory' bit (1-th)
   writer.writeLE8(0x00);   // Length of File Identifier (=L_FI)

   writer.writeLongAD(
       0x800, m_parent ? m_parent->m_sectorNum : m_owner->absoluteSectorNum(),
       0x01, 0);         // parent entry ICB
   writer.writeLE16(0);  // Length of Implementation Use
   ...
   m_owner->writeExtentFileDescriptor(m_fileType, writer.size(),
                                      m_sectorNum + 1, m_subDirs.size() + 1);
...
void FileEntryInfo::addExtent(const Extent& extent) {
   if (m_extents.empty()) {
      ...
      int lastLBN =
          m_extents.rbegin()->lbnPos + m_extents.rbegin()->size / SECTOR_SIZE;
      if (lastLBN == extent.lbnPos &&
          m_extents.rbegin()->size + extent.size < MAX_EXTENT_SIZE)
...
void FileEntryInfo::close() {
   if (m_sectorBufferSize) {
      ...
      m_owner->sectorSeek(IsoWriter::MainPartition,
                          m_extents.rbegin()->lbnPos + delta);
      memset(m_sectorBuffer + m_sectorBufferSize, 0,
             SECTOR_SIZE - m_sectorBufferSize);
      m_owner->writeRawData(m_sectorBuffer, SECTOR_SIZE);
      m_sectorBufferSize = 0;
   }
}
...
FileEntryInfo* FileEntryInfo::subDirByName(const std::string& name) const {
   for (int i = 0; i < m_subDirs.size(); ++i) {
      if (m_subDirs[i]->m_name == name) return m_subDirs[i];
   }
   return 0;
}
...
FileEntryInfo* FileEntryInfo::fileByName(const std::string& name) const {
   for (int i = 0; i < m_files.size(); ++i) {
      if (m_files[i]->m_name == name) return m_files[i];
   }
   return 0;
}
...
bool ISOFile::open(const char* name, unsigned int oflag,
...
bool ISOFile::close() {
   if (m_entry) m_entry->close();
   m_entry = 0;
   return true;
}

void ISOFile::setSubMode(bool value) {
   if (m_entry) m_entry->setSubMode(value);
}
...
void IsoWriter::setVolumeLabel(const std::string& value) {
   m_volumeLabel = value;
   if (m_volumeLabel.empty()) m_volumeLabel = " ";
}

bool IsoWriter::open(const std::string& fileName, int64_t diskSize,
                     int extraISOBlocks) {
   int systemFlags = 0;
   if (!m_file.open(fileName.c_str(), File::ofWrite, systemFlags)) return false;

   if (diskSize > 0) {
      int blocks =
          2 + extraISOBlocks +
          roundUp64(diskSize, META_BLOCK_PER_DATA) / META_BLOCK_PER_DATA;
   ...
   m_systemStreamDir =
       new FileEntryInfo(this, 0, 0, FileType_SystemStreamDirectory);
   m_metadataMappingFile =
       new FileEntryInfo(this, m_systemStreamDir, 0, FileType_File);
   ...
   memset(m_buffer, 0, sizeof(m_buffer));
   int64_t requiredFileSizeLBN =
       METADATA_START_ADDR + m_metadataFileLen / SECTOR_SIZE;
...
FileEntryInfo* IsoWriter::mkdir(const char* name, FileEntryInfo* parent) {
   if (!m_rootDirInfo) return 0;
   if (parent == 0) parent = m_rootDirInfo;

   FileEntryInfo* dir =
       new FileEntryInfo(this, parent, m_objectUniqId++, FileType_Directory);
   dir->setName(name);
...
bool IsoWriter::createInterleavedFile(const std::string& inFile1,
                                      const std::string& inFile2,
                                      const std::string& outFile) {
   FileEntryInfo* inEntry1 =
       getEntryByName(toIsoSeparator(inFile1), FileType_RealtimeFile);
   if (!inEntry1) return false;

   FileEntryInfo* inEntry2 =
       getEntryByName(toIsoSeparator(inFile2), FileType_RealtimeFile);
   if (!inEntry2) return false;

   FileEntryInfo* outEntry =
       getEntryByName(toIsoSeparator(outFile), FileType_RealtimeFile);
...
FileEntryInfo* IsoWriter::createFileEntry(FileEntryInfo* parent,
                                          FileTypes fileType) {
   if (!m_rootDirInfo) return 0;
   if (parent == 0) parent = m_rootDirInfo;

   FileEntryInfo* file =
       new FileEntryInfo(this, parent, m_objectUniqId++, fileType);
   parent->addFile(file);
   return file;
}

FileEntryInfo* IsoWriter::getEntryByName(const std::string& name,
                                         FileTypes fileType) {
   std::vector<std::string> parts = splitStr(name.c_str(), '/');
   FileEntryInfo* entry = m_rootDirInfo;
   if (!entry) return 0;
   bool isDir = fileType == FileType_Directory ||
                fileType == FileType_SystemStreamDirectory;
   int idxMax = isDir ? parts.size() : parts.size() - 1;
   for (int i = 0; i < idxMax; ++i) {
      FileEntryInfo* nextEntry = entry->subDirByName(parts[i]);
      if (!nextEntry) nextEntry = mkdir(parts[i].c_str(), entry);
      entry = nextEntry;
   }
   if (isDir) return 0;

   std::string fileName = *parts.rbegin();
   if (fileName.empty()) return entry;
...
void IsoWriter::allocateMetadata() {
   ...
   if (m_systemStreamLBN + 3 > m_metadataFileLen / SECTOR_SIZE)
      throw std::runtime_error(
          "ISO error: Not enough space in metadata partition. It possible in a "
          "split mode if a lot of files. Please provide addition input "
          "parameter "
          "--extra-iso-space to increate metadata space. Default value for "
          "this "
...
void IsoWriter::close() {
   ...
   m_tagLocationBaseAddr = m_partitionStartAddress;
   writeExtentFileDescriptor(FileType_MetadataMirror, m_metadataFileLen,
   ...
   writeExtentFileDescriptor(FileType_Metadata, m_metadataFileLen,
                             m_metadataLBN - m_partitionStartAddress, 0);
   m_tagLocationBaseAddr =
       m_metadataLBN;  // I don't know why. I doing just as scenarist does
...
void IsoWriter::writeDescriptors() {
   ...
   writeAnchorVolumeDescriptor(
       m_partitionEndAddress +
...
void IsoWriter::sectorSeek(Partition partition, int pos) {
   int64_t offset = (partition == MetadataPartition) ? m_curMetadataPos
                                                     : m_partitionStartAddress;
...
int IsoWriter::allocateEntity(FileEntryInfo* entity, int sectorNum) {
   if ((entity->m_fileType == FileType_File ||
        entity->m_fileType == FileType_RealtimeFile) &&
       entity->m_objectId)
      m_totalFiles++;
   ...
   if (entity->m_objectId)
      m_mappingEntries[entity->m_objectId] =
          MappingEntry(entity->m_parent->m_sectorNum, entity->m_sectorNum);
...
void IsoWriter::writeAllocationExtentDescriptor(ExtentList* extents, int start,
                                                int indexEnd) {
   uint32_t* buff32 = (uint32_t*)m_buffer;
   memset(m_buffer, 0, sizeof(m_buffer));
   writeDescriptorTag(m_buffer, DESC_TYPE_AllocationExtent,
                      absoluteSectorNum());
   ...
   if (indexEnd < extents->size()) {
      buff32[212 / 4] += 0x10;
      writeLongAD(curPos, SECTOR_SIZE + NEXT_EXTENT, absoluteSectorNum() + 1, 1,
                  0);
      curPos += 16;
   }
...
int IsoWriter::writeExtentFileDescriptor(uint8_t fileType, uint64_t len,
                                         uint32_t pos, int linkCount,
                                         ExtentList* extents) {
   int sectorsWrited = 0;
   ...
   // zero Permissions, File Link Count, Record Format, Record Display
   // Attributes
   ...
   buff32[72 / 4] = roundUp64(len, SECTOR_SIZE) /
                    SECTOR_SIZE;  // blocks recorded (matched to bytes)
   ...
   if (fileType != FileType_File && fileType != FileType_RealtimeFile) {
      ...
      buff32[220 / 4] = pos;   // Allocation descriptors, start logical block
                               // number inside volume
      ...
      if (indexEnd < extents->size()) {
         buff32[212 / 4] += 0x10;
         writeLongAD(curPos, SECTOR_SIZE + NEXT_EXTENT, absoluteSectorNum() + 1,
                     1,
                     0);  // continue expected
         curPos += 16;
      }
...
void IsoWriter::writeIcbTag(uint8_t* buffer, uint8_t fileType) {
   ...
   // skip 6 byte zero Parent ICB Location
   if (fileType != FileType_File && fileType != FileType_RealtimeFile)
      buff16[18 / 2] = 0x20;  // flags. Archive: This bit shall be set to ONE
                              // when the file is created or is written.
   else
...
void IsoWriter::writeFileSetDescriptor() {
   ...
   writer.writeCharSpecString("OSTA Compressed Unicode",
                              64);          // File Set Character Set
...
void IsoWriter::writePrimaryVolumeDescriptor() {
   ...
   std::string volumeSetIdentifier =
       volId + std::string("        ") + m_volumeLabel;
...
void IsoWriter::writePartitionDescriptor() {
   ...
   buff32[188 / 4] =
       m_partitionStartAddress;  // Partition Starting Location (576K address )
   buff32[192 / 4] = m_partitionEndAddress -
                     m_partitionStartAddress;  // Partition Length field
...
void IsoWriter::writeLogicalVolumeDescriptor() {
   ...
   strcpy((char*)m_buffer + 21,
          "OSTA Compressed Unicode");  // Descriptor Character Set
   // Logical Volume Identifier
   writeDString(m_buffer + 84, m_volumeLabel.c_str(), 128);
   buff32[212 / 4] = SECTOR_SIZE;  // Logical Block Size
   writeUDFString(m_buffer + 216, "*OSTA UDF Compliant",
   ...
   // Metadata Mirror File Location, to    do: fill me. should be written here
   // in future
   uint32_t mirrorHeaderLocation =
       (m_metadataMirrorLBN - 1 - m_partitionStartAddress);
...
void IsoWriter::writeLogicalVolumeIntegrityDescriptor() {
   memset(m_buffer, 0, sizeof(m_buffer));
   writeDescriptorTag(m_buffer, DESC_TYPE_LogicalVolIntegrity,
                      absoluteSectorNum());
   ...
   buff32[88 / 4] =
       m_partitionEndAddress - m_partitionStartAddress;  // main partition size
