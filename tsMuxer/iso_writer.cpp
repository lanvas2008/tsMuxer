...
int FileEntryInfo::allocateEntity(int sectorNum) {
  ...
  if (!isFile())
    m_sectorsUsed = 2;  // restriction here: amount of files/subdirs is limited
                        // by sector size.
...
void FileEntryInfo::serializeDir() {
  ...
  m_owner->writeExtentFileDescriptor(m_fileType, writer.size(), m_sectorNum + 1,
                                     m_subDirs.size() + 1);
...
int FileEntryInfo::write(const uint8_t* data, uint32_t len) {
  ...
  if (writeLen - dataRest > 0) m_owner->writeRawData(data, writeLen - dataRest);
  if (dataRest) {
...
void IsoWriter::allocateMetadata() {
  ...
  if (m_systemStreamLBN + 3 > m_metadataFileLen / SECTOR_SIZE)
    throw std::runtime_error(
        ...
        "split mode if a lot of files. Please provide addition input parameter "
        "--extra-iso-space to increate metadata space. Default value for this "
...
void IsoWriter::writeLogicalVolumeDescriptor() {
  ...
  // Metadata Mirror File Location, to    do: fill me. should be written here in
  // future
