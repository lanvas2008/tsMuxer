...
void CombinedH264Reader::addDataToPrimary(const uint8_t* data,
...
void CombinedH264Reader::addDataToSecondary(const uint8_t* data,
                                            const uint8_t* dataEnd,
                                            DemuxedData& demuxedData,
                                            int64_t& discardSize) {
...
CombinedH264Reader::ReadState CombinedH264Reader::detectStreamByNal(
    const uint8_t* data, const uint8_t* dataEnd) {
  ...
  if (nalType == nuEOSeq) {
    ...
    if (sps.deserialize() == NOT_ENOUGHT_BUFFER) return ReadState_NeedMoreData;
    m_mvcSPS = sps.seq_parameter_set_id;
    return ReadState_Secondary;
  } else if (nalType == nuPPS) {
    PPSUnit pps;
    pps.decodeBuffer(data, dataEnd);
    if (pps.deserialize() == NOT_ENOUGHT_BUFFER) return ReadState_NeedMoreData;
...
CombinedH264Demuxer::CombinedH264Demuxer(
    const BufferedReaderManager& readManager, const char* streamName)
...
void CombinedH264Demuxer::getTrackList(
    std::map<uint32_t, TrackInfo>& trackList) {
  trackList.insert(std::make_pair(MVC_STREAM_INDEX,
                                  TrackInfo(CODEC_V_MPEG4_H264_DEP, "", 0)));
  trackList.insert(
      std::make_pair(AVC_STREAM_INDEX, TrackInfo(CODEC_V_MPEG4_H264, "", 0)));
}

int CombinedH264Demuxer::simpleDemuxBlock(DemuxedData& demuxedData,
  ...
  if (readedBytes + m_tmpBuffer.size() == 0 ||
  ...
  const uint8_t* nextNal =
      NALUnit::findNALWithStartCode(curNal + 3, dataEnd, true);
...
CombinedH264Filter::CombinedH264Filter(int demuxedPID)
    : CombinedH264Reader(), SubTrackFilter(demuxedPID) {}

int CombinedH264Filter::demuxPacket(DemuxedData& demuxedData,
                                    const PIDSet& acceptedPIDs,
                                    AVPacket& avPacket) {
  if (m_firstDemuxCall) {
