...
struct AVChapter {
  AVChapter() : start(0) {}
  AVChapter(int64_t _start, const std::string _cTitle)
      : start(_start), cTitle(_cTitle) {}
...
struct AVPacket {
  ...
  int stream_index;
  int flags;
  int64_t duration;  //  presentation duration in time_base units (0 if not
                     //  available)
  int64_t pos;       //  byte position in stream, -1 if unknown
  int64_t pcr;       // last program clock reference if packet demuxed from TS
                     // container. used for VBV streaming in RTP/RTSP mode
...
class BaseAbstractStreamReader {
 public:
  ...
  ~BaseAbstractStreamReader() {}
  virtual int writeAdditionData(uint8_t* dst, uint8_t* dstEnd,
                                AVPacket& avPacket,
