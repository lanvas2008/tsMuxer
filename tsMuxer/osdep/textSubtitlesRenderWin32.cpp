...
void TextSubtitlesRenderWin32::setRenderSize(int width, int height) {
  ...
  m_width = width;
  m_height = height;
  m_pbmpInfo = (BITMAPINFO*)new uint8_t[sizeof(BITMAPINFOHEADER) +
                                        256 * sizeof(RGBQUAD)];
  ...
  m_hbmp = ::CreateDIBSection(m_dc, m_pbmpInfo, DIB_RGB_COLORS,
                              (void**)&m_pData, 0, 0);
  SelectObject(m_dc, m_hbmp);
  if (m_hbmp == 0)
    THROW(ERR_COMMON,
          "Can't initialize graphic subsystem for render text subtitles");
...
void TextSubtitlesRenderWin32::drawText(const std::wstring& text, RECT* rect) {
  ...
  path.AddString(text.c_str(), text.length(), &fontFamily, m_font.m_opts & 0xf,
                 m_font.m_size, Gdiplus::Point(rect->left, rect->top),
                 &strformat);
...
void TextSubtitlesRenderWin32::getTextSize(const std::wstring& text,
                                           SIZE* mSize) {
#ifdef OLD_WIN32_RENDERER
  ...
  int lineSpacingPixel =
      font.GetSize() * lineSpacing / fontFamily.GetEmHeight(opts);

  StringFormat strformat;
  GraphicsPath path;
  path.AddString(text.c_str(), text.length(), &fontFamily, opts, m_font.m_size,
...
int TextSubtitlesRenderWin32::getBaseline() {
  ...
  int descentPixel =
      font.GetSize() * descentOffset / fontFamily.GetEmHeight(opts);
  return descentPixel;
