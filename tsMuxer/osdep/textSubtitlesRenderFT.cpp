...
TextSubtitlesRenderFT::TextSubtitlesRenderFT() : TextSubtitlesRender() {
   ...
   if (!initialized) {
      ...
      if (error) THROW(ERR_COMMON, "Can't initialize freeType font library");
      loadFontMap();
      initialized = true;
   }
   m_pData = 0;
...
string TextSubtitlesRenderFT::findAdditionFontFile(const string& fontName,
                                                   const string& fontExt,
...
void TextSubtitlesRenderFT::setFont(const Font& font) {
   if (m_font != font) {
      ...
      if (fileExt.length() > 0) fileExt = string(".") + fileExt;
      fontName = fontName.substr(0, fontName.length() - fileExt.length());
      string postfix;
      string modFontName =
          findAdditionFontFile(fontName, fileExt, font.m_opts & Font::BOLD,
                               font.m_opts & Font::ITALIC);
      int error = loadFont(modFontName, face);
      if (error == FT_Err_Unknown_File_Format) {
         THROW(ERR_COMMON,
               "Unknown font format for file " << modFontName.c_str());
      } else if (error) {
      ...
      if (error) THROW(ERR_COMMON, "Invalid font size " << font.m_size);

      m_line_thickness =
          FT_MulFix(face->underline_thickness, face->size->metrics.y_scale) >>
          6;
      if (m_line_thickness < 1) m_line_thickness = 1;
      m_underline_position =
      ...
      if (m_font.m_charset) {
         ...
         if (error)
            THROW(ERR_COMMON, "Can't load charset " << m_font.m_charset
                                                    << " for font "
                                                    << modFontName.c_str());
      }
   }
...
uint32_t clrMax(uint32_t first, uint32_t second) {
   ...
   int first_yuv = (0.257 * rgba->rgbRed) + (0.504 * rgba->rgbGreen) +
                   (0.098 * rgba->rgbBlue) + 16;
   int second_yuv = (0.257 * rgba2->rgbRed) + (0.504 * rgba2->rgbGreen) +
                    (0.098 * rgba2->rgbBlue) + 16;
   return first_yuv > second_yuv ? first : second;
}

void TextSubtitlesRenderFT::drawHorLine(int left, int right, int top,
                                        RECT* rect) {
   if (top < rect->top || top >= rect->bottom) return;
...
void RenderGlyph(FT_Library& library, wchar_t ch, FT_Face& face, int size,
                 const Pixel32& fontCol, const Pixel32 outlineColOut,
                 const Pixel32 outlineColIner, float outlineWidth, int left,
                 int top, int width, int height, uint32_t* dstData) {
   ...
   if (FT_Get_Glyph(face->glyph, &glyph) == 0) {
      ...
      FT_Stroker_Set(strokerOut, (int)(outlineWidth * 64),
      ...
      FT_Stroker_Set(strokerIn, (int)(outlineWidth * 32),
                     FT_STROKER_LINECAP_ROUND, FT_STROKER_LINEJOIN_ROUND, 0);
      ...
      if (!spans.empty()) {
         ...
         for (Spans::iterator s = outlineSpansOut.begin();
              s != outlineSpansOut.end(); ++s) {
            rect.Include(Vec2(s->x, s->y));
            rect.Include(Vec2(s->x + s->width - 1, s->y));
         }
         for (Spans::iterator s = outlineSpansIner.begin();
              s != outlineSpansIner.end(); ++s) {
            rect.Include(Vec2(s->x, s->y));
            rect.Include(Vec2(s->x + s->width - 1, s->y));
         }
         ...
         for (Spans::iterator s = outlineSpansOut.begin();
            ...
            for (int w = 0; w < s->width; ++w) {
               ...
               if (y >= 0 && y < height && x >= 0 && x < width) {
                  ...
                  if (*(uint32_t*)dst == 0)
                     *dst = Pixel32(outlineColOut.r, outlineColOut.g,
                                    outlineColOut.b,
         ...
         for (Spans::iterator s = outlineSpansIner.begin();
            ...
            for (int w = 0; w < s->width; ++w) {
               ...
               if (y >= 0 && y < height && x >= 0 && x < width) {
                  Pixel32* dst = (Pixel32*)dstData + y * width + x;
                  *dst = Pixel32(outlineColIner.r, outlineColIner.g,
                                 outlineColIner.b, s->coverage);
               }
            }
         }

         // Then loop over the regular glyph spans and blend them into the
         // image.
         for (Spans::iterator s = spans.begin(); s != spans.end(); ++s) {
            for (int w = 0; w < s->width; ++w) {
               int y = imgHeight - 1 - (s->y - rect.ymin) + top;
               ...
               if (y >= 0 && y < height && x >= 0 && x < width) {
                  ...
                  Pixel32 src =
                      Pixel32(fontCol.r, fontCol.g, fontCol.b, s->coverage);
...
void TextSubtitlesRenderFT::drawText(const wstring& text, RECT* rect) {
   ...
   for (int i = 0; i < text.length(); ++i) {
      RenderGlyph(library, text.at(i), face, m_font.m_size, m_font.m_color,
                  Pixel32(0, 0, 0, outColor), Pixel32(0, 0, 0, alpha),
                  m_font.m_borderWidth, pen.x, pen.y, rect->right, rect->bottom,
                  (uint32_t*)m_pData);

      pen.x += face->glyph->advance.x >> 6;
      pen.x += m_font.m_borderWidth / 2;
      if (m_emulateBold || m_emulateItalic) pen.x += m_line_thickness - 1;
      maxX = pen.x + face->glyph->bitmap_left;
   }
   if ((m_font.m_opts & m_font.UNDERLINE) ||
       (m_font.m_opts & m_font.STRIKE_OUT)) {
      // SIZE size;
      // getTextSize(text, &size);
      if (m_font.m_opts & m_font.UNDERLINE) {
         int y = pen.y + (face->size->metrics.ascender >> 6) +
                 m_underline_position - m_line_thickness / 2;
         for (int i = 0; i < m_line_thickness; ++i)
            drawHorLine(rect->left, /*rect->left + size.cx*/ maxX, y + i, rect);
      }
      if (m_font.m_opts & m_font.STRIKE_OUT) {
         int y = pen.y + (face->size->metrics.ascender >> 6) * 2.0 / 3.0 -
                 m_line_thickness / 2;
         for (int i = 0; i < m_line_thickness; ++i)
            drawHorLine(rect->left, /*rect->left + size.cx*/ pen.x, y + i,
                        rect);
      }
   }
}
...
void TextSubtitlesRenderFT::getTextSize(const wstring& text, SIZE* mSize) {
   ...
   for (int i = 0; i < text.length(); ++i) {
      ...
      if (error)
         THROW(ERR_COMMON,
               "Can't load symbol code '" << text.at(i) << "' from font");

      error = FT_Render_Glyph(face->glyph, FT_RENDER_MODE_NORMAL);
      if (error)
         THROW(ERR_COMMON,
               "Can't render symbol code '" << text.at(i) << "' from font");
      pen.x += face->glyph->advance.x >> 6;
      if (m_emulateBold || m_emulateItalic) pen.x += m_line_thickness - 1;
      pen.x += m_font.m_borderWidth / 2;
