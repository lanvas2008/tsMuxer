...
int DVBSubStreamReader::decodeFrame(uint8_t* buff, uint8_t* end, int& skipBytes,
                                    int& skipBeforeBytes) {
  skipBytes = 0;
  skipBeforeBytes = 0;
  int rez;
...
int DVBSubStreamReader::intDecodeFrame(uint8_t* buff, uint8_t* end) {
  ...
  if (buf_size < 10) return NOT_ENOUGHT_BUFFER;
  ...
  if (m_firstFrame) {
    ...
    if (m_big_offsets && AV_RB16(buff) != 0) return BAD_FRAME;
    if (!m_big_offsets && AV_RB16(buff) == 0) return BAD_FRAME;
  }
  ...
  while ((cmd_pos + 2 + m_offset_size) < buf_size) {
    ...
    while (pos < buf_size && !breakFlag) {
      ...
      switch (cmd) {
        ...
        case 0x03:
          // set colormap
          if ((buf_size - pos) < 2) return NOT_ENOUGHT_BUFFER;
        ...
        case 0x83:
          // HD set palette
          if ((buf_size - pos) < 768) return NOT_ENOUGHT_BUFFER;
          yuv_palette = buff + pos;
          pos += 768;
          break;
        case 0x84:
          // HD set contrast (alpha)
          if ((buf_size - pos) < 256) return NOT_ENOUGHT_BUFFER;
          for (i = 0; i < 256; i++) alpha[i] = 0xFF - buff[pos + i];
          pos += 256;
          break;
        case 0xff:
        default:
          if (next_cmd_pos == cmd_pos) return (pos + 1) & 0xfffffffe;
