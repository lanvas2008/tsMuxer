...
HevcUnitWithProfile::HevcUnitWithProfile()
    : profile_idc(0), level_idc(0), interlaced_source_flag(0) {
   memset(sub_layer_profile_present_flag, 0,
          sizeof(sub_layer_profile_present_flag));
   memset(sub_layer_level_present_flag, 0,
          sizeof(sub_layer_level_present_flag));
...
void HevcUnitWithProfile::profile_tier_level(int subLayers) {
   ...
   for (int i = 0; i < subLayers - 1; i++) {
      if (sub_layer_profile_present_flag[i]) {
         ...
         for (int j = 0; j < 32; j++)
            m_reader.skipBit();  // sub_layer_profile_compatibility_flag[ i ][ j
                                 // ] u(1)
...
void HevcSpsUnit::hrd_parameters(bool commonInfPresentFlag,
   ...
   if (commonInfPresentFlag) {
      ...
      if (nal_hrd_parameters_present_flag || vcl_hrd_parameters_present_flag) {
         ...
         if (sub_pic_hrd_params_present_flag) {
            ...
            m_reader.skipBits(
                5);  // du_cpb_removal_delay_increment_length_minus1 u(5)
            m_reader
                .skipBit();  // sub_pic_cpb_params_in_pic_timing_sei_flag u(1)
   ...
   for (int i = 0; i <= maxNumSubLayersMinus1; i++) {
      ...
      if (!low_delay_hrd_flag) cpb_cnt_minus1[i] = extractUEGolombCode();
      if (nal_hrd_parameters_present_flag) sub_layer_hrd_parameters(i);
      if (vcl_hrd_parameters_present_flag) sub_layer_hrd_parameters(i);
   }
}
...
void HevcSpsUnit::short_term_ref_pic_set(int stRpsIdx) {
   ...
   if (stRpsIdx != 0) rps_predict = m_reader.getBit();
   ...
   if (rps_predict) {
      ...
      for (i = 0; i <= rps_ridx->num_delta_pocs; i++) {
         ...
         if (!used) use_delta_flag = m_reader.getBit();
...
void HevcSpsUnit::short_term_ref_pic_set(int stRpsIdx )
{
    ...
    if( inter_ref_pic_set_prediction_flag )
    {
        ...
        int NumDeltaPocs = NumNegativePics[ RefRpsIdx ] + NumPositivePics[
RefRpsIdx ];
...
{ DeltaPocS0[ stRpsIdx ][ i ] = dPoc; UsedByCurrPicS0[ stRpsIdx ][ i++ ] =
...
NumDeltaPocs ];
        ...
        if( deltaRps > 0 && use_delta_flag[ NumDeltaPocs ] )
        {
            ...
            UsedByCurrPicS1[ stRpsIdx ][ i++ ] = used_by_curr_pic_flag[
NumDeltaPocs ];
        }
        for(int j = 0; j < NumPositivePics[ RefRpsIdx ]; j++) {
            int dPoc = DeltaPocS1[ RefRpsIdx ][ j ] + deltaRps;
            if( dPoc > 0 && use_delta_flag[ NumNegativePics[ RefRpsIdx ] + j ] )
{ DeltaPocS1[ stRpsIdx ][ i ] = dPoc; UsedByCurrPicS1[ stRpsIdx ][ i++ ] =
used_by_curr_pic_flag[ NumNegativePics[ RefRpsIdx ] + j ];
            }
        }
...
( delta_poc_s0_minus1 + 1 ); else DeltaPocS0[ stRpsIdx ][ i ] = -(
...
( delta_poc_s1_minus1 + 1 ); else DeltaPocS1[ stRpsIdx ][ i ] = -(
...
void HevcSpsUnit::scaling_list_data() {
   for (int sizeId = 0; sizeId < 4; sizeId++) {
      for (int matrixId = 0; matrixId < ((sizeId == 3) ? 2 : 6); matrixId++) {
         ...
         if (!flag)
            extractUEGolombCode();  // scaling_list_pred_matrix_id_delta[ sizeId
                                    // ][ matrixId ]
         else {
            int nextCoef = 8;
            ...
            if (sizeId > 1) {
               int value =
                   extractSEGolombCode();  // scaling_list_dc_coef_minus8[
                                           // sizeId - 2 ][ matrixId ]
               nextCoef = value + 8;
            }
...
int HevcSpsUnit::deserialize() {
   ...
   try {
      ...
      if (crhomaFormat == 3) separate_colour_plane_flag = m_reader.getBit();
      ...
      if (pcm_enabled_flag) {
         ...
         extractUEGolombCode();  // log2_min_pcm_luma_coding_block_size_minus3
                                 // ue(v)
         extractUEGolombCode();  // log2_diff_max_min_pcm_luma_coding_block_size
                                 // ue(v)
      ...
      */
      ...
      if (long_term_ref_pics_present_flag) {
         ...
         for (int i = 0; i < num_long_term_ref_pics_sps; i++) {
            m_reader.skipBits(log2_max_pic_order_cnt_lsb);  // lt_ref_pic_poc_lsb_sps[
                                                            // i ] u(v)
...
string HevcSpsUnit::getDescription() const {
   ...
   result += string(" Resolution: ") + int32ToStr(pic_width_in_luma_samples) +
             string(":") + int32ToStr(pic_height_in_luma_samples);
...
int HevcSliceHeader::deserialize(const HevcSpsUnit* sps,
                                 const HevcPpsUnit* pps) {
   int rez = HevcUnit::deserialize();
   if (rez) return rez;
   ...
   try {
      ...
      if (!first_slice) {
         ...
         int slice_segment_address = m_reader.getBits(
             sps->PicSizeInCtbsY_bits);  // Ceil( Log2( PicSizeInCtbsY ) )
      }
      if (!dependent_slice_segment_flag) {
         ...
         if (!isIDR()) {
            pic_order_cnt_lsb =
                m_reader.getBits(sps->log2_max_pic_order_cnt_lsb);
         }
      }
...
vector<vector<uint8_t>> hevc_extract_priv_data(const uint8_t* buff, int size,
                                               int* nal_size) {
   *nal_size = 4;

   vector<vector<uint8_t>> spsPps;
   if (size < 23) return spsPps;
   ...
   *nal_size = (buff[21] & 3) + 1;
   ...
   for (int i = 0; i < num_arrays; ++i) {
      if (src + 3 > end) THROW(ERR_MOV_PARSE, "Invalid HEVC extra data format");
      int type = *src++;
      int cnt = AV_RB16(src);
      src += 2;
