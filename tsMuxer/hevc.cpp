...
void HevcUnitWithProfile::profile_tier_level(int subLayers) {
  ...
  for (int i = 0; i < subLayers - 1; i++) {
    if (sub_layer_profile_present_flag[i]) {
      ...
      for (int j = 0; j < 32; j++)
        m_reader
            .skipBit();  // sub_layer_profile_compatibility_flag[ i ][ j ] u(1)
...
void HevcSpsUnit::vui_parameters() {
  ...
  if (vui_timing_info_present_flag) {
    ...
    if (vui_hrd_parameters_present_flag) hrd_parameters(1, max_sub_layers - 1);
  }
  ...
  for (int sizeId = 0; sizeId < 4; sizeId++) {
    for (int matrixId = 0; matrixId < ((sizeId == 3) ? 2 : 6); matrixId++) {
      ...
      if (!flag)
        ...
        if (sizeId > 1) {
          int value = extractSEGolombCode();  // scaling_list_dc_coef_minus8[
                                              // sizeId - 2 ][ matrixId ]
          nextCoef = value + 8;
        }
    ...
    */
    ...
    if (long_term_ref_pics_present_flag) {
      ...
      for (int i = 0; i < num_long_term_ref_pics_sps; i++) {
        m_reader.skipBits(
            log2_max_pic_order_cnt_lsb);  // lt_ref_pic_poc_lsb_sps[ i ] u(v)
...
vector<vector<uint8_t>> hevc_extract_priv_data(const uint8_t* buff, int size,
  ...
  *nal_size = (buff[21] & 3) + 1;
  ...
  for (int i = 0; i < num_arrays; ++i) {
    ...
    for (int j = 0; j < cnt; ++j) {
      if (src + 2 > end) THROW(ERR_MOV_PARSE, "Invalid HEVC extra data format");
