...
int MpegAudioStreamReader::decodeFrame(uint8_t* buff, uint8_t* end,
                                       int& skipBytes, int& skipBeforeBytes) {
   skipBytes = 0;
   skipBeforeBytes = 0;
...
double MpegAudioStreamReader::getFrameDurationNano() {
   ...
   if (m_layer == 3 && m_nb_channels == 1)
      rez /= 2;  // todo: what is that? I myself don't understand why it's
                 // necessary to leave it here.
   return rez;
}
