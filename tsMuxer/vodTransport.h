...
startStreaming(const char* session, int lowLevelTransport, double ntpPos,
               ...
               bool doDownload = false, int64_t downloadLimit = 0);
...
LIB_EXPORT unsigned LIB_STD_CALL startSmartBridgeStreaming(
...
LIB_EXPORT unsigned LIB_STD_CALL milticastJoin(const char* address,
                                               unsigned short port,
                                               const char* ixface);
...
LIB_EXPORT bool LIB_STD_CALL startAutoIndexing(const std::string& dir,
                                               bool createRewind);
...
LIB_EXPORT void LIB_STD_CALL initLog(int messageLevel, const char* logName,
                                     const char* logDir, int logRotationPeriod);

LIB_EXPORT void LIB_STD_CALL getAssetDescription(const char* sessionID,
...
LIB_EXPORT int64_t LIB_STD_CALL getFileLength(
    const char* fileName,
    double fromOffset);  // return whole file length for zerro offset, or
                         // partial file length from specified offset
...
LIB_EXPORT void LIB_STD_CALL setYTCacheParams(
    const std::string& sCacheDirectory, const std::string& sLogDirectory,
    uint64_t nTotalCacheDuration);
LIB_EXPORT void LIB_STD_CALL setRTCacheParams(
    const std::string& sCacheDirectory, uint64_t nTotalCacheDuration);

LIB_EXPORT void LIB_STD_CALL getYTVideoInfo(const std::string& streamName,
                                            uint8_t& existingType, double& fps,
                                            bool cleanUp = false);

LIB_EXPORT void LIB_STD_CALL getRTVideoInfo(const std::string& streamName,
                                            uint8_t& existingType, double& fps,
                                            bool cleanUp = false);

LIB_EXPORT void LIB_STD_CALL getYTDomesticHost(char* dstBuff,
                                               uint32_t& nDstBuffSize);
LIB_EXPORT void LIB_STD_CALL
setYTDomesticHost(const std::string& sDomesticHost);
...
LIB_EXPORT void LIB_STD_CALL
setMediaInterface(const std::string& sMediaInterface);
...
LIB_EXPORT bool LIB_STD_CALL
cyclicMulticastEncoder(std::string name);  // it is a cyclic multicast encoder
...
LIB_EXPORT bool LIB_STD_CALL
isMulticastEncoder(std::string name);  // it is any type multicast encoder
