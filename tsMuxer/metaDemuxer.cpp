    ...
    if (minDtsIndex != -1) {
      if (!m_flushDataMode) {
        if (m_codecInfo[minDtsIndex].lastReadRez !=
          ...
          int res =
              m_codecInfo[minDtsIndex].m_streamReader->readPacket(avPacket);
...
void METADemuxer::openFile(const string& streamName) {
  ...
  while (str.length() > 0) {
    ...
    for (unsigned i = 2; i < params.size(); i++) {
      ...
      addParams.insert(std::make_pair(trimStr(tmp[0]),
                                      trimStr(tmp.size() > 1 ? tmp[1] : "")));
    }
  ...
  if (fileExt == "mpls" || fileExt == "mpl") {
    ...
    for (int k = 0; k < mplsInfoList.size(); ++k) {
        ...
        if (isSubStream) {
          if (mplsInfo.m_mvcFiles.size() == 0) {
            THROW(ERR_INVALID_CODEC_FORMAT,
                  "Current playlist file doesn't has MVC track info. Please, "
      ...
      if (streamInfo.stream_coding_type == 0x90 && streamInfo.isSSPG) {
        ...
        rightEyeSubStreamIdx = addPGSubStream(codec, unquotedStreamName,
                                              addParams, streamInfo.rightEye);
...
DetectStreamRez METADemuxer::DetectStreamReader(
  ...
  if (strEndWith(tmpname, ".m2ts") || strEndWith(tmpname, ".mts") ||
    ...
    if (clpiFileName.size() > 0) clpiParsed = clpi.parse(clpiFileName.c_str());
  ...
  if (demuxer) {
    ...
    for (map<uint32_t, TrackInfo>::const_iterator itr = acceptedPidMap.begin();
    ...
    for (map<uint32_t, StreamData>::iterator itr = demuxedData.begin();
      ...
      CheckStreamRez trackRez =
          detectTrackReader(vect.data(), vect.size(), containerType,
...
CheckStreamRez METADemuxer::detectTrackReader(
  ...
  rez = lpcmReader.checkStream(tmpBuffer, len, containerType, containerDataType,
                               containerStreamIndex);
  if (rez.codecInfo.codecID) return rez;
  ...
  if (codecName == "V_MPEG4/ISO/AVC" || codecName == "V_MPEG4/ISO/MVC") {
    ...
    */
    ...
    if (srtWidth == 0 || srtHeight == 0 || fps == 0)
      THROW(ERR_COMMON,
            "video-width, video-height and fps parameters MUST be provided for "
            "SRT tracks");
...
string METADemuxer::findBluRayFile(const string& streamDir,
  ...
  if (tmp != std::string::npos) {
    ...
    if (!fileExists(fileName)) {
      fileName = dirName + string("BACKUP") + getDirSeparator() + requestDir +
                 getDirSeparator() + requestFile;
      if (!fileExists(fileName))
        return "";
      else
        return fileName;
...
bool ContainerToReaderWrapper::openStream(uint32_t readerID,
  ...
  if (codecInfo &&
      (codecInfo->codecID == CODEC_S_PGS || codecInfo->codecID == CODEC_S_SUP ||
       codecInfo->codecID == CODEC_S_SRT))
