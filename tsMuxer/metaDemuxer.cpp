...
uint64_t METADemuxer::getDemuxedSize() {
   ...
   for (unsigned i = 0; i < m_codecInfo.size(); i++)
      rez += m_codecInfo[i]
                 .m_streamReader
                 ->getProcessedSize();  // m_codecInfo[i].m_dataProcessed;
      ...
      while (allDataDelayed) {
         for (unsigned i = 0; i < m_codecInfo.size(); i++) {
            ...
            if (!m_flushDataMode) {
               ...
               if (streamInfo.lastReadRez ==
                   BufferedFileReader::DATA_NOT_READY) {
               ...
               } else if (streamInfo.m_lastDTS < minDts &&
                          !streamInfo.m_flushed) {
                  minDtsIndex = i;
                  minDts = streamInfo.m_lastDTS;
               }
            } else {
         ...
         if (allDataDelayed)
            for (unsigned i = 0; i < m_codecInfo.size(); i++) {
               ContainerToReaderWrapper* cReader =
                   dynamic_cast<ContainerToReaderWrapper*>(
                       m_codecInfo[i].m_dataReader);
               if (cReader) cReader->resetDelayedMark();
            }
      }
      if (minDtsIndex != -1) {
         if (!m_flushDataMode) {
            if (m_codecInfo[minDtsIndex].lastReadRez !=
                BufferedFileReader::DATA_EOF2) {
               int res = m_codecInfo[minDtsIndex].m_streamReader->readPacket(
                   avPacket);
            ...
            m_codecInfo[minDtsIndex].m_lastDTS =
                avPacket.dts + avPacket.duration;
...
void METADemuxer::openFile(const string& streamName) {
   ...
   while (str.length() > 0) {
      ...
      for (unsigned i = 2; i < params.size(); i++) {
         ...
         addParams.insert(std::make_pair(
             trimStr(tmp[0]), trimStr(tmp.size() > 1 ? tmp[1] : "")));
      }
   ...
   for (int i = 0; i < m_codecInfo.size(); ++i) {
      H264StreamReader* reader =
          dynamic_cast<H264StreamReader*>(m_codecInfo[i].m_streamReader);
      ...
      for (int i = 0; i < m_codecInfo.size(); ++i) {
         ...
         if (reader && reader->isSubStream()) {
            if (!warned && (int)reader->getInsertSEI() != primarySEI) {
               LTRACE(
                   LT_INFO, 2,
                   "Parameter 'insertSEI' for MVC dependent view differs from "
                   "same param for AVC base view. Ignore and apply same value");
               warned = true;
            }
...
std::string METADemuxer::mplsTrackToFullName(const std::string& mplsFileName,
                                             std::string& mplsNum) {
   string path = toNativeSeparators(extractFilePath(mplsFileName));
   int tmp = path.find_last_of(getDirSeparator());
   if (tmp == string::npos) return string();
...
std::string METADemuxer::mplsTrackToSSIFName(const std::string& mplsFileName,
                                             std::string& mplsNum) {
   string path = toNativeSeparators(extractFilePath(mplsFileName));
   int tmp = path.find_last_of(getDirSeparator());
   if (tmp == string::npos) return string();
   path = path.substr(0, tmp + 1) + string("STREAM") + getDirSeparator() +
          string("SSIF") + getDirSeparator();
...
int METADemuxer::addPGSubStream(const string& codec,
                                const string& _codecStreamName,
   ...
   if (subStream->type == 2) params["subClip"] = "1";
   return addStream(codec, _codecStreamName, params);
}

std::vector<MPLSPlayItem> METADemuxer::mergePlayItems(
    const std::vector<MPLSParser>& mplsInfoList) {
...
int METADemuxer::addStream(const string codec, const string& codecStreamName,
                           const map<string, string>& addParams) {
   uint32_t pid = 0;
   map<string, string>::const_iterator tmpitr = addParams.find("track");
   if (tmpitr != addParams.end()) pid = strToInt32(tmpitr->second.c_str());
   ...
   bool isSubStream = (codec == h264DepCodecInfo.programName) ||
                      addParams.find("subClip") != addParams.end();
   if (fileExt == "mpls" || fileExt == "mpl") {
      ...
      for (int k = 0; k < mplsInfoList.size(); ++k) {
         ...
         for (int i = 0; i < mplsInfo.m_playItems.size(); ++i) {
            ...
            if (isSubStream) {
               if (mplsInfo.m_mvcFiles.size() == 0) {
                  THROW(ERR_INVALID_CODEC_FORMAT,
                        "Current playlist file doesn't has MVC track info. "
                        "Please, "
                        "remove MVC track from the track list");
               } else if (mplsInfo.m_mvcFiles.size() <= i)
                  THROW(
                      ERR_INVALID_CODEC_FORMAT,
                      "Bad playlist file: number of CLPI files for AVC and VMC "
                      "parts do not match");
            ...
            string fileName =
                mplsTrackToFullName(unquotedStreamName, playItemName);
            if (mplsInfo.isDependStreamExist && !fileExists(fileName))
               fileName = mplsTrackToSSIFName(unquotedStreamName,
                                              mplsInfo.m_playItems[i].fileName);
         ...
         if (streamInfo.stream_coding_type == 0x90 && streamInfo.isSSPG) {
            ...
            leftEyeSubStreamIdx = addPGSubStream(codec, unquotedStreamName,
                                                 addParams, streamInfo.leftEye);
            rightEyeSubStreamIdx = addPGSubStream(
                codec, unquotedStreamName, addParams, streamInfo.rightEye);
   ...
   for (int i = fileList.size() - 1; i >= 0; --i) {
      string trackKey =
          fileList[i] + string("_#") + int32ToStr(i) + string("_");
   ...
   if (m_containerReader.m_demuxers.find(fileList[0]) ==
       m_containerReader.m_demuxers.end()) {
      File tmpFile;
      for (unsigned i = 0; i < fileList.size(); i++) {
         if (!tmpFile.open(fileList[i].c_str(), File::ofRead))
            THROW(ERR_INVALID_CODEC_FORMAT,
                  "Can't open file: " << fileList[i].c_str());
         uint64_t tmpSize = 0;
   ...
   AbstractStreamReader* codecReader =
       createCodec(codec, addParams, fileList[0], mergePlayItems(mplsInfoList));
   codecReader->setStreamIndex(m_codecInfo.size() + 1);
   codecReader->setTimeOffset(m_timeOffset);

   if (m_codecInfo.size() == 0) codecReader->m_flags |= AVPacket::PCR_STREAM;

   AbstractReader* dataReader;
   string tmpname = strToLowerCase(fileList[0]);
   tmpname = unquoteStr(trimStr(tmpname));

   if (strEndWith(tmpname, ".h264") || strEndWith(tmpname, ".264") ||
       strEndWith(tmpname, ".mvc")) {
      if (pid) {
         dataReader = &m_containerReader;
         codecReader->setSrcContainerType(AbstractStreamReader::ctMultiH264);
         if (listIterator)
            ((ContainerToReaderWrapper*)dataReader)
                ->setFileIterator(fileList[0].c_str(), listIterator);
      } else {
         dataReader = (const_cast<BufferedReaderManager&>(m_readManager))
                          .getReader(fileList[0].c_str());
      }
   } else if (strEndWith(tmpname, ".ts") || strEndWith(tmpname, ".m2ts") ||
              strEndWith(tmpname, ".mts") || strEndWith(tmpname, ".ssif")) {
      if (pid)
         dataReader = &m_containerReader;
      else
         THROW(ERR_INVALID_CODEC_FORMAT,
   ...
   } else if (strEndWith(tmpname, ".vob") || strEndWith(tmpname, ".evo") ||
              strEndWith(tmpname, ".mpg")) {
      if (pid)
         dataReader = &m_containerReader;
      else
         THROW(
   ...
   } else if (strEndWith(tmpname, ".mov") || strEndWith(tmpname, ".mp4") ||
   ...
   if (dataReader == 0) {
      delete codecReader;
      THROW(ERR_INVALID_CODEC_FORMAT,
            "This version do not support multicast or other network steams for "
            "muxing");
   }

   m_codecInfo.push_back(StreamInfo(dataReader, codecReader, fileList[0],
                                    codecStreamName, pid, isSubStream));
   if (listIterator) {
      BufferedFileReader* fileReader =
          dynamic_cast<BufferedFileReader*>(dataReader);
      if (fileReader)
         fileReader->setFileIterator(listIterator,
                                     m_codecInfo.rbegin()->m_readerID);
   }
   ...
   if (itr != addParams.end()) {
      ...
      if (value * coeff > 0) streamInfo.m_lastDTS = value * coeff;
   }

   itr = addParams.find("lang");
   if (itr != addParams.end()) streamInfo.m_lang = itr->second;
...
DetectStreamRez METADemuxer::DetectStreamReader(
    BufferedReaderManager& readManager, const string& fileName,
    bool calcDuration) {
   AVChapters chapters;
   ...
   AbstractStreamReader::ContainerType containerType =
       AbstractStreamReader::ctNone;
   CLPIParser clpi;
   bool clpiParsed = false;
   if (strEndWith(tmpname, ".m2ts") || strEndWith(tmpname, ".mts") ||
       strEndWith(tmpname, ".ssif")) {
      demuxer = new TSDemuxer(readManager, "");
      containerType = AbstractStreamReader::ctM2TS;
      string clpiFileName = findBluRayFile(extractFileDir(tmpname), "CLIPINF",
                                           extractFileName(tmpname) + ".clpi");
   ...
   } else if (strEndWith(tmpname, ".mp4") || strEndWith(tmpname, ".m4a") ||
   ...
   if (demuxer) {
      ...
      for (map<uint32_t, TrackInfo>::const_iterator itr =
               acceptedPidMap.begin();
           itr != acceptedPidMap.end(); ++itr)
         acceptedPidSet.insert(itr->first);
      for (DemuxedData::iterator itr = demuxedData.begin();
           itr != demuxedData.end(); ++itr) {
      ...
      for (map<uint32_t, StreamData>::iterator itr = demuxedData.begin();
           itr != demuxedData.end(); ++itr) {
         StreamData& vect = itr->second;
         CheckStreamRez trackRez = detectTrackReader(
             vect.data(), vect.size(), containerType,
         ...
         if (clpiParsed) {
            map<int, CLPIStreamInfo>::const_iterator clpiStream =
                clpi.m_streamInfo.find(itr->first);
            if (clpiStream != clpi.m_streamInfo.end() &&
                trackRez.lang.size() == 0)
               trackRez.lang = clpiStream->second.language_code;
         }

         if (strStartWith(trackRez.codecInfo.programName, "A_") &&
             dynamic_cast<TSDemuxer*>(demuxer)) {
            if (trackRez.trackID >= 0x1A00) trackRez.isSecondary = true;
         }
      ...
      if (calcDuration) fileDuration = demuxer->getFileDurationNano();
      delete demuxer;
   } else {
      File file;
      containerType = AbstractStreamReader::ctNone;
      if (!file.open(fileName.c_str(), File::ofRead)) return DetectStreamRez();
      ...
      else if (strEndWith(tmpname, ".pcm") || strEndWith(tmpname, ".lpcm") ||
               strEndWith(tmpname, ".wav") || strEndWith(tmpname, ".w64"))
      ...
      CheckStreamRez trackRez =
          detectTrackReader(tmpBuffer, len, containerType, 0, 0);
...
void METADemuxer::addTrack(vector<CheckStreamRez>& rez, CheckStreamRez trackRez,
                           bool insToBegin) {
   if (trackRez.codecInfo.codecID == h264DepCodecInfo.codecID &&
       trackRez.multiSubStream) {
...
CheckStreamRez METADemuxer::detectTrackReader(
    uint8_t* tmpBuffer, int len,
    AbstractStreamReader::ContainerType containerType, int containerDataType,
    int containerStreamIndex) {
   CheckStreamRez rez;

   PGSStreamReader pgsReader;
   rez = pgsReader.checkStream(tmpBuffer, len, containerType, containerDataType,
                               containerStreamIndex);
   if (rez.codecInfo.codecID) return rez;

   SRTStreamReader srtReader;
   rez = srtReader.checkStream(tmpBuffer, len, containerType, containerDataType,
                               containerStreamIndex);
   if (rez.codecInfo.codecID) return rez;
   ...
   rez = lpcmReader.checkStream(tmpBuffer, len, containerType,
   ...
   rez = dtscodec.checkStream(tmpBuffer, len, containerType, containerDataType,
                              containerStreamIndex);
   if (rez.codecInfo.codecID) return rez;

   AC3StreamReader ac3codec;
   rez = ac3codec.checkStream(tmpBuffer, len, containerType, containerDataType,
                              containerStreamIndex);
   if (rez.codecInfo.codecID) return rez;

   AACStreamReader aaccodec;
   rez = aaccodec.checkStream(tmpBuffer, len, containerType, containerDataType,
                              containerStreamIndex);
   if (rez.codecInfo.codecID) return rez;
   ...
   rez = mpegAudioCodec.checkStream(tmpBuffer, len, containerType,
                                    containerDataType, containerStreamIndex);
   if (rez.codecInfo.codecID) return rez;

   DVBSubStreamReader supReader;
   rez = supReader.checkStream(tmpBuffer, len, containerType, containerDataType,
                               containerStreamIndex);
   if (rez.codecInfo.codecID) return rez;
...
VideoAspectRatio arNameToCode(const string& arName) {
   if (arName == "9x16" || arName == "16x9" || arName == "9:16" ||
       arName == "16:9")
      return AR_16_9;
   else if (arName == "3:4" || arName == "4:3" || arName == "4x3" ||
            arName == "3x4")
      return AR_3_4;
   else if (arName == "Square" || arName == "VGA" || arName == "1:1" ||
            arName == "1x1" || arName == "1:1 (Square)")
      return AR_VGA;
   else if (arName == "WIDE" || arName == "1x2,21" || arName == "1x2.21" ||
            arName == "1:2.21" || arName == "1:2,21" || arName == "2.21:1" ||
            arName == "2,21:1" || arName == "2.21x1" || arName == "2,21x1")
      return AR_221_100;
   else
...
AbstractStreamReader* METADemuxer::createCodec(
    const string& codecName, const map<string, string>& addParams,
   ...
   if (codecName == "V_MPEG4/ISO/AVC" || codecName == "V_MPEG4/ISO/MVC") {
      ...
      rez = h264Reader;
      if (codecName == "V_MPEG4/ISO/MVC") h264Reader->setIsSubStream(true);
      ...
      if (itr != addParams.end())
         ((H264StreamReader*)rez)
             ->setForceLevel(strToDouble(itr->second.c_str()) * 10);
      itr = addParams.find("insertSEI");
      if (itr != addParams.end()) {
         ((H264StreamReader*)rez)
             ->setInsertSEI(H264StreamReader::SEI_InsertAuto);
      }
      ...
      if (itr != addParams.end()) {
         ((H264StreamReader*)rez)
             ->setInsertSEI(H264StreamReader::SEI_InsertForce);
      }
      ...
      if (itr != addParams.end())
         ((PGSStreamReader*)rez)
             ->setBottomOffset(strToInt32(itr->second.c_str()));
      ...
      if (itr != addParams.end()) fps = strToDouble(itr->second.c_str());
      itr = addParams.find("video-width");
      if (itr != addParams.end()) width = strToInt32(itr->second.c_str());
      itr = addParams.find("video-height");
      if (itr != addParams.end()) height = strToInt32(itr->second.c_str());
      ((PGSStreamReader*)rez)->setVideoInfo(width, height, fps);
      itr = addParams.find("font-border");
      if (itr != addParams.end())
         ((PGSStreamReader*)rez)
             ->setFontBorder(strToInt32(itr->second.c_str()));

      /*
      map<string,string>::const_iterator itr = addParams.find("fps");
      if (itr != addParams.end())
              ((PGSStreamReader*)
      rez)->setFPS(strToDouble(itr->second.c_str())); itr =
      addParams.find("video-width"); if (itr != addParams.end())
              ((PGSStreamReader*)
      rez)->setVideoWidth(strToInt32(itr->second.c_str())); itr =
      addParams.find("video-height"); if (itr != addParams.end())
              ((PGSStreamReader*)
      rez)->setVideoHeight(strToInt32(itr->second.c_str()));
      */
      ...
      for (map<string, string>::const_iterator itr = addParams.begin();
           itr != addParams.end(); ++itr) {
         if (itr->first == "font-name") {
            ...
            if (s.size() >= 2 && s[0] == '0' && s[1] == 'x')
               font.m_color =
                   strToInt32u(s.substr(2, s.size() - 2).c_str(), 16);
            else if (s.size() >= 1 && s[0] == 'x')
               font.m_color =
                   strToInt32u(s.substr(1, s.size() - 1).c_str(), 16);
            else
               font.m_color = strToInt32u(s.c_str());
            if ((font.m_color & 0xff000000u) == 0) font.m_color |= 0xff000000u;
      ...
      if (srtWidth == 0 || srtHeight == 0 || fps == 0)
         THROW(ERR_COMMON,
               "video-width, video-height and fps parameters MUST be provided "
               "for "
               "SRT tracks");
   ...
   if (codecName[0] == 'A') {
      ...
      if (itr != addParams.end()) {
         ...
         if (divPos == std::string::npos)
            ...
            string secondStr =
                itr->second.substr(divPos + 1, itr->second.size() - divPos - 1);
            ...
            if (dSecond != 0)
               ...
               THROW(ERR_COMMON,
                     "Second argument at stretch parameter can not be 0");
         }
         if (itr != addParams.end())
   ...
   if (itr != addParams.end()) rez->setIsSecondary(true);

   PIPParams pipParams;
   itr = addParams.find("pipCorner");
   if (itr != addParams.end()) pipParams.corner = pipCornerFromStr(itr->second);
   itr = addParams.find("pipHOffset");
   if (itr != addParams.end()) pipParams.hOffset = strToInt32(itr->second);
   itr = addParams.find("pipVOffset");
   if (itr != addParams.end()) pipParams.vOffset = strToInt32(itr->second);
   itr = addParams.find("pipLumma");
   if (itr != addParams.end()) pipParams.lumma = strToInt32(itr->second);
...
const std::vector<MPLSParser> METADemuxer::getMplsInfo(
    const string& mplsFileName) {
   std::vector<MPLSParser> result;

   std::vector<std::string> mplsFiles =
       splitQuotedStr(mplsFileName.c_str(), '+');
   for (int i = 0; i < mplsFiles.size(); ++i) {
      ...
      if (itr != m_mplsStreamMap.end())
         ...
         pair<MPLSCache::iterator, bool> insRez =
             m_mplsStreamMap.insert(make_pair(mplsFiles[i], parser));
...
string METADemuxer::findBluRayFile(const string& streamDir,
                                   const string& requestDir,
   ...
   if (strEndWith(strToLowerCase(dirName), "ssif")) {
      ...
      if (pos > 0) dirName = streamDir.substr(0, pos);
   }
   ...
   if (tmp != std::string::npos) {
      ...
      if (!fileExists(fileName)) {
         fileName = dirName + string("BACKUP") + getDirSeparator() +
                    requestDir + getDirSeparator() + requestFile;
         if (!fileExists(fileName))
            return "";
         else
            return fileName;
...
void METADemuxer::updateReport(bool checkTime) {
   ...
   if (!checkTime ||
       currentTime - m_lastReportTime > std::chrono::microseconds(250000)) {
...
int StreamInfo::read() {
   ...
   if (m_lastAVRez != 0) {
      ...
      if (readRez == BufferedFileReader::DATA_NOT_READY ||
          readRez == BufferedFileReader::DATA_DELAYED) {
         m_lastAVRez = readRez;
         return readRez;
...
uint8_t* ContainerToReaderWrapper::readBlock(uint32_t readerID,
                                             uint32_t& readCnt, int& rez,
                                             bool* firstBlockVar) {
   rez = 0;
   uint8_t* data = 0;
   map<uint32_t, ReaderInfo>::iterator itr = m_readerInfo.find(readerID);
   if (itr == m_readerInfo.end()) return 0;
   ...
   if (demuxerData.m_firstRead) {
      for (map<uint32_t, DemuxerReadPolicy>::iterator itr =
               demuxerData.m_pids.begin();
   ...
   if ((readCnt > 0 &&
   ...
   } else if (demuxerData.lastReadRez[pid] != AbstractReader::DATA_DELAYED ||
              demuxerData.m_allFragmented) {
      map<uint32_t, uint32_t> dataTypeMap;
      int demuxRez = 0;
      do {
         int64_t discardSize = 0;
         demuxRez = demuxerData.m_demuxer->simpleDemuxBlock(
             demuxerData.demuxedData, demuxerData.m_pidSet, discardSize);
         for (DemuxedData::const_iterator itr = demuxerData.demuxedData.begin();
            ...
            if (itr->second.size() > MAX_DEMUX_BUFFER_SIZE)
               THROW(ERR_CONTAINER_STREAM_NOT_SYNC,
                     "Reading buffer overflow. Possible container streams are "
                     "not "
      ...
      } while (demuxRez == 0 && readCnt < MIN_READED_BLOCK &&
               policy != drpFragmented && !m_terminated);

      demuxerData.lastReadCnt[pid] = readCnt;
      data = streamData.data();
      if (readCnt > 0) {
         rez = demuxerData.m_demuxer->getLastReadRez();
      } else if (demuxerData.m_demuxer->getLastReadRez() ==
                 AbstractReader::DATA_EOF)
...
void ContainerToReaderWrapper::terminate() {
   m_terminated = true;
   for (std::map<std::string, DemuxerData>::iterator i = m_demuxers.begin();
        i != m_demuxers.end(); ++i)
      i->second.m_demuxer->terminate();
}

void ContainerToReaderWrapper::resetDelayedMark() {
   for (map<uint32_t, ReaderInfo>::iterator itr = m_readerInfo.begin();
        itr != m_readerInfo.end(); ++itr) {
      DemuxerData& demuxerData = itr->second.m_demuxerData;
      for (map<uint32_t, uint32_t>::iterator itr2 =
               demuxerData.lastReadRez.begin();
           itr2 != demuxerData.lastReadRez.end(); ++itr2)
         if (itr2->second == AbstractReader::DATA_DELAYED) itr2->second = 0;
   }
}
...
void ContainerToReaderWrapper::deleteReader(uint32_t readerID) {
   map<uint32_t, ReaderInfo>::iterator itr = m_readerInfo.find(readerID);
   if (itr == m_readerInfo.end()) return;
...
bool ContainerToReaderWrapper::openStream(uint32_t readerID,
                                          const char* streamName, int pid,
   ...
   if (demuxer == 0) {
      ...
      if ((ext == "264" || ext == "H264" || ext == "MVC") && pid) {
         demuxer = m_demuxers[streamName].m_demuxer =
             new CombinedH264Demuxer(m_readManager, "");
         m_demuxers[streamName].m_streamName = streamName;
      } else if (ext == "TS" || ext == "M2TS" || ext == "MTS" || ext == "M2T" ||
                 ext == "SSIF") {
         demuxer = m_demuxers[streamName].m_demuxer =
             new TSDemuxer(m_readManager, "");
         m_demuxers[streamName].m_streamName = streamName;
      } else if (ext == "EVO" || ext == "VOB" || ext == "MPG" ||
                 ext == "MPEG") {
         demuxer = m_demuxers[streamName].m_demuxer =
             new ProgramStreamDemuxer(m_readManager);
         m_demuxers[streamName].m_streamName = streamName;
      } else if (ext == "MKV" || ext == "MKA") {
         demuxer = m_demuxers[streamName].m_demuxer =
             new MatroskaDemuxer(m_readManager);
         m_demuxers[streamName].m_streamName = streamName;
      } else if (ext == "MOV" || ext == "MP4" || ext == "M4A") {
         demuxer = m_demuxers[streamName].m_demuxer =
             new MovDemuxer(m_readManager);
         m_demuxers[streamName].m_streamName = streamName;
      } else
         THROW(ERR_UNSUPPORTER_CONTAINER_FORMAT,
               "Unsupported container format: " << streamName);
   ...
   if (SubTrackFilter::isSubTrack(pid)) {
      if (!demuxer || !demuxer->isPidFilterSupported())
         THROW(ERR_INVALID_CODEC_FORMAT,
               "Unsupported parameter subTrack for format "
                   << extractFileExt(streamName));
      int srcPID = pid >> 16;
      if (demuxer->getPidFilter(srcPID) == 0) {
         if (codecInfo->codecID == CODEC_V_MPEG4_H264 ||
             codecInfo->codecID == CODEC_V_MPEG4_H264_DEP)
            demuxer->setPidFilter(srcPID, new CombinedH264Filter(srcPID));
         else
            THROW(ERR_INVALID_CODEC_FORMAT,
                  "Unsupported parameter subTrack for codec "
   ...
   if (codecInfo &&
       (codecInfo->codecID == CODEC_S_PGS ||
        codecInfo->codecID == CODEC_S_SUP || codecInfo->codecID == CODEC_S_SRT))
      m_demuxers[streamName].m_pids.insert(std::make_pair(pid, drpFragmented));
   else {
      m_demuxers[streamName].m_pids.insert(
          std::make_pair(pid, drpReadSequence));
      m_demuxers[streamName].m_allFragmented = false;
   }
   m_demuxers[streamName].m_pidSet.insert(pid);
   m_readerInfo.insert(
       std::make_pair(readerID, ReaderInfo(m_demuxers[streamName], pid)));
   return true;
}

void ContainerToReaderWrapper::setFileIterator(const char* streamName,
