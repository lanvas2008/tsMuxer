...
SRTStreamReader::SRTStreamReader() : m_lastBlock(false) {
   // in future version here must be case for destination subtitle format (DVB
   // sub, DVD sub e.t.c)
...
void SRTStreamReader::setAnimation(
    const text_subtitles::TextAnimation& animation) {
...
void SRTStreamReader::setBuffer(uint8_t* data, int dataLen, bool lastBlock) {
   ...
   if (rest > MAX_AV_PACKET_SIZE)
      THROW(ERR_COMMON, "Invalid SRT file or too large text message (>"
                            << MAX_AV_PACKET_SIZE << " bytes)");
   m_tmpBuffer.resize(rest);
   if (rest > 0)
      memcpy(&m_tmpBuffer[0], dataBegin + dataLen + m_tmpBuffer.size() - rest,
             rest);
}

bool SRTStreamReader::detectSrcFormat(uint8_t* dataStart, int len,
                                      int& prefixLen) {
   prefixLen = 0;
   ...
   if (dataStart[0] == 0xEF && dataStart[1] == 0xBB && dataStart[2] == 0xBF) {
      ...
      prefixLen = 3;
   } else if (dataStart[0] == 0 && dataStart[1] == 0 && dataStart[2] == 0xFE &&
              dataStart[3] == 0xFF) {
...
int SRTStreamReader::parseText(uint8_t* dataStart, int len) {
   ...
   if (m_srcFormat == UtfConverter::sfUnknown) {
      if (!detectSrcFormat(dataStart, len, prefixLen)) return false;
   }
   ...
   for (; cur < end; cur += m_charSize) {
      ...
      if (m_charSize == 1 && *cur == '\n' ||
         ...
         m_sourceText.push(UtfConverter::toWideString(
             lastProcessedLine, cur - lastProcessedLine - x, m_srcFormat));
         std::wstring& tmp = m_sourceText.back();
         if (strOnlySpace(tmp)) tmp.clear();
...
int SRTStreamReader::readPacket(AVPacket& avPacket) {
   ...
   if (rez == NEED_MORE_DATA) {
      ...
      if (renderedBuffer) {
         m_dstSubCodec->setBuffer(renderedBuffer - MAX_AV_PACKET_SIZE,
                                  renderedLen,
...
uint8_t* SRTStreamReader::renderNextMessage(uint32_t& renderedLen) {
   uint8_t* rez = 0;
   if (m_sourceText.size() == 0) return 0;
   ...
   if (m_state == PARSE_TIME) {
      if (!parseTime(m_sourceText.front()))
         THROW(ERR_COMMON, "Invalid SRT format. \""
                               << m_sourceText.front().c_str()
                               << "\" is invalid timing info");
      m_state = PARSE_TEXT;
      m_sourceText.pop();
      m_processedSize += m_origSize.front();
      m_origSize.pop();
   ...
   while (m_sourceText.size() > 0 && m_sourceText.front().size() > 0) {
      if (m_renderedText.size() > 0) m_renderedText += '\n';
   ...
   if (m_sourceText.size() == 0) {
      if (m_lastBlock && m_renderedText.size() > 0) {
         ...
         rez = m_srtRender->doConvert(m_renderedText, m_animation, m_inTime,
                                      m_outTime, renderedLen);
      }
      return rez;
   } else {
   ...
   for (int i = 0; i < text.length() - 2; i++) {
      if (text[i] == L'-' && text[i + 1] == L'-' && text[i + 2] == L'>') {
         ...
         for (int j = 0; j < first.length(); j++)
            if (first[j] == L',') first[j] = L'.';
         for (int j = 0; j < second.length(); j++)
            if (second[j] == L',') second[j] = L'.';
...
CheckStreamRez SRTStreamReader::checkStream(uint8_t* buffer, int len,
   ...
   CheckStreamRez rez;
   if ((containerType == ctMKV || containerType == ctMOV) &&
           containerDataType == TRACKTYPE_SRT ||
       containerType == ctSRT) {
      rez.codecInfo = srtCodecInfo;
      rez.streamDescr = "SRT text subtitles";
      if (containerType == ctSRT) rez.lang = detectUTF8Lang(buffer, len);
   }
   return rez;
}
