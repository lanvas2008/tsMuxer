...
class SimplePacketizerReader : public AbstractStreamReader {
 public:
  ...
  void setMPLSInfo(const std::vector<MPLSPlayItem>& mplsInfo) {
    ...
    if (m_mplsInfo.size() > 0) {
      m_curMplsIndex = 0;
      m_lastMplsTime =
          (int64_t)((m_mplsInfo[0].OUT_time - m_mplsInfo[0].IN_time) *
                    (1000000000.0 / 45000.0));
    } else
      m_curMplsIndex = -1;
  }
 ...
 protected:
  virtual unsigned
  getHeaderLen() = 0;  // return fixed frame header size at bytes
  virtual int decodeFrame(
      uint8_t* buff, uint8_t* end, int& skipBytes,
      int& skipBeforeBytes) = 0;  // decode frame parameters. bitrate, channels
                                  // for audio e.t.c
  virtual uint8_t* findFrame(uint8_t* buff,
                             uint8_t* end) = 0;  // find forawrd nearest frame
  ...
  // virtual bool isSubFrame() {return false;} // used for DTS-HD, Dolby-TRUEHD.
  // returns true for MLP data. Data can't be splitted in point where
  // subFrame=true
