...
void TSMuxer::intAddStream(const std::string& streamName,
   ...
   if (codecName == "V_MPEG4/ISO/AVC") {
      ...
      if (dtsReader->getDTSHDMode() && !dtsReader->getDownconvertToDTS()) {
         // if (dtsReader->getDTSHDMode() ==
         // DTSStreamReader::DTS_SUBTYPE_EXPRESS)
...
bool TSMuxer::doFlush(uint64_t newPCR, int64_t pcrGAP) {
   ...
   if (m_sectorSize > 0) {
      // round file to sector size. It's also can be applied to any file in a
      // m2ts mode
      int nullCnt = 0;
   ...
   if (m_owner->isAsyncMode()) {
      if (lastBlockSize > 0) {
         assert(m_sectorSize ==
                0);  // we should not be here in interleaved mode!
...
std::string TSMuxer::getNextName(const std::string curName) {
   ...
   if (m_bluRayMode) {
      ...
      m_curFileNum++;
      result = closeDirPath(filePath) +
               strPadLeft(int32ToStr(origNum), 5, '0') + "." + fileExt;
...
void TSMuxer::writePESPacket() {
   if (m_pesData.size() > 0) {
      ...
      for (int i = 0; i < m_priorityData.size(); ++i) {
         ...
         if (blockPtr > curPtr) {
            tsPackets += writeTSFrames(m_pesPID, curPtr, blockPtr - curPtr,
                                       false, payloadStart);
            payloadStart = false;
         }
         tsPackets += writeTSFrames(
             m_pesPID, blockPtr, m_priorityData[i].second, true, payloadStart);
         curPtr = blockPtr + m_priorityData[i].second;
      }
      tsPackets += writeTSFrames(m_pesPID, curPtr, dataEnd - curPtr, false,
                                 payloadStart);
...
bool TSMuxer::muxPacket(AVPacket& avPacket) {
   ...
   */
   ...
   if (avPacket.dts != m_streamInfo[tsIndex].m_dts ||
       avPacket.pts != m_streamInfo[tsIndex].m_pts ||
            ...
            if (m_lastPESDTS != -1 && m_lastPCR > m_lastPESDTS) {
               LTRACE(LT_ERROR, 2,
                      "VBV buffer overflow at position "
                          << (m_lastPCR - m_fixed_pcr_offset) / 90000.0
                          << " sec");
            }
         }
      }
