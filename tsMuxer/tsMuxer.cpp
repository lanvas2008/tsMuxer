...
uint8_t DefaultSitTableOne[] = {
    0x47, 0x40, 0x1f, 0x10, 0x00, 0x7f, 0xf0, 0x19, 0xff, 0xff, 0xc1, 0x00,
    0x00, 0xf0, 0x0a, 0x63, 0x08, 0xc1, 0x5a, 0xae, 0xff, 0xff, 0xff, 0xff,
    0xff, 0x00, 0x01, 0x80, 0x00, 0x34, 0x1e, 0xe7, 0x4e, 0xff, 0xff, 0xff,
...
TSMuxer::~TSMuxer() {
  delete[] m_outBuf;
  if (!m_isExternalFile) delete m_muxFile;
}

void TSMuxer::setVBVBufferLen(int value) {
  m_vbvLen = value * 90;
  m_fixed_pcr_offset = m_timeOffset - m_vbvLen;
  if (m_fixed_pcr_offset < 0) m_fixed_pcr_offset = 0;
  m_prevM2TSPCR = m_fixed_pcr_offset * 300;
}
...
void TSMuxer::intAddStream(const std::string& streamName,
  ...
  if (itr != params.end()) lang = itr->second;
  ...
  if (m_pcrOnVideo) {
    if (codecName[0] == 'V') m_pmt.pcr_pid = tsStreamIndex;
  } else
  ...
  if (codecName == "A_AC3")
    ...
    m_pesType[tsStreamIndex] =
        PES_PRIVATE_DATA1;  // PES_INT_LPCM_ID; //m_lpcmCnt++;
  ...
  if (codecName == "V_MPEG4/ISO/AVC") {
    ...
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(STREAM_TYPE_VIDEO_H264, tsStreamIndex, descrBuffer,
    ...
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(STREAM_TYPE_VIDEO_H265, tsStreamIndex, descrBuffer,
                      descriptorLen, codecReader, lang, isSecondary)));
  } else if (codecName == "V_MS/VFW/WVC1") {
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(STREAM_TYPE_VIDEO_VC1, tsStreamIndex, descrBuffer,
                      descriptorLen, codecReader, lang, isSecondary)));
  } else if (codecName == "V_MPEG-2") {
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(STREAM_TYPE_VIDEO_MPEG2, tsStreamIndex, descrBuffer,
                      descriptorLen, codecReader, lang, isSecondary)));
  } else if (codecName == "A_AAC")
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(STREAM_TYPE_AUDIO_AAC, tsStreamIndex, descrBuffer,
                      descriptorLen, codecReader, lang, isSecondary)));
  else if (codecName == "S_SUP")
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(STREAM_TYPE_SUB_PGS, tsStreamIndex, descrBuffer,
                      descriptorLen, codecReader, lang, isSecondary)));
  else if (codecName == "S_HDMV/PGS")
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
    ...
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(streamType, tsStreamIndex, descrBuffer, descriptorLen,
    ...
    if (mp3Reader->getLayer() >= 2)
      m_pmt.pidList.insert(std::make_pair(
          tsStreamIndex,
          PMTStreamInfo(STREAM_TYPE_AUDIO_MPEG2, tsStreamIndex, descrBuffer,
                        descriptorLen, codecReader, lang, isSecondary)));
    else
      m_pmt.pidList.insert(std::make_pair(
          tsStreamIndex,
          PMTStreamInfo(STREAM_TYPE_AUDIO_MPEG1, tsStreamIndex, descrBuffer,
                        descriptorLen, codecReader, lang, isSecondary)));
  } else if (codecName == "A_LPCM") {
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(STREAM_TYPE_AUDIO_LPCM, tsStreamIndex, descrBuffer,
                      descriptorLen, codecReader, lang, isSecondary)));
  } else if (codecName == "A_DTS") {
    ...
    if (dtsReader->getDTSHDMode() && !dtsReader->getDownconvertToDTS()) {
      ...
      else if (dtsReader->getDTSHDMode() ==
               DTSStreamReader::DTS_SUBTYPE_MASTER_AUDIO)
    ...
    m_pmt.pidList.insert(std::make_pair(
        tsStreamIndex,
        PMTStreamInfo(audioType, tsStreamIndex, descrBuffer, descriptorLen,
                      codecReader, lang, isSecondary)));
  } else
    THROW(ERR_UNKNOWN_CODEC,
          "Unsupported codec " << codecName << " for TS/M2TS muxing.");
...
bool TSMuxer::doFlush() {
  if (!m_lastPts.empty()) m_lastPts[m_lastPts.size() - 1] += m_additionCLPISize;

  uint64_t newPCR = 0;
  if (m_m2tsMode) {
    newPCR = (m_endStreamDTS - m_minDts) / INT_FREQ_TO_TS_FREQ + 0.5 +
             m_fixed_pcr_offset;
...
bool TSMuxer::doFlush(uint64_t newPCR, int64_t pcrGAP) {
  ...
  if (m_sectorSize > 0) {
    // round file to sector size. It's also can be applied to any file in a m2ts
    // mode
    int nullCnt = 0;
  ...
  if (m_m2tsMode) processM2TSPCR(newPCR, pcrGAP);

  unsigned lastBlockSize = m_outBufLen & (MuxerManager::PHYSICAL_SECTOR_SIZE -
                                          1);  // last 64K of data
  unsigned roundBufLen =
      m_outBufLen & ~(MuxerManager::PHYSICAL_SECTOR_SIZE - 1);
...
bool TSMuxer::close() {
  if (m_isExternalFile) return true;

  if (m_sectorSize) {
    assert(m_outBufLen == 0 && m_muxFile->size() % m_sectorSize == 0);
  }

  if (!m_muxFile->close()) return false;

  if (m_sectorSize) return true;

  if (m_outBufLen > 0) {
    if (!m_muxFile->open(
            m_outFileName.c_str(),
            AbstractOutputStream::ofWrite + AbstractOutputStream::ofAppend))
      return false;
    if (!writeOutFile(m_outBuf, m_outBufLen)) return false;
  }
...
void TSMuxer::processM2TSPCR(int64_t pcrVal, int64_t pcrGAP) {
  ...
  uint64_t pcrValDif =
      hiResPCR - m_prevM2TSPCR;  // m2ts pcr clock based on full 27Mhz counter
...
int gg = 4;
  ...
  if (m_m2tsDelayBlocks.size() > 0) {
    ...
    for (int i = 0; i < m_m2tsDelayBlocks.size(); i++) {
      ...
      if (m_owner->isAsyncMode())
        m_owner->asyncWriteBuffer(this, m_m2tsDelayBlocks[i].first,
                                  m_m2tsDelayBlocks[i].second, m_muxFile);
      else {
        m_owner->syncWriteBuffer(this, m_m2tsDelayBlocks[i].first,
                                 m_m2tsDelayBlocks[i].second, m_muxFile);
...
void TSMuxer::writeEmptyPacketWithPCRTest(int64_t pcrVal) {
  ...
  *initTS = TSPacket::TS_FRAME_SYNC_BYTE;
  ...
  tsPacket->counter =
      m_streamInfo[m_pmt.pcr_pid]
          .m_tsCnt;  // do not increment counter becouse data_exists == 0

  memset(curBuf + tsPacket->getHeaderSize(), 0xff,
...
void TSMuxer::writeEmptyPacketWithPCR(int64_t pcrVal) {
  ...
  *initTS = TSPacket::TS_FRAME_SYNC_BYTE;
  ...
  m_pcrBits += TS_FRAME_SIZE * 8;
  m_muxedPacketCnt[m_muxedPacketCnt.size() - 1]++;
  if (m_m2tsMode) processM2TSPCR(pcrVal, 0);
  writeOutBuffer();
}
...
void TSMuxer::buildPesHeader(int pesStreamID, AVPacket& avPacket, int pid) {
  ...
  AbstractStreamReader* ast =
      dynamic_cast<AbstractStreamReader*>(avPacket.codec);
  if (ast) ast->writePESExtension(pesPacket, avPacket);
  ...
  int additionDataSize = avPacket.codec->writeAdditionData(
      tmpBuffer + pesPacket->getHeaderLength(), tmpBuffer + sizeof(tmpBuffer),
      avPacket, &tmpPriorityData);
  ...
  for (int i = 0; i < tmpPriorityData.size(); ++i)
    m_priorityData.push_back(std::pair<int, int>(
        tmpPriorityData[i].first + pesPacket->getHeaderLength(),
        tmpPriorityData[i].second));
}
...
void TSMuxer::addData(int pesStreamID, int pid, AVPacket& avPacket) {
  ...
  if (oldLen > 100000000)
    THROW(ERR_COMMON,
          "Pes packet len too large ( >100Mb). Bad stream or invalid codec "
          "speciffed.");
  ...
  if (avPacket.flags & AVPacket::PRIORITY_DATA) {
    if (!m_priorityData.empty() &&
        m_priorityData.rbegin()->first + m_priorityData.rbegin()->second ==
            beforePesLen)
      m_priorityData.rbegin()->second += avPacket.size + pesHeaderLen;
    else
      m_priorityData.push_back(
          pair<int, int>(beforePesLen, avPacket.size + pesHeaderLen));
  }
}
...
bool TSMuxer::isSplitPoint(const AVPacket& avPacket) {
  ...
  if (m_splitSize > 0) {
    return m_muxedPacketCnt[m_muxedPacketCnt.size() - 1] * m_frameSize >
           m_splitSize;
...
std::string TSMuxer::getNextName(const std::string curName) {
  ...
  if (m_bluRayMode) {
    ...
    m_curFileNum++;
    result = closeDirPath(filePath) + strPadLeft(int32ToStr(origNum), 5, '0') +
             "." + fileExt;
  } else if (m_splitSize > 0 || m_splitDuration > 0) {
    m_curFileNum++;
    string fileExt = extractFileExt(m_origFileName);
    result =
        m_origFileName.substr(0, m_origFileName.size() - fileExt.size() - 1) +
        ".split." + int32ToStr(m_curFileNum) + "." + fileExt;
  }
...
void TSMuxer::gotoNextFile(uint64_t newPts) {
  // 2. CloseCurrentFile
  if (m_owner->isAsyncMode()) m_owner->waitForWriting();
  ...
  if (m_sublingMuxer) m_outFileName = getNextName(m_outFileName);  // inc by 2
  m_fileNames.push_back(m_outFileName);
  openDstFile();

  for (PIDListMap::iterator itr = m_pmt.pidList.begin();
       itr != m_pmt.pidList.end(); ++itr) {
    PMTStreamInfo& pmtInfo = itr->second;
    pmtInfo.m_codecReader->onSplitEvent();
    if (pmtInfo.m_index.size() == 0) continue;
    ...
    /*
            ...
            if (pmtInfo.m_index[spSize-2].size() > 0) {
                    uint64_t shortPts = (m_lastSplitPts / INT_FREQ_TO_TS_FREQ +
       0.5) + m_timeOffset; uint32_t lastPesSize =
       pmtInfo.m_index[spSize-2].rbegin()->second.m_frameLen;
                    pmtInfo.m_index[spSize-1].insert(make_pair(shortPts,
       PMTIndexData(firstDataFrame, lastPesSize)));
  ...
  m_lastPts[m_lastPts.size() - 1] =
      newPts;  //(curPts-FIXED_PTS_OFFSET)*INT_FREQ_TO_TS_FREQ;
  m_firstPts.push_back(
      newPts);  //((curPts-FIXED_PTS_OFFSET)*INT_FREQ_TO_TS_FREQ);
  m_lastPts.push_back(
      newPts);  //((curPts-FIXED_PTS_OFFSET)*INT_FREQ_TO_TS_FREQ);
...
void TSMuxer::writePESPacket() {
  if (m_pesData.size() > 0) {
    ...
    if (m_computeMuxStats && (pesPacket->flagsLo & 0x80) == 0x80) {
      ...
      if ((pesPacket->flagsLo & 0xc0) == 0xc0) curDts = pesPacket->getDts();

      // LTRACE(LT_INFO, 2, "pts=" << curPts << " dts=" << curDts);

      int idxSize = streamInfo.m_index.size();
      if (idxSize == 0) streamInfo.m_index.push_back(PMTIndex());
      MPEGStreamReader* vCodec =
          dynamic_cast<MPEGStreamReader*>(streamInfo.m_codecReader);
      // bool isH264 = dynamic_cast <H264StreamReader*>
      // (streamInfo.m_codecReader);
      bool SPSRequired = streamInfo.m_codecReader->needSPSForSplit();
      SimplePacketizerReader* aCodec =
          dynamic_cast<SimplePacketizerReader*>(streamInfo.m_codecReader);
      if (vCodec && m_pesIFrame) {
        ...
        if (m_pesSpsPps || !SPSRequired) {
          ...
          if (curIndex.empty() || curPts > curIndex.rbegin()->first) {
            curIndex.insert(std::make_pair(
                curPts, PMTIndexData(
                            m_muxedPacketCnt[m_muxedPacketCnt.size() - 1], 0)));
            updateIdx = true;
          }
        }
      ...
      // else if (vCodec) {
      //	PMTIndex& curIndex = *streamInfo.m_index.rbegin();
      //	curIndex.insert(std::make_pair(curPts,
      // PMTIndexData(m_muxedPacketCnt[m_muxedPacketCnt.size()-1],
      // m_pesDataSize)));
      //}
      else if (aCodec) {
        ...
        if (idxSize == 0 || curPts - curIndex.rbegin()->first >= 90000) {
          curIndex.insert(std::make_pair(
              curPts,
              PMTIndexData(m_muxedPacketCnt[m_muxedPacketCnt.size() - 1], 0)));
          updateIdx = true;
        }
      }
    }
    ...
    for (int i = 0; i < m_priorityData.size(); ++i) {
      ...
      if (blockPtr > curPtr) {
        tsPackets += writeTSFrames(m_pesPID, curPtr, blockPtr - curPtr, false,
                                   payloadStart);
        payloadStart = false;
      }
      tsPackets += writeTSFrames(m_pesPID, blockPtr, m_priorityData[i].second,
                                 true, payloadStart);
      curPtr = blockPtr + m_priorityData[i].second;
    }
    tsPackets +=
        writeTSFrames(m_pesPID, curPtr, dataEnd - curPtr, false, payloadStart);
...
void TSMuxer::writePCR(uint64_t newPCR) {
  ...
  if (m_cbrBitrate != -1 && m_minBitrate != -1 && m_lastPCR != -1) {
    ...
    if (expectedBits > 0) {
      ...
      if (tsFrames * m_frameSize * 8 < expectedBits) tsFrames++;
...
void TSMuxer::flushTSBuffer() {
  ...
  if (writeOutFile(m_outBuf, m_outBufLen) != m_outBufLen)
    THROW(ERR_FILE_COMMON,
          "Can't write last data block to file " << m_outFileName);
  delete[] m_outBuf;
  m_outBufLen = 0;
}

void TSMuxer::finishFileBlock(uint64_t newPts, uint64_t newPCR,
                              bool doChangeFile, bool recursive) {
  ...
  if (doChangeFile) gotoNextFile(newPts);
...
bool TSMuxer::muxPacket(AVPacket& avPacket) {
  ...
  */
  ...
  // if (m_lastProcessedDts != avPacket.dts || m_lastStreamIndex !=
  // avPacket.stream_index)
  //    LTRACE(LT_INFO, 2, "dts=" << avPacket.dts/1000000000.0 << " index=" <<
  //    avPacket.stream_index);
#ifdef _DEBUG
  ...
  if (avPacket.stream_index == m_mainStreamIndex) {
    MPEGStreamReader* mpegReader =
        dynamic_cast<MPEGStreamReader*>(avPacket.codec);
    if (mpegReader)
      m_additionCLPISize = INTERNAL_PTS_FREQ / mpegReader->getFPS();

    if (avPacket.duration > 0)
      *m_lastPts.rbegin() =
          FFMAX(*m_lastPts.rbegin(), avPacket.pts + avPacket.duration);
    else
      *m_lastPts.rbegin() = FFMAX(*m_lastPts.rbegin(), avPacket.pts);
  }
  if (m_firstPts[m_firstPts.size() - 1] ==
      -1 /*|| avPacket.pts < m_firstPts[m_firstPts.size()-1]*/) {
    if (m_firstPts[m_firstPts.size() - 1] == -1) {
      ...
      m_prevM2TSPCR = (m_fixed_pcr_offset - 100) *
                      300;  // add a little extra delta (100) to prevent 0
                            // bitrate for first several packets
    }
  ...
  if (m_minDts == -1) m_minDts = avPacket.dts;
  ...
  bool newPES = false;
  if (avPacket.dts != m_streamInfo[tsIndex].m_dts ||
      avPacket.pts != m_streamInfo[tsIndex].m_pts || tsIndex != m_lastTSIndex ||
  ...
  int64_t newPCR = (avPacket.dts - m_minDts) / INT_FREQ_TO_TS_FREQ + 0.5 +
                   m_fixed_pcr_offset;
  ...
  if (newPES && m_canSwithBlock &&
      (isSplitPoint(avPacket) || isSplitPoint(avPacket))) {
    ...
    if (m_interliaveBlockSize == 0 ||
        avPacket.stream_index == m_mainStreamIndex) {
      writePATPMT(newPCR);
      writePCR(newPCR);
    }
  }
...
int TSMuxer::writeTSFrames(int pid, uint8_t* buffer, int len, bool priorityData,
                           bool payloadStart) {
  int result = 0;
  ...
  while (curPos < end) {
    if (m_cbrBitrate != -1 && m_lastPCR != -1) {
      ...
      if (newPCR - m_lastPCR >= m_pcr_delta && m_lastPCR != -1) {
        ...
        if (m_lastPESDTS != -1 && m_lastPCR > m_lastPESDTS) {
          LTRACE(LT_ERROR, 2,
                 "VBV buffer overflow at position "
                     << (m_lastPCR - m_fixed_pcr_offset) / 90000.0 << " sec");
        }
      }
    }
    ...
    *initTS = TSPacket::TS_FRAME_SYNC_BYTE + TSPacket::DATA_EXIST_BIT_VAL;
    ...
    if (tmpBufferLen < payloadLen) {
      ...
      memset((uint8_t*)tsPacket + tsPacket->getHeaderSize(), 0xff,
...
void TSMuxer::buildNULL() {
  *((uint32_t*)m_nullBuffer) = TSPacket::TS_FRAME_SYNC_BYTE;
  ...
  memset(m_nullBuffer + TSPacket::TS_HEADER_SIZE, 0xff,
         TS_FRAME_SIZE - TSPacket::TS_HEADER_SIZE);
}

void TSMuxer::buildPAT() {
  *((uint32_t*)m_patBuffer) =
      TSPacket::TS_FRAME_SYNC_BYTE + TSPacket::DATA_EXIST_BIT_VAL;
  ...
  uint32_t size = m_pat.serialize(m_patBuffer + TSPacket::TS_HEADER_SIZE,
                                  TS_FRAME_SIZE - TSPacket::TS_HEADER_SIZE);
  memset(m_patBuffer + TSPacket::TS_HEADER_SIZE + size, 0xff,
         TS_FRAME_SIZE - TSPacket::TS_HEADER_SIZE - size);
}

void TSMuxer::buildPMT() {
  *((uint32_t*)m_pmtBuffer) =
      TSPacket::TS_FRAME_SYNC_BYTE + TSPacket::DATA_EXIST_BIT_VAL;
  TSPacket* tsPacket = (TSPacket*)m_pmtBuffer;
  tsPacket->setPID(DEFAULT_PMT_PID);
  tsPacket->dataExists = 1;
  tsPacket->payloadStart = 1;
  uint32_t size = m_pmt.serialize(m_pmtBuffer + TSPacket::TS_HEADER_SIZE, 3864,
                                  !m_bluRayMode);
...
bool TSMuxer::appendM2TSNullPacketToFile(uint64_t curFileSize, int counter,
                                         int* packetsWrited) {
  *packetsWrited = 0;
  ...
  if (readCnt != 192) return false;
  memcpy(tmpBuff + 4, m_nullBuffer, TS_FRAME_SIZE);
  TSPacket* tsPacket = (TSPacket*)(tmpBuff);
  for (uint64_t newFileSize = curFileSize; newFileSize % (2048 * 3) != 0;
       newFileSize += 192) {
    tsPacket->counter = counter++;
    if (file->write(tmpBuff, 192) != 192) return false;
    // m_muxedPacketCnt[m_muxedPacketCnt.size()-1]++;
    (*packetsWrited)++;
  }
  return true;
...
void TSMuxer::writeOutBuffer() {
  if (m_outBufLen >= m_writeBlockSize) {
    int toFileLen =
        m_writeBlockSize & ~(MuxerManager::PHYSICAL_SECTOR_SIZE - 1);
    if (m_owner->isAsyncMode()) {
  ...
  for (int i = 0; i < params.size(); i++) {
    ...
    if (paramPair[0] == "--pcr-on-video-pid")
      ...
      for (int i = paramPair[1].size() - 1; i >= 0; i--)
        if (!(paramPair[1][i] >= '0' && paramPair[1][i] <= '9' ||
              paramPair[1][i] == '.'))
      ...
      if (postfix == "GB")
        ...
        coeff = 1024;
      string prefix =
          paramPair[1].substr(0, paramPair[1].size() - postfix.size());
...
void TSMuxer::setMasterMode(AbstractMuxer* subMuxer,
                            bool flushInterleavedBlock) {
...
void TSMuxer::openDstFile() {
  ...
  int systemFlags = 0;
#ifdef WIN32
  if (m_owner->isAsyncMode()) systemFlags += FILE_FLAG_NO_BUFFERING;
#endif
...
void TSMuxer::alignPTS(TSMuxer* otherMuxer) {
  if (!m_lastPts.empty() && !otherMuxer->m_lastPts.empty()) {
    int64_t minPTS =
        FFMIN(*m_lastPts.rbegin(), *otherMuxer->m_lastPts.rbegin());
...
void TSMuxer::setFileName(const std::string& fileName,
...
std::string TSMuxer::getFileNameByIdx(int idx) {
  if (idx < m_fileNames.size()) return m_fileNames[idx];
  assert(1);
  return std::string();
}

void TSMuxer::setMuxFormat(const std::string& format) {
  m_m2tsMode = format == "M2TS" || format == "M2T" || format == "MTS" ||
               format == "SSIF";
  m_writeBlockSize =
      (m_m2tsMode ? DEFAULT_FILE_BLOCK_SIZE : TS188_ROUND_BLOCK_SIZE);
  m_outBuf = new uint8_t[m_writeBlockSize + 1024];
  m_frameSize = m_m2tsMode ? 192 : 188;
  if (m_m2tsMode) m_sectorSize = 1024 * 6;
}
