  ...
  try {
    if (argc == 2) {
      ...
      if (fileExt == "mpls" || fileExt == "mpl") {
        ...
        if (mplsParser.m_playItems.size() > 0) {
          ...
          if (switchToSsif) {
            string ssifName = streamDir + string("SSIF") + getDirSeparator() +
                              item.fileName + ssifExt;
        ...
        for (int i = 0; i < mplsParser.m_playItems.size(); i++) {
          ...
          LTRACE(
              LT_INFO, 2,
              "Duration: " << floatToTime((mplsParser.m_playItems[i].OUT_time -
          ...
          for (; markIndex < mplsParser.m_marks.size(); markIndex++) {
            ...
            uint64_t time = curMark.m_markTime -
                            mplsParser.m_playItems[i].IN_time + prevFileOffset;
            if (marksPerFile % 5 == 0) {
    ...
    if (argc != 3) {
      /*
                  ...
                  LTRACE(LT_INFO, 2, "For start muxing: " << "tsMuxeR <meta file
         name> <out file/dir name>"); LTRACE(LT_INFO, 2, "For detect stream
         params: " << "tsMuxeR <media file name>"); LTRACE(LT_INFO, 2, "For more
         information about meta file see readme.txt"); cout << endl;
      */
      showHelp();
      return -1;
    }
    ...
    if (muxMode) {
      ...
      if (dt != DT_NONE) {
        ...
        blurayHelper.writeBluRayFiles(insertBlankPL, firstMplsOffset, blankNum,
                                      stereoMode);
      ...
      if (dt != DT_NONE) {
        ...
        if (mainMuxer)
          blurayHelper.createCLPIFile(mainMuxer, mainMuxer->getFirstFileNum(),
                                      true);
        if (subMuxer) {
          blurayHelper.createCLPIFile(subMuxer, subMuxer->getFirstFileNum(),
                                      false);
