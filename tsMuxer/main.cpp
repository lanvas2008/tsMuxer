...
BufferedReaderManager readManager(2, DEFAULT_FILE_BLOCK_SIZE,
...
static const char EXCEPTION_ERR_MSG[] =
    ". It does not have to be! Please contact application support team for "
    "more information.";
...
DiskType checkBluRayMux(const char* metaFileName, int& autoChapterLen,
                        vector<double>& customChaptersList,
                        int& firstMplsOffset, int& firstM2tsOffset,
                        bool& insertBlankPL, int& blankNum, bool& stereoMode,
...
void detectStreamReader(const char* fileName, MPLSParser* mplsParser,
                        bool isSubMode) {
   DetectStreamRez streamInfo =
       METADemuxer::DetectStreamReader(readManager, fileName, mplsParser == 0);
   ...
   for (unsigned i = 0; i < streams.size(); i++) {
      if (streams[i].trackID != 0) {
         if (i > 0) LTRACE(LT_INFO, 2, "");
         LTRACE(LT_INFO, 2, "Track ID:    " << streams[i].trackID);
      }
      if (streams[i].codecInfo.codecID) {
         if (mplsParser) {
            MPLSStreamInfo streamInfo =
                mplsParser->getStreamByPID(streams[i].trackID);
            if (streamInfo.streamPID) {
               if (streamInfo.isSecondary) streams[i].isSecondary = true;
            } else {
               if (!(streams[i].codecInfo.codecID == CODEC_V_MPEG4_H264_DEP &&
                     mplsParser->isDependStreamExist))
         ...
         LTRACE(LT_INFO, 2,
                "Stream type: " << streams[i].codecInfo.displayName << postfix);
         if (streams[i].isSecondary) LTRACE(LT_INFO, 2, "Secondary: 1");
         if (streams[i].unused) LTRACE(LT_INFO, 2, "Unselected: 1");

         LTRACE(LT_INFO, 2,
                "Stream ID:   " << streams[i].codecInfo.programName);
         ...
         if (streams[i].codecInfo.codecID == CODEC_S_PGS) {
            ...
            if (pgTrackNum >= 0 && pgTrackNum < pgStreams3D.size()) {
               ...
               if (pgStreams3D[pgTrackNum].isSSPG) {
                  descr += "   (stereo, right=";
                  descr +=
                      (pgStreams3D[pgTrackNum].rightEye->type == 2 ? "dep-view "
                                                                   : "");
                  descr +=
                      int32ToStr(pgStreams3D[pgTrackNum].rightEye->streamPID);
                  descr += ", left=";
                  descr +=
                      (pgStreams3D[pgTrackNum].leftEye->type == 2 ? "dep-view "
                                                                  : "");
                  descr +=
                      int32ToStr(pgStreams3D[pgTrackNum].leftEye->streamPID);
                  descr += ")";
               }
            }
         }
         ...
         if (streams[i].multiSubStream)
            LTRACE(LT_INFO, 2,
                   "subTrack: " << (streams[i].codecInfo.codecID ==
                                            CODEC_V_MPEG4_H264_DEP
                                        ? 1
                                        : 2));
      } else
   ...
   if (streamInfo.fileDurationNano)
      LTRACE(LT_INFO, 2,
             "Duration: " << floatToTime(streamInfo.fileDurationNano / 1e9));
...
string getBlurayStreamDir(const string& mplsName) {
   ...
   int tmp =
       dirName.substr(0, dirName.size() - 1).find_last_of(getDirSeparator());
   if (tmp != string::npos) {
      dirName = dirName.substr(0, tmp + 1);
      if (strEndWith(dirName, string("BACKUP") + getDirSeparator())) {
         tmp = dirName.substr(0, dirName.size() - 1)
                   .find_last_of(getDirSeparator());
         if (tmp == string::npos) return "";
         dirName = dirName.substr(0, tmp + 1);
      }
...
void muxBlankPL(const string& appDir, BlurayHelper& blurayHelper,
                const PIDListMap& pidList, DiskType dt, int blankNum) {
   int videoWidth = 1920;
   ...
   for (PIDListMap::const_iterator itr = pidList.begin(); itr != pidList.end();
      ...
      const MPEGStreamReader* streamReader =
          dynamic_cast<const MPEGStreamReader*>(streamInfo.m_codecReader);
      if (streamReader) {
   ...
   uint8_t* pattern;
   int patternSize;
   bool isNtsc = videoWidth <= 854 && videoHeight <= 480 &&
                 (fabs(25 - fps) >= 0.5 && fabs(50 - fps) >= 0.5);
   bool isPal = videoWidth <= 1024 && videoHeight <= 576 &&
                (fabs(25 - fps) < 0.5 || fabs(50 - fps) < 0.5);
   if (isNtsc) {
   ...
   auto fname_time =
       std::chrono::duration_cast<std::chrono::microseconds>(
           ...
           .count();
   string tmpFileName =
       appDir + string("blank_") + std::to_string(fname_time) + string(".264");
   File file;
   ...
   videoParams["fps"] = "23.976";
   {
      ...
      muxerManager.parseMuxOpt(
          "MUXOPT --no-pcr-on-video-pid --vbr --avchd --vbv-len=500");
      ...
      blurayHelper.createMPLSFile(tsMuxer, 0, 0, vector<double>(), dt, blankNum,
                                  false);
...
int main(int argc, char** argv) {
   LTRACE(
       LT_INFO, 2,
       "tsMuxeR version " << APP_VERSION << ". github.com/justdan96/tsMuxer");
   ...
   try {
      if (argc == 2) {
         ...
         if (fileExt == "mpls" || fileExt == "mpl") {
            ...
            if (mplsParser.m_playItems.size() > 0) {
               ...
               if (fileExists(itemName)) {
                  if (mode3D && !mplsParser.m_mvcFiles.empty()) {
                     string subItemName =
                         streamDir + mplsParser.m_mvcFiles[0] + mediaExt;
                     if (fileExists(subItemName))
                        detectStreamReader(subItemName.c_str(), &mplsParser,
                                           true);
                     else
                        switchToSsif = true;
                  }
               ...
               if (switchToSsif) {
                  string ssifName = streamDir + string("SSIF") +
                                    getDirSeparator() + item.fileName + ssifExt;
                  if (fileExists(ssifName))
                     itemName =
                         ssifName;  // if m2ts file absent then swith to ssif
               }
            ...
            for (int i = 0; i < mplsParser.m_playItems.size(); i++) {
               ...
               string itemName;
               if (mode3D)
                  itemName = streamDir + string("SSIF") + getDirSeparator() +
                             item.fileName + ".ssif";
               else
               ...
               LTRACE(LT_INFO, 2,
                      "File #" << strPadLeft(int32ToStr(i), 5, '0')
                               << " name=" << buffer);
#else
               LTRACE(LT_INFO, 2,
                      "File #" << strPadLeft(int32ToStr(i), 5, '0')
               ...
               LTRACE(LT_INFO, 2,
                      "Duration: "
                          << floatToTime((mplsParser.m_playItems[i].OUT_time -
                                          mplsParser.m_playItems[i].IN_time) /
                                         (double)45000.0));
               ...
               if (!mplsParser.m_playItems.empty())
                  LTRACE(LT_INFO, 2,
                         "start-time: " << mplsParser.m_playItems[0].IN_time);
               int marksPerFile = 0;
               for (; markIndex < mplsParser.m_marks.size(); markIndex++) {
                  PlayListMark& curMark = mplsParser.m_marks[markIndex];
                  if (curMark.m_playItemID > i) break;
                  uint64_t time = curMark.m_markTime -
                                  mplsParser.m_playItems[i].IN_time +
                                  prevFileOffset;
                  if (marksPerFile % 5 == 0) {
                     if (marksPerFile > 0) LTRACE(LT_INFO, 2, "");
                     LTRACE2(LT_INFO, "Marks: ");
                  }
                  marksPerFile++;
               ...
               if (marksPerFile > 0) LTRACE(LT_INFO, 2, "");
               prevFileOffset += mplsParser.m_playItems[i].OUT_time -
                                 mplsParser.m_playItems[i].IN_time;
            }
         } else
      ...
      if (argc != 3) {
         /*
                     ...
                     LTRACE(LT_INFO, 2, "For start muxing: " << "tsMuxeR <meta
            file name> <out file/dir name>"); LTRACE(LT_INFO, 2, "For detect
            stream params: " << "tsMuxeR <media file name>"); LTRACE(LT_INFO, 2,
            "For more information about meta file see readme.txt"); cout <<
            endl;
         */
         showHelp();
         return -1;
      ...
      string isoDiskLabel;
      DiskType dt = checkBluRayMux(
          argv[1], autoChapterLen, customChapterList, firstMplsOffset,
          firstM2tsOffset, insertBlankPL, blankNum, stereoMode, isoDiskLabel);
      std::string fileExt2 = unquoteStr(fileExt);
      bool muxMode = fileExt2 == "M2TS" || fileExt2 == "TS" ||
                     fileExt2 == "SSIF" || fileExt2 == "ISO" || dt != DT_NONE;
      ...
      if (muxMode) {
         ...
         if (dt != DT_NONE) {
            if (!blurayHelper.open(dstFile, dt, muxerManager.totalSize(),
                                   muxerManager.getExtraISOBlocks()))
               throw runtime_error(string("Can't create output file ") +
                                   dstFile);
            blurayHelper.setVolumeLabel(isoDiskLabel);
            blurayHelper.createBluRayDirs();
            blurayHelper.writeBluRayFiles(insertBlankPL, firstMplsOffset,
                                          blankNum, stereoMode);
         ...
         if (dt != DT_NONE) {
            TSMuxer* mainMuxer =
                dynamic_cast<TSMuxer*>(muxerManager.getMainMuxer());
            TSMuxer* subMuxer =
                dynamic_cast<TSMuxer*>(muxerManager.getSubMuxer());

            if (mainMuxer)
               blurayHelper.createCLPIFile(mainMuxer,
                                           mainMuxer->getFirstFileNum(), true);
            if (subMuxer) {
               blurayHelper.createCLPIFile(subMuxer,
                                           subMuxer->getFirstFileNum(), false);
                  ...
                  for (int i = 0; i < mainMuxer->splitFileCnt(); ++i) {
                     ...
                     if (!file1.empty() && !file2.empty())
                        IsoWriter->createInterleavedFile(
                            file1, file2, blurayHelper.ssifFileName(ssifNum));
                  }
               }
            }
            ...
            // createMPLSFile(dstDir, mainMuxer->getPidList(),
            // *(mainMuxer->getFirstPts().begin()),
            // *(mainMuxer->getLastPts().rbegin()),
            //    autoChapterLen, customChapterList, dt, firstMplsOffset,
            //    firstM2tsOffset);

            // allign last PTS between main and sub muxers

            if (subMuxer) mainMuxer->alignPTS(subMuxer);

            blurayHelper.createMPLSFile(mainMuxer, subMuxer, autoChapterLen,
                                        customChapterList, dt, firstMplsOffset,
                                        muxerManager.isMvcBaseViewR());

            if (insertBlankPL && mainMuxer && !subMuxer) {
               LTRACE(LT_INFO, 2, "Adding blank play list");
               muxBlankPL(extractFileDir(argv[0]), blurayHelper,
                          mainMuxer->getPidList(), dt, blankNum);
            }
         }
         ...
         if (sMuxer.getTrackCnt() == 0) THROW(ERR_COMMON, "No tracks selected");
      ...
      auto seconds =
          std::chrono::duration_cast<std::chrono::seconds>(totalTime);
      auto minutes =
      ...
      if (argc == 2) LTRACE2(LT_ERROR, "Error: ");
#ifdef WIN32
      char buffer[1024 * 16];
      ...
      if (argc == 2) LTRACE2(LT_ERROR, "Error: ");
      LTRACE(LT_ERROR, 2,
             "Bitstream exception " << e.what() << EXCEPTION_ERR_MSG);
      return -3;
   } catch (...) {
      if (argc == 2) LTRACE2(LT_ERROR, "Error: ");
