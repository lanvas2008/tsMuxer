...
struct StreamInfo {
   ...
   StreamInfo(AbstractReader* dataReader, AbstractStreamReader* streamReader,
              const std::string& streamName, const std::string& fullStreamName,
              int pid, bool isSubStream = false) {
      m_streamName = streamName;
      m_fullStreamName = fullStreamName;
      m_dataReader = dataReader;
      m_readerID = dataReader->createReader(streamReader->getTmpBufferSize());
      if (!dataReader->openStream(m_readerID, m_streamName.c_str(), pid,
                                  &streamReader->getCodecInfo()))
...
// drpFragmented   - the stream is fragmented according to the resulting
// container
...
class ContainerToReaderWrapper : public AbstractReader {
  public:
   struct DemuxerData {
      ...
      bool m_firstRead;
      bool m_allFragmented;  // // container reader does not contain any
                             // sequence track reader(s)
   };

   struct ReaderInfo {
      ReaderInfo(DemuxerData& demuxerData, const uint32_t pid)
          : m_demuxerData(demuxerData), m_pid(pid) {}
      DemuxerData& m_demuxerData;
      const uint32_t m_pid;
   };

   ContainerToReaderWrapper(const METADemuxer& owner,
                            const BufferedReaderManager& readManager)
       : m_owner(owner), m_readManager(readManager) {
      BufferedReaderManager& brm =
          const_cast<BufferedReaderManager&>(readManager);
   ...
   virtual uint8_t* readBlock(uint32_t readerID, uint32_t& readCnt, int& rez,
                              bool* firstBlockVar = 0);
   ...
   virtual bool openStream(uint32_t readerID, const char* streamName,
                           int pid = 0, const CodecInfo* codecInfo = 0);
...
class METADemuxer : public AbstractDemuxer {
  public:
   // METADemuxer(const BufferedReaderManager& readManager, const char*
   // streamName);
   ...
   static DetectStreamRez DetectStreamReader(BufferedReaderManager& readManager,
   ...
   std::string mplsTrackToFullName(const std::string& mplsFileName,
                                   std::string& mplsNum);
   std::string mplsTrackToSSIFName(const std::string& mplsFileName,
                                   std::string& mplsNum);
  ...
  private:
   ...
   AbstractStreamReader* createCodec(
   ...
   int addPGSubStream(const std::string& codec,
                      const std::string& _codecStreamName,
                      const std::map<std::string, std::string>& addParams,
                      MPLSStreamInfo* subStream);
   static void addTrack(std::vector<CheckStreamRez>& rez,
                        CheckStreamRez trackRez, bool insToBegin);
   std::vector<MPLSPlayItem> mergePlayItems(
