      ...
      if (st->parsed_priv_data) {
        ...
        if (m_deliveredPacket.size) {
          if (filterItr != m_pidFilters.end()) {
            ...
            st->parsed_priv_data->extractData(&m_deliveredPacket,
                                              &m_tmpChunkBuffer[0], chunkSize);
  ...
  for (int i = 0; i < num_tracks; i++) {
    if (tracks[i]->type != TRACK_TYPE_CONTROL)
      trackList.insert(std::make_pair(
          i + 1,
          TrackInfo(tracks[i]->type == TRACK_TYPE_SUBTITLE ? TRACKTYPE_SRT : 0,
                    tracks[i]->language, 0)));
  }
}
    ...
    if (st->type == TRACK_TYPE_VIDEO) {
      ...
      if (!isom) {
        if (version == 1) {
          ...
          //    st->codec->codec_id =
          //    mov_get_lpcm_codec_id(st->codec->bits_per_coded_sample, flags);
        }
      }
      /*
