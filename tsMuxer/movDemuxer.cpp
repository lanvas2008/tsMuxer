...
static const char* const mov_mdhd_language_map[] = {
    ...
    "lav", NULL, "fo ", NULL, "rus", "chi", NULL, "iri", "alb", "ron", "ces",
    "slk", "slv", "yid", "sr ", "mac", "bul", "ukr", "bel", "uzb", "kaz", "aze",
    /*?*/
    "aze", "arm", "geo", "mol", "kir", "tgk", "tuk", "mon", NULL, "pus", "kur",
    "kas", "snd", "tib", "nep", "san", "mar", "ben", "asm", "guj", "pa ", "ori",
    "mal", "kan", "tam", "tel", NULL, "bur", "khm", "lao",
    /*                   roman? arabic? */
    ...
    NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
    NULL, NULL, NULL, NULL, "wel", "baq", "cat", "lat", "que", "grn", "aym",
    "tat", "uig", "dzo", "jav"};
...
int ff_mov_lang_to_iso639(unsigned code, char* to) {
   ...
   if (code >= sizeof(mov_mdhd_language_map) / sizeof(char*)) return 0;
   if (!mov_mdhd_language_map[code]) return 0;
...
class MovParsedAudioTrackData : public ParsedTrackPrivData {
  public:
   ...
   virtual void setPrivData(uint8_t* buff, int size) {
      ...
      if (size > 0) m_aacRaw.m_profile = (buff[0] >> 3) - 1;
      m_aacRaw.m_layer = 0;
      m_aacRaw.m_rdb = 0;
   }
   virtual void extractData(AVPacket* pkt, uint8_t* buff, int size) {
      ...
      while (buff < srcEnd - 4) {
         ...
         if (frameSize == 0) frameSize = m_sc->m_index[m_sc->m_indexCur++];
         if (isAAC) {
            m_aacRaw.m_channels = m_sc->channels;
      ...
      for (; left > 4; ++i) {
         ...
         if (m_sc->sample_size == 0) {
            if (m_sc->m_indexCur + i >= m_sc->m_index.size())
               THROW(ERR_MOV_PARSE, "Out of index for AAC track #"
                                        << m_sc->ffindex << " at position "
                                        << m_demuxer->getProcessedBytes());
            left -= m_sc->m_index[m_sc->m_indexCur + i];
         }
      }
      if (left < 0 || left > 4)
         THROW(ERR_MOV_PARSE, "Invalid AAC frame for track #"
                                  << m_sc->ffindex << " at position "
                                  << m_demuxer->getProcessedBytes());
      if (!isAAC) i = 0;
...
class MovParsedH264TrackData : public ParsedTrackPrivData {
  public:
   ...
   virtual void setPrivData(uint8_t* buff, int size) {
      spsPpsList.clear();
      if (size < 6) THROW(ERR_MOV_PARSE, "Invalid H.264/AVC extra data format");
...
class MovParsedH265TrackData : public MovParsedH264TrackData {
  public:
   MovParsedH265TrackData(MovDemuxer* demuxer, MOVStreamContext* sc)
       : MovParsedH264TrackData(demuxer, sc) {}
...
class MovParsedSRTTrackData : public ParsedTrackPrivData {
  public:
   ...
   virtual void extractData(AVPacket* pkt, uint8_t* buff, int size) {
      uint8_t* end = buff + size;
      std::string prefix;
      if (m_packetCnt == 0) prefix = "\xEF\xBB\xBF";  // UTF-8 header
   ...
   virtual int newBufferSize(uint8_t* buff, int size) {
      ...
      while (buff < end) {
         if (buff + 2 > end)
            THROW(ERR_MOV_PARSE, "MP4/MOV error: Invalid SRT frame at position "
                                     << m_demuxer->getProcessedBytes());
         uint32_t unitSize = (buff[0] << 8) + buff[1];
         buff += 2;
...
const MovDemuxer::MOVParseTableEntry MovDemuxer::mov_default_parse_table[] = {
    ...
    {MKTAG('c', 't', 't', 's'),
     &MovDemuxer::mov_read_ctts},  //  // composition time to sample
    ...
    {MKTAG('S', 'M', 'I', ' '),
     &MovDemuxer::mov_read_smi},  //  // Sorenson extension ???
    {MKTAG('a', 'l', 'a', 'c'),
     &MovDemuxer::mov_read_extradata},  //  // alac specific atom
    ...
    {MKTAG('s', 't', 's', 'd'),
     &MovDemuxer::mov_read_stsd},  // sample description
    ...
    {MKTAG('t', 'f', 'h', 'd'),
     &MovDemuxer::mov_read_tfhd},  // track fragment header
...
void MovDemuxer::openFile(const std::string& streamName) {
   ...
   m_isEOF = false;
   readHeaders();
   if (m_mdat_pos && m_processedBytes != m_mdat_pos) url_fseek(m_mdat_pos);
   buildIndex();
...
void MovDemuxer::buildIndex() {
   ...
   if (num_tracks == 1 &&
       ((MOVStreamContext*)tracks[0])->chunk_offsets.empty()) {
      chunks.push_back(make_pair(0, 0));
   } else {
      for (int i = 0; i < num_tracks; ++i) {
         MOVStreamContext* st = (MOVStreamContext*)tracks[i];
         for (int j = 0; j < st->chunk_offsets.size(); ++j) {
            if (st->chunk_offsets[j] < m_mdat_pos ||
                st->chunk_offsets[j] > m_mdat_pos + m_mdat_size)
               THROW(ERR_MOV_PARSE,
                     "Invalid chunk offset " << st->chunk_offsets[j]);
...
void MovDemuxer::readHeaders() {
   ...
   atom.size = LLONG_MAX;
   m_mdat_pos = 0;
   if (mov_read_default(atom) < 0) THROW(ERR_MOV_PARSE, "error reading header");
   if (!found_moov) THROW(ERR_MOV_PARSE, "moov atom not found");
}

int MovDemuxer::simpleDemuxBlock(DemuxedData& demuxedData,
   ...
   if (m_firstDemux) {
      ...
      if (m_mdat_pos == 0) {
         readHeaders();
         if (m_lastReadRez == BufferedReader::DATA_EOF) return m_lastReadRez;
         buildIndex();
   ...
   while (m_processedBytes - startPos < m_fileBlockSize &&
          m_curChunk < chunks.size()) {
      ...
      if (filterItr == m_pidFilters.end() &&
          acceptedPIDs.find(trackId + 1) == acceptedPIDs.end()) {
         ...
         if (st->parsed_priv_data) {
            ...
            if (readed == 0) break;
            m_deliveredPacket.size = st->parsed_priv_data->newBufferSize(
                &m_tmpChunkBuffer[0], chunkSize);
            if (m_deliveredPacket.size) {
               if (filterItr != m_pidFilters.end()) {
                  m_filterBuffer.resize(m_deliveredPacket.size);
                  m_deliveredPacket.data = m_filterBuffer.data();
                  st->parsed_priv_data->extractData(
                      &m_deliveredPacket, &m_tmpChunkBuffer[0], chunkSize);
                  int demuxed = filterItr->second->demuxPacket(
                      demuxedData, acceptedPIDs, m_deliveredPacket);
            ...
            if (filterItr != m_pidFilters.end()) {
               ...
               if (readed < chunkSize) m_filterBuffer.grow(readed - chunkSize);
               if (readed == 0) break;
               m_deliveredPacket.data = m_filterBuffer.data();
   ...
   for (int i = 0; i < num_tracks; i++) {
      if (tracks[i]->type != TRACK_TYPE_CONTROL)
         trackList.insert(std::make_pair(
             i + 1,
             TrackInfo(
                 tracks[i]->type == TRACK_TYPE_SUBTITLE ? TRACKTYPE_SRT : 0,
                 tracks[i]->language, 0)));
   }
}
...
int MovDemuxer::mov_read_default(MOVAtom atom) {
   ...
   if (atom.size < 0) atom.size = LLONG_MAX;
...
int MovDemuxer::mov_read_trun(MOVAtom atom) {
   ...
   if (!frag->track_id || frag->track_id > num_tracks) return -1;
...
int MovDemuxer::mov_read_tfhd(MOVAtom atom) {
   ...
   track_id = get_be32();
   if (!track_id || track_id > num_tracks) return -1;
   ...
   if (!trex) THROW(ERR_COMMON, "could not find corresponding trex");
...
int MovDemuxer::mov_read_stts(MOVAtom atom) {
   ...
   for (int i = 0; i < entries; i++) {
      ...
      if (i == 0) {
         // int64_t tmp = (int64_t) st->time_scale * (int64_t)1000000ll /
         // st->stts_data[i].duration; st->fps = tmp / 1000000.0;
...
int MovDemuxer::mov_read_stsz(MOVAtom atom) {
   ...
   if (st->sample_size) return 0;
   if (entries >= UINT_MAX / sizeof(int)) return -1;
...
int MovDemuxer::mov_read_extradata(MOVAtom atom) {
   ...
   if (newSize > INT_MAX || (uint64_t)atom.size > INT_MAX) return -1;
...
int MovDemuxer::mov_read_moov(MOVAtom atom) {
   if (mov_read_default(atom) < 0) return -1;
   found_moov = 1;
   return 0;
}
...
int MovDemuxer::mov_read_mdhd(MOVAtom atom) {
   if (num_tracks == -1) return -1;
   MOVStreamContext* st = (MOVStreamContext*)tracks[num_tracks - 1];
   int version = get_byte();
   if (version > 1) return -1;  // unsupported
   ...
   fileDuration =
       FFMAX(fileDuration, duration / double(st->time_scale) * 1000000000ll);
...
int MovDemuxer::mov_read_stsd(MOVAtom atom) {
   ...
   for (int pseudo_stream_id = 0; pseudo_stream_id < entries;
        pseudo_stream_id++) {
      ...
      if (st->type == TRACK_TYPE_VIDEO) {
         ...
         if (!isom) {
            if (version == 1) {
               ...
               st->bits_per_coded_sample =
                   get_be32();  // bits per channel if sound is uncompressed
               int flags = get_be32();  // lcpm format specific flag
               st->bytes_per_frame =
                   get_be32();  // bytes per audio packet if constant
               st->samples_per_frame =
                   get_be32();  // lpcm frames per audio packet if constant
                                // if (format == MKTAG('l','p','c','m'))
                                //    st->codec->codec_id =
               //    mov_get_lpcm_codec_id(st->codec->bits_per_coded_sample,
               //    flags);
            }
         }
         /*
   ...
   sc->sample_size = (bits_per_sample >> 3) * st->codec->channels;
      ...
      if (a.size > 8) {
         if (mov_read_default(a) < 0) return -1;
      } else if (a.size > 0)
         skip_bytes(a.size);
   }
   return 0;
...
int MovDemuxer::mov_read_stco(MOVAtom atom) {
   ...
   if (entries >= UINT_MAX / sizeof(int64_t)) return -1;
...
int MovDemuxer::mov_read_glbl(MOVAtom atom) {
   if ((uint64_t)atom.size > (1 << 30)) return -1;
   Track* st = tracks[num_tracks - 1];
   delete[] st->codec_priv;
...
int MovDemuxer::mov_read_esds(MOVAtom atom) {
   ...
   get_be16();                            // ID
   if (tag == MP4ESDescrTag) get_byte();  // priority
   ...
   if (tag == MP4DecConfigDescrTag) {
      ...
      if (tag == MP4DecSpecificDescrTag) {
         ...
         if (st->parsed_priv_data) {
            ...
            st->parsed_priv_data->setPrivData(st->codec_priv,
                                              st->codec_priv_size);
