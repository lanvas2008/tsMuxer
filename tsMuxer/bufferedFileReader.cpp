...
bool FileReaderData::openStream() {
  ...
  if (!rez) {
    ...
    FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dw,
                   MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&msgBuf,
                   sizeof(msgBuf), NULL);
...
BufferedFileReader::BufferedFileReader(uint32_t blockSize, uint32_t allocSize,
                                       uint32_t prereadThreshold)
    : BufferedReader(blockSize, allocSize, prereadThreshold) {}

bool BufferedFileReader::openStream(uint32_t readerID, const char* streamName,
                                    int pid, const CodecInfo* codecInfo) {
...
bool BufferedFileReader::gotoByte(uint32_t readerID, uint64_t seekDist) {
  ...
  if (data) {
    data->m_blockSize =
        m_blockSize - (uint32_t)(seekDist % (uint64_t)m_blockSize);
    uint64_t seekRez =
        data->m_file.seek(seekDist + data->m_fileHeaderSize, File::smBegin);
