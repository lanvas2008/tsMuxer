...
enum CompositionMode { CM_Start, CM_Update, CM_Finish };
...
class TextToPGSConverter  //: public TextSubtitlesRenderWin32
{
  public:
   ...
   uint8_t* doConvert(std::wstring& text, const TextAnimation& animation,
                      double inTime, double outTime, uint32_t& dstBufSize);
  ...
  public:
   ...
   long composePresentationSegment(uint8_t* buff, CompositionMode mode,
                                   int64_t pts, int64_t dts, int top,
                                   bool needPgHeader, bool forced);
   long composeWindowDefinition(uint8_t* buff, int64_t pts, int64_t dts,
                                int top, int height, bool needPgHeader = true);
   long composeWindow(uint8_t* buff, int top, int height);
   long composePaletteDefinition(const Palette& palette, uint8_t* buff,
                                 int64_t pts, int64_t dts,
                                 bool needPgHeader = true);
   long composeObjectDefinition(uint8_t* buff, int64_t pts, int64_t dts,
                                int firstLine, int lastLine, bool needPgHeader);
   long composeVideoDescriptor(uint8_t* buff);
   long composeCompositionDescriptor(uint8_t* buff, uint16_t number,
                                     uint8_t state);
   long composeEnd(uint8_t* buff, int64_t pts, int64_t dts,
                   bool needPgHeader = true);
   ...
   int getRepeatCnt(const uint32_t* pos, const uint32_t* end,
                    uint32_t colorMask);
