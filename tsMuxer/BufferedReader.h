...
struct ReaderData {
 ...
 virtual void deleteNextBlocks()
 {
   ...
   */
   ...
   virtual void init() {
      // deleteNextBlocks();
      if (m_nextBlock[0] == 0) m_nextBlock[0] = new uint8_t[m_allocSize];
      if (m_nextBlock[1] == 0) m_nextBlock[1] = new uint8_t[m_allocSize];
   }
...
class BufferedReader : public AbstractReader, TerminatableThread {
  public:
   ...
   BufferedReader(uint32_t blockSize, uint32_t allocSize = 0,
   ...
   virtual void notify(uint32_t readerID,
                       uint32_t dataReaded);  // reader must call notificate
                                              // when part of data handled
