...
const int AACCodec::object_type[9] = {1, // 'AAC_MAIN' -> AAC Main
                                                                   2, //
'AAC_LC' -> AAC Low complexity 3, // 'AAC_SSR' -> AAC SSR 4, // 'AAC_LTP' -> AAC
Long term prediction 5, // HE_AAC 17, // ER_LC 19, // ER_LTP 23, // LD 27
//DRM_ER_LC
                                  };
*/

const int AACCodec::aac_sample_rates[16] = {96000, 88200, 64000, 48000, 44100,
...
uint8_t* AACCodec::findAacFrame(uint8_t* buffer, uint8_t* end) {
   ...
   while (curBuf < end) {
      if (*curBuf < 0xf0)
         curBuf += 2;
      else if (*curBuf == 0xff && curBuf < end - 1 &&
               (curBuf[1] & 0xf6) == 0xf0)
         return curBuf;
      else if ((*curBuf & 0xf6) == 0xf0 && curBuf > buffer &&
               curBuf[-1] == 0xff)
         return curBuf - 1;
      else
         curBuf++;
...
bool AACCodec::decodeFrame(uint8_t* buffer, uint8_t* end) {
   ...
   try {
      ...
      if (!aac_sample_rates[m_sample_rates_index]) return false;
      bits.skipBit();                     /* private_bit */
      m_channels_index = bits.getBits(3); /* channel_configuration */
      if (!aac_channels[m_channels_index]) return false;
...
void AACCodec::readConfig(uint8_t* buff, int size) {
   ...
   if (object_type == 31) object_type = 32 + reader.getBits(6);
   m_profile = (object_type & 0x3) - 1;
   m_sample_rates_index = reader.getBits(4);
   m_sample_rate = m_sample_rates_index == 0x0f
                       ? reader.getBits(24)
                       : aac_sample_rates[m_sample_rates_index];
