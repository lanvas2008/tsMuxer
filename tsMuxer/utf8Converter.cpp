...
std::wstring FromUtf8(const std::string& utf8string) {
   ...
   if (sizeof(wchar_t) == 2) {
      ...
      const UTF8* sourcestart =
          reinterpret_cast<const UTF8*>(utf8string.c_str());
      ...
      *targetstart = 0;
      ...
      if (res != conversionOK) {
         ...
         // throw std::exception("La falla!");
         THROW(ERR_COMMON,
               "Can't convert UTF-8 to UTF-32. Invalid source text");
      }
      *targetstart = 0;
      std::wstring resultstring(widestringnative);
      delete[] widestringnative;
...
std::wstring toWideString(uint8_t* start, size_t widesize,
   ...
   if (widesize == 0) return L"";
      ...
      wchar_t* widestringnative = new wchar_t[widesize + 1];
      int wlen = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, (LPCSTR)start,
                                     widesize, widestringnative, widesize);
   ...
   else if (srcFormat == sfDefault)
   {
           if (cd == 0)
           {
                   ...
                   LTRACE(LT_INFO, 2, "Default text encoding: " <<
   sourceEncoding.c_str()); if (sizeof(wchar_t) == 4) cd = iconv_open("UTF-32",
   sourceEncoding.c_str()); else cd = iconv_open("UTF-16",
   sourceEncoding.c_str()); if (cd == (iconv_t)-1) THROW(ERR_COMMON, "Can't
   initialize iconv library for source encoding " << sourceEncoding);
           }
           ...
           if (wlen == -1)
                   THROW(ERR_COMMON, "Can't convert source string '" << start <<
   "'to unicode"); widestringnative[widesize] = 0;
   ...
   */
      ...
      switch (srcFormat) {
         case sfUnknown:
         case sfUTF8:
            res = ConvertUTF8toUTF16(&sourcestart, sourceend, &targetstart,
                                     targetend, strictConversion);
            *targetstart = 0;
            break;
         case sfUTF16be:
            for (uint16_t* ptr = (uint16_t*)start; ptr < (uint16_t*)sourceend;
                 ++ptr)
               *ptr = my_ntohs(*ptr);
         case sfUTF16le:
            ...
            break;
         case sfUTF32be:
            for (uint32_t* ptr = (uint32_t*)start; ptr < (uint32_t*)sourceend;
                 ++ptr)
               *ptr = my_ntohl(*ptr);
         case sfUTF32le:
            res = ConvertUTF32toUTF16((const UTF32**)&sourcestart,
                                      (const UTF32*)sourceend, &targetstart,
                                      targetend, strictConversion);
            *targetstart = 0;
            break;
      }
      ...
      switch (srcFormat) {
         case sfUnknown:
         case sfUTF8:
            res = ConvertUTF8toUTF32(&sourcestart, sourceend, &targetstart,
                                     targetend, strictConversion);
            *targetstart = 0;
            break;
         case sfUTF16be:
            for (uint16_t* ptr = (uint16_t*)start; ptr < (uint16_t*)sourceend;
               ...
               *ptr = my_ntohs(*ptr);
         case sfUTF16le:
            res = ConvertUTF16toUTF32((const UTF16**)&sourcestart,
                                      (const UTF16*)sourceend, &targetstart,
                                      targetend, strictConversion);
            *targetstart = 0;
            break;
         case sfUTF32be:
