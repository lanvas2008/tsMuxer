...
void TextToPGSConverter::enlargeCrop(int width, int height, int* newWidth,
                                     int* newHeight) {
   *newWidth = width;
   *newHeight = height;
   if (width > 1280) {
   ...
   // else
   {
      ...
      if (itr == m_paletteYUV.end()) {
         if (m_paletteYUV.size() < 255) {
            ...
            THROW(ERR_COMMON,
                  "Can't transform image to YUV: too many colors are used.");
         }
...
int TextToPGSConverter::getRepeatCnt(const uint32_t* pos, const uint32_t* end,
                                     uint32_t colorMask) {
   int rez = 1;
   if (*pos == 0) {
...
YUVQuad TextToPGSConverter::RGBAToYUVA(uint32_t data) {
   ...
   YUVQuad rez;
   rez.Y = (0.257 * rgba->rgbRed) + (0.504 * rgba->rgbGreen) +
           (0.098 * rgba->rgbBlue) + 16.5;
   rez.Cr = (0.439 * rgba->rgbRed) - (0.368 * rgba->rgbGreen) -
            (0.071 * rgba->rgbBlue) + 128.5;
   rez.Cb = -(0.148 * rgba->rgbRed) - (0.291 * rgba->rgbGreen) +
            (0.439 * rgba->rgbBlue) + 128.5;
...
RGBQUAD TextToPGSConverter::YUVAToRGBA(const YUVQuad& yuv) {
   ...
   tmp = 1.164 * (yuv.Y - 16) - 0.813 * (yuv.Cr - 128) -
         0.391 * (yuv.Cb - 128) + 0.5;
...
void TextToPGSConverter::reduceColors(uint8_t mask) {
   ...
   uint32_t* dst =
       (uint32_t*)(m_textRender ? m_textRender->m_pData : m_imageBuffer);
         ...
         if (m_minLine != INT_MAX) {
            ...
            *curPtr++ = 0;  // end of line signal
            if (curPtr >= dstLineEnd) {
               THROW(ERR_COMMON,
                     "Not enought RLE buffer for encoding picture (RLE line "
                     "length "
                     "> width + 16)");
            }
         }
         if (!isEmptyLine) trimPos = curPtr;
      }
      if (m_minLine == INT_MAX) m_minLine = m_maxLine = 0;
      m_rleLen = trimPos - m_renderedData;
      // sort by colors indexes
      m_paletteByColor.clear();
      for (std::map<YUVQuad, uint8_t>::const_iterator itr =
               m_paletteYUV.begin();
           itr != m_paletteYUV.end(); ++itr)
...
TextToPGSConverter::Palette TextToPGSConverter::buildPalette(float opacity) {
   if (opacity == 1.0) return m_paletteByColor;
   Palette result = m_paletteByColor;
   for (Palette::iterator itr = result.begin(); itr != result.end(); ++itr)
      itr->second.alpha =
          FFMIN(255, uint8_t(float(itr->second.alpha) * opacity + 0.5));
   return result;
}
...
uint8_t* TextToPGSConverter::doConvert(std::wstring& text,
                                       const TextAnimation& animation,
   ...
   if (m_rleLen == 0) return 0;  // empty text

   int objectWindowHeight = FFMAX(0, renderedHeight());
   int objectWindowTop =
       FFMAX(0, m_textRender->m_height - objectWindowHeight - m_bottomOffset);

   // static const int64_t PRESENTATION_DTS_DELTA = 5832; // time to render half
   // of frame (so, it is max subtitle size) static const int64_t
   // PRESENTATION_DTS_DELTA = 6900; // time to render half of frame (so, it is
   // max subtitle size)
   ...
   const int64_t PLANEINITIALIZATIONTIME =
       90000.0 * (m_videoWidth * m_videoHeight) / PIXEL_COMPOSITION_RATE +
       0.999;
   ...
   curPos += composePresentationSegment(curPos, CM_Start, inTime,
                                        inTime - PRESENTATION_DTS_DELTA,
                                        objectWindowTop, true, forced);
   curPos += composeWindowDefinition(curPos, inTime - windowsTransferTime,
                                     inTime - PRESENTATION_DTS_DELTA,
                                     objectWindowTop, objectWindowHeight);
   curPos += composePaletteDefinition(buildPalette(toCurve(opacity)), curPos,
                                      inTime - PRESENTATION_DTS_DELTA,
                                      inTime - PRESENTATION_DTS_DELTA);
   int64_t odfPTS = inTime - PRESENTATION_DTS_DELTA + compositionDecodeTime;
   curPos +=
       composeObjectDefinition(curPos, odfPTS, inTime - PRESENTATION_DTS_DELTA,
                               m_minLine, m_maxLine, true);
   ...
   while (updateTime <= lastAnimateTime + FLOAT_EPS &&
          opacity <= 1.0 + FLOAT_EPS) {
      palette_update_flag = 1;
      m_paletteVersion++;
      curPos += composePresentationSegment(curPos, CM_Update, updateTime,
   ...
   updateTime =
       alignToGrid(FFMAX(updateTime, outTime - (fadeOutFrames - 1) * fpsPts));
   ...
   palette_update_flag = 0;
   curPos += composePresentationSegment(curPos, CM_Finish, outTime,
                                        outTime - windowsTransferTime - 90,
                                        objectWindowTop, true, false);
   curPos += composeWindowDefinition(curPos, outTime - windowsTransferTime,
                                     outTime - windowsTransferTime - 90,
                                     objectWindowTop, objectWindowHeight);
...
long TextToPGSConverter::composePresentationSegment(uint8_t* buff,
                                                    CompositionMode mode,
                                                    int64_t pts, int64_t dts,
                                                    int top, bool needPGHeader,
                                                    bool forced) {
   uint8_t* curPos = buff;
   if (needPGHeader) curPos += writePGHeader(curPos, pts, dts);
   *curPos++ = PCS_DEF_SEGMENT;
   curPos += 2;  // skip length field
   uint8_t* startPos = curPos;

   curPos += composeVideoDescriptor(curPos);
   curPos += composeCompositionDescriptor(
       curPos, m_composition_number++,
       mode == CM_Start ? EPOTH_START : EPOTH_NORMAL);
   *curPos++ = palette_update_flag
               << 7;         // palette_update_flag = 0 and 7 reserved bits
   ...
   *curPos++ = mode != CM_Finish ? 1 : 0;  // number_of_composition_objects
   ...
   if (mode != CM_Finish) {
      ...
      *curPos++ = 0;  // windowID ref
      if (forced)
         *curPos++ =
             0x40;  // object_cropped_flag = false, forced_on_flag = true
      else
...
long TextToPGSConverter::composeWindowDefinition(uint8_t* buff, int64_t pts,
                                                 int64_t dts, int top,
                                                 int height,
                                                 bool needPgHeader) {
   uint8_t* curPos = buff;
...
long TextToPGSConverter::composeCompositionDescriptor(uint8_t* buff,
                                                      uint16_t number,
                                                      uint8_t state) {
   uint8_t* curPos = buff;
...
long TextToPGSConverter::composePaletteDefinition(const Palette& palette,
                                                  uint8_t* buff, int64_t pts,
                                                  int64_t dts,
                                                  bool needPgHeader) {
   uint8_t* curPos = buff;
   if (needPgHeader) curPos += writePGHeader(curPos, pts, dts);
   ...
   *curPos++ = m_paletteVersion;  // palette version number
   for (Palette::const_iterator itr = palette.begin(); itr != palette.end();
        ++itr) {
      *curPos++ = itr->first;
      *curPos++ = itr->second.Y;
...
long TextToPGSConverter::composeObjectDefinition(uint8_t* buff, int64_t pts,
                                                 int64_t dts, int firstLine,
                                                 int lastLine,
                                                 bool needPgHeader) {
   ...
   do {
      ...
      *curPos++ = 0;  // 0xc0; // sequence descriptor: first=true, last=true
      ...
      if (blocks == 0) MAX_PG_PACKET -= 7;
      int size = FFMIN(m_rleLen - srcProcessed, MAX_PG_PACKET);
      memcpy(curPos, srcData + srcProcessed, size);
      srcProcessed += size;
      curPos += size;

      AV_WB16(fragmentStart - 2,
              curPos - fragmentStart);  // correct length field
      blocks++;
...
long TextToPGSConverter::composeEnd(uint8_t* buff, int64_t pts, int64_t dts,
                                    bool needPgHeader) {
   uint8_t* curPos = buff;
   if (needPgHeader) curPos += writePGHeader(curPos, pts, dts);
...
long TextToPGSConverter::writePGHeader(uint8_t* buff, int64_t pts,
                                       int64_t dts) {
   if (dts > pts) dts = pts;
   *buff++ = 'P';
   *buff++ = 'G';
