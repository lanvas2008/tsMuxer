...
RGBQUAD TextToPGSConverter::YUVAToRGBA(const YUVQuad& yuv) {
  ...
  tmp = 1.164 * (yuv.Y - 16) - 0.813 * (yuv.Cr - 128) - 0.391 * (yuv.Cb - 128) +
        0.5;
...
bool TextToPGSConverter::rlePack(uint32_t colorMask) {
  try {
      ...
      if (m_minLine != INT_MAX) {
        ...
        *curPtr++ = 0;  // end of line signal
        if (curPtr >= dstLineEnd) {
          THROW(ERR_COMMON,
                "Not enought RLE buffer for encoding picture (RLE line length "
                "> width + 16)");
        }
      }
    ...
    for (std::map<YUVQuad, uint8_t>::const_iterator itr = m_paletteYUV.begin();
...
uint8_t* TextToPGSConverter::doConvert(std::wstring& text,
  ...
  const int64_t PLANEINITIALIZATIONTIME =
      90000.0 * (m_videoWidth * m_videoHeight) / PIXEL_COMPOSITION_RATE + 0.999;
