...
class AC3StreamReader : public SimplePacketizerReader, public AC3Codec {
 public:
  ...
  virtual int getAltFreq() {
    if (m_downconvertToAC3)
      ...
      return mh.subType == MLPHeaderInfo::stUnknown ? AC3Codec::m_sample_rate
  ...
  virtual bool isIFrame(AVPacket* packet) override {
    return isPriorityData(packet);
  }
 ...
 protected:
  ...
  virtual const CodecInfo& getCodecInfo() { return AC3Codec::getCodecInfo(); }
  virtual const std::string getStreamInfo() {
    return AC3Codec::getStreamInfo();
  }
  virtual void writePESExtension(PESPacket* pesPacket,
