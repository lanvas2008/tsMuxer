...
// 0.0, 23.97602397602397, 24.0, 25.0, 29.97002997002997,
// 30, 50.0, 59.94005994005994, 60.0
//};
...
bool MPEGRawDataHeader::addRawData(uint8_t* buffer, int len,
                                   bool headerIncluded, bool isHeader) {
...
uint8_t* MPEGSequenceHeader::deserialize(uint8_t* buf, int buf_size) {
   ...
   if (aspect_ratio_info == 0) return 0;
   frame_rate_index = bitReader.getBits(4);
   if (frame_rate_index == 0 || frame_rate_index > 13) return 0;
...
uint8_t* MPEGSequenceHeader::deserializeExtension(BitStreamReader& bitReader) {
 ...
 bitrate:%d\n", s->avctx->profile, s->avctx->level, s->avctx->rc_buffer_size,
 s->bit_rate);
   */
...
uint8_t* MPEGSequenceHeader::deserializeMatrixExtension(
    BitStreamReader& bitReader) {
...
uint8_t* MPEGSequenceHeader::deserializeDisplayExtension(
    BitStreamReader& bitReader) {
   // MpegEncContext *s= &s1->mpeg_enc_ctx;
   ...
   if (color_description) {
      ...
      transfer_characteristics =
          bitReader.getBits(8);                   /* transfer_characteristics */
...
double MPEGSequenceHeader::getFrameRate() {
   if (frame_rate_index > 0 &&
       frame_rate_index < sizeof(frame_rates) / sizeof(double))
...
void MPEGSequenceHeader::setFrameRate(uint8_t* buff, double fps) {
   ...
   THROW(ERR_MPEG2_ERR_FPS,
         "Can't set fps to value "
             << fps << ". Only standart fps values allowed for mpeg2 streams.");
}
...
uint32_t MPEGGOPHeader::serialize(uint8_t* buffer) {
   ...
   // init_bitWriter.putBits(buffer, 8*8); // 8*8 maximum gop header buffer size
   // at bits
...
void MPEGPictureHeader::buildCodingExtension() {
   BitStreamWriter bitWriter;
   // init_bitWriter.putBits(m_data_buffer + m_data_buffer_len, (m_max_data_len
   // - m_data_buffer_len)*8);
...
uint8_t* MPEGPictureHeader::deserialize(uint8_t* buf, int buf_size) {
   ...
   if (pict_type == 0 || pict_type > 3) return 0;
...
// int gg = 0;
uint8_t* MPEGPictureHeader::deserializeCodingExtension(
    BitStreamReader& bitReader) {
 ...
 ff_alternate_vertical_scan); ff_init_scantable(s->dsp.idct_permutation,
 &s->intra_scantable  , ff_alternate_vertical_scan); }else{
   ff_init_scantable(s->dsp.idct_permutation, &s->inter_scantable  ,
 ff_zigzag_direct); ff_init_scantable(s->dsp.idct_permutation,
 &s->intra_scantable  , ff_zigzag_direct);
 }
 ...
 uint8_t* MPEGPictureHeader::deserializeDisplayExtension(BitStreamReader&
 bitReader)
 {
...
void MPEGPictureHeader::setVbvDelay(uint16_t val) {
   if (m_headerIncludedToBuff) {
      uint8_t* vbv_delay_ptr =
          &m_data_buffer[5];  // skip sequence header and 1-st byte of ref
...
bool MPEGPictureHeader::addRawData(uint8_t* buffer, int len,
                                   bool headerIncluded, bool isHeader)

{
   bool rez =
       MPEGRawDataHeader::addRawData(buffer, len, headerIncluded, isHeader);
   if (!isHeader) m_picture_data_len += len;
   return rez;
}
