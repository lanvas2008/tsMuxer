...
int MPEG2StreamReader::getTSDescriptor(uint8_t* dstBuff) {
   try {
      for (uint8_t* nal = MPEGHeader::findNextMarker(m_buffer, m_bufEnd);
           nal <= m_bufEnd - 32;
...
CheckStreamRez MPEG2StreamReader::checkStream(uint8_t* buffer, int len) {
   ...
   for (uint8_t* nal = MPEGHeader::findNextMarker(buffer, end); nal <= end - 32;
      ...
      try {
         if (unitType >= SLICE_MIN_START_SHORT_CODE &&
             unitType <= SLICE_MAX_START_SHORT_CODE)
            sliceFound = true;
         else
            switch (unitType) {
   ...
   if (spsFound && pictureFound /*&& gopFound*/ &&
       sliceFound /*&& seqExtFound*/) {
      rez.codecInfo = mpeg2CodecInfo;
      rez.streamDescr = m_sequence.getStreamDescr();
      if (pulldownFound) rez.streamDescr += " (pulldown)";
   }
   return rez;
}
...
int MPEG2StreamReader::decodePicture(uint8_t* buff) {
   if (!spsFound) return NALUnit::SPS_OR_PPS_NOT_READY;

   if (!m_streamMsgPrinted) {
      LTRACE(LT_INFO, 2,
             "Decoding MPEG2 stream (track "
   ...
   if (rez == NOT_ENOUGHT_BUFFER) return rez;
   ...
   if (m_frame.ref != m_lastRef) {
      ...
      // more careful handling of top_field_first, repeat_first_field and
      // progressive_frame
   ...
   /*
   LTRACE(LT_INFO, 0, "frame num: " << m_totalFrameNum << " type=" <<
   frameTypeDescr[m_frame.pict_type] << " ps=" << m_frame.progressive_frame << "
   rf=" << m_frame.repeat_first_field);
   */
   return 0;
}

int MPEG2StreamReader::findFrameExt(uint8_t* buffer) {
   for (uint8_t* nal = MPEGHeader::findNextMarker(buffer, m_bufEnd);
        nal < m_bufEnd - 4;
        nal = MPEGHeader::findNextMarker(nal + 4, m_bufEnd)) {
      if (nal[3] == EXT_START_SHORT_CODE) {
         ...
         try {
            ...
            if (extType == PICTURE_CODING_EXT) {
               ...
               if (m_removePulldown) {
                  updateBits(bitReader, m_frame.repeat_first_field_bitpos, 1,
                             0);
                  if (m_sequence.progressive_sequence)
                     updateBits(bitReader, m_frame.top_field_first_bitpos, 1,
                                0);
               }
               return 0;
            }
...
void MPEG2StreamReader::updateStreamFps(void* nalUnit, uint8_t* buff,
                                        uint8_t* nextNal, int oldSpsLen) {
   m_sequence.setFrameRate(buff + 1, m_fps);
}

void MPEG2StreamReader::updateStreamAR(void* nalUnit, uint8_t* buff,
                                       uint8_t* nextNal, int oldSpsLen) {
   if (m_ar != AR_KEEP_DEFAULT)
      m_sequence.setAspectRation(buff + 1,
                                 m_ar);  // todo: delete this line! debug only
}
