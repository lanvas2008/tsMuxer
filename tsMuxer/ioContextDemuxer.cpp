...
double av_int2dbl(int64_t v) {
   if (v + v > 0xFFEULL << 52) return 0;  // 0.0/0.0;
   return ldexp((double)((v & ((1LL << 52) - 1)) + (1LL << 52)) * (v >> 63 | 1),
                (v >> 52 & 0x7FF) - 1075);
}

float av_int2flt(int32_t v) {
   if (v + v > 0xFF000000U) return 0;  // 0.0/0.0;
   return ldexp((float)((v & 0x7FFFFF) + (1 << 23)) * (v >> 31 | 1),
                (v >> 23 & 0xFF) - 150);
}
...
IOContextDemuxer::IOContextDemuxer(const BufferedReaderManager& readManager)
   ...
   m_bufferedReader =
       (const_cast<BufferedReaderManager&>(m_readManager)).getReader("");
...
int IOContextDemuxer::get_byte() {
   ...
   if (m_curPos == m_bufEnd) {
      uint8_t* data =
          m_bufferedReader->readBlock(m_readerID, readedBytes,
...
uint32_t IOContextDemuxer::get_buffer(uint8_t* binary, int size) {
   ...
   while (dst < dstEnd) {
      uint8_t* data =
          m_bufferedReader->readBlock(m_readerID, readedBytes, readRez);
      if (readedBytes > 0 && readRez == 0)
         m_bufferedReader->notify(m_readerID, readedBytes);
