...
struct PESPacket {
   ...
   */
   ...
   //Indicates the nature of alignment of the first start code occurring in the
   payload.
   //The type of data in the payload is indicated by the
   ...
   //Indicates the priority of this packet with respect to other packets: 1=high
   priority; 0=no priority. int pesp:1;
   ...
   //The flag is set as necessary to indicate that extension flags are set in
   the PES header. Its use includes support of private data.
   //1 - Field is present.
   ...
   //Indicates the presence of an 8 bit field describing the DSM (Digital
   Storage Media) operating mode:
   //1 - Field is present.
   //0 - Field is not present. (For broadcasting purposes, set = 0.)
   int tm:1;

   //Indicates whether the Elementary Stream Rate field is present in the PES
   header int rate:1;

   //Indicates whether the Elementary Stream Clock Reference field is present in
   the PES header int escr:1;
   ...
   */
   ...
   uint64_t getDts() {
      return get_pts((uint8_t *)this + HEADER_SIZE + PTS_SIZE);
   }
