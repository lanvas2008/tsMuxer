...
CheckStreamRez HEVCStreamReader::checkStream(uint8_t* buffer, int len) {
   ...
   for (uint8_t* nal = NALUnit::findNextNAL(buffer, end); nal < end - 4;
      ...
      if (*nal & 0x80) return rez;  // invalid nal
      int nalType = (*nal >> 1) & 0x3f;
      switch (nalType) {
         case NAL_VPS: {
            uint8_t* nextNal = NALUnit::findNALWithStartCode(nal, end, true);
            if (!m_vps) m_vps = new HevcVpsUnit();
            m_vps->decodeBuffer(nal, nextNal);
            if (m_vps->deserialize() != 0) return rez;
            break;
         }
         case NAL_SPS: {
            uint8_t* nextNal = NALUnit::findNALWithStartCode(nal, end, true);
            if (!m_sps) m_sps = new HevcSpsUnit();
            m_sps->decodeBuffer(nal, nextNal);
            if (m_sps->deserialize() != 0) return rez;
            break;
         }
         case NAL_PPS: {
            uint8_t* nextNal = NALUnit::findNALWithStartCode(nal, end, true);
            if (!m_pps) m_pps = new HevcPpsUnit();
            m_pps->decodeBuffer(nal, nextNal);
            if (m_pps->deserialize() != 0) return rez;
            break;
         }
      }
   }

   if (m_vps && m_sps && m_pps && m_sps->vps_id == m_vps->vps_id &&
       m_pps->sps_id == m_sps->sps_id) {
      rez.codecInfo = hevcCodecInfo;
      rez.streamDescr =
          m_sps->getDescription() + string(" ") + m_vps->getDescription();
   }
...
void HEVCStreamReader::updateStreamFps(void* nalUnit, uint8_t* buff,
                                       uint8_t* nextNal, int) {
   ...
   long newSpsLen =
       vps->serializeBuffer(tmpBuffer, tmpBuffer + vps->nalBufferLen() + 16);
...
bool HEVCStreamReader::isSlice(int nalType) const {
   if (!m_sps || !m_vps || !m_pps) return false;
...
void HEVCStreamReader::incTimings() {
   if (m_totalFrameNum++ > 0) m_curDts += m_pcrIncPerFrame;
...
int HEVCStreamReader::toFullPicOrder(HevcSliceHeader* slice, int pic_bits) {
   if (slice->isIDR()) {
      ...
      if (slice->pic_order_cnt_lsb < m_prevPicOrder &&
          m_prevPicOrder - slice->pic_order_cnt_lsb >= range / 2)
         m_picOrderMsb += range;
      else if (slice->pic_order_cnt_lsb > m_prevPicOrder &&
               slice->pic_order_cnt_lsb - m_prevPicOrder >= range / 2)
         m_picOrderMsb -= range;
...
void HEVCStreamReader::storeBuffer(MemoryBlock& dst, const uint8_t* data,
...
int HEVCStreamReader::intDecodeNAL(uint8_t* buff) {
   ...
   while (curPos < m_bufEnd) {
      ...
      if (isSlice(nalType)) {
         if (curPos >= m_bufEnd - 2) return NOT_ENOUGHT_BUFFER;
         ...
         if (curPos[2] & 0x80)  // slice.first_slice
         {
            if (sliceFound) {
               ...
               slice.decodeBuffer(curPos,
                                  FFMIN(curPos + MAX_SLICE_HEADER, nextNal));
               rez = slice.deserialize(m_sps, m_pps);
               if (rez) return rez;  // not enough buffer or error
               // if (slice.slice_type == HEVC_IFRAME_SLICE)
               if (nalType >= NAL_BLA_W_LP) m_lastIFrame = true;
               m_fullPicOrder =
                   toFullPicOrder(&slice, m_sps->log2_max_pic_order_cnt_lsb);
            }
         }
         sliceFound = true;
      } else {
         ...
         switch (nalType) {
            case NAL_VPS:
               ...
               nextNalWithStartCode =
                   nextNal[-4] == 0 ? nextNal - 4 : nextNal - 3;
...
uint8_t* HEVCStreamReader::writeBuffer(MemoryBlock& srcData, uint8_t* dstBuffer,
                                       uint8_t* dstEnd) {
   if (srcData.isEmpty()) return dstBuffer;
   int bytesLeft = dstEnd - dstBuffer;
   int requiredBytes = srcData.size() + 3 + (m_shortStartCodes ? 0 : 1);
   if (bytesLeft < requiredBytes) return dstBuffer;
...
int HEVCStreamReader::writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd,
                                        AVPacket& avPacket,
   ...
   bool needInsSpsPps =
       m_firstFileFrame && !(avPacket.flags & AVPacket::IS_SPS_PPS_IN_GOP);
   if (needInsSpsPps) {
