...
typedef void(LIB_STD_CALL* EVENT_HANDLER_PROC)(const long handle,
                                               const int eventID,
                                               const char* message);
struct MuxTrackInfo {
...
class AbstractCoreContext {
  public:
   ...
   virtual unsigned startStreaming(const char* session, int streamingMode,
                                   double ntpPos, int scale,
                                   const char* fileName, const char* address,
                                   unsigned short port, const char* rtpInfo,
                                   long tcpSocketHandle) = 0;

   virtual unsigned startSmartBridgeStreaming(const char* sessionID,
                                              double ntpPos, const char* url,
                                              const char* address,
                                              unsigned short port,
                                              const char* rtpInfo,
                                              long tcpSocketHandle) = 0;

   virtual unsigned seekYouTubeStreaming(const char* streamName,
                                         double ntpPos) = 0;
   virtual uint64_t getYTStreamDuration(const std::string& streamName) = 0;

   virtual unsigned seekRuTubeStreaming(const char* streamName,
                                        double ntpPos) = 0;
   virtual uint64_t getRTStreamDuration(const std::string& streamName) = 0;

   virtual void setYTCacheParams(const std::string& sCacheDirectory,
                                 const std::string& sLogDirectory,
                                 uint64_t nTotalCacheDuration) = 0;
   virtual void setRTCacheParams(const std::string& sCacheDirectory,
                                 uint64_t nTotalCacheDuration) = 0;

   virtual void setYTVideoInfo(const std::string& streamName,
                               uint8_t existingType, double fps) = 0;
   virtual void getYTVideoInfo(const std::string& streamName,
                               uint8_t& existingType, double& fps,
                               bool cleanUp = false) = 0;

   virtual void setRTVideoInfo(const std::string& streamName,
                               uint8_t existingType, double fps) = 0;
   virtual void getRTVideoInfo(const std::string& streamName,
                               uint8_t& existingType, double& fps,
                               bool cleanUp = false) = 0;

   virtual void setYTDemuxer(const std::string& streamName,
                             AbstractDemuxer* demuxer) = 0;
   virtual void removeYTDemuxer(const std::string& streamName) = 0;

   virtual void setRTDemuxer(const std::string& streamName,
                             AbstractDemuxer* demuxer) = 0;
   ...
   virtual int createStreamingGroup(const char* address, const char* name,
                                    double nptPos, int scale,
                                    int lowLevelTransport, int loopCount,
                                    int eoplDelay, int eofDelay,
                                    bool suspended) = 0;
   ...
   virtual uint32_t addDestination(const char* session, const char* fileName,
   ...
   virtual unsigned createIndexFile(const char* fileName,
                                    bool createRewind) = 0;
   virtual unsigned createIndexFileAsync(const char* displayName,
                                         const char* fileName,
                                         bool createRewind) = 0;
   ...
   virtual long multicastJoin(const char* assetName, const char* address,
                              int port, bool overWrite, int cycleBufferLen,
                              const char* ixface, bool createRewind,
                              int nBlockSize, int splitByDurationSize) = 0;
   virtual int multicastLeave(const char* encoderName) = 0;
   virtual void setReadThreadCnt(int readThreadCnt) = 0;
   virtual void initLog(int messageLevel, const char* logName,
                        const char* logDir, int logRotationPeriod) = 0;
   virtual void getAssetDescription(const char* sessionID,
                                    const char* cfileName, int descrType,
                                    char* dstBuffer, unsigned len) = 0;
   virtual void registerEventHandler(EVENT_HANDLER_PROC eventHandler) = 0;
   virtual int getAssetInfoFromFile(const char* fileName, char* dstBuffer,
                                    int bufSize) = 0;
   ...
   virtual int LIB_STD_CALL muxTSFile(std::map<int, MuxTrackInfo> trackInfo,
                                      ...
                                      int nBitrate, int nVBVLen,
                                      int nFileBlockSize) = 0;
   ...
   virtual void setYTVideoSize(const uint32_t width, const uint32_t height,
                               const bool autoPal) = 0;
   ...
   virtual void setRTVideoSize(const uint32_t width, const uint32_t height,
                               const bool autoPal) = 0;
  ...
  protected:
   ...
   bool m_ytCacheDisabled, m_ytMp4TranscodingDisabled, m_ytAutoPal,
       m_sendBlockBySinglePackets, m_rtAutoPal, m_rtCacheDisabled,
       m_cacheDemoMode, m_maxSpeedMode;

   uint32_t m_ytVideoWidth, m_ytVideoHeight, m_rtVideoWidth, m_rtVideoHeight;

   uint32_t m_tsMtuSize, m_maxMIpPacketCount, m_maxUIpPacketCount,
       m_trafficBandWidth;
