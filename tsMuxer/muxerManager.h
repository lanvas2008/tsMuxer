...
class MuxerManager {
  public:
   static const uint32_t PHYSICAL_SECTOR_SIZE =
       2048;  // minimum write align requirement. Should be readed from OS in a
              // next version
   static const int BLURAY_SECTOR_SIZE =
       PHYSICAL_SECTOR_SIZE * 3;  // real sector size is 2048, but M2TS frame
                                  // required addition rounding by 3 blocks

   MuxerManager(const BufferedReaderManager& readManager,
                AbstractMuxerFactory& factory);
   virtual ~MuxerManager();

   void setAsyncMode(bool val) { m_asyncMode = val; }

   // void setFileBlockSize ( uint32_t nFileBlockSize) { m_fileBlockSize =
   // nFileBlockSize; } int32_t getFileBlockSize() const { return
   // m_fileBlockSize;
   // }
   ...
   virtual void asyncWriteBuffer(AbstractMuxer* muxer, uint8_t* buff, int len,
                                 AbstractOutputStream* dstFile);
   virtual int syncWriteBuffer(AbstractMuxer* muxer, uint8_t* buff, int len,
                               AbstractOutputStream* dstFile);
