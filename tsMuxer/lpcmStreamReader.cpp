        ...
        if (m_channels == 3) {
          LTRACE(LT_WARN, 2,
                 "Warning! Multi channels WAVE file for stream "
                     << m_streamIndex
                     << " do not contain channels configuration info. Applying "
...
int LPCMStreamReader::decodeFrame(uint8_t* buff, uint8_t* end, int& skipBytes,
  ...
  if (m_headerType == htM2TS) {
    ...
    // 'F' && buff[3] == 'F') || (buff[0] == 'r' && buff[1] == 'i' && buff[2] ==
    // 'f' && buff[3] == 'f'))
    ...
    if (m_frameRest == 0) {
      ...
      int64_t reqFrameLen = (m_bitsPerSample == 20 ? 3 : m_bitsPerSample / 8) *
                            m_channels * m_freq / 200;
...
int LPCMStreamReader::readPacket(AVPacket& avPacket) {
  ...
  if (frameLen > MAX_AV_PACKET_SIZE)
    THROW(ERR_AV_FRAME_TOO_LARGE,
          "AV frame too large (" << frameLen << " bytes). Increase AV buffer.");
