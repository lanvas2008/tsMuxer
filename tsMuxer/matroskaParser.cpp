...
ParsedH264TrackData::ParsedH264TrackData(uint8_t* buff, int size)
    : ParsedTrackPrivData(buff, size) {
   m_firstExtract = true;
   if (buff == 0) return;
      ...
      for (int i = 0; i < spsCnt; i++) {
         ...
         if (spsLen > 0) {
            ...
            std::vector<uint8_t>& curData =
                m_spsPpsList[m_spsPpsList.size() - 1];
...
void ParsedH264TrackData::extractData(AVPacket* pkt, uint8_t* buff, int size) {
   ...
   if (m_firstExtract && spsppsExists(buff, size)) m_firstExtract = false;

   if (m_firstExtract) newBufSize += getSPSPPSLen();
   ...
   if (curPos > end) {
      LTRACE(LT_ERROR, 2,
             "Matroska parse error: invalid H264 NAL unit size. NAL unit "
             "truncated.");
      curPos = end;
   }
...
// ----------- H.265 -----------------
ParsedH265TrackData::ParsedH265TrackData(uint8_t* buff, int size)
    : ParsedH264TrackData(0, 0) {
...
ParsedVC1TrackData::ParsedVC1TrackData(uint8_t* buff, int size)
    : ParsedTrackPrivData(buff, size) {
   if (size < MS_BIT_MAP_HEADER_SIZE)
      THROW(ERR_MATROSKA_PARSE,
            "Matroska parse error: Invalid or unsupported VC-1 stream");
...
void ParsedVC1TrackData::extractData(AVPacket* pkt, uint8_t* buff, int size) {
   ...
   bool addFrameHdr =
       !(size >= 4 && buff[0] == 0 && buff[1] == 0 && buff[2] == 1);
   if (addFrameHdr) pkt->size += 4;
...
ParsedAACTrackData::ParsedAACTrackData(uint8_t* buff, int size)
    : ParsedTrackPrivData(buff, size) {
...
ParsedLPCMTrackData::ParsedLPCMTrackData(MatroskaTrack* track)
   ...
   buildWaveHeader(m_waveBuffer, audiotrack->samplerate, m_channels,
                   m_channels >= 6, m_bitdepth);

   if (track->codec_priv_size == sizeof(WAVEFORMATPCMEX))
      memcpy(m_waveBuffer.data() + 20, track->codec_priv,
...
ParsedAC3TrackData::ParsedAC3TrackData(uint8_t* buff, int size)
    : ParsedTrackPrivData(buff, size) {
   m_firstPacket = true;
...
void ParsedAC3TrackData::extractData(AVPacket* pkt, uint8_t* buff, int size) {
   if (m_firstPacket && size > 2) {
      if (AV_RB16(buff) != 0x0b77) m_shortHeaderMode = true;
   }
   m_firstPacket = false;
...
ParsedSRTTrackData::ParsedSRTTrackData(uint8_t* buff, int size)
    : ParsedTrackPrivData(buff, size) {
   m_packetCnt = 0;
}
...
void ParsedSRTTrackData::extractData(AVPacket* pkt, uint8_t* buff, int size) {
   ...
   memcpy(pkt->data + prefix.length() + size, postfix.c_str(),
          postfix.length());
}
