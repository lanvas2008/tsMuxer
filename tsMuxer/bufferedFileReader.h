...
class FileListIterator : public FileNameIterator {
 public:
  ...
  virtual std::string getNextName() override {
    if (++m_index < m_files.size()) return m_files[m_index];
    /*
  ...
  void addFile(const std::string& fileName) {
    /*
            for(unsigned i = 0; i < m_files.size(); i++)
                    if (m_files[i] == fileName)
                            THROW(ERR_FILE_EXISTS, "File name " << fileName << "
       already exists.");
    */
...
struct FileReaderData : public ReaderData {
 ...
 public:
  ...
  virtual bool incSeek(int64_t offset) {
    return m_file.seek(offset, File::smCurrent) != uint64_t(-1);
  }
...
class BufferedFileReader : public BufferedReader {
 public:
  BufferedFileReader(uint32_t blockSize, uint32_t allocSize = 0,
 ...
 protected:
  ReaderData* intCreateReader() {
    return new FileReaderData(m_blockSize, m_allocSize);
  }
};
