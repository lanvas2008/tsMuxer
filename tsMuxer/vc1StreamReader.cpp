...
void VC1StreamReader::writePESExtension(PESPacket* pesPacket,
...
int VC1StreamReader::writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd,
                                       AVPacket& avPacket,
                                       PriorityDataInfo* priorityData) {
   // uint8_t* afterPesData = (uint8_t*)pesPacket +
   ...
   if ((m_totalFrameNum > 1 && m_spsFound < 2 && m_frame.pict_type == I_TYPE) ||
      ...
      if (m_seqBuffer.size() > 0) {
         if (dstEnd - curPtr < m_seqBuffer.size())
            THROW(ERR_COMMON,
                  "VC1 stream error: Not enought buffer for write headers");
...
int VC1StreamReader::getTSDescriptor(uint8_t* dstBuff) {
   for (uint8_t* nal = VC1Unit::findNextMarker(m_buffer, m_bufEnd);
        nal <= m_bufEnd - 32;
      ...
      switch (unitType) {
         case VC1_CODE_SEQHDR:
            ...
            if (sequence.decode_sequence_header() != 0) return 0;
            ...
            if (sequence.profile == 0)
               dstBuff[7] =
                   (sequence.profile << 4) + 0x11 + (sequence.level >> 1);
            else if (sequence.profile == 1)
               dstBuff[7] =
                   (sequence.profile << 4) + 0x41 + (sequence.level >> 1);
...
CheckStreamRez VC1StreamReader::checkStream(uint8_t* buffer, int len) {
   ...
   for (uint8_t* nal = VC1Unit::findNextMarker(buffer, end); nal <= end - 32;
      ...
      switch (unitType) {
         ...
         case VC1_USER_CODE_FRAME:
            ...
            if (m_frame.pict_type == I_TYPE) iFrameFound = true;
            break;
         case VC1_CODE_ENTRYPOINT:
         case VC1_USER_CODE_ENTRYPOINT:
            nextNal = VC1Unit::findNextMarker(nal + 4, end);
            m_sequence.vc1_unescape_buffer(nal + 4, nextNal - nal - 4);
            if (m_sequence.decode_entry_point() != 0) break;
            break;
         case VC1_CODE_SEQHDR:
         case VC1_USER_CODE_SEQHDR:
            nextNal = VC1Unit::findNextMarker(nal + 4, end);
            m_sequence.vc1_unescape_buffer(nal + 4, nextNal - nal - 4);
            if (m_sequence.decode_sequence_header() != 0) break;
            spsFound = true;
            break;
         default:
            return rez;
   ...
   if (spsFound && iFrameFound) {
      ...
      if (pulldown) rez.streamDescr += " (pulldown)";
   }
   return rez;
}
...
int VC1StreamReader::intDecodeNAL(uint8_t* buff) {
   ...
   switch (*buff) {
      ...
      case VC1_CODE_ENDOFSEQ:
         ...
         if (!m_eof && nextNal >= m_bufEnd) return NOT_ENOUGHT_BUFFER;
         break;
      case VC1_CODE_SEQHDR:
         m_spsPpsFound = true;
...
int VC1StreamReader::decodeSeqHeader(uint8_t* buff) {
   ...
   fillAspectBySAR(m_sequence.sample_aspect_ratio.num /
                   (double)m_sequence.sample_aspect_ratio.den);

   updateFPS(0, buff, nextNal, oldSpsLen);
   if (m_spsFound == 0) {
      LTRACE(LT_INFO, 2,
             "Decoding VC-1 stream (track "
                 << m_streamIndex << "): " << m_sequence.getStreamDescr());
   }
   if (m_sequence.profile != PROFILE_ADVANCED)
      THROW(
          ERR_VC1_ERR_PROFILE,
          "Only ADVANCED profile are supported now. For feature request contat "
          "to: r_vasilenko@smlabs.net.");
...
int VC1StreamReader::decodeFrame(uint8_t* buff) {
   if (!m_spsFound) return NALUnit::SPS_OR_PPS_NOT_READY;
   ...
   if (m_sequence.max_b_frames > 0 &&
       (m_frame.pict_type == I_TYPE || m_frame.pict_type == P_TYPE)) {
      nextBFrameCnt = getNextBFrames(buff, bTiming);
      if (nextBFrameCnt == -1) return NOT_ENOUGHT_BUFFER;
   }
   ...
   if (m_removePulldown) {
      ...
      if (m_sequence.pulldown) {
         if (!m_sequence.interlace || m_sequence.psf) {
            ...
            if (m_frame.rff > 0)
               updateBits(m_frame.getBitReader(), m_frame.rptfrmBitPos + 1, 1,
                          0);
            m_frame.rff = 0;
         }
      }
...
int VC1StreamReader::getNextBFrames(uint8_t* buffer, int64_t& bTiming) {
   int bFrameCnt = 0;
   bTiming = 0;
   for (uint8_t* nal = VC1Unit::findNextMarker(buffer, m_bufEnd);
        nal < m_bufEnd - 4; nal = VC1Unit::findNextMarker(nal + 4, m_bufEnd)) {
      ...
      if (nal[3] == VC1_CODE_FRAME || nal[3] == VC1_USER_CODE_FRAME) {
         ...
         if (bFrameCnt == m_sequence.max_b_frames) return bFrameCnt;
      }
   }
   if (m_eof) {
...
uint8_t* VC1StreamReader::findNextFrame(uint8_t* buffer) {
   for (uint8_t* nal = VC1Unit::findNextMarker(buffer, m_bufEnd);
        nal < m_bufEnd - 4; nal = VC1Unit::findNextMarker(nal + 4, m_bufEnd)) {
      if (nal[3] != VC1_CODE_FIELD && nal[3] != VC1_USER_CODE_FIELD) return nal;
   }
   if (m_eof)
      return m_bufEnd;
   else
...
void VC1StreamReader::updateStreamFps(void* nalUnit, uint8_t* buff,
