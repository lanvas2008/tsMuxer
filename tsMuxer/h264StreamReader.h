...
class H264StreamReader : public MPEGStreamReader {
 ...
 protected:
  ...
  virtual void updateStreamFps(void* nalUnit, uint8_t* buff, uint8_t* nextNal,
                               int oldSpsLen);
 ...
 private:
  ...
  int deserializeSliceHeader(SliceUnit& slice, uint8_t* buff,
                             uint8_t* sliceEnd);
 ...
 private:
  ...
  void updateSpsFps(SPSUnit* sps, uint8_t* buff, uint8_t* nextNal,
                    int oldSpsLen);
  ...
  int writeSEIMessage(uint8_t* dstBuffer, uint8_t* dstEnd, SEIUnit& sei,
                      uint8_t payloadType);

  uint8_t* m_nextFrameAddr;

  std::vector<uint8_t> m_bdRomMetaDataMsg;  // copy metadata to buffer, then
                                            // isert during sei rebuild process
