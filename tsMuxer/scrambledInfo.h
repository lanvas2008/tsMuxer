...
struct ScrambledData {
   ...
   uint32_t m_len;
   int m_code;
   ScrambledData(uint32_t start, uint32_t len, int code)
       : m_start(start), m_len(len), m_code(code) {}
};
...
class ScrambledInfo {
  public:
   ...
   void addValue(uint32_t start, uint32_t len, int code) {
      if (m_data.size() > 0 && m_data[m_data.size() - 1].m_start == start &&
          m_data[m_data.size() - 1].m_code == code)
   ...
   const uint32_t getDataStart(size_t index) {
      return m_data[index].m_start - m_offset;
   }
