...
class SRTStreamReader : public AbstractStreamReader {
 public:
  ...
  virtual int flushPacket(AVPacket& avPacket) {
    return m_dstSubCodec->flushPacket(avPacket);
  }
  ...
  CheckStreamRez checkStream(uint8_t* buffer, int len,
                             ContainerType containerType, int containerDataType,
  ...
  virtual void setDemuxMode(bool value) {
    ...
    if (pgsReader) pgsReader->setDemuxMode(value);
  }
  void setVideoInfo(int width, int height, double fps) {
    m_srtRender->setVideoInfo(width, height, fps);
    PGSStreamReader* pgsReader = dynamic_cast<PGSStreamReader*>(m_dstSubCodec);
    if (pgsReader) pgsReader->setVideoInfo(0, 0, fps);
  }
  void setFont(const text_subtitles::Font& font) {
    m_srtRender->m_textRender->setFont(font);
  }
 ...
 protected:
  virtual int writeAdditionData(uint8_t* dstBuffer, uint8_t* dstEnd,
    ...
    return m_dstSubCodec->writeAdditionData(dstBuffer, dstEnd, avPacket,
                                            priorityData);
  }
