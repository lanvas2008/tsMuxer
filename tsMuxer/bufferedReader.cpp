...
BufferedReader::BufferedReader(uint32_t blockSize, uint32_t allocSize,
   ...
   // size of the memory reserved per-block (can exceed the block size, in which
   // case the rest of the memory can be used by the user)
...
BufferedReader::~BufferedReader() {
   ...
   m_readQueue.push(0);
   join();
   for (std::map<uint32_t, ReaderData*>::iterator itr = m_readers.begin();
        itr != m_readers.end(); ++itr) {
      int i = 0;
...
uint32_t BufferedReader::createReader(int readBuffOffset) {
   ...
   LTRACE(LT_INFO, 0,
          "Reader #" << m_id << ". Start new stream " << newReaderID
                     << ". stream(s): " << (uint32_t)rSize);
...
void BufferedReader::deleteReader(uint32_t readerID) {
   ...
   {
      std::lock_guard<std::mutex> lock(m_readersMtx);
      std::map<uint32_t, ReaderData*>::iterator iterator =
          m_readers.find(readerID);
      if (iterator == m_readers.end()) return;
      ReaderData* data = iterator->second;
      if (data->m_atQueue > 0)
         data->m_deleted = true;  // There are requests in the queue for reading
                                  // into this structure.
      else {
         delete iterator->second;  // No outstanding requests for reading in the
                                   // queue. Delete immediately.
         m_readers.erase(iterator);
      }
      rSize = m_readers.size();
   }
   // LTRACE(LT_INFO, 0, "stop stream " << readerID << ". stream(s): " <<
   // (uint32_t) rSize);
}

uint8_t* BufferedReader::readBlock(uint32_t readerID, uint32_t& readCnt,
                                   int& rez, bool* firstBlockVar) {
   ReaderData* data = 0;
   {
      std::lock_guard<std::mutex> lock(m_readersMtx);
      map<uint32_t, ReaderData*>::iterator itr = m_readers.find(readerID);
      if (itr != m_readers.end()) {
         ...
         if (!data->m_nextBlockSize &&
             !data->m_notified) {  // No fragments of this block found, and no
                                   // requests for reading this block
   ...
   data->m_notified = false;
   if (firstBlockVar) *firstBlockVar = data->m_firstBlock;
...
void BufferedReader::thread_main() {
   try {
      while (!m_terminated) {
         ...
         if (data) {
            uint8_t* buffer =
                data->m_nextBlock[data->m_bufferIndex] + data->m_readOffset;
            if (!data->m_deleted) {
               ...
               if ((bytesReaded < data->m_blockSize && data->itr) ||
                   bytesReaded <= 0) {
                  if (data->itr) {
                     ...
                     if (nextFileName != data->m_streamName) {
                        ...
                        if (!data->m_streamName.empty() && data->openStream()) {
                           if (bytesReaded == 0) {
                              ...
                              data->m_firstBlock = true;
                              bytesReaded =
                                  data->readBlock(buffer, m_blockSize);
      ...
      LTRACE(LT_ERROR, 0,
             "BufferedReader::thread_main() throws exception: " << e.what());
   } catch (...) {
      LTRACE(LT_ERROR, 0,
             "BufferedReader::thread_main() throws unknown exception");
   }
}
...
void BufferedReader::setFileIterator(FileNameIterator* itr, int readerID) {
   ...
   if (reader != m_readers.end()) reader->second->itr = itr;
}
