...
void buildWaveHeader(MemoryBlock& waveBuffer, int samplerate, int channels,
                     bool lfeExist, int bitdepth) {
   waveBuffer.clear();
   ...
   *fmtSize = sizeof(WAVEFORMATPCMEX);
   ...
   waveFormatPCMEx->dwChannelMask =
       getWaveChannelMask(channels, lfeExist);  // Specify PCM
...
void toLittleEndian(uint8_t* dstData, const uint8_t* srcData, int size,
                    int bitdepth) {
   if (bitdepth == 16) {
      ...
      THROW(ERR_WAV_PARSE,
            "Unsupported LPCM big depth " << bitdepth << " for /LIT codec");
   }
}
