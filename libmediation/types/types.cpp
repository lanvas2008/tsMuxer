...
uint64_t my_ntohll(const uint64_t& original) {
  ...
  return original;
#else  // little endian
  return (((uint64_t)my_ntohl((uint32_t)original)) << 32) |
         ((uint64_t)my_ntohl((uint32_t)(original >> 32)));
#endif
}
...
string doubleToStr(const double& x, int precision) {
  std::ostringstream str;
  if (precision > 0) str << fixed << std::setprecision(precision);
  str << x;
  return str.str();
}
...
vector<string> splitStr(const char* str, char splitter) {
  ...
  if (buf > prevPos) rez.push_back(string(prevPos, buf - prevPos));
  return rez;
}
...
vector<wstring> splitStrW(const wchar_t* str, wchar_t splitter) {
  ...
  if (buf > prevPos) rez.push_back(wstring(prevPos, buf - prevPos));
  return rez;
}
...
string extractFileName2(const string& src, bool withExt) {
  ...
  if (dirSep == string::npos) dirSep = fileName.find_last_of('\\');

  if (extSep != string::npos && !withExt) fileName = fileName.substr(0, extSep);
...
string closeDirPath(const string& src, char delimiter) {
  if (delimiter == ' ') delimiter = getDirSeparator();
  if (src.length() == 0) return src;
  if (src[src.length() - 1] == '/' || src[src.length() - 1] == '\\') return src;
...
wstring trimStrW(const wstring& value) {
  ...
  for (; chBeg < value.length() &&
         (value[chBeg] == '\n' || value[chBeg] == '\r' || value[chBeg] == ' ');
       chBeg++)
    ;
  for (; chEnd >= chBeg &&
         (value[chEnd] == '\n' || value[chEnd] == '\r' || value[chEnd] == ' ');
       chEnd--)
    ;
