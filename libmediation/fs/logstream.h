...
Из класса VoipSession перенесены в VoipProcessor функции по обработке сессии. В
классе VoipSession оставлены только данные, связанные с сессией.
...
class LogStream : public AlternativeFileStream {
  public:
   ...
   LogStream(uint32_t logLevel, const std::string& fileName,
             const std::string& oldFilesDir, const uint32_t secRotationPeriod,
             const uint32_t maxFileSizeBytes = 128 * 1024 * 1024,
             const bool rotateAtInitialization = true,
             AlternativeFileStream::DestDirectoryDetalization dirDetalization =
                 AlternativeFileStream::None,
             uint32_t systemMessages = MessageLevel | MessageDate |
                                       MessageTime | MessageThreadID,
   ...
   virtual AlternativeFileStream& operator<<(
       AlternativeFileStream& (*_Pfn)(AlternativeFileStream&));
...
//------------------------------------------------------------------------------------
void bufToLogStream(const char* const buf, const uint32_t& length,
                    LogStream* const stream, bool printLength = true);

AlternativeFileStream& operator<<(AlternativeFileStream&,
                                  const loglevel& manip);
