...
string extractFileDir(const string& fileName) {
   ...
   if (index != string::npos) return fileName.substr(0, index + 1);
#ifdef WIN32
...
bool createDir(const std::string& dirName, bool createParentDirs) {
   ...
   if (createParentDirs) {
      for (string::size_type
               separatorPos = dirName.find_first_not_of(getDirSeparator()),
               dirEnd = string::npos;
           separatorPos != string::npos;
           dirEnd = separatorPos, separatorPos = dirName.find_first_not_of(
                                      getDirSeparator(), separatorPos)) {
         ...
         if (dirEnd != string::npos) {
            ...
            if (parentDir.size() == 0 ||
                ...
                (strStartWith(parentDir, "\\\\.\\") &&
                 parentDir[parentDir.size() - 1] == '}'))  // UNC patch prefix
               continue;
...
bool findFiles(const string& path, const string& fileMask,
               vector<string>* fileList, bool savePaths) {
   WIN32_FIND_DATA fileData;  // Data structure describes the file found
   HANDLE hSearch;            // Search handle returned by FindFirstFile

   hSearch =
       FindFirstFile(TEXT(string(path + '/' + fileMask).c_str()), &fileData);
...
bool findFilesRecursive(const string& path, const string& mask,
