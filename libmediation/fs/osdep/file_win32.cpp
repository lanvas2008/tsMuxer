...
void makeWin32OpenFlags(unsigned int oflag, DWORD* const dwDesiredAccess,
   ...
   *dwCreationDisposition = CREATE_ALWAYS;
   ...
   if (oflag & File::ofAppend) *dwCreationDisposition = OPEN_ALWAYS;
   if (oflag & File::ofNoTruncate) *dwCreationDisposition = OPEN_ALWAYS;
   if (oflag & File::ofOpenExisting) *dwCreationDisposition = OPEN_EXISTING;
   if (oflag & File::ofCreateNew) *dwCreationDisposition = CREATE_NEW;
}
...
File::File(const char* fName, unsigned int oflag,
   ...
   DWORD dwShareMode = 0;
   makeWin32OpenFlags(oflag, &dwDesiredAccess, &dwCreationDisposition,
                      &dwShareMode);
   ...
   m_impl = CreateFile(fName, dwDesiredAccess, dwShareMode, NULL,
                       dwCreationDisposition, systemDependentFlags, NULL);
...
bool File::open(const char* fName, unsigned int oflag,
...
uint64_t File::seek(int64_t offset, SeekMethod whence) {
   ...
   DWORD newPointerLow = SetFilePointer(m_impl, distanceToMoveLow,
...
bool File::truncate(uint64_t newFileSize) {
   ...
   LONG distanceToMoveHigh =
       (uint32_t)((newFileSize & 0xffffffff00000000ull) >> 32);
   DWORD newPointerLow = SetFilePointer(m_impl, distanceToMoveLow,
