...
void makeUnixOpenFlags(unsigned int oflag, int* const unixOflag) {
  ...
  if (oflag & File::ofCreateNew) sysFlags |= O_CREAT | O_EXCL;
  *unixOflag = sysFlags;
}
...
File::File(const char* fName, unsigned int oflag,
  ...
  m_impl = (void*)::open(fName, sysFlags | O_LARGEFILE | systemDependentFlags,
                         S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
  if ((long)m_impl == -1) {
    std::ostringstream ss;
    ss << "Error opening file " << fName << ": " << strerror(errno) << "("
       << errno << ")";
